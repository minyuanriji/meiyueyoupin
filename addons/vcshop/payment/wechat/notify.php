...
class VcShopWechatPay
{
	...
	{
		global $_W;
		// pdo_insert('a',array('info'=>'a'));
		if (!$this->publicMethod()) {
		// pdo_insert('a',array('info'=>'b'));
		...
		if (strexists($tid, 'GJ')) {
			...
			if (100 <= $ordersn2) {
				pdo_update('vcshop_order', array('ordersn2' => $ordersn2), array('ordersn' => $tid, 'openid' => $openid));
			}
		}
		...
		// pdo_insert('a',array('info'=>'a4'));
			...
			if ($count_ordersn == 2) {
		// pdo_insert('a',array('info'=>'a5'));
				pdo_update('vcshop_order', array('tradepaytype' => 21, 'isborrow' => $isborrow, 'borrowopenid' => $borrowopenid, 'apppay' => $this->isapp ? 1 : 0, 'transid' => $transaction_id), array('ordersn_trade' => $log['tid'], 'uniacid' => $log['uniacid']));
			}
			else {
		// pdo_insert('a',array('info'=>'a6'));
				pdo_update('vcshop_order', array('paytype' => 21, 'isborrow' => $isborrow, 'borrowopenid' => $borrowopenid, 'apppay' => $this->isapp ? 1 : 0, 'transid' => $transaction_id), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
			}
		...
		// pdo_insert('a',array('info'=>'c'));

			if (!is_error($site)) {
		// pdo_insert('a',array('info'=>'d'));
				$method = 'payResult';

				if (method_exists($site, $method)) {
		// pdo_insert('a',array('info'=>'e'));
		...
		// pdo_insert('a',array('info'=>'f'));
					...
					// pdo_insert('a',array('info'=>'method error'));

					}
				}else{
					// pdo_insert('a',array('info'=>'pay error'));
				}
			}else{
					// pdo_insert('a',array('info'=>'pay error!!!!'));
	...
	public function recharge()
	{
		...
		if ($OK) {
			pdo_update('vcshop_member_log', array('status' => 1, 'rechargetype' => 'wechat', 'isborrow' => $isborrow, 'borrowopenid' => $borrowopenid, 'apppay' => $this->isapp ? 1 : 0), array('id' => $log['id']));
	...
	public function coupon()
	{
		...
		$coupon = pdo_fetchcolumn('select money from ' . tablename('vcshop_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
	...
	public function threen()
	{
		...
		if ($this->is_jie) {
			pdo_update('vcshop_threen_log', array('isborrow' => '1', 'borrowopenid' => $this->get['openid']), array('logno' => $orderno, 'uniacid' => $_W['uniacid']));
		}
	...
	{
		global $_W;
		$setting = pdo_fetch('select * from ' . tablename('vcshop_system_grant_setting') . ' where id = 1 limit 1 ');
		...
		if (0 < $setting['weixin']) {
			...
			if ($this->sign == $this->get['sign']) {
				$order = pdo_fetch('select * from ' . tablename('vcshop_system_grant_order') . ' where logno = \'' . $this->get['out_trade_no'] . '\'');
				pdo_update('vcshop_system_grant_order', array('paytime' => time(), 'paystatus' => 1), array('logno' => $this->get['out_trade_no']));
				...
				foreach ($plugind as $key => $value) {
					...
					$data['pluginid'] = $value;
					pdo_insert('vcshop_system_grant_log', $data);
					$id = pdo_insertid();
	...
	{
		global $_W;
		$setting = pdo_fetch('select * from ' . tablename('vcshop_system_plugingrant_setting') . ' where 1 = 1 limit 1 ');
		...
		if (0 < $setting['weixin']) {
			...
			if ($this->sign == $this->get['sign']) {
				$order = pdo_fetch('select * from ' . tablename('vcshop_system_plugingrant_order') . ' where logno = \'' . $this->get['out_trade_no'] . '\'');
				pdo_update('vcshop_system_plugingrant_order', array('paytime' => time(), 'paystatus' => 1), array('logno' => $this->get['out_trade_no']));
				$plugind = explode(',', $order['pluginid']);
	...
	public function mr()
	{
		...
		if (p('mr')) {
			$price = pdo_fetchcolumn('select payprice from ' . tablename('vcshop_mr_order') . ' where ordersn=:ordersn limit 1', array(':ordersn' => $ordersn));

			if ($price == $this->total_fee) {
				if ($isborrow == 1) {
					pdo_update('vcshop_order', array('isborrow' => $isborrow, 'borrowopenid' => $borrowopenid), array('ordersn' => $ordersn));
				}
		...
		if (!empty($log) && $log['status'] == '0' && $log['fee'] == $this->total_fee) {
			...
			if (!is_error($site)) {
				...
				if (method_exists($site, $method)) {
					...
					$ret['fee'] = $log['fee'];
					$ret['tag'] = $log['tag'];
					pdo_update('vcshop_order', array('paytype' => 21, 'apppay' => 2, "transid" => $transaction_id), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
					$result = $site->{$method}($ret);
	...
	public function wxapp_recharge()
	{
		...
		if ($OK) {
			pdo_update('vcshop_member_log', array('status' => 1, 'rechargetype' => 'wechat', 'apppay' => 2), array('id' => $log['id']));
			$shopset = m('common')->getSysset('shop');
			m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, $shopset['name'] . '会员充值:微信充值:余额:' . $log['money']));
			m('member')->setRechargeCredit($log['openid'], $log['money']);
			com_run('sale::setRechargeActivity', $log);
			...
			if ($log['money'] == $this->total_fee) {
				pdo_update('vcshop_member_log', array('rechargetype' => 'wechat', 'apppay' => 2), array('id' => $log['id']));
			}
		}
	}
	...
	public function wxapp_groups()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('vcshop_groups_paylog') . ' WHERE `tid`=:orderno  limit 1';
		$params = array();
		$params[':orderno'] = $orderno;
		$log = pdo_fetch($sql, $params);
		if (!empty($log) && $log['status'] == '0' && $log['fee'] == $this->total_fee) {
			if (p('groups')) {
				pdo_update('vcshop_groups_paylog', array('status' => '1'), array('id' => $log['id']));
	...
	public function membercard()
	{
		...
		if ($this->is_jie) {
			pdo_update('vcshop_member_card_order', array('isborrow' => '1', 'borrowopenid' => $this->get['openid']), array('orderno' => $orderno, 'uniacid' => $_W['uniacid']));
		}
