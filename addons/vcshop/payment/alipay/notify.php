...
class aliApy
{
		...
		if (!empty($log) && $log['status'] == '0') {
			...
			if (!is_error($site)) {
				...
				if (method_exists($site, $method)) {
					...
					pdo_update('vcshop_order', array('paytype' => 22), array('uniacid' => $log['uniacid'], 'ordersn' => $log['tid']));
					...
					if ($result) {
						...
						pdo_update('vcshop_order', array('paytype' => 22, 'apppay' => $this->isapp ? 1 : 0, 'transid' => $this->post['trade_no']), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
						exit('success');
					}
				}
			}
	...
	public function threen()
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('vcshop_threen_log') . ' WHERE `uniacid`=:uniacid and logno = :logno limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
		if (!$this->isapp && $this->post['sign_type'] == 'RSA') {
	...
	public function recharge()
	{
		...
		if (!empty($log) && empty($log['status'])) {
			pdo_update('vcshop_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'apppay' => $this->isapp ? 1 : 0, 'transid' => $this->post['trade_no']), array('id' => $log['id']));
	...
	{
		global $_W;
		$apply = pdo_fetch('select * from ' . tablename('vcshop_commission_apply') . ' where id=:id limit 1', array(':id' => $id));
		...
		$list = pdo_fetchall('select id,agentid, ordersn,price,goodsprice, dispatchprice,createtime, paytype from ' . tablename('vcshop_order') . ' where  id in ( ' . implode(',', $ids) . ' );');
		$totalcommission = 0;
		$totalpay = 0;
		$totalmoney = 0;
	...
	{
		$log = pdo_fetch('select * from ' . tablename('vcshop_member_log') . ' where id=:id limit 1', array(':id' => $id));
	...
	{
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_order') . ' WHERE id = :id limit 1', array(':id' => $id));
