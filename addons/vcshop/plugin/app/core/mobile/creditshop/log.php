...
class Log_VcShopPage extends AppMobilePage
{
    ...
    public function paydispatch()
    {
        ...
        if ($paytype == "wechat") {
            ...
            if (empty($dispatchno)) {
                ...
                pdo_update("vcshop_creditshop_log", array("dispatchno" => $dispatchno, "addressid" => $addressid, "dispatch" => $dispatchprice), array("id" => $log["id"]));
            }
            $tid = $dispatchno;
    ...
    public function paydispatchresult()
    {
        ...
        pdo_update("vcshop_creditshop_log", array("dispatchstatus" => 1), array("id" => $log["id"]));
        return app_json();
    }
    public function express()
    {
        ...
        $order = pdo_fetch("select expresscom,expresssn,addressid,status,express from " . tablename("vcshop_creditshop_log") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid));
        if (empty($order)) {
            return app_error(AppError::$OrderNotFound);
        }
        ...
        $goods = pdo_fetchall("select og.goodsid,g.thumb  from " . tablename("vcshop_creditshop_log") . " og " . " left join " . tablename("vcshop_creditshop_goods") . " g on g.id=og.goodsid " . " where og.id=:orderid and og.uniacid=:uniacid ", array(":uniacid" => $uniacid, ":orderid" => $orderid));
