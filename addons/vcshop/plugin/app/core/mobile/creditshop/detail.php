...
class Detail_VcShopPage extends AppMobilePage
{
    ...
    public function option()
    {
        ...
        $goods = pdo_fetch("select id,thumb,credit,money,total,title from " . tablename("vcshop_creditshop_goods") . " where id= :id and uniacid = :uniacid ", array(":id" => $id, ":uniacid" => $uniacid));
        ...
        unset($spec);
        $options = pdo_fetchall("select * from " . tablename("vcshop_creditshop_option") . " where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc", array(":goodsid" => $id, ":uniacid" => $uniacid));
        if (!empty($specs)) {
    ...
    public function pay($a = array(), $b = array())
    {
        ...
        if (0 < $goods["money"] || 0 < floatval($goods["dispatch"])) {
            pdo_delete("vcshop_creditshop_log", array("goodsid" => $id, "openid" => $openid, "status" => 0, "paystatus" => 0));
            $needpay = true;
            ...
            pdo_delete("vcshop_creditshop_log", array("goodsid" => $id, "openid" => $openid, "status" => 0));
        }
        $dispatchstatus = 0;
        ...
        $address = false;
        if (!empty($addressid)) {
            $address = pdo_fetch("select id,realname,mobile,address,province,city,area from " . tablename("vcshop_member_address") . "\r\n            where id=:id and uniacid=:uniacid limit 1", array(":id" => $addressid, ":uniacid" => $_W["uniacid"]));
            if (empty($address)) {
        ...
        if ($needpay) {
            if ($paytype == "credit") {
                if ($goods["money"] + $goods["dispatch"] < $money) {
                    $paystatus = 0;
                    pdo_update("vcshop_creditshop_log", array("paytype" => $paystatus), array("id" => $logid));
                } else {
    ...
    public function express()
    {
        ...
        $order = pdo_fetch("select expresscom,expresssn,addressid,status,express from " . tablename("vcshop_creditshop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid));
        if (empty($order)) {
            return app_error(AppError::$OrderNotFound);
        }
        ...
        $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids  from " . tablename("vcshop_order_goods") . " og " . " left join " . tablename("vcshop_creditshop_order") . " g on g.id=og.goodsid " . " where og.orderid=:orderid and og.uniacid=:uniacid ", array(":uniacid" => $uniacid, ":orderid" => $orderid));
