...
class Verify_VcShopPage extends AppMobilePage
{
	public function log()
	{
		...
		$sql = 'select vgl.id as logid,vg.*,g.id as goodsid ,g.title,g.thumb,o.ordersn,vgl.verifydate,vgl.verifynum,o.paytime,s.storename,sa.salername,vgl.remarks,o.openid  from ' . tablename('vcshop_verifygoods_log') . '   vgl
	...
	public function get_detail()
	{
		...
		$sql = 'select vg.*,g.id as goodsid ,g.title,g.thumb,o.ordersn,vgl.verifydate,vgl.verifynum,o.paytime,s.storename,sa.salername,vgl.remarks,o.openid  from ' . tablename('vcshop_verifygoods_log') . '   vgl
		 left join ' . tablename('vcshop_verifygoods') . ' vg on vg.id = vgl.verifygoodsid
		 left join ' . tablename('vcshop_store') . ' s  on s.id = vgl.storeid
		 left join ' . tablename('vcshop_saler') . ' sa  on sa.id = vgl.salerid
		 left join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
