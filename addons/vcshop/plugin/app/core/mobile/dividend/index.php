...
class Index_VcShopPage extends Base_VcShopPage
{
	...
	public function register()
	{
		...
		if ($set['condition'] == 1) {
			$status = 1;
			$membercount = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member') . ' where  agentid = ' . $member['id'] . ' and id <> ' . $member['id'] . '  and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid']));
			...
			$status = 1;
			$commissiondownlinecount = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member') . ' where  agentid = ' . $member['id'] . ' and isagend = 1 and id <> ' . $member['id'] . '  and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid']));
	...
	public function main()
	{
		...
		$member['applycount'] = pdo_fetchcolumn('select count(id) from ' . tablename('vcshop_dividend_apply') . ' where mid=:mid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
