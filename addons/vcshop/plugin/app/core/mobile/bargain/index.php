...
class Index_VcShopPage extends AppMobilePage
{
	...
	public function get_list()
	{
		...
		$sql = 'SELECT bg.* ,g.id as gid,g.title,g.subtitle,g.thumb,g.marketprice,g.productprice,g.minprice,g.maxprice,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.sales,g.salesreal,g.total,g.description,g.bargain,g.`type` as gtype,g.ispresell,g.`virtual`,g.hasoption,g.video FROM ' . tablename('vcshop_bargain_goods') . ' bg ' . ' LEFT JOIN ' . tablename('vcshop_goods') . ' g ON g.id = bg.goods_id ' . (' WHERE ' . $condition . ' ORDER BY bg.id DESC LIMIT ') . ($pindex - 1) * $pagesize . ',' . $pagesize;
		$list = pdo_fetchall($sql, $params);
		$data = date('Y-m-d h:i:s');
		$sqlcount = 'SELECT bg.id,bg.start_time,bg.end_time FROM ' . tablename('vcshop_bargain_goods') . ' bg ' . ' LEFT JOIN ' . tablename('vcshop_goods') . ' g ON g.id = bg.goods_id ' . (' WHERE ' . $condition . '  ORDER BY bg.id DESC ');
	...
	public function get_detail()
	{
		...
		if (!empty($res)) {
			$goods = pdo_fetch('SELECT * FROM ' . tablename('vcshop_goods') . ' WHERE id = :id AND status = \'1\' AND deleted = \'0\'', array(':id' => $res['goods_id']));
	...
	public function bargain()
	{
		...
		$thumbs = array($vc_detail['thumb']);
		$res2['thumb'] = tomedia($thumbs[0]);
		$vc_detail['content'] = m('common')->html_to_images($vc_detail['content']);
		...
		$res3 = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_bargain_record') . ' WHERE actor_id = :actor_id ORDER BY id DESC LIMIT 0,10', array(':actor_id' => $id));
		...
		$res4 = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_bargain_actor') . (' WHERE goods_id = ' . $res2['id'] . ' AND account_id=:uniacid and bargain_price <= \'' . $account_set['partin'] . '\' ORDER BY bargain_price ASC LIMIT 10'), array(':uniacid' => $_W['uniacid']));
	...
	public function join()
	{
		...
		$res = pdo_fetch('SELECT * FROM ' . tablename('vcshop_bargain_goods') . ' WHERE id = :id', array(':id' => $bargain_id));
		...
		if (!empty($res['initiate'])) {
			$count = pdo_get('vcshop_bargain_actor', array('goods_id' => $bargain_id, 'openid' => $_W['openid'], 'status' => 0, 'order' => 0), 'id');
		...
		if (!empty($user_info['openid'])) {
			...
			pdo_query('UPDATE ' . tablename('vcshop_bargain_goods') . ' SET act_times=act_times+1 WHERE id= :id', array(':id' => $bargain_id));
		}
		else {
	...
	private function get_bargain_share()
	{
		...
		if (!empty($share_res['mall_logo'])) {
			...
			$share['logo'] = $_W['siteroot'] . 'addons/vcshop/plugin/bargain/static/images/share_logo.jpg';
		}
