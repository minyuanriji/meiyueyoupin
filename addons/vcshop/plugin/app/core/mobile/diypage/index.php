...
class Index_VcShopPage extends AppMobilePage
{
	public function main()
	{
		...
		if (is_array($page['data']['page']) && !empty($page['data']['page']['diyadv'])) {
			$startadvitem = pdo_fetch('SELECT * FROM ' . tablename('vcshop_wxapp_startadv') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => intval($page['data']['page']['diyadv']), ':uniacid' => $_W['uniacid']));
		...
		if (!empty($dataurl)) {
			if (strpos($dataurl, '/pages/') === false) {
				...
				if ($dataParams[0] == 'category') {
					$pcate = $dataParams[1];
					$goodsql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' . tablename('vcshop_goods') . ' WHERE FIND_IN_SET(' . $pcate . ',cates) AND status = 1 AND deleted = 0 AND uniacid =' . $_W['uniacid'] . ' ORDER BY displayorder DESC' . ' limit 0,' . $goodsnum;
					$count = pdo_fetch('SELECT count(id) as count FROM ' . tablename('vcshop_goods') . ' WHERE FIND_IN_SET(' . $pcate . ',cates) AND status = 1 AND deleted = 0 AND uniacid =' . $_W['uniacid']);
					...
					foreach ($list['list'] as $k => $v) {
						...
						if ($v['hasoption'] == 1) {
							$pricemax = array();
							$options = pdo_fetchall('select * from ' . tablename('vcshop_goods_option') . ' where goodsid=:goodsid and                               uniacid=:uniacid order by displayorder asc', array(':goodsid' => $v['id'], ':uniacid' => $_W['uniacid']));
				...
				if ($dataParams[0] == 'groups') {
					...
					$goodsql = 'SELECT id,displayorder,title,subtitle,thumb,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' . tablename('vcshop_goods') . ' WHERE id in(' . $goodsid . ') AND status = 1 AND deleted = 0 AND uniacid =' . $_W['uniacid'] . ' ORDER BY displayorder DESC' . ' limit 0,' . $goodsnum;
					$count = pdo_fetch('SELECT count(id) as count FROM ' . tablename('vcshop_goods') . ' WHERE id in(' . $goodsid . ') AND status = 1 AND deleted = 0 AND uniacid =' . $_W['uniacid']);
					$list['list'] = pdo_fetchall($goodsql);
					$list['count'] = $count['count'];
				...
				if ($dataParams[0] == 'goodsids') {
					...
					if (!empty($goodsids)) {
						...
						$sql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' . tablename('vcshop_goods') . ' WHERE id in(' . $goodsid . ') AND uniacid =' . $_W['uniacid'] . ' ORDER BY displayorder DESC' . ' limit 0,' . $goodsnum;
						$count = pdo_fetch('SELECT count(id) as count FROM ' . tablename('vcshop_goods') . ' WHERE id in(' . $goodsid . ') AND uniacid =' . $_W['uniacid']);
					...
					if ($dataParams[0] == 'stores') {
						...
						if (!empty($storesids)) {
							...
							$sql = 'SELECT id,storename,displayorder FROM ' . tablename('vcshop_store') . ' WHERE id in(' . $storesid . ') AND uniacid =' . $_W['uniacid'] . ' ORDER BY displayorder DESC' . ' limit 0,' . $storenum;
