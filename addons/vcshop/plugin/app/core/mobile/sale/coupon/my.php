...
class My_VcShopPage extends AppMobilePage
{
	public function getlist()
	{
		...
		$psize = 10;
		$time = time();
		$sql = 'select d.id,d.couponid,d.gettime,c.timelimit,c.coupontype,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.bgcolor,c.thumb,c.merchid,c.tagtitle,c.settitlecolor,c.titlecolor from ' . tablename('vcshop_coupon_data') . ' d';
		...
		pdo_update('vcshop_coupon_data', array('isnew' => 0), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		...
		foreach ($coupons as $i => &$row) {
			...
			$row['check'] = $check;
			$row['title2'] = $title2;
			$row['iconurl'] = $_W['siteroot'] . 'addons/vcshop/template/mobile/default/static/images/coupon/' . $row['imgname'] . '.png';
			unset($row['timelimit']);
	...
	public function getdetail()
	{
		...
		$data = pdo_fetch('select * from ' . tablename('vcshop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		$num = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('vcshop_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and used=0 ', array(':couponid' => $coupon['id'], ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
	...
	public function showcoupon2()
	{
		...
		$data = pdo_fetch('select c.*  from ' . tablename('vcshop_coupon_data') . '  cd inner join  ' . tablename('vcshop_coupon') . ' c on cd.couponid = c.id  where cd.id=:id and cd.uniacid=:uniacid and coupontype =0  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($data['limitgoodcatetype'] == 1 && !empty($data['limitgoodcateids'])) {
			...
			if (0 < count($limitcateids)) {
				...
				foreach ($limitcateids as $cateid) {
					...
					$table .= 'select * from ' . tablename('vcshop_goods') . ' where FIND_IN_SET(' . $cateid . ',cates)';
				}
