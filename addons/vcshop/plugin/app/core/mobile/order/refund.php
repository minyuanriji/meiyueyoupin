...
class Refund_VcShopPage extends AppMobilePage
{
    protected function globalData()
    {
        ...
        $order = pdo_fetch("select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,`virtual`,refundstate,merchid from " . tablename("vcshop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid));
        if (empty($order)) {
        ...
        if (1 < $order["status"]) {
            $goods = pdo_fetchall("select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund,g.refund,g.returngoods,g.exchange, g.thumb, g.type, g.title from" . tablename("vcshop_order_goods") . " og left join " . tablename("vcshop_goods") . " g on g.id=og.goodsid where og.orderid=" . $order["id"]);
            if (!empty($goods)) {
                foreach ($goods as $g) {
