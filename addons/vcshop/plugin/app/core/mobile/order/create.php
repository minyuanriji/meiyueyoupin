...
class Create_VcShopPage extends AppMobilePage
{
		...
		if (!$packageid) {
			...
			if (empty($id)) {
				if (!empty($quickid)) {
					...
					$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,g.isfullback,' . ' g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.manydeduct2,g.`virtual`,o.`virtual` as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale' . ' FROM ' . tablename('vcshop_member_cart') . ' c ' . ' left join ' . tablename('vcshop_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('vcshop_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc';
					$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
				}
				...
				if (!empty($optionid)) {
					$option = pdo_fetch('select * from ' . tablename('vcshop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $optionid));
			...
			foreach ($goods as &$g) {
				...
				if (!empty($g['seckillinfo']) && $g['seckillinfo']['status'] == 0) {
					...
					if (0 < $g['usermaxbuy']) {
						$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $g['goodsid'], ':uniacid' => $uniacid, ':openid' => $openid));
			...
			if ($isverify) {
				...
				if (empty($storeids)) {
					if (0 < $merchid) {
						$stores = pdo_fetchall('select * from ' . tablename('vcshop_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
					}
					else {
						$stores = pdo_fetchall('select * from ' . tablename('vcshop_store') . ' where  uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
					}
				}
				else if (0 < $merchid) {
					$stores = pdo_fetchall('select * from ' . tablename('vcshop_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else {
					$stores = pdo_fetchall('select * from ' . tablename('vcshop_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
				}
				...
				if ($isforceverifystore) {
					...
					if (0 < $merch_id) {
						$store_list = pdo_fetchall('select * from ' . tablename('vcshop_merch_store') . ' where ' . $storeids_condition . '  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merch_id));
					}
					else {
						$store_list = pdo_fetchall('select * from ' . tablename('vcshop_store') . ' where  ' . $storeids_condition . '  uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
					}
				}
			}
			else {
			...
			if ($g['seckillinfo'] && $g['seckillinfo']['status'] == 0) {
				...
				if ($giftid) {
					$gift = array();
					$giftdata = pdo_fetch('select giftgoodsid,activity,orderprice from ' . tablename('vcshop_gift') . ' where uniacid = ' . $uniacid . ' and id = ' . $giftid . ' and status = 1 and starttime <= ' . time() . ' and endtime >= ' . time() . ' ');
					...
					foreach ($gifts as $key => $value) {
						...
						foreach ($giftgoods as $k => $val) {
							$giftinfo = pdo_fetch('select id,title,thumb,marketprice,total from ' . tablename('vcshop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2  and id = ' . $val . ' ');
			...
			foreach ($g as $key => $value) {
				$goods[$key] = pdo_fetch('select id,title,thumb,marketprice,merchid,dispatchtype,dispatchid,dispatchprice from ' . tablename('vcshop_goods') . '
				...
				if (0 < $value['optionid']) {
					$option = pdo_fetch('select title,packageprice from ' . tablename('vcshop_package_goods_option') . '
					...
					$packagegoods = pdo_fetch('select title,packageprice from ' . tablename('vcshop_package_goods') . '
		...
		if (0 < $packageid) {
			...
			if (is_array($goodsarr)) {
				...
				foreach ($goodsarr as $key => $value) {
					$goods[$key] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('vcshop_goods') . '
                            where id = ' . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
					$option = array();
					$packagegoods = array();
			...
			if (is_array($goodsarr)) {
				...
				foreach ($goodsarr as &$g) {
					...
					if (!empty($optionid)) {
						$option = pdo_fetch('select id,title,marketprice,presellprice,goodssn,productsn,stock,`virtual`,weight,cycelbuy_periodic from ' . tablename('vcshop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
	...
	public function submit()
	{
		...
		if ($giftid) {
			...
			if ($giftdata['giftgoodsid']) {
				...
				foreach ($giftgoodsid as $key => $value) {
					$giftinfo = pdo_fetch('select id as goodsid,title,thumb,status from ' . tablename('vcshop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and id = ' . $value . ' and deleted = 0 ');
					$gift[$key] = $giftinfo;
		...
		foreach ($goods as $g) {
			...
			if ($data['seckillinfo'] && $data['seckillinfo']['status'] == 0) {
				...
				if (0 < $data['usermaxbuy']) {
					$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $data['goodsid'], ':uniacid' => $uniacid, ':openid' => $openid));
			...
			if (!empty($optionid)) {
				...
				if ($packageid) {
					$packageoption = pdo_fetch('select packageprice from ' . tablename('vcshop_package_goods_option') . '
				...
				if ($packageid) {
					$pg = pdo_fetch('select packageprice from ' . tablename('vcshop_package_goods') . '
			...
			if (intval($_GPC['fromcart']) == 1) {
				...
				$cartdata = pdo_fetch('select id,diyformdataid,diyformfields,diyformdata from ' . tablename('vcshop_quick_cart') . ' ' . ' where goodsid=:goodsid and optionid=:optionid and openid=:openid and deleted=0 order by id desc limit 1', array(':goodsid' => $data['goodsid'], ':optionid' => intval($data['optionid']), ':openid' => $openid));
		...
		if (is_array($carrier)) {
			...
			pdo_update('vcshop_member', $up, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
		...
		if (empty($virtualid)) {
			...
			if (isset($allgoods[0])) {
				$vgoods = $allgoods[0];
				pdo_update('vcshop_goods', array('sales' => $vgoods['sales'] + $vgoods['total']), array('id' => $vgoods['goodsid']));
			}
		}
