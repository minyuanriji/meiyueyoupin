...
class Article_VcShopPage extends AppMobilePage
{
	public function get_list()
	{
		global $_GPC;
		global $_W;
		$categorys = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_article_category') . ' WHERE uniacid=:uniacid and isshow=1 order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
		$page = intval($_GPC['page']);
		$article_sys = pdo_fetch('select * from ' . tablename('vcshop_article_sys') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		...
		if (intval($article_sys['article_temp']) == 0) {
			$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('vcshop_article') . ' a left join ' . tablename('vcshop_article_category') . ' c on c.id=a.article_category  WHERE a.article_state=1 and article_visit=0 and c.isshow=1 and a.uniacid= :uniacid', array(':uniacid' => $_W['uniacid']));
			$articles = pdo_fetchall('SELECT a.id, a.article_title, a.resp_img, a.article_rule_credit, a.article_rule_money, a.resp_desc, a.article_category FROM ' . tablename('vcshop_article') . ' a left join ' . tablename('vcshop_article_category') . ' c on c.id=a.article_category  WHERE a.article_state=1 and article_visit=0 and c.isshow=1 and a.uniacid= :uniacid order by a.displayorder desc, a.article_date desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid']));
			$articles = set_medias($articles, 'resp_img');
		}
		else if ($article_sys['article_temp'] == 1) {
			$total = pdo_fetchcolumn('SELECT count(distinct article_date_v) FROM ' . tablename('vcshop_article') . ' a left join ' . tablename('vcshop_article_category') . ' c on c.id=a.article_category WHERE a.article_state=1 and article_visit=0 and c.isshow=1 and a.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
			$list = pdo_fetchall('SELECT distinct article_date_v FROM ' . tablename('vcshop_article') . ' a left join ' . tablename('vcshop_article_category') . ' c on c.id=a.article_category WHERE a.article_state=1 and article_visit=0 and c.isshow=1 and a.uniacid=:uniacid order by a.article_date_v desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid']), 'article_date_v');
			foreach ($list as &$a) {
				$a['articles'] = pdo_fetchall('SELECT id,article_title,article_date_v,resp_img,resp_desc,article_category FROM ' . tablename('vcshop_article') . ' WHERE article_state=1 and article_visit=0 and uniacid=:uniacid and article_date_v=:article_date_v order by article_date desc ', array(':uniacid' => $_W['uniacid'], ':article_date_v' => $a['article_date_v']));
			...
			if ($article_sys['article_temp'] == 2) {
				...
				$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('vcshop_article') . ' a left join ' . tablename('vcshop_article_category') . ' c on c.id=a.article_category WHERE a.article_state=1 and c.isshow=1 and a.uniacid=:uniacid ' . $where, array(':uniacid' => $_W['uniacid']));
				$articles = pdo_fetchall('SELECT a.id, a.article_title, a.resp_img, a.article_rule_credit, a.article_rule_money, a.article_author, a.article_date_v, a.resp_desc, a.article_category FROM ' . tablename('vcshop_article') . ' a left join ' . tablename('vcshop_article_category') . ' c on c.id=a.article_category WHERE a.article_state=1 and c.isshow=1 and a.uniacid=:uniacid ' . $where . ' order by a.displayorder desc, a.article_date_v desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid']));
				$articles = set_medias($articles, 'resp_img');
			}
		}
	...
	public function get_detail()
	{
		...
		$article = pdo_fetch('SELECT * FROM ' . tablename('vcshop_article') . ' WHERE id=:aid and article_state=1 and uniacid=:uniacid limit 1 ', array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
		...
		if (!empty($openid) && !empty($myid)) {
			$state = pdo_fetch('SELECT * FROM ' . tablename('vcshop_article_log') . ' WHERE openid=:openid and aid=:aid and uniacid=:uniacid limit 1 ', array(':openid' => $openid, ':aid' => $article['id'], ':uniacid' => $_W['uniacid']));

			if (empty($state['id'])) {
				$insert = array('aid' => $aid, 'read' => 1, 'uniacid' => $_W['uniacid'], 'openid' => $openid);
				pdo_insert('vcshop_article_log', $insert);
				$sid = pdo_insertid();
				pdo_update('vcshop_article', array('article_readnum' => $article['article_readnum'] + 1), array('id' => $article['id']));
			}
			else {
				...
				if ($state['read'] < $readtime) {
					pdo_update('vcshop_article_log', array('read' => $state['read'] + 1), array('id' => $state['id']));
					pdo_update('vcshop_article', array('article_readnum' => $article['article_readnum'] + 1), array('id' => $article['id']));
				}
			}
...
/*
	...
	public function like()
	{
		...
		if (!empty($aid) && !empty($openid)) {
			$state = pdo_fetch('SELECT * FROM ' . tablename('vcshop_article_log') . ' WHERE openid=:openid and aid=:aid and uniacid=:uniacid limit 1 ', array(':openid' => $_W['openid'], ':aid' => $aid, ':uniacid' => $_W['uniacid']));
			if (empty($state['like'])) {
				pdo_update('vcshop_article', 'article_likenum=article_likenum+1', array('id' => $aid));
				pdo_update('vcshop_article_log', array('like' => $state['like'] + 1), array('id' => $state['id']));
	...
	public function confirmjoin()
	{
		...
		$user = pdo_fetch('select * from ' . tablename('vcshop_merch_user') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) 
		{
			pdo_update('vcshop_merch_user', array('status'=>1,'jointime'=>time()), array('id' => $user['id']));
