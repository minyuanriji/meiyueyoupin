...
class Sign_VcShopPage extends AppMobilePage
{
...
//print_r($month);exit;
	...
	public function getRecords()
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('vcshop_sign_records') . ' log where ' . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT * FROM ' . tablename('vcshop_sign_records') . ' where ' . $condition . ' ORDER BY `time` DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
