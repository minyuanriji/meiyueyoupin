...
class Cart_VcShopPage extends AppMobilePage
{
	public function get_cart()
	{
		...
		$sql = 'SELECT f.id,f.total,g.status,f.goodsid,g.total as stock, g.discounts,g.isdiscount_discounts,g.deleted,o.stock as optionstock, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,' . ' g.productprice,o.title as optiontitle,f.optionid,o.specs,g.minbuy,g.maxbuy,g.unit,f.merchid,g.merchsale' . ' ,f.selected FROM ' . tablename('vcshop_member_cart') . ' f ' . ' left join ' . tablename('vcshop_goods') . ' g on f.goodsid = g.id ' . ' left join ' . tablename('vcshop_goods_option') . ' o on f.optionid = o.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC ';
	...
	public function add()
	{
		...
		$goods = pdo_fetch('select id,marketprice,`type`,total,diyformid,diyformtype,diyfields, isverify,merchid,cannotrefund,hasoption from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function update()
	{
		...
		$data = pdo_fetch('select * from ' . tablename('vcshop_member_cart') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));

		if (empty($data)) {
			return app_error(AppError::$NotInCart);
		}

		$goods = pdo_fetch('select id,maxbuy,minbuy,total,unit from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid and status=1 and deleted=0', array(':id' => $data['goodsid'], ':uniacid' => $_W['uniacid']));
		...
		pdo_update('vcshop_member_cart', $arr, array('id' => $id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		return app_json();
	}
	...
	public function select()
	{
		...
		if (!empty($id)) {
			...
			pdo_update('vcshop_member_cart', array('selected' => $select), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		}
	...
	public function count()
	{
		...
		$cartcount = (int) pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('vcshop_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 and selected =1', $params);
