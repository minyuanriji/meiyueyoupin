...
class Team_VcShopPage extends AppMobilePage
{
	...
	public function details()
	{
		...
		$myorder = pdo_fetch('SELECT * FROM ' . tablename('vcshop_groups_order') . ' WHERE uniacid = ' . $uniacid . (' and openid = \'' . $openid . '\' and teamid = ' . $teamid . ' and paytime>0'));
		...
		foreach ($orders as $key => $value) {
			...
			if ($orders[$key]['avatar'] == '') {
				$orders[$key]['avatar'] = tomedia('../addons/vcshop/plugin/groups/template/mobile/default/images/user/' . mt_rand(1, 20) . '.jpg');
			}
		}
		...
		if ($tuan_first_order['is_ladder'] == 1) {
			$ladder = pdo_get('vcshop_groups_ladder', array('id' => $tuan_first_order['ladder_id']));
