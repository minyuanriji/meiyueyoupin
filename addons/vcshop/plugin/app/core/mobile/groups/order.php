...
class Order_VcShopPage extends AppMobilePage
{
	...
	public function details()
	{
		...
		if ($order['more_spec'] == 1) {
			$order['option'] = pdo_get('vcshop_groups_order_goods', array('groups_order_id' => $order['id']));
		}

		if ($order['is_ladder'] == 1) {
			$order['ladder'] = pdo_get('vcshop_groups_ladder', array('id' => $order['ladder_id']));
		}
		...
		$goodRefund = false;
		$groupsSet = pdo_fetch('select goodsid,refundday from ' . tablename('vcshop_groups_set') . 'where uniacid = :uniacid ', array(':uniacid' => $uniacid));
	...
	public function create_order()
	{
		...
		$groups_option = pdo_get('vcshop_groups_goods_option', array('id' => $groups_option_id));
		...
		if ($goods['is_ladder'] == 1 && $type == 'groups') {
			...
			$is_ladder = 1;
			$ladder = pdo_get('vcshop_groups_ladder', array('id' => $ladder_id));
			$sql = 'select count(1) from ' . tablename('vcshop_groups_order') . ' as o
		...
		if (!empty($teamid)) {
			...
			if ($goods['is_ladder'] == 0) {
				$num = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_order') . ' where teamid = :teamid and status > :status and goodid = :goodid and uniacid = :uniacid ', array(':teamid' => $teamid, ':status' => 0, ':goodid' => $goods['id'], ':uniacid' => $uniacid));
				...
				$num = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_order') . ' where teamid = :teamid and status > :status and goodid = :goodid and uniacid = :uniacid and ladder_id = :ladder_id', array(':teamid' => $teamid, ':status' => 0, ':goodid' => $goods['id'], ':uniacid' => $uniacid, ':ladder_id' => $ladder_id));
		...
		if ($_W['ispost'] == true) {
			...
			if (!empty($goods['more_spec'])) {
				...
				pdo_insert('vcshop_groups_order_goods', $groups_order_goods);
			}
	...
	public function finish()
	{
		...
		$order = pdo_fetch('select * from ' . tablename('vcshop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $open_id));
	...
	public function delete()
	{
		...
		$open_id = $_W['openid'];
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select id,status from ' . tablename('vcshop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $open_id));
	...
	public function cancel()
	{
		...
		pdo_update('vcshop_groups_order', array('status' => -1, 'canceltime' => time(), 'cancel_reason' => $cancel_reason), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
		p('groups')->sendTeamMessage($orderid);
		return app_json();
	}
