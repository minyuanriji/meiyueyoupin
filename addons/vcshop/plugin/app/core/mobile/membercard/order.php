...
class Order_VcShopPage extends AppMobilePage
{
	public function create_order()
	{
		...
		$card = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_card') . ' 
		...
		$wait_pay = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_card_order') . (' 
				WHERE status =0 ' . $condition . ' limit 1'), $params);

		if ($wait_pay) {
			$result = pdo_update('vcshop_member_card_order', $data, array('id' => $wait_pay['id'], 'uniacid' => $_W['uniacid']));
			$order_id = $wait_pay['id'];
		}
		else {
			$result = pdo_insert('vcshop_member_card_order', $data);
	...
	public function pay($params = array(), $mine = array())
	{
		...
		$order = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_card_order') . (' 
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . '
		 WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'vcshop', ':tid' => $order['orderno']));
		...
		if (empty($log)) {
			$log = array('uniacid' => $uniacid, 'openid' => $member['uid'], 'module' => 'vcshop', 'tid' => $order['orderno'], 'fee' => $order['total'], 'status' => 0);
		...
		if (!empty($set['pay']['wxapp']) && 0 < $order['total'] && $this->iswxapp) {
			...
			if (!is_error($res)) {
				...
				if (!empty($res['package']) && strexists($res['package'], 'prepay_id=')) {
					...
					pdo_update('vcshop_member_card_order', array('wxapp_prepay_id' => $prepay_id), array('id' => $order_id, 'uniacid' => $_W['uniacid']));
				}
			}
			else {
				$wechat['payinfo'] = $res;
