...
class Package_VcShopPage extends AppMobilePage
{
	/**
     * 商品列表
     */
	public function get_list()
	{
		...
		if ($goodsid) {
			...
			$packages = pdo_fetchall('SELECT id,title,thumb,price,goodsid FROM ' . tablename('vcshop_package') . '
		...
		foreach ($packages as $key => $value) {
			...
			foreach ($goods as $k => $val) {
				$g = pdo_fetch('SELECT id,marketprice,title,thumb FROM ' . tablename('vcshop_goods') . '
	...
	public function get_option()
	{
		...
		$optionid = array();
		$option = array();
		$packgoods = pdo_fetch('SELECT id,title,`option` FROM ' . tablename('vcshop_package_goods') . '
		...
		foreach ($optionid as $key => $value) {
			$option[$key] = pdo_fetch('SELECT id,title,packageprice,optionid,goodsid FROM ' . tablename('vcshop_package_goods_option') . '
