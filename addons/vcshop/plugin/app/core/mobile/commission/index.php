...
class Index_VcShopPage extends Base_VcShopPage
{
	public function main()
	{
		...
		$member['applycount'] = pdo_fetchcolumn('select count(id) from ' . tablename('vcshop_commission_apply') . ' where mid=:mid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
		$openselect = false;
		...
		$posercount = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('vcshop_wxapp_poster') . ' WHERE uniacid=:uniacid AND status=1', array(':uniacid' => $_W['uniacid']));
	...
	public function withdraw()
	{
		...
		if (!empty($agentid)) {
			$data = pdo_fetch('select sum(deductionmoney) as sumcharge from ' . tablename('vcshop_commission_log') . ' where mid=:mid and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $agentid));
	...
	public function apply()
	{
		...
		if (1 <= $level) {
			$level1_orders = pdo_fetchall('select distinct o.id from ' . tablename('vcshop_order') . ' o ' . ' left join  ' . tablename('vcshop_order_goods') . ' og on og.orderid=o.id ' . (' where o.agentid=:agentid and o.status>=3  and og.status1=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.uniacid=:uniacid  group by o.id'), array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
		...
		if ($_W['ispost']) {
			...
			if ($open_redis) {
				...
				$redis_key = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_' . $_W['uniacid'] . '_commission_apply_token_' . $openid;
				$redis = redis();
	...
	public function register()
	{
		...
		if ($_W['ispost']) {
			...
			if ($template_flag == 1) {
				...
				pdo_update('vcshop_member', $m_data, array('id' => $member['id']));
				...
				pdo_update('vcshop_member', $data, array('id' => $member['id']));
		...
		if (empty($set['become'])) {
			...
			$status = 1;
			$moneycount = pdo_fetchcolumn('select sum(price) from ' . tablename('vcshop_order') . (' where uniacid=:uniacid and openid=:openid and status>=' . $order_status . ' limit 1'), array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
