...
class Index_VcShopPage extends AppMobilePage
{
	...
	public function getareas()
	{
		global $_W;
		$set = m('util')->get_area_config_set();
		$path = VCSHOP_PATH . 'static/js/dist/area/Area.xml';
		$path_full = VCSHOP_STATIC . 'js/dist/area/Area.xml';

		if (!empty($set['new_area'])) {
			$path = VCSHOP_PATH . 'static/js/dist/area/AreaNew.xml';
			$path_full = VCSHOP_STATIC . 'js/dist/area/AreaNew.xml';
		}
	...
	public function getstreet()
	{
		...
		if (!empty($citycode) && !empty($areacode)) {
			...
			$path = VCSHOP_STATIC . 'js/dist/area/list/' . $city2 . '/' . $citycode . '.xml';
