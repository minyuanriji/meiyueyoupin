...
class Statistics_VcShopPage extends AppMobileAuthPage
{
	public function main()
	{
		$notice = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_system_copyright_notice') . ' WHERE status=1 ORDER BY displayorder ASC,createtime DESC LIMIT 10');
	...
	public function goods()
	{
		...
		$sql = 'select og.price,og.total,o.createtime,o.ordersn,g.title,g.thumb,g.goodssn,op.goodssn as optiongoodssn,op.title as optiontitle from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id = og.goodsid ' . ' left join ' . tablename('vcshop_goods_option') . ' op on op.id = og.optionid ' . (' where 1 ' . $condition . ' order by ' . $orderby . ' desc ');
		...
		unset($row);
		$total = pdo_fetchcolumn('select  count(*) from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id = og.goodsid ' . (' where 1 ' . $condition), $params);
