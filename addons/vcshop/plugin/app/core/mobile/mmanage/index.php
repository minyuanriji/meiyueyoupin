...
class Index_VcShopPage extends AppMobileAuthPage
{
	...
	{
		global $_W;
		$notices = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_system_copyright_notice') . ' WHERE status=1 ORDER BY displayorder ASC,createtime DESC LIMIT 10');
		$order = $this->order(0);
		...
		if ($open_redis) {
			// $redis_key = 'vc_' . $_W['uniacid'] . '_member_mmanage_index1';
			$redis_key = $_W['setting']['site']['key'] . '_' . $_W['account']['key'].'_' . 'vc_' . $_W['uniacid'] . '_member_mmanage_index1';
			$member_count = m('member')->memberRadisCount($redis_key, false);

			if (!$member_count) {
				$member_count = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	private function selectOrderPrice($day = 0)
	{
		...
		$sql = 'select id,price,createtime from ' . tablename('vcshop_order') . ' where uniacid = :uniacid and ismr=0 and isparent=0 and (status > 0 or ( status=0 and paytype=3)) and deleted=0 and createtime between :createtime1 and :createtime2';
	...
	private function newmember($day = 0)
	{
		...
		if ($open_redis) {
			// $redis_key = 'vc_' . $_W['uniacid'] . '_member_mmanage_index2';
			$redis_key = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_' . 'vc_' . $_W['uniacid'] . '_member_mmanage_index2';
			$member_count = m('member')->memberRadisCount($redis_key, false);

			if (!$member_count) {
				$member_count = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member') . ' where uniacid=:uniacid', $param);
				m('member')->memberRadisCount($redis_key, $member_count);
			}
		}
		else {
	...
	private function selectMemberCreate($day = 0)
	{
		...
		$sql = 'select count(*) from ' . tablename('vcshop_member') . ' where uniacid = :uniacid and createtime between :createtime1 and :createtime2';
		$param = array(':uniacid' => $_W['uniacid'], ':createtime1' => $createtime1, ':createtime2' => $createtime2);
