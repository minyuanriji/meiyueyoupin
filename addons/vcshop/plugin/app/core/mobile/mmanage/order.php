...
require VCSHOP_PLUGIN . 'app/core/page_auth_mobile.php';
class Order_VcShopPage extends AppMobileAuthPage
{
	...
	public function get_list()
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$sqlcondition = ' inner join ( select  og.orderid from ' . tablename('vcshop_order_goods') . ' og left join ' . tablename('vcshop_goods') . (' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id');
			}
			else if ($searchfield == 'goodssn') {
				$sqlcondition = ' inner join ( select og.orderid from ' . tablename('vcshop_order_goods') . ' og left join ' . tablename('vcshop_goods') . (' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id');
			}
			else if ($searchfield == 'goodsoptiontitle') {
				$sqlcondition = ' inner join ( select  og.orderid from ' . tablename('vcshop_order_goods') . ' og left join ' . tablename('vcshop_goods') . (' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,og.optionname)>0)) gs on gs.orderid=o.id');
			}
			else {
		...
		if ($condition != ' o.uniacid = :uniacid and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
			$sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress,
                  ...
                  r.rtype,r.status as rstatus,o.sendtype from ' . tablename('vcshop_order') . ' o' . ' left join ' . tablename('vcshop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('vcshop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . ' left join ' . tablename('vcshop_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('vcshop_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('vcshop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('vcshop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . (' ' . $sqlcondition . ' where ' . $condition . ' ' . $statuscondition . ' GROUP BY o.id ORDER BY o.createtime DESC  ');
			...
			$sql = 'select * from ' . tablename('vcshop_order') . (' where uniacid = :uniacid and deleted=0 and isparent=0 ' . $status_condition . ' GROUP BY id ORDER BY createtime DESC  ');
			...
			if (!empty($list)) {
				...
				$openid_array = pdo_fetchall('SELECT openid,nickname,id as mid,realname as mrealname,mobile as mmobile FROM ' . tablename('vcshop_member') . (' WHERE openid IN (' . $openid . ') AND uniacid=' . $_W['uniacid']), array(), 'openid');
		...
		if (!empty($list)) {
			...
			foreach ($list as &$value) {
				...
				$order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' left join ' . tablename('vcshop_goods_option') . ' op on og.optionid = op.id ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $uniacid, ':orderid' => $value['id']));
...
';
		...
		if ($condition != ' o.uniacid = :uniacid and and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('vcshop_order') . ' o ' . ' left join ' . tablename('vcshop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('vcshop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('vcshop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('vcshop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('vcshop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . (' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition), $paras);
		}
		else {
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('vcshop_order') . (' WHERE uniacid = :uniacid and deleted=0 and isparent=0 ' . $status_condition), $paras);
		}
	...
	public function get_detail()
	{
		...
		if (0 < $item['sendtype']) {
			...
			foreach ($order_goods as $key => $value) {
				$order_goods[$key]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $value['sendtype'] . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $id));
		...
		if (empty($item['addressid'])) {
			...
			if (!is_array($user_address)) {
				$user_address = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
			}
	...
	public function remarksaler()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('vcshop_order', array('remarksaler' => $remarksaler), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	public function payorder()
	{
		...
		if (!empty($item['virtual']) && com('virtual')) {
			...
			pdo_update('vcshop_order', array('status' => 1, 'paytype' => 11, 'paytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function send()
	{
		...
		if ($_W['ispost']) {
			if ($item['city_express_state'] == 0) {
				...
				if (!empty($sendtype)) {
					...
					$ogoods = array();
					$ogoods = pdo_fetchall('select sendtype from ' . tablename('vcshop_order_goods') . '
					...
					if ($goodsids) {
						foreach ($goodsids as $key => $value) {
							pdo_update('vcshop_order_goods', $data, array('orderid' => $item['id'], 'goodsid' => $value, 'uniacid' => $_W['uniacid']));
						}
					}

					$send_goods = pdo_fetch('select * from ' . tablename('vcshop_order_goods') . '
					...
					pdo_update('vcshop_order', $senddata, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
				}
				else {
					$data['status'] = 2;
					pdo_update('vcshop_order', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
				}
			}
			else {
			...
			if (!empty($item['refundid'])) {
				$refund = pdo_fetch('select * from ' . tablename('vcshop_order_refund') . ' where id=:id limit 1', array(':id' => $item['refundid']));

				if (!empty($refund)) {
					pdo_update('vcshop_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $item['refundid']));
					pdo_update('vcshop_order', array('refundstate' => 0), array('id' => $item['id']));
				}
			}
		...
		if (0 < $item['sendtype']) {
			$noshipped = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = 0 and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$i = 1;

			while ($i <= $item['sendtype']) {
				$row = array('sendtype' => $i, 'goods' => pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = ' . $i . ' and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id'])));
		...
		$order_goods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
		$order_goods = set_medias($order_goods, 'thumb');
		$address = iunserializer($item['address']);

		if (!is_array($address)) {
			$address = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
		}
...
' . $remark;
			...
			if (0 < $item['sendtype']) {
				...
				if (is_array($sendtype)) {
					foreach ($sendtype as $key => $value) {
						$data['sendtype'] = 0;
						pdo_update('vcshop_order_goods', $data, array('orderid' => $item['id'], 'sendtype' => $value, 'uniacid' => $_W['uniacid']));
						$order = pdo_fetch('select sendtype from ' . tablename('vcshop_order') . ' where id = ' . $item['id'] . ' and uniacid = ' . $_W['uniacid'] . ' ');
						pdo_update('vcshop_order', array('sendtype' => $order['sendtype'] - 1, 'status' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
					}
				}
			}
			else {
		...
		if (0 < $sendtype) {
			$sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $sendtype . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			...
			if (0 < $item['sendtype']) {
				...
				while ($i <= intval($item['sendtype'])) {
					$bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $i . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function changeaddress()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('vcshop_order', array('address' => $address_array), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	public function changeexpress()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $item['sendtype']) {
				...
				if (is_array($sendtype)) {
					foreach ($sendtype as $key => $value) {
						pdo_update('vcshop_order_goods', $change_data, array('orderid' => $orderid, 'sendtype' => $value, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			else {
				pdo_update('vcshop_order', $change_data, array('id' => $orderid, 'uniacid' => $_W['uniacid']));
			}
		...
		if (0 < $sendtype) {
			$sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $sendtype . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$sendgoods = set_medias($sendgoods, 'thumb');
		}
		else {
			if (0 < $item['sendtype']) {
				...
				while ($i <= intval($item['sendtype'])) {
					$bundle = array('goods' => pdo_fetchall('select g.id,g.title,og.optionname,og.total,g.thumb,og.sendtype,g.ispresell from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $i . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id'])), 'sendtype' => $i);
	...
	public function finish()
	{
		...
		$orderid = intval($_GPC['id']);
		$item = $this->getOrder($orderid);
		pdo_update('vcshop_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function fetch()
	{
		...
		pdo_update('vcshop_order', $d, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		...
		if (!empty($item['refundid'])) {
			...
			if (!empty($refund)) {
				pdo_update('vcshop_order_refund', array('status' => -1), array('id' => $item['refundid']));
				pdo_update('vcshop_order', array('refundstate' => 0), array('id' => $item['id']));
			}
		}
	...
	public function changeprice()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($changegoodsprice as $ogid => $change) {
				$og = pdo_fetch('select price,realprice from ' . tablename('vcshop_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));
			...
			if (!empty($orderupdate)) {
				pdo_update('vcshop_order', $orderupdate, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
			...
			foreach ($changegoodsprice as $ogid => $change) {
				$og = pdo_fetch('select price,realprice,changeprice from ' . tablename('vcshop_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));

				if (!empty($og)) {
					$realprice = $og['realprice'] + $change;
					$changeprice = $og['changeprice'] + $change;
					pdo_update('vcshop_order_goods', array('realprice' => $realprice, 'changeprice' => $changeprice), array('id' => $ogid));
				}
			}
