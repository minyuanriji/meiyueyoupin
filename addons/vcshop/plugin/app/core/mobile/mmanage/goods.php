...
class Goods_VcShopPage extends AppMobileAuthPage
{
	...
	public function get_list()
	{
		...
		$sql = 'SELECT count(g.id) FROM ' . tablename('vcshop_goods') . 'g' . $condition;
		$total = pdo_fetchcolumn($sql, $params);

		if (0 < $total) {
			$presize = ($pindex - 1) * $psize - $offset;
			$sql = 'SELECT g.id, g.title, g.thumb, g.merchid, g.minprice, g.maxprice, g.total, g.sales,g.salesreal FROM ' . tablename('vcshop_goods') . 'g' . $condition . ' ORDER BY g.`status` DESC, g.`displayorder` DESC,
	...
	public function get_detail()
	{
		...
		if (!empty($id)) {
			...
			$item = pdo_fetch('SELECT ' . $fields . ' FROM ' . tablename('vcshop_goods') . ' WHERE id = :id and uniacid = :uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		$dispatch_data = pdo_fetchall('select id, dispatchname from ' . tablename('vcshop_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid'], ':merchid' => intval($merchid)));
		...
		if (com('virtual')) {
			$virtual_list = pdo_fetchall('SELECT id,uniacid,title,usedata,alldata FROM ' . tablename('vcshop_virtual_type') . ' WHERE uniacid=:uniacid AND merchid=:merchid AND recycled = 0 ORDER BY id ASC', array(':uniacid' => $_W['uniacid'], ':merchid' => 0));
		}
		...
		if (!empty($item['hasoption'])) {
			$specs = pdo_fetchall('select id,title as value from ' . tablename('vcshop_goods_spec') . ' where goodsid = :goodsid and uniacid = :uniacid order by displayorder asc ', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));

			foreach ($specs as $key => $value) {
				$specid[$key] = $value['id'];
				$spectitle[$key] = $value['value'];
				$specs[$key]['specitem'] = pdo_fetchall('select id,title as value from ' . tablename('vcshop_goods_spec_item') . ' where specid = :specid and uniacid = :uniacid order by displayorder asc ', array(':specid' => $value['id'], ':uniacid' => $_W['uniacid']));
			}

			$optionlist = pdo_fetchall('select id as optionid,specs as id,title as value,stock,productprice,marketprice from ' . tablename('vcshop_goods_option') . ' where goodsid = :goodsid and uniacid = :uniacid order by displayorder asc ', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function submit()
	{
		...
		if (!empty($id)) {
			...
			$item = pdo_fetch('SELECT ' . $fields . ' FROM ' . tablename('vcshop_goods') . ' WHERE id = :id and uniacid = :uniacid LIMIT 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
		...
		if (!empty($item)) {
			pdo_update('vcshop_goods', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		...
		if (!empty($data['hasoption'])) {
			...
			pdo_update('vcshop_goods', array('total' => $totalstocks), array('id' => $id, 'uniacid' => $_W['uniacid']));
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id, title FROM ' . tablename('vcshop_goods') . (' WHERE id in( ' . $id . ' ) AND uniacid=:uniacid'), array(':uniacid' => $_W['uniacid']));

		if (!empty($items)) {
			foreach ($items as $item) {
				pdo_update('vcshop_goods', array('deleted' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function status()
	{
		...
		if (!empty($items)) {
			foreach ($items as $item) {
				pdo_update('vcshop_goods', array('status' => $status), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function restore()
	{
		...
		if (!empty($items)) {
			foreach ($items as $item) {
				pdo_update('vcshop_goods', array('deleted' => 0, 'status' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
