...
require VCSHOP_PLUGIN . "app/core/page_mobile.php";
class Goods_VcShopPage extends AppMobilePage
{
    /**
     * 商品列表
     */
    ...
    public function get_detail()
    {
        ...
        $goods["params"] = pdo_fetchall("SELECT * FROM " . tablename("vcshop_goods_param") . " WHERE uniacid=:uniacid and goodsid=:goodsid order by displayorder asc", array(":uniacid" => $uniacid, ":goodsid" => $goods["id"]));
        ...
        $grftarray = array();
        $i = 0;
        $gifts = pdo_fetchall("select id,goodsid,giftgoodsid,thumb,title from " . tablename("vcshop_gift") . " where uniacid = :uniacid and activity = 2 and status = 1 and starttime <= :starttime and endtime >= :endtime ", array(":uniacid" => $uniacid, ":starttime" => time(), ":endtime" => time()));
            ...
            foreach ($gid as $ke => $val) {
                if ($val == $id) {
                    ...
                    foreach ($giftgoods as $k => $val) {
                        $giftdata = pdo_fetch("select id,title,thumb,marketprice from " . tablename("vcshop_goods") . " where uniacid = :uniacid and deleted = 0 and total > 0 and status = 2 and id = :id ", array(":uniacid" => $uniacid, ":id" => $val));
                        if (!empty($giftdata)) {
                            $isgift = 1;
        ...
        $labelstyle = pdo_fetch("SELECT id,uniacid,style FROM " . tablename("vcshop_goods_labelstyle") . " WHERE uniacid=:uniacid LIMIT 1", array(":uniacid" => $uniacid));
        ...
        if ($goods["isverify"] == 2) {
            ...
            if (empty($storeids)) {
                if (0 < $merchid) {
                    $stores = pdo_fetchall("select * from " . tablename("vcshop_merch_store") . " where  uniacid=:uniacid and merchid=:merchid and status=1 ", array(":uniacid" => $_W["uniacid"], ":merchid" => $merchid));
                } else {
                    $stores = pdo_fetchall("select * from " . tablename("vcshop_store") . " where  uniacid=:uniacid and status=1", array(":uniacid" => $_W["uniacid"]));
                }
            } else {
                if (0 < $merchid) {
                    $stores = pdo_fetchall("select * from " . tablename("vcshop_merch_store") . " where id in (" . implode(",", $storeids) . ") and uniacid=:uniacid and merchid=:merchid and status=1", array(":uniacid" => $_W["uniacid"], ":merchid" => $merchid));
                } else {
                    $stores = pdo_fetchall("select * from " . tablename("vcshop_store") . " where id in (" . implode(",", $storeids) . ") and uniacid=:uniacid and status=1", array(":uniacid" => $_W["uniacid"]));
                }
            }
        }
        unset($goods["pcate"]);
    ...
    public function get_content()
    {
        ...
        $uniacid = $_W["uniacid"];
        $id = intval($_GPC["id"]);
        $goods = pdo_fetch("select content from " . tablename("vcshop_goods") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $id, ":uniacid" => $_W["uniacid"]));
    ...
    public function get_picker()
    {
        ...
        if (!empty($goods) && $goods["hasoption"]) {
            ...
            foreach ($specs as &$spec) {
                $spec["items"] = pdo_fetchall("select *  from " . tablename("vcshop_goods_spec_item") . " where specid=:specid and `show`=1 order by displayorder asc", array(":specid" => $spec["id"]));
            }
            unset($spec);
            $options = pdo_fetchall("select *  from " . tablename("vcshop_goods_option") . " where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc", array(":goodsid" => $id, ":uniacid" => $_W["uniacid"]));
        }
        $minprice = $goods["minprice"];
        $maxprice = $goods["maxprice"];
        if ($goods["isdiscount"] && time() <= $goods["isdiscount_time"]) {
        ...
        if ($goods["isnodiscount"] == 0) {
            ...
            if (!empty($member_levelid)) {
                $member_level = pdo_fetch("select * from " . tablename("vcshop_member_level") . " where id=:id and uniacid=:uniacid and enabled=1 limit 1", array(":id" => $member_levelid, ":uniacid" => $_W["uniacid"]));
        ...
        if ($sale_plugin) {
            ...
            $gifts = array();
            $giftgoods = array();
            $gifts = pdo_fetchall("select id,goodsid,giftgoodsid,thumb,title from " . tablename("vcshop_gift") . " where uniacid = " . $_W["uniacid"] . " and activity = 2 and status = 1 and starttime <= " . time() . " and endtime >= " . time() . "  ");
            foreach ($gifts as $key => &$value) {
                $gid = explode(",", $value["goodsid"]);
                foreach ($gid as $ke => $val) {
                    if ($val == $id) {
                        $giftgoods = explode(",", $value["giftgoodsid"]);
                        foreach ($giftgoods as $k => $val) {
                            $giftdata = pdo_fetch("select id,title,thumb,marketprice,total from " . tablename("vcshop_goods") . " where uniacid = " . $_W["uniacid"] . " and deleted = 0 and total>0  and status = 2 and id = " . $val . " ");
                            if (!empty($giftdata)) {
                                $isgift = 1;
                                $gifts[$key]["gift"][$k] = $giftdata;
