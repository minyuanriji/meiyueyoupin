...
if (!class_exists('AppModel')) {
	class AppModel extends PluginModel
	{
		...
		public function wxpay($params, $type = 0)
		{
			...
			$package['notify_url'] = $_W['siteroot'] . 'addons/vcshop/payment/wechat/notify.php';
		...
		public function getAccessToken()
		{
			...
			$cacheKey = 'vcshop:wxapp:accesstoken:' . $_W['uniacid'];
		...
		{
			global $_W;
			$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_wxapp_tmessage') . ' WHERE id=:id AND uniacid=:uniacid AND status=1 LIMIT 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		public function getGlobal()
		{
			...
			$path = IA_ROOT . '/addons/vcshop/data/global';
								...
								if ($item['id'] == 'menu' && !empty($item['style']['showtype'])) {
									...
									if ($item['id'] == 'picturew' && !empty($item['params']['showtype'])) {
										...
										if ($item['id'] == 'notice' && empty($item['params']['noticedata'])) {
											...
											$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('vcshop_notice') . (' where uniacid=:uniacid and status=1 order by displayorder desc limit ' . $limit), array(':uniacid' => $_W['uniacid']));
											$item['data'] = array();
											...
											if (!empty($item['data'])) {
												...
												if ($item['params']['saleout'] == 0 && !empty($_W['shopset']['shop']['saleout'])) {
													...
													if (empty($item['params']['saleout'])) {
														$item['params']['saleout'] = tomedia('../addons/vcshop/plugin/diypage/static/images/default/saleout-2.png');
													}
												}
												else {
													if ($item['params']['saleout'] == 1 && !empty($item['style']['saleoutstyle'])) {
														$item['params']['saleout'] = tomedia('../addons/vcshop/plugin/diypage/static/images/default/saleout-' . $item['style']['saleoutstyle'] . '.png');
													}
												}
											}
											else {
				...
				if (empty($item['params']['goodsdata']) && !empty($item['data']) && is_array($item['data'])) {
					...
					if ($item['params']['goodsdata'] == 1 && !empty($item['params']['cateid']) && $mobile) {
						...
						if ($item['params']['goodsdata'] == 2 && !empty($item['params']['groupid']) && $mobile) {
							$group = pdo_fetch('SELECT * FROM ' . tablename('vcshop_goods_group') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['groupid'], ':uniacid' => $_W['uniacid']));
							if (!empty($group) && !empty($group['goodsids'])) {
								...
								$item['params']['goodsnum'] = !empty($item['params']['goodsnum']) ? $item['params']['goodsnum'] : 20;
								$goods = pdo_fetchall('select id, title, subtitle, thumb, minprice, sales, salesreal, total, showlevels, showgroups, bargain, productprice,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,video,`type` from ' . tablename('vcshop_goods') . (' where id in( ' . $group['goodsids'] . ' ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid ') . $orderby . (' limit ' . $item['params']['goodsnum']), array(':uniacid' => $_W['uniacid']));
							}
						}
						else {
				...
				if (!empty($goods) && is_array($goods)) {
					...
					foreach ($goods as $key => $value) {
						if ($value['hasoption'] == 1) {
							$pricemax = array();
							$options = pdo_fetchall('select * from ' . tablename('vcshop_goods_option') . ' where goodsid=:goodsid and                               uniacid=:uniacid order by displayorder asc', array(':goodsid' => $value['id'], ':uniacid' => $_W['uniacid']));
        ...
        //return error(-1, '站点未注册');
			...
			load()->func('communication');
			$request = ihttp_get(VCSHOP_AUTH_WXAPP . 'index/index/getAuth.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].$uri);
...
public function getRelease($authid,$uri='')
    ...
    global $_W;
			...
			load()->func('communication');
			$request = ihttp_get(VCSHOP_AUTH_WXAPP . 'index/index/getRelease2.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].$uri);
		...
		public function getReleaseList()
		{
			...
			load()->func('communication');
			$request = ihttp_get(VCSHOP_AUTH_WXAPP . ('xcxapi/auth-xcx-info/list?site_id=' . $siteid . '&uniacid=' . $_W['uniacid']));
		...
		{
			load()->func('communication');
			$request = ihttp_get(VCSHOP_AUTH_URL . ('wxapp_log?pindex=' . $page . '&psize=' . $psize));
			$content = json_decode($request['content'], true);
			return $content;
		}
		...
		{
			$path1 = __DIR__ . '/mobile/vcshop_api.php';
			$path2 = IA_ROOT . '/app/vcshop_api.php';
		...
		public function createAuth()
		{
			...
			$auth = get_auth();
			load()->func('communication');
			$resp = ihttp_request(VCSHOP_AUTH_URL . 'app', array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain, 'appid' => $_W['account']['key']), NULL, 1);
		...
		public function diyMenu($menuid)
		{
			...
			$menu = pdo_fetch('select * from ' . tablename('vcshop_diypage_menu') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		public function getUrl($url)
		{
			...
			if (strexists($url, './index.php?') && strexists($url, 'vcshop') && strexists($url, 'mobile')) {
				$parse = parse_url($url);
				...
				if ($newVars['m'] != 'vcshop' || $newVars['do'] != 'mobile') {
		...
		{
			$file = VCSHOP_PLUGIN . 'app/static/iconfont.json';
		...
		{
			global $_W;
			$arr = array('app_id' => $config['appid'], 'method' => 'alipay.trade.app.pay', 'format' => 'JSON', 'charset' => 'utf-8', 'sign_type' => 'RSA2', 'timestamp' => date('Y-m-d H:i:s', time()), 'version' => '1.0', 'notify_url' => $_W['siteroot'] . 'addons/vcshop/payment/alipay/notify.php', 'biz_content' => json_encode(array('timeout_express' => '90m', 'product_code' => 'QUICK_MSECURITY_PAY', 'total_amount' => $params['total_amount'], 'subject' => $params['subject'], 'body' => $params['body'], 'out_trade_no' => $params['out_trade_no'])));
			ksort($arr);
			$string1 = '';
