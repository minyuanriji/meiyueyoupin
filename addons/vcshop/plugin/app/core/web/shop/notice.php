...
class Notice_VcShopPage extends WebPage
{
	...
	protected function post()
	{
		...
		$notice = pdo_fetch('SELECT * FROM ' . tablename('vcshop_notice') . ' WHERE id =:id and uniacid=:uniacid and iswxapp=1 limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		include $this->template();
	}
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_notice') . (' WHERE id in( ' . $id . ' ) and iswxapp=1 AND uniacid=') . $_W['uniacid']);
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_notice') . (' WHERE id in( ' . $id . ' ) and iswxapp=1 AND uniacid=') . $_W['uniacid']);
