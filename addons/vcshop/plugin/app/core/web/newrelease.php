...
class Newrelease_VcShopPage extends PluginWebPage
{
	...
	public function main()
	{
		...
		if (is_error($auth)) {
			...
			$authUrl = VCSHOP_AUTH_WXAPP . 'auth/auth?id=' . $auth['id'];
			...
			$log = pdo_fetchall('select * from ' . tablename('vcshop_upwxapp_log') . ' where uniacid=:uniacid and type=1 order by id desc', array(':uniacid' => $_W['uniacid']));
			$test_code = IA_ROOT . '/addons/vcshop/plugin/app/static/images/test_code_' . $_W['uniacid'] . '.jpg';
			$version_time = 0;
			...
			$wxcode = IA_ROOT . '/addons/vcshop/plugin/app/static/images/wxcode_' . $_W['uniacid'] . '.jpg';
	...
	public function upload()
	{
		...
		$last_log = pdo_fetch('select * from ' . tablename('vcshop_upwxapp_log') . ' where uniacid=:uniacid and type=1 order by id desc limit 1', array(':uniacid' => $_W['uniacid']));
		@session_start();
		...
		if (empty($ticket)) {
			$need_scan = 1;
			load()->func('communication');
			$res = ihttp_get(VCSHOP_AUTH_WXAPP . 'generate/getqrcode');
		...
		$auth = get_auth();
		$response = ihttp_get(VCSHOP_AUTH_WXAPP . 'generate/check-version?site_id=' . $auth['id']);
	...
	public function getstatus()
	{
		...
		$res = ihttp_get(VCSHOP_AUTH_WXAPP . 'generate/getstatus?uuid=' . $uuid);
		$content = json_decode($res['content'], true);
	...
	public function getticket()
	{
		...
		$res = ihttp_get(VCSHOP_AUTH_WXAPP . 'generate/getticket?code=' . $code);
		$content = json_decode($res['content'], true);
	...
	public function submit()
	{
		...
		$diy_str = '';
		$list = pdo_fetchall('SELECT `data` FROM ' . tablename('vcshop_wxapp_page') . ' WHERE uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		...
		load()->func('communication');
		ihttp_request(VCSHOP_AUTH_WXAPP . 'generate/upload?id=' . $auth['id'], array('version' => $version, 'describe' => $describe, 'tabBar' => $tabBar, 'ticket' => $ticket, 'appIds' => $appIds, 'is_goods' => $isGoods), array('Content-Type' => 'application/x-www-form-urlencoded'), 3);
		...
		pdo_insert('vcshop_upwxapp_log', $data);
		show_json(1);
	}
	...
	public function uploadstatus()
	{
		...
		load()->func('communication');
		$response = ihttp_get(VCSHOP_AUTH_WXAPP . 'generate/uploadstatus?id=' . $auth['id']);
		...
		if (empty($response)) {
			...
			if (intval($data['status']) == 202) {
				...
				$wxcode = IA_ROOT . '/addons/vcshop/plugin/app/static/images/test_code_' . $_W['uniacid'] . '.jpg';
