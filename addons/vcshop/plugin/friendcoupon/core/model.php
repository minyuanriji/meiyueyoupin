...
class FriendcouponModel extends PluginModel
{
	...
	{
		global $_W;
		return pdo_fetch('select * from ' . tablename('vcshop_coupon_data') . ' where uniacid = :uniacid and openid = :openid and friendcouponid = :friendcoupoinid', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':friendcouponid' => $friendcouponid));
	}
	...
	{
		global $_W;
		return pdo_fetchall('select * from ' . tablename('vcshop_friendcoupon') . ' where uniacid = :uniacid and deleted = :deleted order by create_time desc', array(':uniacid' => $_W['uniacid'], ':deleted' => 0));
	}
	...
	{
		global $_W;
		return pdo_fetch('select * from ' . tablename('vcshop_friendcoupon_data') . ' where uniacid = :uniacid and openid = :openid and activity_id = :activity_id', array(':openid' => $openid, ':uniacid' => $_W['uniacid'], ':activity_id' => $activity_id));
	}
	...
	{
		global $_W;
		return pdo_fetchall('select * from ' . tablename('vcshop_friendcoupon_data') . ' where uniacid = :uniacid and activity_id = :activity_id and headerid = :headerid', array(':headerid' => $headerid, ':activity_id' => $activity_id, ':uniacid' => $_W['uniacid']));
	}
	...
	{
		global $_W;
		$table = tablename('vcshop_friendcoupon');
	...
	{
		global $_W;
		return pdo_fetchall('select * from ' . tablename('vcshop_friendcoupon_data') . ' where uniacid = :uniacid and headerid = :headerid and activity_id = :activity_id and openid <> \'\'', array(':uniacid' => $_W['uniacid'], ':headerid' => $headerid, ':activity_id' => $activity_id));
	}
	...
	{
		global $_W;
		return pdo_fetch('select * from ' . tablename('vcshop_member') . ' where uniacid = :uniacid and openid = :openid', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
	}
	public function getmemberwx($openid_wx)
    {
        global $_W;
        return pdo_fetch("select * from " . tablename("vcshop_member") . " where uniacid = :uniacid and openid_wa = :openid_wa", array(":uniacid" => $_W["uniacid"], ":openid_wa" => $openid_wx));
    }
	public function getTakePartInPeopleData($activity_id, $headerid)
	{
		global $_W;
		$r = pdo_fetchall('select * from ' . tablename('vcshop_friendcoupon_data') . ' where uniacid = :uniacid and headerid = :headerid and activity_id = :activity_id', array(':uniacid' => $_W['uniacid'], ':headerid' => $headerid, ':activity_id' => $activity_id));
	}
	...
	{
		$path = IA_ROOT . '/addons/vcshop/plugin/friendcoupon/core/redPackBuilder.php';
		require_once $path;
	}
	...
	public function getTemplateMessage($type)
	{
		...
		if ($data['messageId'] == 'default') {
			return pdo_fetch('select * from ' . tablename('vcshop_member_wxapp_message_template_default') . ' where uniacid = :uniacid and typecode = :typecode', array(':uniacid' => $_W['uniacid'], ':typecode' => $typecode));
		}

		return pdo_fetch('select * from ' . tablename('vcshop_wxapp_tmessage') . ' where id = :id', array('id' => $data['messageId']));
	}
