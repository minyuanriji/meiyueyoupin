...
ignore_user_abort();
$time = time();
$table1 = tablename('vcshop_friendcoupon');
$table2 = tablename('vcshop_friendcoupon_data');
...
foreach ($ret as $currentActivityInfo) {
	...
	if ($tm['friendcoupon_launch_close_advanced']) {
		...
		if (!is_error($sendResult)) {
			pdo_update('vcshop_friendcoupon_data', array('send_failed_message' => 1), array('id' => $currentActivityInfo['id']));
		}
	}
	else {
