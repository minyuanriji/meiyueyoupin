...
require_once VCSHOP_INC . 'plugin_processor.php';
...
class PosterProcessor extends PluginProcessor
{
    ...
    {
        global $_W;
        if (is_file(VCSHOP_CORE . 'queue.pid')) {
    ...
    private function responseScan($obj)
    {
        ...
        $poster = pdo_fetch('select * from ' . tablename('vcshop_poster') . ' where type=4 and isdefault=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
        if (empty($poster)) {
            return $this->responseDefault($obj);
        }
        ...
        if (0 < $poster['ismembergroup'] && empty($member['groupid']) && 0 < $poster['membergroupid']) {
            pdo_update('vcshop_member', array('groupid' => $poster['membergroupid']), array('openid' => $openid, 'uniacid' => $_W['uniacid']));
        }
    ...
    private function responseSubscribe($obj)
    {
        ...
        if ($member['isnew']) {
            pdo_update('vcshop_poster', array('follows' => $poster['follows'] + 1), array('id' => $poster['id']));
        }
        $qrmember = m('member')->getMember($qr['openid']);
        $log = pdo_fetch('select * from ' . tablename('vcshop_poster_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid']));
        if (empty($log) && $openid != $qr['openid']) {
            ...
            pdo_insert('vcshop_poster_log', $log);
            $log['id'] = pdo_insertid();
            if (0 < $poster['ismembergroup'] && empty($member['groupid']) && 0 < $poster['membergroupid']) {
                pdo_update('vcshop_member', array('groupid' => $poster['membergroupid']), array('openid' => $openid, 'uniacid' => $_W['uniacid']));
            }
            ...
            if (!empty($upgrade)) {
                pdo_update('vcshop_poster_log', $upgrade, array('id' => $log['id']));
            }
        }
            ...
            if (!empty($cset) && $member['isagent'] != 1 && $qrmember['isagent'] == 1 && $qrmember['status'] == 1) {
                if (!empty($poster['bedown']) && empty($member['agentid']) && empty($member['fixagentid'])) {
                    ...
                    if ($author) {
                        ...
                        pdo_update('vcshop_member', array('agentid' => $qrmember['id'], 'childtime' => $time, 'authorid' => $authorid), array('id' => $member['id']));
                    } else {
                        pdo_update('vcshop_member', array('agentid' => $qrmember['id'], 'childtime' => $time), array('id' => $member['id']));
                    }
                    if (p('dividend')) {
                        $p->saveRelation($member['id'], $qrmember['id'], 1);
                        ...
                        if (!empty($author_set['become']) && ($author_set['become'] == '2' || $author_set['become'] == '5')) {
                            ...
                            if ($can_author) {
                                ...
                                if (empty($member['authorblack'])) {
                                    pdo_update('vcshop_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $qrmember['id']));
                                    if ($become_check == 1) {
