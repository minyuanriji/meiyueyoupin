...
class Index_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_poster') . (' WHERE 1 ' . $condition . ' ORDER BY isdefault desc,createtime desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);

		foreach ($list as &$row) {
			$row['times'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_poster_scan') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['follows'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_poster_log') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
		}

		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_poster') . (' where 1 ' . $condition . ' '), $params);
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_poster') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if ($item['keyword2'] != $data['keyword2'] && !empty($keyword)) {
				if ($keyword['name'] != 'vcshop:poster:' . $data['type']) {
					show_json(0, '关键字已存在!');
				}
			}
			...
			if ($data['isdefault'] == 1) {
				pdo_update('vcshop_poster', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $data['type']));
			}

			if (!empty($id)) {
				pdo_update('vcshop_poster', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'vcshop', ':name' => 'vcshop:poster:' . $data['type']));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'vcshop:poster:' . $data['type'], 'module' => 'vcshop', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'vcshop', 'content' => $data['type'] == 3 ? '^' . trim($data['keyword2']) . '\\+*[0-9]{1,}$' : trim($data['keyword2']), 'type' => $data['type'] == 3 ? 3 : 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
			else {
				$isDefault = pdo_fetchcolumn("select `isdefault` from " . tablename("vcshop_poster") . " where uniacid = :uniacid and id=:id", array(":uniacid" => $_W["uniacid"], ":id" => $id));
                if ($isDefault) {
			...
			$ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'vcshop', ':name' => 'vcshop:poster:auto'));

			if (empty($ruleauto)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'vcshop:poster:auto', 'module' => 'vcshop', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'vcshop', 'content' => 'VCSHOP_POSTER', 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
	...
	public function delete()
	{
		...
		$posters = pdo_fetchall('SELECT id,title,keyword,keyword2 FROM ' . tablename('vcshop_poster') . (' WHERE id in ( ' . $id . ' ) and uniacid=') . $_W['uniacid']);
		...
		foreach ($posters as $poster) {
			...
			pdo_delete('vcshop_poster', array('id' => $poster['id'], 'uniacid' => $_W['uniacid']));
			pdo_delete('vcshop_poster_log', array('posterid' => $poster['id'], 'uniacid' => $_W['uniacid']));
	...
	public function setdefault()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$poster = pdo_fetch('SELECT * FROM ' . tablename('vcshop_poster') . (' WHERE id = \'' . $id . '\''));

		if (empty($poster)) {
			show_json(0, '抱歉，海报不存在或是已经被删除！');
		}
		$rule = pdo_fetch("select * from " . tablename("rule") . " where uniacid=:uniacid and module=:module and name=:name  limit 1", array(":uniacid" => $_W["uniacid"], ":module" => "vcshop", ":name" => "vcshop:poster:" . $poster["type"]));
		pdo_update('vcshop_poster', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $poster['type']));
		pdo_update('vcshop_poster', array('isdefault' => 1), array('uniacid' => $_W['uniacid'], 'id' => $poster['id']));
