...
class GroupsProcessor extends PluginProcessor
{
	public function __construct()
	{
		...
		$this->sessionkey = VCSHOP_PREFIX . 'gorder_wechat_verify_info';
		$this->codekey = VCSHOP_PREFIX . 'gorder_wechat_verify_code';
	}
			...
			if ($obj->inContext) {
				if (is_numeric($content)) {
					if (8 <= strlen($content)) {
						...
						$order = pdo_fetch('select id,orderno,price,goodid from ' . tablename('vcshop_groups_order') . '
