...
class Goods_VcShopPage extends PluginMobileLoginPage
{
	public function main()
	{
		...
		$uniacid = $_W['uniacid'];
		$id = intval($_GPC['id']);
		$groupsset = pdo_fetch('select description,groups_description,discount,headstype,headsmoney,headsdiscount,followbar from ' . tablename('vcshop_groups_set') . 'where uniacid = :uniacid ', array(':uniacid' => $uniacid));
		...
		if ($goods['more_spec'] == 1) {
			...
			$specArr = pdo_getall('vcshop_goods_spec', array('goodsid' => $group_goods['gid'], 'uniacid' => $_W['uniacid']), array('id', 'title'), '', array('displayorder' => 'desc'));

			foreach ($specArr as $k => $v) {
				$specArr[$k]['item'] = pdo_getall('vcshop_goods_spec_item', array('uniacid' => $_W['uniacid'], 'specid' => $v['id']), array('id', 'specid', 'title', 'thumb'), '', array('displayorder' => 'desc'));
			}
		}
	...
	public function openGroups()
	{
		...
		$groupsset = pdo_fetch('SELECT followbar FROM ' . tablename('vcshop_groups_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $uniacid));
		$goods = pdo_fetch('select * from ' . tablename('vcshop_groups_goods') . '
					where id = :id and status = :status and uniacid = :uniacid and deleted = 0 order by displayorder desc', array(':id' => $id, ':uniacid' => $uniacid, ':status' => 1));
		$goods['fightnum'] = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_order') . ' where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ', array(':goodid' => $goods['id'], ':uniacid' => $uniacid));
		$goods['fightnum'] = $goods['teamnum'] + $goods['fightnum'];
		...
		if ($goods['is_ladder'] == 1 && $_GPC['is_ladder'] == 1) {
			$ladder = pdo_getall('vcshop_groups_ladder', array('goods_id' => $id, 'uniacid' => $_W['uniacid']));
			$info = pdo_fetchall('SELECT count(ladder_id) as order_num FROM ' . tablename('vcshop_groups_order') . ' WHERE `uniacid` = :uniacid and `openid`!=:openid and success = 0 and is_team =1 and status>0 and ladder_id >0 and goodid = :goodid group by ladder_id', array(':uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodid' => $id));
		...
		if ($goods['more_spec'] == 1 && $_GPC['more_spec'] == 1) {
			...
			$order_num = pdo_fetchcolumn('SELECT count(id) as order_num FROM ' . tablename('vcshop_groups_order') . ' WHERE `uniacid` = :uniacid and `openid`!=:openid and success = 0 and status>0 and more_spec =1 and is_team =1 and `goodid`=:goodid ', array(':uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodid' => $id));
		}

		if ($goods['is_ladder'] == 0 && $goods['more_spec'] == 0) {
			$order_num = pdo_fetchcolumn('SELECT count(id) as order_num FROM ' . tablename('vcshop_groups_order') . ' WHERE `uniacid` = :uniacid and `openid`!=:openid and success = 0 and is_team =1 and status>0 and `goodid`=:goodid ', array(':uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodid' => $id));
		}

		$teams = pdo_fetchall('select * from ' . tablename('vcshop_groups_goods') . ' where deleted = 0 and status = 1 and uniacid = :uniacid order by sales desc limit 4', array(':uniacid' => $uniacid));

		foreach ($teams as $key => $value) {
			$value['fightnum'] = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_order') . ' where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ', array(':goodid' => $value['id'], ':uniacid' => $uniacid));
	...
	public function fightGroups()
	{
		...
		if ($goods['is_ladder'] == 1 && 0 < $_GPC['ladder_id']) {
			$teams = pdo_fetchall('select o.paytime,o.id,o.goodid,o.ladder_id,o.is_ladder,o.teamid,m.nickname,m.realname,m.mobile,m.avatar,g.endtime,g.groupnum,g.thumb_url,l.ladder_num,l.ladder_price from ' . tablename('vcshop_groups_order') . ' as o
			...
			$teams = pdo_fetchall('select o.paytime,o.id,o.goodid,o.ladder_id,o.is_ladder,o.teamid,m.nickname,m.realname,m.mobile,m.avatar,g.endtime,g.groupnum,g.thumb_url,g.more_spec from ' . tablename('vcshop_groups_order') . ' as o
				left join ' . tablename('vcshop_member') . ' as m on m.openid=o.openid and m.uniacid =  o.uniacid
				left join ' . tablename('vcshop_groups_goods') . ' as g on g.id = o.goodid
		...
		try {
			...
			if ($goods['is_ladder'] == 1) {
				...
				if ($_GPC['fightgroups'] == 1) {
					$info = pdo_fetchall('SELECT ladder_id,count(ladder_id) as order_num FROM ' . tablename('vcshop_groups_order') . ' WHERE `uniacid` = :uniacid and `openid`!=:openid and success = 0 and status>0 and ladder_id >0 and goodid = :goodid group by ladder_id', array(':uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodid' => $id));
	...
	public function get_option()
	{
		...
		if (!empty($specArr)) {
			...
			$goods_option = pdo_get('vcshop_groups_goods_option', array('specs' => $spec_id, 'uniacid' => $_W['uniacid']));
