...
class Orders_VcShopPage extends PluginMobileLoginPage
{
	...
	public function detail()
	{
		global $_W;
		global $_GPC;
		$data = pdo_fetch('SELECT * FROM ' . tablename('vcshop_groups_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
		$openid = $_W['openid'];
		$uniacid = $_W['uniacid'];
		$orderid = intval($_GPC['orderid']);
		$teamid = intval($_GPC['teamid']);
		$condition = ' and openid=:openid  and uniacid=:uniacid and id = :orderid and teamid = :teamid ';
		$order = pdo_fetch('select * from ' . tablename('vcshop_groups_order') . '
		...
		if ($order['refundid'] != 0) {
			$refund = pdo_fetch('SELECT *  FROM ' . tablename('vcshop_groups_order_refund') . ' WHERE orderid = :orderid and uniacid=:uniacid order by id desc', array(':orderid' => $order['id'], ':uniacid' => $_W['uniacid']));
		}

		if ($order['more_spec'] == 1) {
			$option = pdo_fetch('select * from ' . tablename('vcshop_groups_order_goods') . '
				where uniacid=:uniacid and groups_order_id = :orderid', array(':uniacid' => $uniacid, ':orderid' => $orderid));
		}

		if ($order['is_ladder'] == 1) {
			$ladder = pdo_fetch('select * from ' . tablename('vcshop_groups_ladder') . '
				where uniacid=:uniacid and id = :id', array(':uniacid' => $uniacid, ':id' => $order['ladder_id']));
		}

		$good = pdo_fetch('select * from ' . tablename('vcshop_groups_goods') . '
		...
		if (!empty($order['isverify'])) {
			...
			$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_verify') . ' where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':orderid' => $order['id'], ':openid' => $order['openid'], ':uniacid' => $order['uniacid'], ':verifycode' => $order['verifycode']));

			if ($order['verifytype'] == 0) {
				$verify = pdo_fetch('select isverify from ' . tablename('vcshop_groups_verify') . ' where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':orderid' => $order['id'], ':openid' => $order['openid'], ':uniacid' => $order['uniacid'], ':verifycode' => $order['verifycode']));
			}
	...
	public function express()
	{
		...
		$order = pdo_fetch('select * from ' . tablename('vcshop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		$goods = pdo_fetch('select *  from ' . tablename('vcshop_groups_goods') . '  where id=:id and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':id' => $order['goodid']));
	...
	public function cancel()
	{
		...
		try {
			$orderid = intval($_GPC['id']);
			$order = pdo_fetch('select id,orderno,openid,status,credit,teamid,groupnum,creditmoney,price,freight,pay_type,discount,success from ' . tablename('vcshop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
			$total = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_order') . '  where teamid = :teamid  ', array(':teamid' => $order['teamid']));
			...
			pdo_update('vcshop_groups_order', array('status' => -1, 'canceltime' => time()), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		global $_GPC;
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select id,status from ' . tablename('vcshop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		...
		pdo_update('vcshop_groups_order', array('deleted' => 1), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
		show_json(1);
	}
	...
	public function get_list()
	{
		...
		$orders = pdo_fetchall('select o.id,o.orderno,o.createtime,o.price,o.more_spec,o.is_ladder,o.freight,o.ladder_id,o.specs,o.creditmoney,o.goodid,o.teamid,o.status,o.is_team,o.success,o.teamid,o.openid,
				...
				left join ' . tablename('vcshop_groups_goods') . ' as g on g.id = o.goodid
				left join ' . tablename('vcshop_groups_ladder') . ' as l on l.id = o.ladder_id
				left join ' . tablename('vcshop_groups_order_goods') . (' as p on p.groups_order_id = o.id
				where 1 ' . $condition . ' order by o.createtime desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_order') . (' as o where 1 ' . $condition), $params);

		foreach ($orders as $key => $value) {
			$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_verify') . ' where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':orderid' => $value['id'], ':openid' => $value['openid'], ':uniacid' => $value['uniacid'], ':verifycode' => $value['verifycode']));
	...
	public function confirm()
	{
		...
		if ($goods['is_ladder'] == 1) {
			$ladder = pdo_get('vcshop_groups_ladder', array('goods_id' => $goodid, 'uniacid' => $_W['uniacid']));
		}

		if (0 < $_GPC['ladder_id']) {
			$ladder = pdo_get('vcshop_groups_ladder', array('id' => $_GPC['ladder_id'], 'uniacid' => $_W['uniacid']));
		}
		...
		if ($goods['more_spec'] == 1) {
			...
			if (!empty($option_id)) {
				$option = pdo_fetch('select * from ' . tablename('vcshop_groups_goods_option') . ' where goods_option_id=:goods_option_id and groups_goods_id=:groups_goods_id and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':groups_goods_id' => $goods['id'], ':goods_option_id' => $option_id));
			}
		}
		...
		if ($type == 'groups') {
			$ordernum = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_order') . ' as o
		...
		if (!empty($teamid)) {
			$orders = pdo_fetchall('select * from ' . tablename('vcshop_groups_order') . '
			...
			$num = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_order') . ' as o where teamid = :teamid and status > :status and goodid = :goodid and uniacid = :uniacid ', array(':teamid' => $teamid, ':status' => 0, ':goodid' => $goods['id'], ':uniacid' => $uniacid));
		...
		$set = pdo_fetch('select discount,headstype,headsmoney,headsdiscount from ' . tablename('vcshop_groups_set') . '
		...
		if (!empty($goods['isverify'])) {
			...
			while (1) {
				$count = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_groups_order') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
			...
			$address = pdo_fetch('select * from ' . tablename('vcshop_member_address') . '
				where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1', array(':uniacid' => $uniacid, ':openid' => $openid));
		}

		$creditdeduct = pdo_fetch('SELECT creditdeduct,groupsdeduct,credit,groupsmoney FROM' . tablename('vcshop_groups_set') . 'WHERE uniacid = :uniacid ', array(':uniacid' => $uniacid));
		...
		if ($_W['ispost']) {
			...
			if (0 < intval($_GPC['aid']) && !$isverify) {
				$order_address = pdo_fetch('select * from ' . tablename('vcshop_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid   limit 1', array(':uniacid' => $uniacid, ':openid' => $openid, ':id' => intval($_GPC['aid'])));
			...
			$order_insert = pdo_insert('vcshop_groups_order', $data);
			...
			if (empty($teamid) && $type == 'groups') {
				pdo_update('vcshop_groups_order', array('teamid' => $orderid), array('id' => $orderid));
			}
			...
			if (!empty($orderid) && $goods['more_spec'] == 1) {
				...
				pdo_insert('vcshop_groups_order_goods', $_data);
			}
	...
	public function finish()
	{
		...
		global $_GPC;
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select * from ' . tablename('vcshop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		...
		if (0 < $order['refundstate'] && !empty($order['refundid'])) {
			...
			pdo_update('vcshop_groups_order_refund', $change_refund, array('id' => $order['refundid'], 'uniacid' => $_W['uniacid']));
		}

		pdo_update('vcshop_groups_order', array('status' => 3, 'finishtime' => time(), 'refundstate' => 0), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
		p('groups')->sendTeamMessage($orderid);
		show_json(1);
	}
}
