...
class Team_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$teams = pdo_fetchall('SELECT o.* FROM ' . tablename('vcshop_groups_order') . (' as o
					WHERE ' . $condition . ' ORDER BY o.createtime DESC limit ') . ($pindex - 1) * $psize . ',' . $psize, $params);

		foreach ($teams as $key => $value) {
			$good = pdo_fetch('select title from ' . tablename('vcshop_groups_goods') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $value['goodid']);
			$teams[$key]['title'] = $good['title'];
			$teams[$key]['num'] = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('vcshop_groups_order') . ' as o
		...
		if (empty($teams) && !empty($_GPC['keyword']) && $_GPC['searchfield'] == 'orderno') {
			$grouporder = pdo_fetch('select teamid from ' . tablename('vcshop_groups_order') . ' where uniacid = :uniacid and orderno like :orderno ', array(':uniacid' => intval($_W['uniacid']), ':orderno' => '%' . $_GPC['keyword'] . '%'));
			$teams = pdo_fetchall('select * from ' . tablename('vcshop_groups_order') . ' where id = :teamid and uniacid = :uniacid and  paytime > 0 and heads = 1 and is_team = 1', array(':teamid' => $grouporder['teamid'], ':uniacid' => intval($_W['uniacid'])));
	...
	public function detail()
	{
		...
		global $_GPC;
		$teamid = $_GPC['teamid'];
		$teaminfo = pdo_fetch('SELECT o.*,g.id as gid,g.title,g.thumb,g.thumb_url FROM ' . tablename('vcshop_groups_order') . ' as o
					left join ' . tablename('vcshop_groups_goods') . ' as g on g.id = o.goodid
		...
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_groups_order') . ' as o where o.teamid =:teamid and o.uniacid=:uniacid and o.is_team = :is_team and status > :status', array(':uniacid' => $_W['uniacid'], ':teamid' => $teamid, ':is_team' => 1, ':status' => 0));
		$orders = pdo_fetchall('SELECT o.*,g.thumb,g.thumb_url FROM ' . tablename('vcshop_groups_order') . ' as o
					left join ' . tablename('vcshop_groups_goods') . ' as g on g.id = o.goodid
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('vcshop_dispatch') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $teaminfo['dispatchid'], ':uniacid' => $_W['uniacid']));
	...
	public function group()
	{
		...
		foreach ($teamid as $key => $value) {
			$order = pdo_fetch('select id,uniacid,groupnum,goodid,endtime from ' . tablename('vcshop_groups_order') . '
					where teamid = :teamid and heads = 1 and success = 0 and uniacid = :uniacid ', array(':teamid' => $value, ':uniacid' => $uniacid));
			$order_count = pdo_fetchcolumn('select COUNT(1) from ' . tablename('vcshop_groups_order') . ' where teamid = :teamid and status = 1 and success = 0 and uniacid = :uniacid ', array(':teamid' => $value, ':uniacid' => $uniacid));
			...
			while ($i < $num) {
				...
				$order_insert = pdo_insert('vcshop_groups_order', $system_order_data);
				++$i;
			}

			pdo_update('vcshop_groups_order', array('success' => 1), array('teamid' => $value, 'uniacid' => $uniacid, 'status' => 1));
			pdo_update('vcshop_groups_order', array('status' => -1), array('teamid' => $value, 'uniacid' => $uniacid, 'status' => 0));
	...
	public function cancel_group()
	{
		...
		$orderNum = pdo_fetchcolumn('select COUNT(id) from ' . tablename('vcshop_groups_order') . ' where teamid = :teamid and uniacid = :uniacid and status >1 and success = 1 ', array(':teamid' => $teamid, ':uniacid' => $uniacid));

		if (!empty($orderNum)) {
			show_json(0, '该团商品已发货或已使用，不可取消！');
		}

		$headgroup = pdo_fetch('select * from ' . tablename('vcshop_groups_order') . ' where heads = 1 and teamid = :teamid and uniacid = :uniacid ', array(':teamid' => $teamid, ':uniacid' => $uniacid));

		if ($headgroup['success'] < 1) {
			show_json(0, '该团不满足取消条件！');
		}

		pdo_update('vcshop_groups_order', array('success' => -1), array('teamid' => $teamid, 'uniacid' => $uniacid, 'status' => 1));
		pdo_update('vcshop_groups_order', array('status' => -1), array('teamid' => $teamid, 'uniacid' => $uniacid, 'status' => 0));
		show_json(1);
	}
