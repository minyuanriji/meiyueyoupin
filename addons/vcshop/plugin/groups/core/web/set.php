...
class Set_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$set = pdo_fetch('SELECT * FROM ' . tablename('vcshop_groups_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $uniacid));
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'vcshop:groups') {
					show_json(0, '关键字已存在!');
				}
			}

			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'vcshop', ':name' => 'vcshop:groups'));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'vcshop:groups', 'module' => 'vcshop', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'vcshop', 'content' => trim($exchangekeyword), 'type' => 1, 'displayorder' => 0, 'status' => 1);
			...
			if (!empty($set)) {
				$set_update = pdo_update('vcshop_groups_set', $data, array('id' => $set['id'], 'uniacid' => $uniacid));
			}
			else {
				$set_insert = pdo_insert('vcshop_groups_set', $data);
			}

			pdo_update('vcshop_groups_goods', array('rights' => 1), array('uniacid' => $uniacid));
			$goodsid = explode(',', $data['goodsid']);

			foreach ($goodsid as $value) {
				$goods_update = pdo_update('vcshop_groups_goods', array('rights' => 0), array('id' => intval($value), 'uniacid' => $uniacid));
			}
		...
		$data2 = $this->set;
		$data = pdo_fetch('SELECT * FROM ' . tablename('vcshop_groups_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $uniacid));
		$data["rules"] = m("common")->html_to_images($data["rules"]);
		if ($data['goodsid']) {
			$goods = pdo_fetchall('SELECT *,groupsprice as marketprice  FROM ' . tablename('vcshop_groups_goods') . ('
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,title,rights,thumb FROM ' . tablename('vcshop_groups_goods') . (' WHERE 1 ' . $condition . ' order by createtime desc'), $params);
