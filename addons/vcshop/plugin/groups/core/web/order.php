...
class Order_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT o.id,o.orderno,o.status,og.option_name as optiontitle,o.expresssn,o.paytime,o.addressid,o.express,o.remark,o.is_team,o.pay_type,o.isverify,o.refundtime,o.price,o.creditmoney,o.refundstate,o.refundid,o.message,
				...
				left join ' . tablename('vcshop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid
				left join ' . tablename('vcshop_member_address') . ' a on a.id=o.addressid
				left join ' . tablename('vcshop_groups_order_goods') . (' as og on og.groups_order_id = o.id
		...
		foreach ($list as $key => $value) {
			if (!empty($value['address'])) {
				...
				if (!is_array($user)) {
					$user = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $value['addressid'], ':uniacid' => $_W['uniacid']));
				}
		...
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('vcshop_groups_order') . ' as o
				left join ' . tablename('vcshop_groups_goods') . ' as g on g.id = o.goodid
				left join ' . tablename('vcshop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid
				left join ' . tablename('vcshop_member_address') . (' a on a.id=o.addressid
				WHERE 1 ' . $condition . ' '), $params);
		$totalmoney = pdo_fetchcolumn('SELECT ifnull(sum(o.price),0) FROM ' . tablename('vcshop_groups_order') . ' as o\n\t\t\t\tleft join ' . tablename('vcshop_groups_goods') . ' as g on g.id = o.goodid\n\t\t\t\tleft join ' . tablename('vcshop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid\n\t\t\t\tleft join ' . tablename('vcshop_member_address') . ' a on a.id=o.addressid\n\t\t\t\tWHERE 1 ' . $condition . ' ', $params);
	...
	public function detail()
	{
		...
		$order = pdo_fetch('SELECT o.*,g.title,g.category,op.title as optiontitle,g.groupsprice,g.singleprice,g.goodsnum,g.thumb,g.thumb_url,g.id as gid FROM ' . tablename('vcshop_groups_order') . ' as o
				    left join ' . tablename('vcshop_groups_goods') . ' as g on g.id = o.goodid
					left join ' . tablename('vcshop_groups_goods_option') . ' as op on op.specs = o.specs
				WHERE o.id = :orderid ', $params);
		$order = set_medias($order, 'thumb');
		if (0 < $order['more_spec'] && empty($order['optiontitle'])) {
			$data = pdo_fetch('select option_name from ' . tablename('vcshop_groups_order_goods') . ' where groups_order_id = ' . $order['id'] . ' ');
		...
		if ($order['verifytype'] == 0) {
			$verify = pdo_fetch('select * from ' . tablename('vcshop_groups_verify') . ' where orderid = ' . $order['id'] . ' ');

			if (!empty($verify['verifier'])) {
				$saler = m('member')->getMember($verify['verifier']);
				$saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('vcshop_saler') . ' where openid=:openid and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $verify['verifier']));
			}

			if (!empty($order['storeid'])) {
				$store = pdo_fetch('select * from ' . tablename('vcshop_store') . ' where id=:storeid limit 1 ', array(':storeid' => $verify['storeid']));
			}
		}
		else {
			if ($order['verifytype'] == 1) {
				$verifyinfo = pdo_fetchall('select v.*,sm.id as salerid,sm.nickname as salernickname,s.salername,store.storename from ' . tablename('vcshop_groups_verify') . ' as v
					left join ' . tablename('vcshop_saler') . ' s on s.openid = v.verifier and s.uniacid = v.uniacid
					left join ' . tablename('vcshop_member') . ' sm on sm.openid = s.openid and sm.uniacid = s.uniacid
					left join ' . tablename('vcshop_store') . ' store on store.id = v.storeid and store.uniacid = v.uniacid
		...
		if (!empty($order['address'])) {
			...
			if (!is_array($user)) {
				$user = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $order['addressid'], ':uniacid' => $_W['uniacid']));
			}
	...
	protected function opData()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_groups_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,isverify,refundid  FROM ' . tablename('vcshop_groups_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));

		if (empty($item)) {
			show_json(0, '抱歉，订单不存在或是已经被删除！');
		}

		pdo_delete('vcshop_groups_order', array('id' => $id, 'uniacid' => $_W['uniacid']));

		if (0 < $item['isverify']) {
			pdo_delete('vcshop_groups_verify', array('orderid' => $id, 'uniacid' => $_W['uniacid']));
		}

		if (0 < $item['refundid']) {
			pdo_delete('vcshop_groups_order_refund', array('orderid' => $id, 'uniacid' => $_W['uniacid']));
		}
	...
	public function remarksaler()
	{
		...
		if ($_W['ispost']) {
			pdo_update('vcshop_groups_order', array('remark' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function changeaddress()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				$item = pdo_fetch('SELECT id, orderno, address FROM ' . tablename('vcshop_groups_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('vcshop_groups_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function pay($a = array(), $b = array())
	{
		...
		if (!empty($item['virtual']) && c('virtual')) {
			...
			pdo_update('vcshop_groups_order', array('status' => 1, 'pay_type' => 'other', 'paytime' => time(), 'starttime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function send()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('vcshop_groups_order', array('status' => 2, 'express' => trim($_GPC['express']), 'expresscom' => trim($_GPC['expresscom']), 'expresssn' => trim($_GPC['expresssn']), 'sendtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));

			if (!empty($item['refundid'])) {
				$refund = pdo_fetch('select * from ' . tablename('vcshop_groups_order_refund') . ' where id=:id limit 1', array(':id' => $item['refundid']));

				if (!empty($refund)) {
					pdo_update('vcshop_groups_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $item['refundid']));
					pdo_update('vcshop_groups_order', array('refundstate' => 0), array('id' => $item['id']));
				}
			}
...
' . $remark;
			...
			pdo_update('vcshop_groups_order', array('status' => 1, 'sendtime' => 0, 'remarksend' => $remark), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function changeexpress()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($expresssn)) {
				...
				pdo_update('vcshop_groups_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function finish()
	{
		...
		$opdata = $this->opData();
		extract($opdata);
		pdo_update('vcshop_groups_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		p('groups')->sendTeamMessage($item['id']);
	...
	public function enabled()
	{
		...
		$items = pdo_fetchall('SELECT id,name FROM ' . tablename('vcshop_groups_order') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('vcshop_groups_order', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
	...
	protected function selectOrderPrice($day = 0)
	{
		...
		$sql = 'select id,price,freight,starttime,openid,creditmoney from ' . tablename('vcshop_groups_order') . ' where uniacid = :uniacid and status > 0 and paytime between :createtime1 and :createtime2 ';
	...
	protected function selectteamPrice($success = 0)
	{
		...
		$sql = 'select id,price,freight,starttime from ' . tablename('vcshop_groups_order') . ' where uniacid = :uniacid and paytime > 0 and is_team = 1 and heads = :heads and success = :success ';
	...
	public function close()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $item['refundstate'] && !empty($item['refundid'])) {
				...
				pdo_update('vcshop_groups_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid']));
			}

			pdo_update('vcshop_groups_order', array('status' => -1, 'refundstate' => 0, 'canceltime' => $time, 'remarkclose' => trim($_GPC['remark'])), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function refund($tid, $fee = 0, $reason = '')
	{
		...
		if ($_W['ispost']) {
			...
			$order = pdo_fetch('SELECT id,orderno,credit,creditmoney,price,freight,status,pay_type,is_team,apppay FROM ' . tablename('vcshop_groups_order') . '
			...
			pdo_update('vcshop_groups_order', $update, array('id' => $item['id'], 'uniacid' => $uniacid));
			$sales = pdo_fetch('select id,sales,stock from ' . tablename('vcshop_groups_goods') . ' where id = :id and uniacid = :uniacid ', array(':id' => $item['goodid'], ':uniacid' => $uniacid));
			pdo_update('vcshop_groups_goods', array('sales' => $sales['sales'] - 1, 'stock' => $sales['stock'] + 1), array('id' => $sales['id'], 'uniacid' => $uniacid));
