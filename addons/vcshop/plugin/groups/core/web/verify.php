...
class Verify_VcShopPage extends PluginWebPage
{
                    ...
                    if ($searchfield == "goodstitle") {
                        ...
                        if ($searchfield == "goodssn") {
                            ...
                            if ($searchfield == "saler") {
                                ...
                                $sqlcondition = " left join " . tablename("vcshop_saler") . " as s on s.openid = v.verifier and s.uniacid=v.uniacid\n\t\t\t\tleft join " . tablename("vcshop_member") . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid ";
                            } else {
                                if ($searchfield == "store") {
                                    $condition .= " AND (locate(:keyword,store.storename)>0)";
                                    $sqlcondition = " left join " . tablename("vcshop_store") . " store on store.id = v.storeid and store.uniacid=o.uniacid ";
                                }
                            }
                        }
                    }
    ...
    public function detail()
    {
        ...
        $order = pdo_fetch("SELECT o.*,g.title,g.category,op.title as optiontitle,g.groupsprice,g.singleprice,g.thumb,g.thumb_url,g.id as gid FROM " . tablename("vcshop_groups_order") . " as o\n\t\t\t\tleft join " . tablename("vcshop_groups_goods") . " as g on g.id = o.goodid\n\t\t\t\tleft join " . tablename("vcshop_groups_goods_option") . " as op on op.specs = o.specs\n\t\t\t\tWHERE o.id = :orderid ", $params);
