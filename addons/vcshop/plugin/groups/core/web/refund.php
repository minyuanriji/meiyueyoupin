...
class Refund_VcShopPage extends PluginWebPage
{
	protected function opData()
	{
		...
		if (!empty($refundid)) {
			$refund = pdo_fetch('select * from ' . tablename('vcshop_groups_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
	...
	public function main()
	{
		...
		try {
			...
			$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_groups_order_refund') . ' as ore
                left join ' . tablename('vcshop_groups_order') . ' as o on o.id = ore.orderid
	...
	public function submit()
	{
		...
		if ($_W['ispost']) {
			...
			$setting = pdo_fetch('SELECT refundday FROM ' . tablename('vcshop_groups_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
			$refundtime = $item['finishtime'] + 3600 * 24 * $setting['refundday'];
			if ($item['status'] == 3 && $refundtime < time()) {
				pdo_update('vcshop_groups_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			...
			if (empty($refund['refundno'])) {
				...
				pdo_update('vcshop_groups_order_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
			}
			...
			if ($refundstatus == 0) {
				...
				if ($raid != 0) {
					$raddress = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $raid, ':uniacid' => $uniacid));
				}
				else {
					$raddress = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $raid, ':uniacid' => $uniacid));
				}
				...
				if (empty($raddress)) {
					$raddress = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where uniacid=:uniacid order by id desc limit 1', array(':uniacid' => $uniacid));
				}
				...
				pdo_update('vcshop_groups_order_refund', $change_refund, array('id' => $item['refundid']));
				...
				pdo_update('vcshop_groups_order_refund', $refund_data, array('id' => $item['refundid'], 'uniacid' => $uniacid));
				$order_data = array();
				$order_data['refundstate'] = 0;
				$order_data['status'] = 3;
				$order_data['refundtime'] = $time;
				pdo_update('vcshop_groups_order', $order_data, array('id' => $item['id'], 'uniacid' => $uniacid));
				p('groups')->sendTeamMessage($item['id'], true);
			}
				...
				pdo_update('vcshop_groups_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));
				...
				pdo_update('vcshop_groups_order_refund', array('reply' => $refundcontent, 'refundstatus' => -1, 'endtime' => $time), array('id' => $item['refundid']));
				p('groups')->sendTeamMessage($item['id'], true);
				plog('groups.refund', '订单退款拒绝 ID: ' . $item['id'] . ' 订单号: ' . $item['orderno'] . ' 原因: ' . $refundcontent);
				pdo_update('vcshop_groups_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid));
			}
			else {
				if ($refundstatus == 2) {
					$refundtype = 2;
		...
		$refund_address = pdo_fetchall('select * from ' . tablename('vcshop_refund_address') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
