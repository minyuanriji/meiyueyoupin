...
class PolyapiModel extends PluginModel
{
	public function get_key_info($array)
	{
		...
		$item = pdo_fetch('select * from ' . tablename('vcshop_polyapi_key') . ' where appkey=:appkey and token=:token limit 1', $params);
		return $item;
	}
	...
	public function GetOrder($array)
	{
		...
		if (!empty($list)) {
			foreach ($list as $k => $v) {
				...
				if (0 < $v['parentid']) {
					$parent_item = pdo_fetch('SELECT id,ordersn,ordersn2,price FROM ' . tablename('vcshop_order') . ' WHERE id = :id and uniacid=:uniacid Limit 1', array(':id' => $v['parentid'], ':uniacid' => $uniacid));
				...
				if (empty($v['addressid'])) {
					...
					if (!is_array($user)) {
						$user = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $v['addressid'], ':uniacid' => $uniacid));
					}
				...
				if (!empty($refundid)) {
					$refund = pdo_fetch('select status from ' . tablename('vcshop_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
				}
				...
				$order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,g.hasoption,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell,og.id as ogid from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' left join ' . tablename('vcshop_goods_option') . ' op on og.optionid = op.id ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $uniacid, ':orderid' => $v['id']));
	...
	public function Send($array)
	{
		...
		if (empty($IsSplit)) {
			...
			pdo_update('vcshop_order', $change_data, array('id' => $item['id'], 'uniacid' => $uniacid));
		}
		else {
			...
			foreach ($goodsid as $key => $value) {
				pdo_update('vcshop_order_goods', $change_data, array('orderid' => $item['id'], 'goodsid' => $value, 'uniacid' => $uniacid));
			}
			...
			pdo_update('vcshop_order', $senddata, array('id' => $item['id'], 'uniacid' => $uniacid));
		}
	...
	public function DownloadProduct($array)
	{
		...
		if (!empty($total)) {
			...
			foreach ($list as $k => &$v) {
				...
				if ($v['hasoption'] == 1) {
					$options = pdo_fetchall('select * from ' . tablename('vcshop_goods_option') . ' where goodsid=:id order by id asc', array(':id' => $v['id']));
	...
	public function SyncStock($array)
	{
		...
		$item = pdo_fetch('select * from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid Limit 1', array(':id' => $PlatProductID, ':uniacid' => $uniacid));
		...
		if ($item['hasoption'] == 1) {
			...
			pdo_update('vcshop_goods_option', array('stock' => $Quantity), array('id' => $SkuID, 'goodsid' => $PlatProductID));
			pdo_update('vcshop_goods', array('total' => $goods_total), array('id' => $PlatProductID));
		}
		else {
			pdo_update('vcshop_goods', array('total' => $Quantity), array('id' => $PlatProductID));
		}
	...
	{
		global $_W;
		$express = pdo_fetch('select * from ' . tablename('vcshop_express') . ' where code=:code limit 1', array(':code' => $LogisticType));
		return $express;
	}
	...
	{
		global $_W;
		$item = pdo_fetch('select * from ' . tablename('vcshop_order') . ' where  ordersn=:ordersn and uniacid=:uniacid Limit 1', array(':ordersn' => $ordersn, ':uniacid' => $uniacid));
		return $item;
	}
	...
	public function insert_data($data, $uniacid)
	{
		...
		pdo_insert('vcshop_polyapi_data', $insert_data);
	}
	...
	public function get_data($uniacid)
	{
		...
		$params = array();
		$params[':uniacid'] = $uniacid;
		$sql = 'SELECT * FROM ' . tablename('vcshop_polyapi_data') . ' WHERE uniacid=:uniacid order by id desc';
		$list = pdo_fetchall($sql, $params);
