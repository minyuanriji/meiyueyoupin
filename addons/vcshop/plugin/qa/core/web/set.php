...
class Set_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'vcshop:qa') {
					show_json(0, '关键字已存在！');
				}
			}

			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'vcshop:qa'));
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'vcshop:qa', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			...
			if (!empty($set)) {
				pdo_update('vcshop_qa_set', $data, array('id' => $set['id']));
			}
			else {
				$data['uniacid'] = $_W['uniacid'];
				pdo_insert('vcshop_qa_set', $data);
				$id = pdo_insertid();
			}
