...
class Batch_VcShopPage extends PluginWebPage
{
		...
		if ($_W['ispost']) {
			...
			if (is_array($ordersns)) {
				...
				foreach ($ordersns as $ordersn) {
					pdo_update('vcshop_order', array('address_send' => $address_send), array('ordersn' => $ordersn, 'merchid' => 0));
				}
