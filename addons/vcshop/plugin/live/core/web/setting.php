...
class Setting_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$data = pdo_fetch('select * from ' . tablename('vcshop_live_setting') . ' where uniacid = ' . $uniacid . ' ');
		...
		if ($_W['ispost']) {
			...
			if (!empty($data)) {
				pdo_update('vcshop_live_setting', $set_data, array('uniacid' => $uniacid));
			}
			else {
				$set_data['uniacid'] = $uniacid;
				pdo_insert('vcshop_live_setting', $set_data);
			}
