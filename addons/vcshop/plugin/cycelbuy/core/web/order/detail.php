...
class Detail_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$notArray = array();
		$periods = pdo_getall('vcshop_cycelbuy_periods', array('orderid' => $id), array(), '', 'id');
		...
		foreach ($goods as &$r) {
			...
			if (empty($r['optionname']) && !empty($r['specs'])) {
				$option_title = pdo_fetch('SELECT title FROM' . tablename('vcshop_goods_spec_item') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($r['specs']));
	...
	public function option_title($orderid)
	{
		...
		$sql = 'select o.specs from ' . tablename('vcshop_order_goods') . ' g join ' . tablename('vcshop_goods_option') . ' o on o.id = g.optionid where g.orderid = :orderid';
