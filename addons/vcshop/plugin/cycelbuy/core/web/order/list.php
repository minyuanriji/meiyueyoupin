	...
	protected function orderData($status, $st)
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$sqlcondition = ' inner join ( select  DISTINCT(og.orderid) from ' . tablename('vcshop_order_goods') . ' og left join ' . tablename('vcshop_goods') . (' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,og.optionname)>0)) gs on gs.orderid=o.id');
			}
			else {
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 ' || !empty($sqlcondition)) {
			...
			$sql = 'select * from ' . tablename('vcshop_order') . (' where uniacid = :uniacid and iscycelbuy = 1 and ismr=0 and deleted=0 and isparent=0 ' . $status_condition . ' GROUP BY id ORDER BY createtime DESC  ');
		...
		if (!empty($list)) {
			...
			foreach ($list as &$value) {
				...
				$order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell,g.costprice,op.costprice as option_costprice from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' left join ' . tablename('vcshop_goods_option') . ' op on og.optionid = op.id ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $uniacid, ':orderid' => $value['id']));
				$goods = '';
...
';
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
			if ($searchfield == 'member') {
				$openidArr = pdo_fetchall('SELECT openid FROM ' . tablename('vcshop_member') . ' WHERE uniacid = ' . $_W['uniacid'] . $priceCondition);
				...
				if (!empty($openidArr)) {
					...
					$orderPrice = pdo_fetch('SELECT count(1) as count,sum(price) as sumprice FROM ' . tablename('vcshop_order') . ' o WHERE o.uniacid = ' . $_W['uniacid'] . ' AND o.deleted=0 AND o.isparent=0 AND o.openid IN (' . $inOpenid . ')' . $priceStatus);
				}
				else {
			...
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('vcshop_order') . (' WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 and iscycelbuy = 1 ' . $status_condition), $paras);
			$total = $t['count'];
			$totalmoney = $t['sumprice'];
		}
