...
if (!class_exists('CycelbuyModel')) {
	class CycelbuyModel extends PluginModel
	{
		public function cycelbuy_periodic($orderid)
		{
			...
			global $_W;
			$openid = $_W['openid'];
			$sql = 'SELECT id,ordersn,status,cycelbuy_periodic,cycelbuy_predict_time,iscycelbuy,addressid,dispatchprice,dispatchtype FROM ' . tablename('vcshop_order') . ' WHERE id = :id and iscycelbuy=1 and uniacid =:uniacid';
			...
			if ($order['status'] == 1) {
				...
				$addressSql = 'SELECT * FROM ' . tablename('vcshop_member_address') . ' WHERE id=:id and uniacid=:uniacid';
				...
				while ($i < $cycelbuy_num) {
					...
					pdo_insert('vcshop_cycelbuy_periods', $data);
					++$i;
				}
			}
		}
