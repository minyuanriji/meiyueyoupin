...
require VCSHOP_PLUGIN . 'cycelbuy/core/page_mobile.php';
class List_VcShopPage extends PluginMobilePage
{
...
paytype,expresssn,refundstate,dispatchtype,verifyinfo,merchid,isparent,iscycelbuy,cycelbuy_periodic,userdeleted' . $s_string . '
 from ' . tablename('vcshop_order') . (' where 1 ' . $condition . ' order by createtime desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);
	...
	public function address()
	{
		...
		if (!empty($orderid)) {
			$address = pdo_fetch('select * from ' . tablename('vcshop_cycelbuy_periods') . ' where id = :id and orderid=:orderid and status = 0', array(':orderid' => $orderid, ':id' => intval($_GPC['id'])));
			$address = iunserializer($address['address']);
		}

		if (!empty($applyforid)) {
			$new_address = pdo_fetch('select * from ' . tablename('vcshop_address_applyfor') . ' where cycleid = :cycleid and isdelete = 0 order by createtime desc limit 1', array(':cycleid' => intval($_GPC['id'])));
	...
	public function submit()
	{
		...
		$orderid = intval($_GPC['orderid']);
		$order = pdo_fetch('SELECT openid,ordersn FROM ' . tablename('vcshop_order') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
		...
		if (!empty($applyid)) {
			...
			pdo_insert('vcshop_address_applyfor', $new_data);
			$res = pdo_insertid();
		}
	...
	public function applyfor()
	{
		...
		$data = pdo_get('vcshop_address_applyfor', array('id' => $apply_id, 'isdelete' => 0));
	...
	public function cancelApply()
	{
		...
		global $_GPC;
		$id = $_GPC['applyid'];
		$result = pdo_update('vcshop_address_applyfor', array('isdelete' => 1), array('id' => $id));
