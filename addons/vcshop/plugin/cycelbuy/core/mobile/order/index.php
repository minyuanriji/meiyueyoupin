...
class Index_VcShopPage extends PluginMobilePage
{
	...
	public function detail()
	{
		...
		if ($order['refundid'] != 0) {
			$refund = pdo_fetch('SELECT *  FROM ' . tablename('vcshop_order_refund') . ' WHERE orderid = :orderid and uniacid=:uniacid order by id desc', array(':orderid' => $order['id'], ':uniacid' => $_W['uniacid']));
		}
		...
		$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,g.status, g.cannotrefund, og.total,g.credit,og.optionid,
            ...
            og.seckill_taskid' . $diyformfields . $condition1 . ',og.prohibitrefund  from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . (' where ' . $scondition . ' and og.uniacid=:uniacid '), $param);
		...
		if (0 < $order['sendtype'] && 1 <= $order['status']) {
			$order_goods = pdo_fetchall('select orderid,goodsid,sendtype,expresscom,expresssn,express,sendtime from ' . tablename('vcshop_order_goods') . '
	...
	public function express()
	{
		...
		$order = pdo_fetch('select * from ' . tablename('vcshop_cycelbuy_periods') . ' where id=:id  and orderid = :orderid and uniacid=:uniacid limit 1', array(':id' => $id, ':orderid' => $orderid, ':uniacid' => $uniacid));
		...
		$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,og.expresssn,og.express,
            ...
            from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid ' . $condition . ' and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
		$expresslist = m('util')->getExpressList($order['express'], $order['expresssn']);
		include $this->template();
	}
	...
	public function cycledetail()
	{
		...
		$oSql = 'select id,status,refundstate from ' . tablename('vcshop_order') . (' where id = ' . $orderid . ' and uniacid = ' . $_W['uniacid']);
		$oData = pdo_fetch($oSql);
		$sql = 'SELECT * FROM ' . tablename('vcshop_cycelbuy_periods') . ' WHERE orderid = :orderid AND uniacid = :uniacid ';
		...
		$cycelbuy_periodic = pdo_fetchcolumn('select cycelbuy_periodic from ' . tablename('vcshop_order') . ' where id=:orderid and uniacid=:uniacid limit 1', $params);
