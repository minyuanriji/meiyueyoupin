...
class Refund_VcShopPage extends OrderMobilePage
{
	protected function globalData()
	{
		...
		if ($order['status'] == 2) {
			$goods = pdo_fetchall('select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund, g.thumb, g.title,g.isfullback from' . tablename('vcshop_order_goods') . ' og left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid where og.orderid=' . $order['id']);
		...
		$fullback_log = pdo_fetch('select * from ' . tablename('vcshop_fullback_log') . ' where orderid = ' . $orderid . ' and uniacid = ' . $uniacid . ' ');

		if ($fullback_log) {
			$fullbackgoods = pdo_fetch('select refund from ' . tablename('vcshop_fullback_goods') . ' where goodsid = ' . $fullback_log['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
