...
class Detail_VcShopPage extends PluginMobilePage
{
			...
			foreach ($gid as $ke => $val) {
				if ($val == $id) {
					...
					foreach ($giftgoods as $k => $val) {
						$isgift = 1;
						$gifts[$key]['gift'][$k] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('vcshop_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and total > 0 and status = 2 and id = ' . $val . ' ');
	...
	public function querygift()
	{
		...
		$uniacid = $_W['uniacid'];
		$giftid = $_GPC['id'];
		$gift = pdo_fetch('select * from ' . tablename('vcshop_gift') . ' where uniacid = ' . $uniacid . ' and status = 1 and id = ' . $giftid . ' ');
		show_json(1, $gift);
	}
