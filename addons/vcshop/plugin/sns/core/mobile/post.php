...
class Post_VcShopPage extends SnsMobilePage
{
	public function main()
	{
		...
		pdo_update('vcshop_sns_post', array('views' => $post['views'] + 1), array('id' => $post['id']));
		$goodcount = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_sns_like') . ' where pid=:pid limit 1', array(':pid' => $post['id']));
		$replycount = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_sns_post') . ' where pid=:pid and deleted=0 and checked=1 limit 1', array(':pid' => $post['id']));
		$isgood = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_sns_like') . ' where uniacid=:uniacid and pid=:pid and openid=:openid  limit 1', array(':uniacid' => $_W['uniacid'], ':pid' => $post['id'], ':openid' => $_W['openid']));
		$set = $this->getSet();
		...
		$catelist = pdo_fetchall('SELECT id,name FROM ' . tablename('vcshop_sns_complaincate') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY displayorder asc'));
	...
	public function checkPost()
	{
		...
		$post = pdo_fetch('select pid,nickname from ' . tablename('vcshop_sns_post') . ' where id = ' . $postid . ' ');
	...
	public function submit()
	{
		...
		pdo_insert('vcshop_sns_post', $data);
	...
	public function complain()
	{
		...
		$id = intval($_GPC['id']);
		$openid = $_W['openid'];
		$posts = pdo_fetch('SELECT id,pid,openid FROM ' . tablename('vcshop_sns_post') . ' WHERE uniacid = ' . $uniacid . ' AND id = ' . $id . ' AND deleted = 0 ');
		...
		pdo_insert('vcshop_sns_complain', $data);
	...
	public function reply()
	{
		...
		pdo_insert('vcshop_sns_post', $data);
		pdo_update('vcshop_sns_post', array('replytime' => $time), array('id' => $pid, 'uniacid' => $_W['uniacid']));
	...
	public function getlist()
	{
		...
		$sql = 'select id,bid,rpid,title,createtime,content,images ,openid, nickname,avatar,checked from ' . tablename('vcshop_sns_post') . ('  where 1 ' . $condition . ' ORDER BY createtime desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
		...
		foreach ($list as $key => &$row) {
			...
			$isgood = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_sns_like') . ' where uniacid=:uniacid and pid=:pid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':pid' => $row['id'], ':openid' => $_W['openid']));
			$row['isgood'] = $isgood;
			$row['goodcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_sns_like') . ' where uniacid=:uniacid and pid=:pid  limit 1', array(':uniacid' => $_W['uniacid'], ':pid' => $row['id']));
	...
	public function like()
	{
		...
		$isgood = 1;
		$like = pdo_fetch('select id from ' . tablename('vcshop_sns_like') . ' where pid=:pid and openid=:openid limit 1', array(':pid' => $pid, ':openid' => $_W['openid']));

		if (!empty($like)) {
			$isgood = 0;
			pdo_delete('vcshop_sns_like', array('id' => $like['id']));
		}
		else {
			$like = array('uniacid' => $_W['uniacid'], 'pid' => $pid, 'openid' => $_W['openid']);
			pdo_insert('vcshop_sns_like', $like);
		}

		$time = time();
		pdo_update('vcshop_sns_post', array('replytime' => $time), array('id' => $pid, 'uniacid' => $_W['uniacid']));
		$goodcount = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_sns_like') . ' where pid=:pid limit 1', array(':pid' => $pid));
	...
	public function delete()
	{
		...
		pdo_update('vcshop_sns_post', array('deleted' => 1, 'deletedtime' => time()), array('id' => $pid));
	...
	public function check()
	{
		...
		if (!$post['checked']) {
			pdo_update('vcshop_sns_post', array('checked' => 1, 'checktime' => time()), array('id' => $pid));
	...
	public function best()
	{
		...
		if ($post['isboardbest']) {
			$isbest = 0;
			pdo_update('vcshop_sns_post', array('isboardbest' => 0), array('id' => $pid));
			$this->model->setCredit($post['openid'], $bid, SNS_CREDIT_BEST_BOARD_CANCEL);
		}
		else {
			pdo_update('vcshop_sns_post', array('isboardbest' => 1), array('id' => $pid));
	...
	public function top()
	{
		...
		if ($post['isboardtop']) {
			$istop = 0;
			pdo_update('vcshop_sns_post', array('isboardtop' => 0), array('id' => $pid));
			$this->model->setCredit($post['openid'], $bid, SNS_CREDIT_TOP_BOARD_CANCEL);
		}
		else {
			pdo_update('vcshop_sns_post', array('isboardtop' => 1), array('id' => $pid));
	...
	public function allbest()
	{
		...
		if ($post['isbest']) {
			$isbest = 0;
			pdo_update('vcshop_sns_post', array('isbest' => 0), array('id' => $pid));
			$this->model->setCredit($post['openid'], $bid, SNS_CREDIT_BEST_CANCEL);
		}
		else {
			pdo_update('vcshop_sns_post', array('isbest' => 1), array('id' => $pid));
	...
	public function alltop()
	{
		...
		if ($post['istop']) {
			$istop = 0;
			pdo_update('vcshop_sns_post', array('istop' => 0), array('id' => $pid));
			$this->model->setCredit($post['openid'], $bid, SNS_CREDIT_TOP_CANCEL);
		}
		else {
			pdo_update('vcshop_sns_post', array('istop' => 1), array('id' => $pid));
