...
class Board_VcShopPage extends SnsMobilePage
{
	...
	public function getlist()
	{
		...
		foreach ($list as &$row) {
			...
			$row['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_sns_post') . ' where pid=:pid and deleted = 0 limit 1', array(':pid' => $row['id']));
	...
	public function relate()
	{
		...
		$sql = 'select id,title,logo,`desc`  from ' . tablename('vcshop_sns_board') . (' where 1 ' . $condition . ' order by displayorder desc');
		$list = pdo_fetchall($sql, $params);
	...
	public function get_boardlist()
	{
		...
		$list = pdo_fetchall('SELECT sb.id,sb.title,sb.logo FROM ' . tablename('vcshop_sns_board') . ' as sb
                left join ' . tablename('vcshop_sns_category') . (' as sc on sc.id = sb.cid
		...
		unset($row);
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('vcshop_sns_board') . (' as sb WHERE 1 ' . $condition), $params);
		show_json(1, array('list' => $list, 'pagesize' => $psize, 'total' => $total));
	}
	...
	public function follow()
	{
		...
		$follow = pdo_fetch('select id from ' . tablename('vcshop_sns_board_follow') . ' where bid=:bid and openid=:openid limit 1', array(':bid' => $bid, ':openid' => $_W['openid']));

		if (!empty($follow)) {
			pdo_delete('vcshop_sns_board_follow', array('id' => $follow['id']));
			show_json(1, array('isfollow' => false));
		}
		else {
			$follow = array('uniacid' => $_W['uniacid'], 'bid' => $bid, 'openid' => $_W['openid'], 'createtime' => time());
			pdo_insert('vcshop_sns_board_follow', $follow);
