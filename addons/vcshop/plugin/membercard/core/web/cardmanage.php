...
class Cardmanage_VcShopPage extends PluginWebPage
{
    ...
    protected function post()
    {
        ...
        if ($_W["ispost"]) {
            ...
            if (!empty($id)) {
                ...
                $goods_update = pdo_update("vcshop_member_card", $update_data, array("id" => $id, "uniacid" => $_W["uniacid"]));
                if (!$goods_update) {
                ...
                $goods_insert = pdo_insert("vcshop_member_card", $data);
                if (!$goods_insert) {
                    show_json(0, "会员卡添加失败！");
                }
                $id = pdo_insertid();
            ...
            if ($_GPC["cardmodel"] == 2 || $item["cardmodel"] == 2) {
                $sql = "UPDATE " . tablename("vcshop_goods") . " SET membercardpoint = 1 WHERE id in (" . $goodsids . ")";
                pdo_fetch($sql);
                $goodsids_del = "";
                ...
                if (!empty($goodsids_old)) {
                    ...
                    $sql = "UPDATE " . tablename("vcshop_goods") . " SET membercardpoint = 0 WHERE id in (" . $goodsids_del . ")";
                    pdo_fetch($sql);
                }
            }
        ...
        $sql = "SELECT id,maxbuy,title,thumb,total FROM " . tablename("vcshop_goods") . " WHERE id in (" . $item["goodsids"] . ")";
    ...
    public function status()
    {
        ...
        $item = pdo_fetch("SELECT id,name,status FROM " . tablename("vcshop_member_card") . " WHERE id = '" . $id . "' AND uniacid=" . $_W["uniacid"] . "");
        if (empty($item)) {
            show_json(0, "会员卡不存在或者已经被删除");
        }
        $status = abs(1 - $item["status"]);
        $result = pdo_update("vcshop_member_card", array("status" => $status), array("id" => $id, "uniacid" => $_W["uniacid"]));
        if (!$result) {
            show_json(0, "操作失败");
        }
    ...
    public function delete()
    {
        ...
        global $_GPC;
        $id = intval($_GPC["id"]);
        $item = pdo_fetch("SELECT id,name FROM " . tablename("vcshop_member_card") . " WHERE id = '" . $id . "' AND uniacid=" . $_W["uniacid"] . "");
        if (empty($item)) {
            show_json(0, "抱歉,会员卡不存在或者已经被删除");
        }
        pdo_update("vcshop_member_card", array("isdelete" => 1, "del_time" => time()), array("id" => $id, "uniacid" => $_W["uniacid"]));
        pdo_update("vcshop_member_card_history", array("isdelete" => 1, "del_time" => time()), array("member_card_id" => $id, "uniacid" => $_W["uniacid"]));
    ...
    public function querycoupon($couponid = array())
    {
        ...
        foreach ($couponid as $ck => $cv) {
            ...
            $cpsql = "SELECT * FROM " . tablename("vcshop_coupon") . $where;
