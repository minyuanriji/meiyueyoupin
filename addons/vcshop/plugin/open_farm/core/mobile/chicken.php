...
class Chicken_VcShopPage extends PluginMobilePage
{
	/**
     * 当前数据表名称
     * @var string
     */
	private $table = 'vc_open_farm_chicken';
	/**
     * 当前类的所有字段
     * @var array
     */
	...
	{
		global $_W;
		$surprised = new Surprised_VcShopPage();
		...
		if ($surprised === 'yes') {
			$surprisedTable = 'vc_open_farm_surprised';
			...
			$userSurprised = 'vc_open_farm_user_surprised';
	...
	public function feeding()
	{
		...
		if (0 < $egg && 0 < count($eggs)) {
			foreach ($eggs as $value) {
				$this->eggLog($value);
				$presentation = new Presentation_VcShopPage();
	...
	public function getSurprisedList($mode = false)
	{
		...
		$table = 'vc_open_farm_user_surprised';
		...
		if ($data && 0 < count($data)) {
			foreach ($data as $key => $value) {
				...
				case '优惠券':
					...
					$gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $surprised['value'], ':uniacid' => $_W['uniacid']));
	...
	public function getSurprised($method = false)
	{
		...
		if ($query['category'] === '优惠券') {
			...
			$fields = array('couponname', 'enough', 'backtype', 'deduct', 'discount', 'backmoney', 'backcredit', 'backredpack', 'total');
			$info = pdo_get($table, $where, $fields);
			$gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $surprisedInfo['value'], ':uniacid' => $_W['uniacid']));
			$left_count = $info['total'] - $gettotal;
			$left_count = intval($left_count);
			if ($info['total'] != -1 && $left_count <= 0) {
				$query = array();
	...
	{
		$sql = ' UPDATE `ims_vc_open_farm_egg` ' . ' SET `receive` = \'是\' ' . (' WHERE `id` IN (' . $idStr . '); ');
		pdo_query($sql);
	}
