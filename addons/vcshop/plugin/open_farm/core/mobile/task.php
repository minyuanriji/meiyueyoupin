...
class Task_VcShopPage extends PluginMobilePage
{
	/**
     ...
     */
	private $table = 'vc_open_farm_task';
	/**
	...
	public function signIn($data)
	{
		...
		if (!$query || count($query) < 1) {
			...
			if ($log) {
				$chicken = new Chicken_VcShopPage();
	...
	public function core($data)
	{
		...
		if (count($query) <= 0) {
			...
			pdo_insert('vc_open_farm_user_task', $userTask);
		}
