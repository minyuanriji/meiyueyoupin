...
class Surprised_VcShopPage extends PluginWebPage
{
	/**
     ...
     */
	private $table = 'vc_open_farm_surprised';
	/**
     ...
     * Surprised_VcShopPage constructor.
	...
	public function getCoupon($surprisedList)
	{
		...
		$couponList = pdo_fetchall('SELECT  `id`,`couponname` FROM ' . tablename('vcshop_coupon') . (' WHERE `id` in(' . $couponId . ') '));
	...
	{
		$table = tablename('vc_open_farm_surprised');
		$field = 'category';
