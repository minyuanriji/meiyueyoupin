...
class Task_VcShopPage extends PluginWebPage
{
	/**
	...
	private function getTaskName($data)
	{
		...
		$idStr = implode(',', $idArr);
		$field = ' `id`,`title` ';
		$taskArr = pdo_fetchall(' SELECT ' . $field . ' FROM ' . tablename('vcshop_task_list') . (' WHERE `id` in(' . $idStr . ') '));
