...
class Index_VcShopPage extends PluginWebPage
{
	...
	public function notice()
	{
		...
		if (isset($data['openid1'])) {
			if (!empty($data['openid1'])) {
				...
				@$salers1 = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('vcshop_member') . ' where openid in (' . implode(',', $openids1) . (') and uniacid=' . $_W['uniacid']));
			}
		}
		$salers3 = array();
		...
		$template_lists = pdo_fetchall('SELECT id,title,typecode FROM ' . tablename('vcshop_member_message_template') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
	...
	public function set()
	{
		...
		if ($_W['ispost']) {
			...
			// 20200513 lin
			$data['showchild'] = json_encode($data['showchild']);
			$data['showteam'] = json_encode($data['showteam']);
	...
	public function goodsquery()
	{
		...
		$goodsids = pdo_fetchall('SELECT goodsids FROM ' . tablename('vcshop_commission_level') . ' WHERE uniacid=:uniacid', array('uniacid' => $_W['uniacid']));
		$newarray = array();
