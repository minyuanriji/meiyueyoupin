...
class Order_VcShopPage extends WebPage
{
	protected function orderData($status, $st)
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$condition .= ' AND o.openid in (select openid from ' . tablename('vcshop_member') . ' where realname like :keyword or mobile=:keyword or nickname like :keyword)';
			}
				...
				$condition .= ' AND verifyopenid in (select openid from ' . tablename('vcshop_member') . ' where realname like :keyword or mobile=:keyword or nickname like :keyword)';
			}
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.agentid>0' || !empty($sqlcondition)) {
			$sql = 'select o.*  from ' . tablename('vcshop_order') . ' o' . (' ' . $sqlcondition . ' where ' . $condition . ' ' . $statuscondition . ' GROUP BY o.id ORDER BY o.createtime DESC  ');
			...
			$sql = 'select * from ' . tablename('vcshop_order') . (' where uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 and agentid > 0 ' . $status_condition . ' GROUP BY id ORDER BY createtime DESC  ');
...
';
				...
				if (!empty($agentid) && 0 < $level) {
					...
					if($reward_agent){
						...
						if(!empty($reward_logs) && is_array($reward_logs)){
							// $commission_level_express = 0;
							// $commission_level_team_bonus = 0;
							foreach ($reward_logs as $team) {
								if($team['type'] == 2){
