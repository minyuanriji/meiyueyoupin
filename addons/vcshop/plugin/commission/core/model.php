...
if (!class_exists('CommissionModel')) {
    class CommissionModel extends PluginModel {
        ...
        public function calculatestar($orderid=0,$update = false){
            ...
            $lock = pdo_fetchcolumn('SELECT ifnull(count(*),0) FROM ' . tablename('vcshop_lock') . ' WHERE uniacid=:uniacid AND mid=:mid AND type=3 AND orderid=:orderid',array(':uniacid' => $_W['uniacid'], ':mid' => $member['id'],':orderid'=>$orderid));
            
            if($lock > 0){
                return false;
            }else{
                pdo_insert('vcshop_lock',array('uniacid' => $_W['uniacid'], 'mid' => $member['id'], 'uid' => $member['uid'], 'status' => 1, 'type' => 3,'orderid'=>$orderid));
            }
            ...
            $findchargeagnt = 0;
            // 20200417 lin
            $lastMonth = $this->getThisMonthStar($agent['openid'],true);
            $thisMonth = $this->getThisMonthStar($agent['openid'],false);
            if ($lastMonth['level'] >= $thisMonth['level']) {
                $thismonelevel = $lastMonth;
            }else{
                $thismonelevel = $thisMonth;
            }
            ...
            if(!empty($thismonelevel)){
                ...
                if($thismonelevel['is_team'] == 1){
                    ...
                    $desc = '直推会员-['.$member['id'].']'.$member['nickname'].'购买产品,获奖励';
                }
                ...
                if($allbonus>0){
                    ...
                    $rewardlog = array(
                        ...
                        'type' => 3,
                        'level' => $a_level['id'],
                        'levelweight' => $a_level['level'],
                        'floot' => $r_level,
                        'status'    => 0,
                        'orderid'   => $orderid,
                        'abonus'   => json_encode($allbonus_data),
                        'create_time'   => time()
                    );
            ...
            if(($thismonelevel && $thismonelevel['is_team'] == 0) || $thismonelevel == false){
                ...
                if($teamagent){
                    ...
                    $tlastMonth = $this->getThisMonthStar($teamagent['openid'],true);
                    $tthisMonth = $this->getThisMonthStar($teamagent['openid'],false);
                    if ($tlastMonth['level'] >= $tthisMonth['level']) {
                        $team_thismonelevel = $tlastMonth;
                    }else{
                        $team_thismonelevel = $tthisMonth;
                    }
            ...
            if(!empty($findchargeagnt) && $allbonus > 0){
                ...
                if($chargeagent){
                    ...
                    $clastMonth = $this->getThisMonthStar($chargeagent['openid'],true);
                    $cthisMonth = $this->getThisMonthStar($chargeagent['openid'],false);
                    if ($clastMonth['level'] >= $cthisMonth['level']) {
                        $team_thismonelevel = $clastMonth;
                    }else{
                        $team_thismonelevel = $cthisMonth;
                    }
        ...
        public function getThisMonthStar($openid,$monthtime){
            global $_W;
            $uniacid = $_W['uniacid'];
            if(empty($openid)){
                return false;
            ...
            // 20200417 lin
            if ($monthtime) {
                $startime = strtotime(date('Y-m-01 00:00:00',strtotime('-1 month'))); //上个月第一天
                $endtime = mktime(0,0,0,date('m'),1)-1; //上个月最后一天
            }else{
                $startime = mktime(0,0,0,date('m'),1);//本月
                $endtime = time();
            ...
            // 20200422 如有指定直接切
            if ($agent['starpoint'] == 1) {
                $data = pdo_fetch('SELECT * FROM ' . tablename('vcshop_commission_star') . ' WHERE uniacid=:uniacid AND id = :starid limit 1',array(':uniacid' => $_W['uniacid'],':starid'=>$agent['starlevel']));
                return $data;
            }
            ...
            foreach ($starlevel as $key => $value) {
                ...
                if(!$conditiondown && !empty($value['downcount']) && !empty($value['downlevel'])){
                    ...
                    if(!empty($level_ids)){
                        ...
                        // 20200418 lin
                        $user = pdo_fetchcolumn('select count(r.id) from ' . tablename('vcshop_commission_relation'). ' r left join' .tablename('vcshop_member') . ' m on m.id=r.id '  . ' where m.uniacid=:uniacid and m.status=1 and m.isagent=1 and r.level=1 and m.agentlevel in('.$level_ids_str.') and m.agenttime BETWEEN '.$startime.' AND '.$endtime.' and r.pid=:id',array(':uniacid'=>$uniacid,':id'=>$agent['id']));//获取本月之前的分销商
        ...
        public function calculatebonus($orderid=0,$update = false){
            ...
            // han 20200422 防止重复奖励 s
            $open_redis = function_exists('redis') && !(is_error(redis()));
            if ($open_redis) 
            {
                $redis_key = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_' . $_W['uniacid'] . '_order_reward_' . $orderid;
                $redis = redis();
                if (!(is_error($redis))) 
                {
         ...
         * @return bol  返回
         */
        public function calculatesecond($orderid=0,$update = false){
            global $_W;
            $set        = $this->getSet();
        ...
        public function checkOrderPay($orderid = '0') {
            ...
            $this->calculatesecond($orderid,true);//lin 20191206
        ...
        public function upgradeLevelByGoods($openid, $orderid,$afterbuy=false) {
            ...
            }elseif($newlevel['id'] == $set['quota_second_level']){
