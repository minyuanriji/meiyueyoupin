...
if (!class_exists('CommissionModel')) {
    class CommissionModel extends PluginModel {
        ...
        public function calculatebonus($orderid=0,$update = false){
            ...
            $check = ($member['agentlevel'] == $set['quota_level'] && $member['status'] == 1 && $member['isagent'] == 1);
            if(!$check){
                // 不是指定分销等级出去
                return null;
            }
            $has_rec = pdo_fetchcolumn('SELECT ifnull(count(*),0) FROM ' . tablename('vcshop_rewardlog') . ' WHERE uniacid=:uniacid AND type in(1,2) AND mid=:mid AND levelweight>=:levelweight',array(':uniacid'=>$_W['uniacid'],':mid' =>$member['id'],':levelweight'=>$level['level']));
        ...
        public function getInfo($openid, $options = NULL) {
            ...
            if (1 <= $level) {
                ...
                if (in_array('ok', $options)) {
                    ...
                    // team_bonus_data && express
                    $level1_commissions_reward = pdo_fetchcolumn('SELECT sum(money) FROM ' . tablename('vcshop_rewardlog') . ' t ' . ' LEFT JOIN ' . tablename('vcshop_order') . ' o on o.id = t.orderid' . ' WHERE t.agentid=:agentid AND o.status>=3 AND (' . $time . ' - o.finishtime > ' . $day_times . ') AND t.status=0 AND o.uniacid=:uniacid AND o.isparent=0 AND t.is_delete=0',array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                ...
                if (in_array('apply', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('vcshop_order_goods') . ' og ' . ' left join  ' . tablename('vcshop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                    ...
                    // team_bonus_data && express
                    $level1_commissions2_reward = pdo_fetchcolumn('SELECT sum(money) FROM ' . tablename('vcshop_rewardlog') . ' t ' . ' LEFT JOIN ' . tablename('vcshop_order') . ' o on o.id = t.orderid' . ' WHERE t.agentid=:agentid AND o.status>=3 AND t.status=1 AND o.uniacid=:uniacid AND o.isparent=0 AND t.is_delete=0',array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    $commission_apply += $level1_commissions2_reward;
                }
                if (in_array('check', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('vcshop_order_goods') . ' og ' . ' left join  ' . tablename('vcshop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                    foreach ($level1_commissions2 as $c) {
                    ...
                    // team_bonus_data && express
                    $level1_commissions2_reward = pdo_fetchcolumn('SELECT sum(money) FROM ' . tablename('vcshop_rewardlog') . ' t ' . ' LEFT JOIN ' . tablename('vcshop_order') . ' o on o.id = t.orderid' . ' WHERE t.agentid=:agentid AND o.status>=3 AND t.status=2 AND o.uniacid=:uniacid AND o.isparent=0 AND t.is_delete=0',array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    $commission_check += $level1_commissions2_reward;
                }
                if (in_array('pay', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('vcshop_order_goods') . ' og ' . ' left join  ' . tablename('vcshop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                    foreach ($level1_commissions2 as $c) {
                    ...
                    // team_bonus_data && express
                    $level1_commissions2_reward = pdo_fetchcolumn('SELECT sum(money) FROM ' . tablename('vcshop_rewardlog') . ' t ' . ' LEFT JOIN ' . tablename('vcshop_order') . ' o on o.id = t.orderid' . ' WHERE t.agentid=:agentid AND o.status>=3 AND t.status=3 AND o.uniacid=:uniacid AND o.isparent=0 AND t.is_delete=0',array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
...
感谢您的支持',
         ...
         * @param,int,$id,升级人id
         */
        public function agentReward($id,$level)
        {
            global $_W;
            ...
            $tm         = $set['tm'];
            foreach ($parents as $key => $value) {
                if($value['levelweight'] >= $level['level']){
                    break;
                }
                $message = $tm['commission_teamupgrade'];
                if (empty($message)) {
