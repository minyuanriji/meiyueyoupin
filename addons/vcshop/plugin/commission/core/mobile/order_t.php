...
class Order_VcShopPage extends CommissionMobileLoginPage
{
	...
	public function get_list()
	{
		...
		if (1 <= $level) {
			...
			$level1_orders_team_bonus = pdo_fetchall('SELECT o.id,o.createtime,t.money FROM ' . tablename('vcshop_rewardlog') . ' t' . ' LEFT JOIN ' . tablename('vcshop_order') . ' o ON o.id=t.orderid ' . ' WHERE t.uniacid=:uniacid AND t.agentid=:agentid  AND type=2 ' . $condition, array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (!empty($this->set['openorderdetail'])) {
					...
					foreach ($goods as &$g) {
						...
						// else if ($orderids[$row['id']]['level'] == 2) {
							...
							if(in_array(4, $orderids[$row['id']]['levels'])){
								$reward_logs = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_rewardlog') . ' WHERE uniacid=:uniacid AND orderid=:orderid AND type=2 ORDER BY id ASC,floot ASC',array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
