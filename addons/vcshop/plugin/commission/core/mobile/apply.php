...
class Apply_VcShopPage extends CommissionMobileLoginPage
{
	public function main()
	{
		...
		$orderids = array();
		$paystatus = 1;
		$maxnum = 200;//20201211 单次审核最多提交数量
		if (1 <= $level) {
			// o.status>=3
			...
			foreach ($level1_orders as $o) {
				...
				if(count($orderids) > $maxnum){
					// han 20201211 数量超过300会导致后台审核漏记
					break;
				}
				// han 20190619 add ,'isteam_bonus'=>false,'isexpress'=>false
				// han 20200402 add ,'isstar_bonus'=>false
				$orderids[] = array('orderid' => $o['id'], 'level' => 1,'isone'=>false,'isteam_bonus'=>false,'isstar_bonus'=>false);
			}
		}
		...
		foreach ($orderids as &$o) {
			...
			foreach ($reward_log as $tk => $t) {
				if (($o['level'] == 1) && ($t['status'] == 0)  && ($t['agentid'] == $member['id'])) {
					...
					if($t['type'] == 3){
						$o['isstar_bonus'] = true;// han 20200402
					}
		...
		if ($_W['ispost']) {
			...
			foreach ($orderids as $o) {
				...
				if($o['level'] == 1){
					...
					// han 20200402 提成
					if($o['isstar_bonus']){
						pdo_update('vcshop_rewardlog', array('status' => 1, 'applytime' => $time), array('orderid' => $o['orderid'], 'uniacid' => $_W['uniacid'], 'agentid'=>$member['id'],'type'=>3,'status'=>0));
					}
				}else{
					pdo_update('vcshop_order_goods', array('status' . $o['level'] => 1, 'applytime' . $o['level'] => $time), array('orderid' => $o['orderid'], 'uniacid' => $_W['uniacid']));
				}
