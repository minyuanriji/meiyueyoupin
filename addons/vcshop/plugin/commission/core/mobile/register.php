...
class Register_VcShopPage extends CommissionMobileLoginPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $icode) {
				if (p('offic')) {
					$iagent = pdo_fetch('select * from ' . tablename('vcshop_member') . ' where mobile = :mobile and uniacid = :uniacid limit 1', array(':mobile' => $icode, ':uniacid' => intval($_W['uniacid'])));
		...
		if (empty($set['become'])) {
			...
			if ($set['become'] == 4) {
				...
				$goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order_goods') . ' og ' . '  left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid' . (' where og.goodsid=:goodsid and o.openid=:openid and o.status>=' . $order_status . '  and og.createtime >= ' . $time . '  limit 1'), array(':goodsid' => $set['become_goodsid'], ':openid' => $openid));
				...
				if ($goodscount <= 0) {
					...
					if (empty($member['isagent'])) {
						pdo_update('vcshop_member', array('applyagenttime' => time()), array('id' => $member['id']));
					}
				}
				else {
					$to_check_agent = true;
