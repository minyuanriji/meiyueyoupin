...
class Index_VcShopPage extends PluginMobilePage
{
    ...
    public function select()
    {
        ...
        if (!empty($shop["selectgoods"])) {
            $goodsids = explode(",", $shop["goodsids"]);
            if (!empty($goodsids)) {
                $goods = pdo_fetchall("select id,title,marketprice,thumb from " . tablename("vcshop_goods") . " where uniacid=:uniacid and id in ( " . trim($shop["goodsids"]) . ")", array(":uniacid" => $_W["uniacid"]));
