...
require_once VCSHOP_PATH . 'vendor/phpquery/phpQuery.php';
class Api_VcShopPage extends PluginWebPage
{
	...
	private function api_taobao($cath_id = 0, $catch_url = '', $pageSource = '')
	{
		...
		if (!empty($pageSource)) {
			...
			foreach ($value as $key => $val) {
				pdo_insert('vcshop_goods_param', array('goodsid' => $goodsid, 'uniacid' => $_W['uniacid'], 'title' => $key, 'value' => $val));
			}
			...
			if (!empty($hasoption) && !empty($spec_list)) {
				foreach ($spec_list as $k => $v) {
					pdo_insert('vcshop_goods_spec', array('uniacid' => $_W['uniacid'], 'goodsid' => $goodsid, 'title' => $k));
					$specId = pdo_insertid();

					foreach ($v['spec_item'] as $key => $val) {
						$title = $val;
						$thumb = '';
						pdo_insert('vcshop_goods_spec_item', array('specid' => $specId, 'title' => $title, 'thumb' => empty($thumb) ? '' : $thumb, 'show' => 1));
