...
class Apply_VcShopPage extends DividendMobileLoginPage
{
	public function main()
	{
		...
		$dividend_ok = 0;
		$orders = pdo_fetchall('select id,dividend from ' . tablename('vcshop_order') . (' where headsid=:headsid and status>=3  and dividend_status=0 and (' . $time . ' - finishtime > ' . $day_times . ') and uniacid=:uniacid  group by id'), array(':uniacid' => $_W['uniacid'], ':headsid' => $member['id']));
		...
		if ($this->set['cashother'] == 1) {
			...
			if ($this->set['cashcard'] == 1) {
				...
				$banklist = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_dividend_bank') . (' WHERE 1 ' . $condition . '  ORDER BY displayorder DESC'), $params);
			}
		}
		...
		if ($_W['ispost']) {
			...
			foreach ($orders as $o) {
				$orderids[] = $o['id'];
				pdo_update('vcshop_order', array('dividend_status' => 1, 'dividend_applytime' => $time), array('id' => $o['id'], 'uniacid' => $_W['uniacid']));
			}
			...
			pdo_insert('vcshop_dividend_apply', $apply);
