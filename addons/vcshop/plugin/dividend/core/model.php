...
if (!class_exists('DividendModel')) {
	class DividendModel extends PluginModel
	{
		...
		public function getInfo($openid, $options = NULL)
		{
			...
			$groupscount = 0;
			$groupscounts = pdo_fetch('select count(id) as counts from ' . tablename('vcshop_member') . ' where headsid = :headsid and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('total', $options)) {
				$order_all = pdo_fetchall('select id,headsid,price,deductcredit2,dispatchprice,dividend from ' . tablename('vcshop_order') . ' where 1 and headsid = :headsid and status >= 1 and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('ok', $options)) {
				$divedend_order_ok = pdo_fetchall('select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from ' . tablename('vcshop_order') . ' where 1 and headsid = :headsid and status >= 3 and dividend_status=0 and (' . $time . '-finishtime > ' . $day_times . ') and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('check', $options)) {
				$check_apply = pdo_fetchall('select id,orderids,dividend from ' . tablename('vcshop_dividend_apply') . ' where mid = :mid and status = 2 and uniacid = :uniacid', array(':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('check', $options)) {
				$check_apply = pdo_fetchall('select id,orderids,dividend from ' . tablename('vcshop_dividend_apply') . ' where mid = :mid and status = 1 and uniacid = :uniacid', array(':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('pay', $options)) {
				$check_apply_pay = pdo_fetchall('select id,orderids,dividend from ' . tablename('vcshop_dividend_apply') . ' where mid = :mid and status = 3 and uniacid = :uniacid', array(':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('lock', $options)) {
				$check_apply_lock = pdo_fetchall('select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from ' . tablename('vcshop_order') . ' where 1 and headsid = :headsid and status >= 3 and dividend_status=0 and (' . $time . '-finishtime <= ' . $day_times . ') and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('wait', $options)) {
				$divedend_order_wait = pdo_fetchall('select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from ' . tablename('vcshop_order') . ' where 1 and headsid = :headsid and status in (1,2) and (' . $time . '-finishtime > ' . $day_times . ') and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('fail', $options)) {
				$check_apply_fail = pdo_fetchall('select id,orderids,dividend from ' . tablename('vcshop_dividend_apply') . ' where mid = :mid and status = -1 and uniacid = :uniacid', array(':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('ordercount0', $options)) {
				$order0 = pdo_fetch('select sum(price) as ordermoney,count(distinct id) as ordercount from ' . tablename('vcshop_order') . ' where headsid = :headsid and status >=0 and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('ordercount', $options)) {
				$order = pdo_fetch('select sum(price) as ordermoney,count(distinct id) as ordercount from ' . tablename('vcshop_order') . ' where headsid = :headsid and status >=1 and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
			...
			if (in_array('ordercount3', $options)) {
				$order3 = pdo_fetch('select sum(price) as ordermoney,count(distinct id) as ordercount from ' . tablename('vcshop_order') . ' where headsid = :headsid and status >=3 and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
		...
		public function getSimpleInfo($openid, $options = NULL)
		{
			...
			if (in_array('total', $options)) {
				$order_all = pdo_fetchall('select id,headsid,price,deductcredit2,dispatchprice,dividend from ' . tablename('vcshop_order') . ' where 1 and headsid = :headsid and status >= 0 and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
		...
		public function getHeadsDownNum($openid = NULL)
		{
			...
			$itemscount = 0;
			$count = (int) pdo_fetchcolumn('select count(id) from ' . tablename('vcshop_member') . ' where headsid=:agentid and headsstatus = 0 and  and uniacid=:uniacid limit 1', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));
		...
		public function checkOrderConfirm($orderid = '0')
		{
			...
			$order = pdo_fetch('select id,openid,ordersn,goodsprice,agentid,paytime,officcode,dispatchprice,deductcredit2,price,deductcredit2 from ' . tablename('vcshop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
			...
			pdo_update('vcshop_order', array('headsid' => $member['headsid'], 'dividend' => iserializer($divedend)), array('id' => $orderid));
			return true;
		}
		...
		public function orderFinishTask($order, $self_buy = false, $member)
		{
			...
			$order_goods_list = pdo_fetchall('SELECT commissions FROM ' . tablename('vcshop_order_goods') . ' WHERE orderid = :order_id AND uniacid = :uniacid AND nocommission = 0', array(':order_id' => $order_id, ':uniacid' => $_W['uniacid']));
...
请您注意审核团队分红的分红提现申请，及时处理哦', 'color' => '#000000')
										...
										if ($message_type == TM_DIVIDEND_DOWNLINE_BECOME && empty($usernotice['dividend_downline_become'])) {
											...
											$heads = pdo_fetch('select id,openid from ' . tablename('vcshop_member') . ' where id=:id', array(':id' => $member['headsid']));
...
感谢您的支持', 'color' => '#000000')
		...
		public function getRepurchase($openid, array $time)
		{
			...
			foreach ($time as $value) {
				...
				if (!empty($repurchase_price)) {
					$order_price = (double) pdo_fetchcolumn('SELECT SUM(price) as price FROM ' . tablename('vcshop_order') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `status`>2 AND `createtime` BETWEEN :time1 AND :time2', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':time1' => $time1, ':time2' => $time2));
					$year_month = explode('-', $value);
					$year_month[0] = (int) $year_month[0];
					$year_month[1] = (int) $year_month[1];
					$residue_price = (double) pdo_fetchcolumn('SELECT SUM(repurchase) FROM ' . tablename('vcshop_commission_repurchase') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `year`=:year AND `month`=:month', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':year' => $year_month[0], ':month' => $year_month[1]));
		...
		public function getAllDown($openid)
		{
			...
			if ($uid == 0) {
				$info = pdo_fetch('select id,openid,uniacid,agentid,isagent,status from ' . tablename('vcshop_member') . ' where  openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			...
			$agents = pdo_fetchall('select id,openid,uniacid,agentid,isagent,nickname,agenttime,createtime,avatar,status,realname,mobile,weixin from ' . tablename('vcshop_member') . ' where uniacid=:uniacid and agentid=:agentid', array(':uniacid' => $_W['uniacid'], ':agentid' => $uid));
			$ids = array();
			$openids = array();
			$users = array();
		...
		public function getAllDownOrder($openid, $start = 0, $end = 0)
		{
			...
			if (!empty($down['openids'])) {
				$order = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_order') . ' WHERE uniacid=:uniacid AND openid IN (\'' . implode('\',\'', $down['openids']) . '\') AND createtime BETWEEN :time1 AND :time2 AND ccard>0', array(':uniacid' => $_W['uniacid'], ':time1' => $start, ':time2' => $end));
		...
		public function update_headsid($memberid, $agentid)
		{
			global $_W;
			global $_GPC;
			$agent = pdo_fetch('select id,isheads,headsid,headsstatus from ' . tablename('vcshop_member') . ' where id = :id', array(':id' => $agentid));
			if (!empty($agent['isheads']) && !empty($agent['headsstatus'])) {
				pdo_update('vcshop_member', array('headsid' => $agentid), array('id' => $memberid, 'uniacid' => $_W['uniacid']));
			}
			else {
				if (empty($agent['isheads']) && !empty($agent['headsid'])) {
					pdo_update('vcshop_member', array('headsid' => $agent['headsid']), array('id' => $memberid, 'uniacid' => $_W['uniacid']));
				}
			}
		}
	}
