...
class Index_VcShopPage extends DividendWebPage
{
	...
	public function init()
	{
		...
		$count = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('vcshop_member') . ' WHERE uniacid=:uniacid AND agentid>0', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['isajax']) {
			...
			$list = pdo_fetchall('SELECT id,agentid FROM ' . tablename('vcshop_member') . ' WHERE uniacid=:uniacid AND agentid>0  ORDER BY id asc limit ' . ($pindex - 1) * $psize . ',' . $psize, array(':uniacid' => $_W['uniacid']));

			if (!empty($list)) {
				foreach ($list as $member) {
					if ($member['id'] != $member['agentid']) {
						p('commission')->delRelation($member['id']);// han 20190907 add
						...
						if (is_array($mem)) {
							...
							pdo_insert('a',array('info'=>json_encode($errData)));
	...
	public function refresh()
	{
		...
		$data['init'] = 0;
		m('common')->updatePluginset(array('dividend' => $data));
		pdo_delete('vcshop_commission_relation');
