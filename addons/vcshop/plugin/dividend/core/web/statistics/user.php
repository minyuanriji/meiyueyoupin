...
class User_VcShopPage extends DividendWebPage
{
	public function main()
	{
		...
		$psize = 100;
		$sql = 'select * from ' . tablename('vcshop_member') . ' where headsid = :headsid and uniacid = :uniacid ORDER BY headstime desc';
		...
		if (!empty($searchstart)) {
			...
			$total_sql = 'select COUNT(1) from ' . tablename('vcshop_member') . ' where headsid = :headsid and uniacid = :uniacid';
		...
		if (!empty($userlist)) {
			...
			foreach ($userlist as $k => $v) {
				...
				$level11 = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member') . ' where isagent=0 and agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $agentid, ':uniacid' => $_W['uniacid']));
				$condition = '';
				$params = array();
				...
				if ($hasagent) {
					$child_sql = 'select dm.* from ' . tablename('vcshop_member') . ' dm ' . ' left join ' . tablename('mc_mapping_fans') . ('f on f.openid=dm.openid  and f.uniacid=' . $_W['uniacid']) . ' where dm.uniacid = ' . $_W['uniacid'] . (' ' . $condition . '   ORDER BY dm.agenttime desc');
...
';
	...
	public function check()
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,status FROM ' . tablename('vcshop_member') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		$time = time();
	...
	public function agentblack()
	{
		...
		foreach ($members as $member) {
			...
			if ($agentblack == 1) {
				pdo_update('vcshop_member', array('isagent' => 1, 'status' => 0, 'agentblack' => 1), array('id' => $member['id']));
				plog('commission.agent.agentblack', '设置黑名单 <br/>分销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else {
				pdo_update('vcshop_member', array('isagent' => 1, 'status' => 1, 'agentblack' => 0), array('id' => $member['id']));
