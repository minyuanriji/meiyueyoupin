...
if (!class_exists('AbonusModel')) {
	class AbonusModel extends PluginModel
	{
		...
		public function getLevels($all = true, $default = false)
		{
			...
			if ($all) {
				$levels = pdo_fetchall('select * from ' . tablename('vcshop_abonus_level') . ' where uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
			}
			else {
				$levels = pdo_fetchall('select * from ' . tablename('vcshop_abonus_level') . ' where uniacid=:uniacid and (ordermoney>0 or bonusmoney>0) order by bonus asc', array(':uniacid' => $_W['uniacid']));
			}
		...
		public function getBonus($openid = '', $params = array())
		{
			...
			if (in_array('ok', $params)) {
				$ret['ok'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('ok1', $params)) {
				$ret['ok1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('ok2', $params)) {
				$ret['ok2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('ok3', $params)) {
				$ret['ok3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)   from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('lock', $params)) {
				$ret['lock'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('lock1', $params)) {
				$ret['lock1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('lock2', $params)) {
				$ret['lock2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('lock3', $params)) {
				$ret['lock3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)  from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('total', $params)) {
				$ret['total'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('total1', $params)) {
				$ret['total1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('total2', $params)) {
				$ret['total2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('total3', $params)) {
				$ret['total3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)  from ' . tablename('vcshop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
...
感谢您的支持', 'color' => '#000000')
		...
		public function getLevel($openid)
		{
			...
			$level = pdo_fetch('select * from ' . tablename('vcshop_abonus_level') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['aagentlevel']));
			return $level;
		}
		...
		public function upgradeLevelByOrder($openid)
		{
			...
			if ($leveltype == 4 || $leveltype == 5) {
				...
				if ($leveltype == 4) {
					$newlevel = pdo_fetch('select * from ' . tablename('vcshop_abonus_level') . (' where uniacid=:uniacid  and ' . $ordermoney . ' >= ordermoney and ordermoney>0  order by ordermoney desc limit 1'), array(':uniacid' => $_W['uniacid']));
					...
					if ($leveltype == 5) {
						$newlevel = pdo_fetch('select * from ' . tablename('vcshop_abonus_level') . (' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount and ordercount>0  order by ordercount desc limit 1'), array(':uniacid' => $_W['uniacid']));
				...
				pdo_update('vcshop_member', array('aagentlevel' => $newlevel['id']), array('id' => $m['id']));
				...
				if (0 <= $leveltype && $leveltype <= 3) {
					...
					foreach ($agents as $agent) {
						...
						pdo_update('vcshop_member', array('aagentlevel' => $newlevel['id']), array('id' => $agent['id']));
			...
			if ($leveltype == 6 || $leveltype == 8) {
				...
				foreach ($agents as $agent) {
					...
					if ($leveltype == 6) {
						...
						$newlevel = pdo_fetch('select * from ' . tablename('vcshop_abonus_level') . (' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1'), array(':uniacid' => $_W['uniacid']));
					}
					else {
						if ($leveltype == 8) {
		...
		public function upgradeLevelByCommissionOK($openid)
		{
			...
			$newlevel = pdo_fetch('select * from ' . tablename('vcshop_abonus_level') . (' where uniacid=:uniacid  and ' . $commissionmoney . ' >= commissionmoney and commissionmoney>0  order by commissionmoney desc limit 1'), array(':uniacid' => $_W['uniacid']));
		...
		public function upgradeLevelByBonus($openid)
		{
			...
			$newlevel = pdo_fetch('select * from ' . tablename('vcshop_abonus_level') . (' where uniacid=:uniacid  and ' . $bonusmoney['ok'] . ' >= bonusmoney and bonusmoney>0  order by bonusmoney desc limit 1'), array(':uniacid' => $_W['uniacid']));
		...
		public function getBonusData($year = 0, $month = 0, $week = 0, $openid = '')
		{
			...
			$bill = pdo_fetch('select * from ' . tablename('vcshop_abonus_bill') . ' where uniacid=:uniacid and `year`=:years and `month`=:months and `week`=:week limit 1', array(':uniacid' => $_W['uniacid'], ':years' => $year, ':months' => $month, ':week' => $week));
			...
			$ordermoney = 0;
			$bonusmoney = 0;
			$orders = pdo_fetchall('select id,openid,price,addressid, address from ' . tablename('vcshop_order') . (' where uniacid=' . $_W['uniacid'] . ' and addressid<>0 and status=3 and isabonus=0 and finishtime + ' . $settletimes . '>= ' . $starttime . ' and  finishtime + ' . $settletimes . '<=' . $endtime), array(), 'id');
			$pcondition = '';
			...
			foreach ($orders as $o) {
				...
				foreach ($aagents as &$a) {
					...
					if (!is_array($address)) {
						$address = pdo_fetch('select * from ' . tablename('vcshop_member_address') . ' where id=:id limit 1', array(':id' => $o['addressid']));
					}
		...
		{
			global $_W;
			return array('total0' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_abonus_bill') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])), 'total1' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_abonus_bill') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])), 'total2' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_abonus_bill') . ' where uniacid=:uniacid and status=2  limit 1', array(':uniacid' => $_W['uniacid'])));
		}
	}
}
