...
class Level_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_abonus_level') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc'));
	...
	protected function post()
	{
		...
		if ($id == 'default') {
			...
			$level = pdo_fetch('SELECT * FROM ' . tablename('vcshop_abonus_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => intval($id), ':uniacid' => $_W['uniacid']));
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if ($id == 'default') {
					...
					pdo_update('vcshop_abonus_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					plog('abonus.level.edit', '修改代理商等级 ID: ' . $id . $updatecontent);
				}
			}
			else {
				pdo_insert('vcshop_abonus_level', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('vcshop_abonus_level') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('vcshop_abonus_level', array('id' => $item['id']));
