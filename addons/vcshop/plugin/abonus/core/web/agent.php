...
class Agent_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('vcshop_member') . ' dm  ' . ' left join ' . tablename('vcshop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('vcshop_abonus_level') . ' l on l.id = dm.aagentlevel' . ' where dm.uniacid =' . $_W['uniacid'] . (' and dm.isaagent =1  ' . $condition), $params);
		$geo_key = pdo_fetchcolumn('SELECT geo_key FROM ' . tablename('vcshop_city_express') . ' WHERE uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		foreach ($members as $member) {
			pdo_update('vcshop_member', array('isaagent' => 0, 'aagentstatus' => 0), array('id' => $member['id']));
	...
	public function check()
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,aagentstatus,aagenttype,aagentareas,aagentprovinces,aagentcitys FROM ' . tablename('vcshop_member') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		$time = time();
		...
		foreach ($members as $member) {
			...
			if ($status == 1) {
				...
				pdo_update('vcshop_member', array('aagentstatus' => 1, 'aagenttime' => $time), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
				...
				pdo_update('vcshop_member', array('aagentstatus' => 0, 'aagenttime' => 0), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
	...
	public function aagentblack()
	{
		...
		foreach ($members as $member) {
			...
			if ($aagentblack == 1) {
				pdo_update('vcshop_member', array('isaagent' => 1, 'aagentstatus' => 0, 'aagentblack' => 1), array('id' => $member['id']));
				plog('abonus.agent.aagentblack', '设置黑名单 <br/>区域代理信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else {
				pdo_update('vcshop_member', array('isaagent' => 1, 'aagentstatus' => 1, 'aagentblack' => 0), array('id' => $member['id']));
