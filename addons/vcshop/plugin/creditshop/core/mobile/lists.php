...
class Lists_VcShopPage extends CreditshopMobilePage
{
	public function main()
	{
		...
		if (!empty($cateid)) {
			$cate = pdo_fetch('select id,name from ' . tablename('vcshop_creditshop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cateid, ':uniacid' => $uniacid));
		}
		...
		if (0 < intval($_GPC['merchid'])) {
			...
			if (!empty($merch_category)) {
				foreach ($merch_category as $index => $row) {
					if (0 < $row) {
						$category[$index] = pdo_fetchall('select id,name,thumb,isrecommand from ' . tablename('vcshop_creditshop_category') . '
	...
	public function getlist()
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('vcshop_creditshop_goods') . (' where 1 ' . $condition);
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT id,title,thumb,subtitle,`type`,price,credit,money,goodstype,hasoption,mincredit,minmoney FROM ' . tablename('vcshop_creditshop_goods') . '
