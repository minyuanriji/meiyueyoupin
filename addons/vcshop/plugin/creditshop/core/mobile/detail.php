...
require VCSHOP_PLUGIN . 'creditshop/core/page_mobile.php';
class Detail_VcShopPage extends CreditshopMobilePage
{
	public function main()
	{
		...
		if (!empty($goods)) {
			pdo_update('vcshop_creditshop_goods', array('views' => $goods['views'] + 1), array('id' => $id));
		...
		$log = array();
		$log = pdo_fetchall('select openid,createtime from ' . tablename('vcshop_creditshop_log') . '
		...
		$logtotal = 0;
		$logtotal = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_creditshop_log') . ' where uniacid = ' . $uniacid . ' and goodsid = ' . $id . ' and status > 0 ');
		$replys = array();
		$replys = pdo_fetchall('select * from ' . tablename('vcshop_creditshop_comment') . '
		...
		$replytotal = 0;
		$replytotal = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_creditshop_comment') . '
		...
		$goodsrec = pdo_fetchall('select id,thumb,title,credit,money,mincredit,minmoney from ' . tablename('vcshop_creditshop_goods') . '
	...
	public function getlistlog()
	{
		...
		$logtotal = 0;
		$logtotal = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_creditshop_log') . ' where uniacid = ' . $uniacid . ' and goodsid = ' . $goodsid . ' and status > 0 ');
	...
	public function option()
	{
		...
		$goods = pdo_fetch('select id,thumb,credit,money,total,title from ' . tablename('vcshop_creditshop_goods') . ' where id= ' . $id . ' and uniacid = ' . $uniacid . ' ');
		$goods = set_medias($goods, 'thumb');
		$specs = false;
		$options = false;
		$specs = pdo_fetchall('select * from ' . tablename('vcshop_creditshop_spec') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));

		foreach ($specs as &$spec) {
			$spec['items'] = pdo_fetchall('select * from ' . tablename('vcshop_creditshop_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $spec['id']));
		}

		unset($spec);
		$options = pdo_fetchall('select * from ' . tablename('vcshop_creditshop_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function pay($a = array(), $b = array())
	{
		...
		if ($goods['hasoption'] && $optionid) {
			$option = pdo_fetch('select total from ' . tablename('vcshop_creditshop_option') . ' where uniacid = ' . $uniacid . ' and id = ' . $optionid . ' and goodsid = ' . $id . ' ');
		...
		if (0 < $goods['money'] || $goods['dispatch']) {
			$needpay = true;
			$lastlog = pdo_fetch('select * from ' . tablename('vcshop_creditshop_log') . ' where goodsid=:goodsid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':goodsid' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
		...
		if (!empty($addressid)) {
			$address = pdo_fetch('select id,realname,mobile,address,province,city,area,street from ' . tablename('vcshop_member_address') . '
		...
		pdo_insert('vcshop_creditshop_log', $log);
		$logid = pdo_insertid();
		...
		if ($needpay) {
			...
			if ($paytype == 'balance') {
				...
				pdo_update('vcshop_creditshop_log', array('paytype' => $paytype), array('id' => $logid));
			}
	...
	public function wechat_complete()
	{
		...
		if (empty($log)) {
			...
			$log = pdo_fetch('select * from ' . tablename('vcshop_creditshop_log') . ' where logno=:logno and uniacid=:uniacid limit 1', array(':logno' => $logno, ':uniacid' => $_W['uniacid']));
		}
		...
		if (!empty($log)) {
			...
			if (!is_error($payquery) || !is_error($payqueryborrow)) {
				if ($log['status'] < 1) {
					...
					$record['paystatus'] = 1;
					$record['paytype'] = 1;
					pdo_update('vcshop_creditshop_log', $record, array('logno' => $log['logno']));
					...
					if (is_h5app()) {
						pdo_update('vcshop_creditshop_log', array('apppay' => 1), array('logno' => $log['logno']));
					}
				}
	...
	public function creditshop_complete()
	{
		...
		if ($lastlog['status'] < 1) {
			$record = array();
			$record['paystatus'] = 1;
			pdo_update('vcshop_creditshop_log', $record, array('logno' => $tid));
			$creditlog = pdo_fetch('select id from ' . tablename('vcshop_creditshop_log') . '
	...
	public function lottery()
	{
		...
		if (empty($log)) {
			$logno = $_GPC['logno'];
			$log = pdo_fetch('select * from ' . tablename('vcshop_creditshop_log') . ' where logno=:logno and uniacid=:uniacid limit 1', array(':logno' => $logno, ':uniacid' => $uniacid));
		}
		...
		if ($status == 2 && $update['creditpay'] == 1) {
			...
			if ($status == 3) {
				pdo_query('update ' . tablename('vcshop_creditshop_goods') . ' set total=total-' . $number . ' where id=' . $log['goodsid']);
			}
			...
			if ($goods['goodstype'] == 3 && $status == 2) {
				pdo_query('update ' . tablename('vcshop_creditshop_goods') . ' set packetsurplus=packetsurplus-' . $number . ' where id=' . $log['goodsid']);
			}

			if ($goods['hasoption'] && $log['optionid']) {
				pdo_query('update ' . tablename('vcshop_creditshop_option') . ' set total=total-' . $number . ' where id=' . $log['optionid']);
			}
		}
