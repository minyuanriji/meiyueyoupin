...
class Index_VcShopPage extends CreditshopMobilePage
{
	public function main()
	{
		...
		$advs = pdo_fetchall('select id,advname,link,thumb from ' . tablename('vcshop_creditshop_adv') . ' where ' . $contation . ' and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		...
		if (0 < intval($_GPC['merchid'])) {
			...
			if (!empty($merch_category)) {
				...
				foreach ($merch_category as $index => $row) {
					if (0 < $row) {
						$list = pdo_fetch('select id,name,thumb,isrecommand from ' . tablename('vcshop_creditshop_category') . '
			...
			$category = pdo_fetchall('select id,name,thumb,isrecommand from ' . tablename('vcshop_creditshop_category') . ' where uniacid=:uniacid and  enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
			$category = set_medias($category, 'thumb');
		}

		array_values($category);
		$lotterydraws = pdo_fetchall('select id, title,goodstype, subtitle, credit, money, thumb,`type`,price from ' . tablename('vcshop_creditshop_goods') . '
		...
		$exchanges = pdo_fetchall('select id, title,goodstype, subtitle, credit, money, thumb,`type` from ' . tablename('vcshop_creditshop_goods') . '
		...
		$coupons = pdo_fetchall('select id, title, subtitle, credit, money, thumb,`type` from ' . tablename('vcshop_creditshop_goods') . '
		...
		$balances = pdo_fetchall('select id, title, subtitle, credit, money, thumb,`type` from ' . tablename('vcshop_creditshop_goods') . '
		...
		$redbags = pdo_fetchall('select id, title, subtitle, credit, money, thumb,`type` from ' . tablename('vcshop_creditshop_goods') . '
