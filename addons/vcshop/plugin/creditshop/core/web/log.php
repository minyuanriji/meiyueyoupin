...
class Log_VcShopPage extends PluginWebPage
{
	...
	public function goodsfinish()
	{
		...
		$log = pdo_fetch('select * from ' . tablename('vcshop_creditshop_log') . ' where id=:id ' . $condition . ' limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid']));
	...
	protected function getData($type)
	{
		...
		foreach ($list as $key => &$row) {
			if ($row['hasoption'] == 1 && 0 < $row['optionid']) {
				$option = pdo_fetch('select total,credit,money,title as optiontitle,weight from ' . tablename('vcshop_creditshop_option') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $row['optionid'] . ' and goodsid = ' . $row['goodsid'] . ' ');
		...
		foreach ($list as &$row) {
			...
			if (!empty($row['addressid'])) {
				$row['address'] = pdo_fetch('select realname,mobile,address,province,city,area,street from ' . tablename('vcshop_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $row['addressid'], ':uniacid' => $_W['uniacid']));
			}
			else {
		...
		if (!empty($goods['isverify'])) {
			if (0 < $log['merchid']) {
				if (!empty($log['storeid'])) {
					$verifys = pdo_fetchall('select cv.verifytime,cv.verifier,s.storename,s.address,m.realname,m.nickname,m.mobile from ' . tablename('vcshop_creditshop_verify') . ' as cv
			        left join ' . tablename('vcshop_merch_store') . ' as s on s.uniacid = cv.uniacid and s.id = cv.storeid and s.merchid = cv.merchid
			        left join ' . tablename('vcshop_member') . ' as m on m.openid = cv.verifier and m.uniacid = cv.uniacid 
				...
				if (!empty($log['storeid'])) {
					$verifys = pdo_fetchall('select cv.verifytime,cv.verifier,s.storename,s.address,m.realname,m.nickname,m.mobile from ' . tablename('vcshop_creditshop_verify') . 'as cv
			        left join ' . tablename('vcshop_store') . ' as s on s.uniacid = cv.uniacid and s.id = cv.storeid
			        left join ' . tablename('vcshop_member') . ' as m on m.openid = cv.verifier and m.uniacid = cv.uniacid 
			        where cv.logid = :logid and cv.uniacid = :uniacid order by cv.verifytime DESC ', array(':logid' => $log['id'], ':uniacid' => $_W['uniacid']));
				}
			}
			...
			if (empty($address)) {
				$address = pdo_fetch('select realname,mobile,address,province,city,area,street from ' . tablename('vcshop_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $_W['uniacid']));
			}
		}
		...
		if ($_W['ispost']) {
			if ($goods['goodstype'] == 0) {
				...
				if (0 < $goods['isverify']) {
					...
					pdo_update('vcshop_creditshop_log', array('status' => 3, 'usetime' => time(), 'time_send' => time(), 'expresscom' => $_GPC['expresscom'], 'expresssn' => $_GPC['expresssn'], 'express' => $_GPC['express']), array('id' => $id));
				}
			}
			else {
				if ($goods['goodstype'] == 3) {
					...
					$goods = pdo_fetch('select surplusmoney from ' . tablename('vcshop_creditshop_goods') . ' where id = ' . $log['goodsid'] . ' ');
	...
	public function remarksaler()
	{
		...
		if ($_W['ispost']) {
			pdo_update('vcshop_creditshop_log', array('remarksaler' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	protected function logData()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_creditshop_log') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
