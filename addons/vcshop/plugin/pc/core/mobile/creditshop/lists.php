...
class Lists_VcShopPage extends CreditshopMobilePage 
{
	public function main() 
	{
		...
		$category = pdo_fetchall('select id,name,thumb,isrecommand from ' . tablename('vcshop_creditshop_category') . "\n\t\t\t\t\t\t" . 'where uniacid=:uniacid and  enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
	...
	public function getlist() 
	{
		...
		if (!(empty($total))) 
		{
			$sql = 'SELECT id,title,thumb,subtitle,`type`,price,credit,money,goodstype FROM ' . tablename('vcshop_creditshop_goods') . "\n" . '            ' . "\t\t" . 'where 1 ' . $condition . ' ORDER BY displayorder desc,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
