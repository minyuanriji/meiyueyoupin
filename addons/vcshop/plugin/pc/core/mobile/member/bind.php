...
class Bind_VcShopPage extends PcMobileLoginPage 
{
	protected $member;
	...
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$member2 = pdo_fetch('select * from ' . tablename('vcshop_member') . ' where mobile=:mobile and uniacid=:uniacid and mobileverify=1 limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
			if (!(empty($bind))) 
			{
			...
			if (!(empty($member2))) 
			{
				if (strexists($member2['openid'], 'sns_wx_') || strexists($member2['openid'], 'sns_qq_') || strexists($member2['openid'], 'wap_user_')) 
				{
					$tables = pdo_fetchall('SHOW TABLES like \'%_vcshop_%\'');
					...
					pdo_update("vcshop_commission_apply", array("mid" => $member['id']), array('uniacid' => $_W['uniacid'], 'mid' => $member2['id']));
					pdo_update("vcshop_order", array("agentid" => $member['id']), array('agentid' => $member2['id']));
					pdo_update("vcshop_member", array("agentid" => $member['id']), array('agentid' => $member2['id']));
					...
					pdo_delete("vcshop_member", array("id" => $member2['id'], 'uniacid' => $_W['uniacid']));
					$changeAgentid = false;
					...
					if ($changeAgentid) 
					{
						pdo_update('vcshop_member', array('agentid' => $member2['agentid']), array('id' => $member['id']));
					}
					if ($member2['isagent'] && $member2['status']) 
					{
						pdo_update('vcshop_member', array('isagent' => 1, 'status' => 1, 'agenttime' => $member2['agenttime']), array('id' => $member['id']));
					}
					else if ($member2['isagent']) 
					{
						if (!($member['isagent']) && !($member['status'])) 
						{
							pdo_update('vcshop_member', array('isagent' => 1, 'status' => 0), array('id' => $member['id']));
						}
					}
					$memberid = $member['id'];
				...
				{
					pdo_update("vcshop_member", array("pwd" => '', "salt" => '', "mobileverify" => 0), array("id" => $member2['id'], 'uniacid' => $_W['uniacid']));
					$memberid = $member2['id'];
				}
			...
			pdo_update("vcshop_member", array("mobile" => $mobile, 'pwd' => md5($pwd . $salt), 'salt' => $salt, 'mobileverify' => 1, 'createtime' => $createtime), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			unset($_SESSION[$key]);
			m("account")->setLogin($member['id']);
			show_json(1);
		}
