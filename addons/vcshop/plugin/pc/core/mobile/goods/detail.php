...
require VCSHOP_PLUGIN . "pc/core/page_mobile.php";
class Detail_VcShopPage extends PcMobilePage 
{
	public function main() 
	{
		...
		$params = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_goods_param') . ' WHERE uniacid=:uniacid and goodsid=:goodsid AND title<>"" order by displayorder asc', array(':uniacid' => $uniacid, ':goodsid' => $goods['id']));
		$goods = set_medias($goods, 'thumb');
		$goods['canbuy'] = !(empty($goods['status'])) && empty($goods['deleted']);
		if (!(empty($goods['hasoption']))) 
		{
			$options = pdo_fetchall('select id,stock from ' . tablename('vcshop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $goods['id'], ':uniacid' => $_W['uniacid']), 'stock');
	...
	public function get_history() 
	{
		...
		$sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,f.createtime,g.merchid FROM ' . tablename('vcshop_member_history') . ' f ' . ' left join ' . tablename('vcshop_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY f.`id` DESC LIMIT ' . $psize;
