...
class Clearing_VcShopPage extends PluginWebPage
{
	protected function main($status = 0)
	{
		...
		$sql = 'select u.merchname,u.mobile,u.realname,u.logo,o.* from ' . tablename('vcshop_merch_clearing') . ' o ' . ' left join ' . tablename('vcshop_merch_user') . ' u on u.id=o.merchid' . (' where 1 ' . $condition . ' ORDER BY o.id DESC');
		...
		$total = pdo_fetchcolumn('select COUNT(u.id) from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_order') . ' o on u.id=o.merchid' . (' where 1 ' . $condition . ' GROUP BY u.id'), $params);
		$total = count($total);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if ($id) {
				...
				pdo_update('vcshop_merch_clearing', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			}
			else {
				pdo_insert('vcshop_merch_clearing', $data);
				$id = pdo_insertid();
			}
		...
		if ($id) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_merch_clearing') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			empty($item) && $this->message('未找到该账单', '', 'error');
			$merch = pdo_fetch('select id,merchname,logo,realname,mobile from ' . tablename('vcshop_merch_user') . ' where id=:id and uniacid=:uniacid', array(':id' => $item['merchid'], ':uniacid' => $_W['uniacid']));
		}
		else {
			if ($merchid) {
				$merch = pdo_fetch('select id,merchname,logo,realname,mobile from ' . tablename('vcshop_merch_user') . ' where id=:id and uniacid=:uniacid', array(':id' => $merchid, ':uniacid' => $_W['uniacid']));
			}
		}
