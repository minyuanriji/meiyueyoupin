...
class Reg_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select  *  from ' . tablename('vcshop_merch_reg') . ('   where uniacid=:uniacid ' . $condition . ' ORDER BY  applytime desc');
		...
		$total = pdo_fetchcolumn('select count(*) from' . tablename('vcshop_merch_reg') . ('  where uniacid = :uniacid ' . $condition), $params);
	...
	public function detail()
	{
		...
		$id = intval($_GPC['id']);
		$item = pdo_fetch('select * from ' . tablename('vcshop_merch_reg') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			pdo_update('vcshop_merch_reg', $item, array('id' => $item['id']));

			if ($status == 1) {
				$usercount = pdo_fetch('select * from ' . tablename('vcshop_merch_user') . ' where regid=:regid and uniacid=:uniacid limit 1', array(':regid' => $item['id'], ':uniacid' => $_W['uniacid']));
				...
				if (empty($usercount)) {
					...
					$user['regid'] = $item['id'];
					$user['status'] = 0;
					pdo_insert('vcshop_merch_user', $user);
					...
					unset($user['reason']);
					$user['status'] = 0;
					pdo_update('vcshop_merch_user', $user, array('uniacid' => $_W['uniacid'], 'regid' => $item['id']));
					pdo_update('vcshop_merch_reg', $item, array('id' => $item['id']));
					m("notice")->sendMerchSuccess($item["id"]);
					if (ca('merch.user.edit')) {
	...
	public function delete()
	{
		...
		$regs = pdo_fetchall('SELECT id,merchname FROM ' . tablename('vcshop_merch_reg') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($regs as $reg) {
			pdo_delete('vcshop_merch_reg', array('id' => $reg['id']));
