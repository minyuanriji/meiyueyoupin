...
class Credit_VcShopPage extends PluginWebPage
{
	protected function main($status = 1)
	{
		...
		$_sql = 'select iscreditmoney,creditrate from ' . tablename('vcshop_merch_user') . ('where 1 ' . $_condition);
		...
		if ($type == 1) {
			...
			foreach ($orderids as $key => $orderid) {
				if (in_array($orderid, $bpid)) {
					pdo_update('vcshop_creditshop_log', array('merchapply' => 2), array('id' => $orderid));
				}
				else {
					pdo_update('vcshop_creditshop_log', array('merchapply' => -1), array('id' => $orderid));
				}
			}
		}
		else {
