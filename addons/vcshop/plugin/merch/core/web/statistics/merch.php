...
class Merch_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$total = pdo_fetchall('select u.id from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_order') . ' o on u.id=o.merchid' . (' where 1 ' . $condition . ' GROUP BY u.id'), $params);
		$total = count($total);
	...
	public function ajax_user_data()
	{
		...
		$merch = pdo_fetch('select * from ' . tablename('vcshop_merch_user') . ' where id = :id', array(':id' => $merchid));
