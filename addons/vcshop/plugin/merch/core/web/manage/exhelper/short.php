...
class Short_VcShopPage extends MerchWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT id,title,shorttitle,status FROM ' . tablename('vcshop_goods') . (' where 1 and ' . $condition . ' ORDER BY  shorttitle desc, status desc, id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_goods') . (' where 1 and ' . $condition), $params);
	...
	public function edit()
	{
		...
		$goods = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_goods') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid'] . ' AND merchid=' . $merchid);

		foreach ($goods as $good) {
			pdo_update('vcshop_goods', array('shorttitle' => $shorttitle), array('id' => $good['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $merchid));
