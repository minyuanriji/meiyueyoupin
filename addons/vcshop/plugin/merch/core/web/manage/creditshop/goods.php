...
class Goods_VcShopPage extends MerchWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('vcshop_creditshop_goods') . (' where  1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_creditshop_goods') . ' WHERE id =:id and merchid = ' . $merchid . ' and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if (!empty($item['storeids'])) {
			if (0 < $merchid) {
				$stores = pdo_fetchall('select * from ' . tablename('vcshop_merch_store') . ' where id in (' . $item['storeids'] . ') and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
			}
			else {
				$stores = pdo_fetchall('select * from ' . tablename('vcshop_store') . ' where id in (' . $item['storeids'] . ') and uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
			}
		}
...
<a href="javascript:;" class="fa fa-angle-double-down" title="批量设置" onclick="setCol(\'option_money\');"></a></span></div></div></th>';
	...
	public function query()
	{
		...
		$list = pdo_fetchall('SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice,costprice,total,content,hasoption
              FROM ' . tablename('vcshop_goods') . (' WHERE 1 ' . $condition . ' order by createtime desc'), $params);
