...
class Index_VcShopPage extends MerchWebPage
{
	...
	public function shop()
	{
		global $_W;
		global $_GPC;
		$item = pdo_fetch('select * from ' . tablename('vcshop_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_W['uniaccount']['merchid'], ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			pdo_update('vcshop_merch_user', $data, array('id' => $_W['uniaccount']['merchid']));
			show_json(1);
		}
	...
	public function notice()
	{
		...
		if (empty($data)) {
			...
			if ($merchid) {
				$merch_user_set = pdo_get('vcshop_merch_user', array('id' => $merchid));
