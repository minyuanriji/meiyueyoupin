...
class Detail_VcShopPage extends MerchWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			pdo_update('vcshop_order', array('remark' => trim($_GPC['remark'])), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('vcshop_dispatch') . ' WHERE id = :id and uniacid=:uniacid and merchid = :merchid', array(':id' => $item['dispatchid'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		$refund = pdo_fetch('SELECT * FROM ' . tablename('vcshop_order_refund') . ' WHERE orderid = :orderid and uniacid=:uniacid and merchid = :merchid order by id desc', array(':orderid' => $item['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		$diyformfields = '';
		$showdiyform = false;
		...
		if (com('verify')) {
			...
			if (!empty($item['verifyopenid'])) {
				...
				$saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('vcshop_merch_saler') . ' where openid=:openid and uniacid=:uniacid and merchid = :merchid limit 1 ', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'], ':openid' => $item['verifyopenid']));
			}
