...
class List_VcShopPage extends MerchWebPage
{
	protected function orderData($status, $st)
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$sqlcondition = ' left join ' . tablename('vcshop_merch_store') . ' store on store.id = o.verifystoreid and store.uniacid=o.uniacid';
			}
			...
			foreach ($list as &$row) {
				...
				if (com('verify')) {
					...
					if (!empty($row['verifyopenid'])) {
						...
						$merch_saler = pdo_fetch('select id,salername from ' . tablename('vcshop_merch_saler') . ' where openid=:openid and uniacid=:uniacid and merchid = :merchid limit 1 ', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'], ':openid' => $row['verifyopenid']));
...
';
		...
		if ($searchfield == 'member') {
			...
			if (!empty($openidArr)) {
				...
				$orderPrice = pdo_fetch('SELECT count(1) as count,sum(price) as sumprice FROM ' . tablename('vcshop_order') . ' WHERE uniacid = ' . $_W['uniacid'] . ' AND merchid = ' . $merchid . ' AND deleted=0 AND isparent=0 AND openid IN (' . $inOpenid . ')' . $priceStatus);
			}
			else {
			...
			$orderPrice = pdo_fetch('SELECT count(1) as count,sum(o.price) as sumprice FROM ' . tablename('vcshop_order') . ' o left join ' . tablename('vcshop_member_address') . ' a on o.addressid = a.id WHERE o.uniacid = ' . $_W['uniacid'] . '  AND o.deleted=0 AND o.isparent=0 AND o.merchid = ' . $merchid . $priceCondition . $statuscondition);
			$totalmoney = $orderPrice['sumprice'];
			$total = $orderPrice['count'];
			...
			$t = pdo_fetch('SELECT count(DISTINCT(o.id)) as count,sum(o.price) as sumprice FROM ' . tablename('vcshop_order') . ' o ' . ' left join ' . tablename('vcshop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('vcshop_merch_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid and s.merchid=o.merchid' . ' left join ' . tablename('vcshop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . (' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition), $paras);
			$total = $t['count'];
			$totalmoney = $t['sumprice'];
		}

		$pager = pagination2($total, $pindex, $psize);
		$stores = pdo_fetchall('select id,storename from ' . tablename('vcshop_merch_store') . ' where uniacid=:uniacid and merchid = :merchid', array(':uniacid' => $uniacid, ':merchid' => $_W['merchid']));
