...
class Refund_VcShopPage extends MerchWebPage
{
	protected function opData()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_order') . ' WHERE id = :id and uniacid=:uniacid and merchid = :merchid  Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
		...
		if (!empty($refundid)) {
			$refund = pdo_fetch('select * from ' . tablename('vcshop_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
	...
	public function submit()
	{
		...
		if ($_W['ispost']) {
			...
			if ($refund['status'] < 0 || $refund['status'] == 1) {
				pdo_update('vcshop_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
				show_json(0, '未找需要处理的维权申请，不需处理！');
			}

			if (empty($refund['refundno'])) {
				$refund['refundno'] = m('common')->createNO('order_refund', 'refundno', 'SR');
				pdo_update('vcshop_order_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
			}
			...
			if ($refundstatus == 0) {
				...
				if ($raid == 0) {
					$raddress = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where isdefault=1 and uniacid=:uniacid and merchid = :merchid limit 1', array(':uniacid' => $uniacid, ':merchid' => $merchid));
				}
				else {
					$raddress = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $raid, ':uniacid' => $uniacid, ':merchid' => $merchid));
				}

				if (empty($raddress)) {
					$raddress = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where uniacid=:uniacid and merchid = :merchid order by id desc limit 1', array(':uniacid' => $uniacid, ':merchid' => $merchid));
				}
				...
				pdo_update('vcshop_order_refund', $change_refund, array('id' => $item['refundid']));
				...
				pdo_update('vcshop_order_refund', $refund_data, array('id' => $item['refundid'], 'uniacid' => $uniacid));
				$order_data = array();
				$order_data['refundstate'] = 0;
				$order_data['status'] = 3;
				$order_data['refundtime'] = $time;
				pdo_update('vcshop_order', $order_data, array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
				m('notice')->sendOrderMessage($item['id'], true);
			}
				...
				$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice,g.isfullback FROM ' . tablename('vcshop_order_goods') . ' o left join ' . tablename('vcshop_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid', array(':orderid' => $item['id'], ':uniacid' => $uniacid));
				$refundtype = 0;
				...
				if ($item['paytype'] == 1) {
					...
					if ($item['apppay'] == 2) {
						...
						$pid = $ispeerpay['id'];
						$peerpaysql = 'SELECT * FROM ' . tablename('vcshop_order_peerpay_payinfo') . ' WHERE pid = :pid';
				...
				m('notice')->sendOrderMessage($item['id'], true);
				pdo_update('vcshop_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));

				foreach ($goods as $g) {
					$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid and o.merchid=:merchid limit 1', array(':goodsid' => $g['id'], ':uniacid' => $uniacid, 'merchid' => $merchid));
				...
				pdo_update('vcshop_order_refund', array('reply' => $refundcontent, 'status' => -1, 'endtime' => $time), array('id' => $item['refundid']));
				m('notice')->sendOrderMessage($item['id'], true);
				plog('order.op.refund', '订单退款拒绝 ID: ' . $item['id'] . ' 订单号: ' . $item['ordersn'] . ' 原因: ' . $refundcontent);
				pdo_update('vcshop_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
			}
			else {
				if ($refundstatus == 2) {
					$refundtype = 2;
					...
					m('notice')->sendOrderMessage($item['id'], true);
					pdo_update('vcshop_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
					$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice FROM ' . tablename('vcshop_order_goods') . ' o left join ' . tablename('vcshop_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid and o.merchid = :merchid', array(':orderid' => $item['id'], ':uniacid' => $uniacid, ':merchid' => $merchid));
					$credits = m('order')->getGoodsCredit($goods);
		...
		$refund_address = pdo_fetchall('select * from ' . tablename('vcshop_refund_address') . ' where uniacid=:uniacid and merchid = :merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
	...
	{
		pdo_update('vcshop_order_peerpay_payinfo', array('refundstatus' => 1, 'refundprice' => $refundprice), array('id' => $id));
	}
}
