...
class Single_Refund_VcShopPage extends MerchWebPage
{
	...
	public function refund_after($item, $time, $shopset)
	{
		...
		if ($refund_num == count($order_goods)) {
			...
			pdo_update('vcshop_order', array('status' => -1, 'canceltime' => $time), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			plog('order.op.close', '订单关闭 ID: ' . $item['id'] . ' 订单号: ' . $item['ordersn']);
		}

		if (empty($apply_refund_num)) {
			pdo_update('vcshop_order', array('refundtime' => $time), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		}
