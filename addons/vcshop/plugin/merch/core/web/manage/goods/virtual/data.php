...
class Data_VcShopPage extends MerchWebPage
{
	...
	public function main()
	{
		...
		$psize = 60;
		$type = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid ', array(':id' => $typeid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		$items = pdo_fetchall('SELECT d.*,m.avatar,m.nickname,m.realname,m.mobile,o.status  FROM ' . tablename('vcshop_virtual_data') . ' d ' . ' left join ' . tablename('vcshop_member') . ' m on d.openid<>\'\' and m.openid = d.openid and m.uniacid= d.uniacid ' . ' left join ' . tablename('vcshop_order') . ' o on d.orderid<>0 and  o.id = d.orderid ' . (' where  1 ' . $condition . ' order by ' . $order . ' limit ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		...
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_virtual_data') . ' d ' . ' left join ' . tablename('vcshop_member') . ' m on d.openid<>\'\' and m.openid = d.openid and m.uniacid= d.uniacid ' . ' left join ' . tablename('vcshop_order') . ' o on d.orderid<>0 and  o.id = d.orderid ' . (' where 1 ' . $condition . ' '), $params);
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid ', array(':id' => $_GPC['typeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		if (!empty($editid)) {
			$data = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_data') . ' WHERE id=:id and typeid=:typeid and uniacid=:uniacid and merchid=:merchid ', array(':id' => $editid, ':typeid' => $typeid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			$data['edit'] = $editid;
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($typeid)) {
				$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid ', array(':id' => $typeid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
				$item['fields'] = iunserializer($item['fields']);
				...
				if (!empty($item['fields'])) {
					...
					foreach ($tpids as $index => $id) {
						...
						$datas = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_data') . ' WHERE id=:id and typeid=:typeid and uniacid=:uniacid and merchid=:merchid ', array(':id' => $id, ':typeid' => $typeid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));

						if (empty($datas)) {
							$keydata = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_data') . ' WHERE pvalue=:pvalue  and typeid=:typeid and uniacid=:uniacid and merchid=:merchid', array(':pvalue' => $insert['pvalue'], ':typeid' => $typeid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));

							if (empty($keydata)) {
								pdo_insert('vcshop_virtual_data', $insert);
								pdo_update('vcshop_virtual_type', 'alldata=alldata+1', array('id' => $item['id']));
							}
							else {
								pdo_update('vcshop_virtual_data', $insert, array('id' => $keydata['id']));
							}
						}
						else {
							$keydata = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_data') . ' WHERE pvalue=:pvalue and id<>:id and typeid=:typeid and uniacid=:uniacid and merchid=:merchid', array(':pvalue' => $insert['pvalue'], ':id' => $id, ':typeid' => $typeid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));

							if (empty($keydata)) {
								pdo_update('vcshop_virtual_data', $insert, array('id' => $datas['id']));
							}
							else {
	...
	public function delete()
	{
		...
		$types = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_virtual_data') . (' WHERE id in( ' . $id . ' ) AND typeid=' . $typeid . ' and  uniacid=') . $_W['uniacid'] . ' and  merchid=' . $_W['merchid']);
		...
		foreach ($types as $type) {
			...
			pdo_delete('vcshop_virtual_data', array('id' => $type['id']));
			pdo_update('vcshop_virtual_type', 'alldata=alldata-1', array('id' => $type['typeid']));
	...
	public function export()
	{
		...
		global $_GPC;
		$typeid = intval($_GPC['typeid']);
		$type = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid limit 1 ', array(':id' => $typeid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		$list = pdo_fetchall('SELECT d.*,o.carrier,m.avatar,m.nickname FROM ' . tablename('vcshop_virtual_data') . ' d ' . ' left join ' . tablename('vcshop_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid and d.merchid=0' . ' left join ' . tablename('vcshop_order') . ' o on o.id = d.orderid ' . (' where  1 ' . $condition . ' order by usetime desc'), $params);
	...
	public function temp()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid ', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		foreach ($datas as $d) {
			if ($d['pvalue'] != '') {
				if (!empty($d['status']) && $d['status'] == 'del') {
					$olddata = pdo_fetch('SELECT * FROM ' . tablename('vcshop_virtual_data') . ' WHERE pvalue=:pvalue and typeid=:typeid and uniacid=:uniacid and merchid=' . $_W['merchid'], array(':pvalue' => $d['pvalue'], ':typeid' => $_GPC['typeid'], ':uniacid' => $_W['uniacid']));
					$d['createtime'] = TIMESTAMP;

					if (!empty($olddata)) {
						if (empty($olddata['openid'])) {
							pdo_delete('vcshop_virtual_data', array('id' => $olddata['id']));
							pdo_update('vcshop_virtual_type', 'alldata=alldata-1', array('id' => $item['id']));
						}
						else {
							$noinsert .= $d['pvalue'] . ',';
					...
					if (empty($olddata)) {
						...
						pdo_update('vcshop_virtual_data', $d, array('id' => $olddata['id']));
					}
					else {
						$noinsert .= $d['pvalue'] . ',';
					}
