...
class Index_VcShopPage extends MerchWebPage
{
	public function main($goodsfrom = '')
	{
		...
		$sql = 'SELECT COUNT(g.`id`) FROM ' . tablename('vcshop_goods') . 'g' . $sqlcondition . $condition . $groupcondition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT g.* FROM ' . tablename('vcshop_goods') . 'g' . $sqlcondition . $condition . $groupcondition . ' ORDER BY g.`status` DESC, g.`merchdisplayorder` DESC,
	...
	public function change()
	{
		...
		$goods = pdo_fetch('select id,hasoption from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		pdo_update('vcshop_goods', array($type => $value, 'checked' => $checked), array('id' => $id));
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice FROM ' . tablename('vcshop_goods') . (' WHERE 1 ' . $condition . ' order by createtime desc'), $params);
