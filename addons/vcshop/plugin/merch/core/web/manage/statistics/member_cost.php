...
class Member_cost_VcShopPage extends MerchWebPage
{
	public function main()
	{
		...
		$condition1 = ' and m.uniacid=:uniacid and m.openid in ( SELECT distinct openid from ' . tablename('vcshop_order') . '   WHERE uniacid =:uniacid and merchid=:merchid)';
		...
		$sql = 'SELECT m.realname, m.mobile,m.avatar,m.nickname,m.openid,l.levelname,' . '(select ifnull( count(o.id) ,0) from  ' . tablename('vcshop_order') . (' o where  o.openid=m.openid and o.status>=1 ' . $condition . ')  as ordercount,') . '(select ifnull(sum(o.price),0) from  ' . tablename('vcshop_order') . (' o where o.openid=m.openid  and o.status>=1 ' . $condition . ')  as ordermoney') . ' from ' . tablename('vcshop_member') . ' m  ' . ' left join ' . tablename('vcshop_member_level') . ' l on l.id = m.level' . (' where 1 ' . $condition1 . ' order by ' . $orderby . ' desc');
		...
		$total = pdo_fetchcolumn('select  count(1) from ' . tablename('vcshop_member') . ' m ' . (' where 1 ' . $condition1 . ' '), $params1);
