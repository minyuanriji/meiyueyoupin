...
class Goods_VcShopPage extends MerchWebPage
{
	public function main()
	{
		...
		$sql = 'select og.price,og.total,o.createtime,o.ordersn,g.title,g.thumb,g.goodssn,op.goodssn as optiongoodssn from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id = og.goodsid ' . ' left join ' . tablename('vcshop_goods_option') . ' op on op.id = og.optionid ' . (' where 1 ' . $condition . ' order by ' . $orderby . ' desc ');
