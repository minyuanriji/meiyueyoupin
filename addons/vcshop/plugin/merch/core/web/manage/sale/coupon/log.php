...
class Log_VcShopPage extends MerchWebPage
{
	...
	public function main()
	{
		...
		if (!empty($couponid)) {
			$coupon = pdo_fetch('select * from ' . tablename('vcshop_coupon') . ' where id=:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		$sql = 'SELECT d.*, c.coupontype,c.couponname,m.nickname,m.avatar,m.realname,m.mobile FROM ' . tablename('vcshop_coupon_data') . ' d ' . ' left join ' . tablename('vcshop_coupon') . ' c on d.couponid = c.id ' . ' left join ' . tablename('vcshop_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid ' . (' where  1 and ' . $condition . ' ORDER BY gettime DESC');
		...
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_coupon_data') . ' d ' . ' left join ' . tablename('vcshop_coupon') . ' c on d.couponid = c.id ' . ' left join ' . tablename('vcshop_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid ' . ('where 1 and ' . $condition), $params);
