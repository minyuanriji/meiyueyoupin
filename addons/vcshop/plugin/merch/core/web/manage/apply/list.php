...
class List_VcShopPage extends MerchWebPage
{
	protected function applyData($status, $st)
	{
		...
		$sql = 'select b.* from ' . tablename('vcshop_merch_bill') . ' b ' . ' left join ' . tablename('vcshop_merch_user') . ' u on b.merchid = u.id' . (' where 1 ' . $condition . ' ORDER BY ' . $orderby . ' desc ');
		...
		$total = pdo_fetchcolumn('select count(b.id) from' . tablename('vcshop_merch_bill') . ' b ' . ' left join ' . tablename('vcshop_merch_user') . ' u on b.merchid = u.id' . (' where 1 ' . $condition), $params);
	...
	protected function post()
	{
		...
		$total = pdo_fetchcolumn('select count(o.id) from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_order') . ' o on u.id=o.merchid' . ' left join ' . tablename('vcshop_order_refund') . ' r on o.refundid=r.id' . ' left join ' . tablename('vcshop_merch_commission_orderprice') . ' comprice on o.id=comprice.order_id' . (' where ' . $condition . ' ' . $conditionrefund), $params);
		...
		if ($_W['ispost']) {
			...
			pdo_insert('vcshop_merch_bill', $insert);
			$billid = pdo_insertid();
			...
			foreach ($lists as $k => $v) {
				...
				pdo_insert('vcshop_merch_billo', $insert_data);
				$change_order_data = array();
				$change_order_data['merchapply'] = 1;
				pdo_update('vcshop_order', $change_order_data, array('id' => $orderid));
			}
	...
	public function getLastApply($merchid, $applytype = -1)
	{
		global $_W;
		$params = array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid);
		$sql = 'select applytype,alipay,bankname,bankcard,applyrealname from ' . tablename('vcshop_merch_bill') . ' where merchid=:merchid and uniacid=:uniacid';
