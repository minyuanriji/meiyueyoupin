...
class Credit_VcShopPage extends MerchWebPage
{
	protected function applyData($status, $st)
	{
		...
		$sql = 'select b.*,u.merchname,u.realname,u.mobile,u.iscreditmoney,b.creditrate,b.isbillcredit from ' . tablename('vcshop_merch_bill') . ' b ' . ' left join ' . tablename('vcshop_merch_user') . ' u on b.merchid = u.id' . (' where 1 ' . $condition . ' ORDER BY ' . $orderby . ' desc ');
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($list as $k => $v) {
				...
				pdo_update('vcshop_creditshop_log', $change_order_data, array('id' => $orderid));
			}
