...
class Pages_VcShopPage extends MerchWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM' . tablename('vcshop_quick') . $condition . ' ORDER BY createtime DESC' . $limit, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM' . tablename('vcshop_quick') . $condition, $params);
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT * FROM' . tablename('vcshop_quick') . ' WHERE id=:id AND uniacid=:uniacid AND merchid=:merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($arr['keyword'])) {
				...
				if (!empty($keyword)) {
					if ($keyword['name'] != 'vcshop:quick:' . $id) {
			...
			if (empty($item)) {
				...
				$arr['createtime'] = time();
				pdo_insert('vcshop_quick', $arr);
				$id = pdo_insertid();
				plog('quick.pages.add', '添加购买页面 ID: ' . $id . ' 标题: ' . $arr['title']);
			}
			else {
				pdo_update('vcshop_quick', $arr, array('uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid'], 'id' => $id));
				plog('quick.pages.add', '添加购买页面 ID: ' . $id . ' 标题: ' . $arr['title']);
			}

			if (!empty($arr['keyword'])) {
				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'vcshop', ':name' => 'vcshop:quick:' . $id));

				if (!empty($rule)) {
					pdo_update('rule_keyword', array('content' => $arr['keyword']), array('rid' => $rule['id']));
				}
				else {
					$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'vcshop:quick:' . $id, 'module' => 'vcshop', 'displayorder' => 0, 'status' => 1);
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title,cart,keyword FROM ' . tablename('vcshop_quick') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		if (!empty($items)) {
			foreach ($items as $item) {
				pdo_delete('vcshop_quick', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
				plog('quick.pages.delete', '删除购买页面 ID: ' . $item['id'] . ' 标题: ' . $item['title'] . ' ');

				if (!empty($item['cart'])) {
					pdo_delete('vcshop_quick_cart', array('quickid' => $item['id'], 'uniacid' => $_W['uniacid']));
				}
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_quick') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		if (!empty($items)) {
			foreach ($items as $item) {
				pdo_update('vcshop_quick', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
