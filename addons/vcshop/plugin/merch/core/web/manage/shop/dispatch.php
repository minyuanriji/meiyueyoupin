...
class Dispatch_VcShopPage extends MerchWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_dispatch') . (' WHERE 1 ' . $condition . '  ORDER BY displayorder DESC limit ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('vcshop_dispatch') . (' WHERE 1 ' . $condition), $params);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if ($data['isdefault']) {
				pdo_update('vcshop_dispatch', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			}

			if (!empty($id)) {
				mplog('shop.dispatch.edit', '修改配送方式 ID: ' . $id);
				pdo_update('vcshop_dispatch', $data, array('id' => $id, 'merchid' => $_W['merchid']));
			}
			else {
				pdo_insert('vcshop_dispatch', $data);
				$id = pdo_insertid();
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('vcshop_dispatch') . (' WHERE id = \'' . $id . '\' and uniacid = \'' . $_W['uniacid'] . '\' and merchid=\'' . $_W['merchid'] . '\''));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('vcshop_dispatch') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid'] . ' AND merchid=' . $_W['merchid']);

		foreach ($items as $item) {
			pdo_delete('vcshop_dispatch', array('id' => $item['id']));
	...
	public function enabled()
	{
		...
		foreach ($items as $item) {
			pdo_update('vcshop_dispatch', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
	...
	public function setdefault()
	{
		...
		foreach ($items as $item) {
			pdo_update('vcshop_dispatch', array('isdefault' => intval($_GPC['isdefault'])), array('id' => $item['id']));
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('vcshop_dispatch') . (' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' and merchid=' . $_W['merchid']));

		if (!empty($item)) {
			pdo_update('vcshop_dispatch', array('displayorder' => $displayorder), array('id' => $id));
