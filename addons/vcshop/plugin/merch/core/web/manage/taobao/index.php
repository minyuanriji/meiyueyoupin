...
class Index_VcShopPage extends MerchWebPage
{
	public function main()
	{
		...
		if (mcv('taobao.main')) {
			$sql = 'SELECT * FROM ' . tablename('vcshop_category') . ' WHERE `uniacid` = :uniacid ORDER BY `parentid`, `displayorder` DESC';
			$category = m('shop')->getFullCategory(true, true);
			$set = m('common')->getSysset(array('shop'));
			$shopset = $set['shop'];
			load()->func('tpl');
			$merchGroup = $item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_merch_group') . ' WHERE id = :id ', array(':id' => $_W['merch_user']['groupid']));
