...
class MerchModel extends PluginModel
{
	...
	{
		global $_W;
		return pdo_fetchall('select id,groupname from ' . tablename('vcshop_merch_group') . ' where uniacid=:uniacid and status=1 order by isdefault desc , id asc', array(':uniacid' => $_W['uniacid']), 'id');
	}
	...
	public function getCategory($data = array())
	{
		...
		return pdo_fetchall('select ' . $res['field'] . ' from ' . tablename('vcshop_merch_category') . $res['condition'], $res['params'], $res['column']);
	}
	...
	public function getCategorySwipe($data = array())
	{
		...
		$params = array(':uniacid' => $_W['uniacid']);
		$res = $this->build($condition, $params, $data);
		return pdo_fetchall('select ' . $res['field'] . ' from ' . tablename('vcshop_merch_category_swipe') . $res['condition'], $res['params'], $res['column']);
	}
	...
	public function getMerch($data = array())
	{
		...
		$params = array(':uniacid' => $_W['uniacid']);
		$res = $this->build($condition, $params, $data);
		return pdo_fetchall('select ' . $res['field'] . ' from ' . tablename('vcshop_merch_user') . $res['condition'], $res['params'], $res['column']);
	}
	...
	public function updateSet($values = array(), $merchid = 0)
	{
		...
		pdo_update('vcshop_merch_user', array('sets' => iserializer($sets)), array('id' => $merchid));
	...
	public function refreshSet($merchid = 0)
	{
		...
		$merch_set = pdo_fetch('select sets from ' . tablename('vcshop_merch_user') . ' where uniacid=:uniacid and id=:id limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $merchid));
	...
	{
		global $_W;
		return array('sale' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . ' where checked=0 and status=1 and deleted=0 and total>0 and uniacid=:uniacid and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])), 'out' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . ' where checked=0 and  status=1 and deleted=0 and total=0 and uniacid=:uniacid and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])), 'check' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . ' where checked=1 and deleted=0 and uniacid=:uniacid and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])), 'stock' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . ' where checked=0 and status=0 and deleted=0 and uniacid=:uniacid  and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])), 'cycle' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . ' where deleted=1 and uniacid=:uniacid and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])));
	}
	...
	public function getOrderTotals()
	{
		...
		$totals['all'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and deleted=0'), $paras);
		$totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and status=-1 and refundtime=0 and deleted=0'), $paras);
		$totals['status0'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and status=0 and paytype<>3 and deleted=0'), $paras);
		$totals['status1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and ( status=1 or ( status=0 and paytype=3) ) and deleted=0'), $paras);
		$totals['status2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and status=2 and deleted=0'), $paras);
		$totals['status3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and status=3 and deleted=0'), $paras);
		$totals['status4'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and refundstate>0 and refundid<>0 and deleted=0'), $paras);
		$totals['status5'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and refundtime<>0 and deleted=0'), $paras);
		return $totals;
	}
	...
	public function getListUser($list, $return = 'all')
	{
		...
		if (!empty($merch)) {
			...
			$merch_user = pdo_fetchall('select * from ' . tablename('vcshop_merch_user') . ' where uniacid=:uniacid and id in(' . implode(',', $merch_ids) . ')', array(':uniacid' => $_W['uniacid']), 'id');
	...
	public function getListUserOne($merchid)
	{
		...
		if ($merchid) {
			$merch_user = pdo_fetch('select * from ' . tablename('vcshop_merch_user') . (' where uniacid=:uniacid and id=' . $merchid), array(':uniacid' => $_W['uniacid']));
			return $merch_user;
		}
	...
	private function check($permtype = '')
	{
		...
		$user = pdo_fetch('select u.status as userstatus,r.status as rolestatus,r.perms as roleperms,u.roleid from ' . tablename('vcshop_merch_account') . ' u ' . ' left join ' . tablename('vcshop_merch_perm_role') . ' r on u.roleid = r.id ' . ' where u.id=:merchuid limit 1 ', array(':merchuid' => $uid));
	...
	public function log($type = '', $op = '')
	{
		...
		pdo_insert('vcshop_merch_perm_log', $log);
	}
	...
	public function select_operator($merchid = 0)
	{
		global $_W;
		$merchid = empty($merchid) ? $_W['merchid'] : intval($merchid);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('vcshop_merch_account') . ' WHERE 1  and uniacid = :uniacid and merchid = :merchid and isfounder<>1', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		return $total;
	}
	...
	public function getUserTotals()
	{
		global $_W;
		global $_GPC;
		$totals = array('reg0' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_merch_reg') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])), 'reg_1' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_merch_reg') . ' where uniacid=:uniacid and status=-1 limit 1', array(':uniacid' => $_W['uniacid'])), 'user0' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_merch_user') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])), 'user1' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_merch_user') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])), 'user2' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_merch_user') . ' where uniacid=:uniacid and status=2 limit 1', array(':uniacid' => $_W['uniacid'])), 'user3' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_merch_user') . ' where uniacid=:uniacid and status=1 and TIMESTAMPDIFF(DAY,now(),FROM_UNIXTIME(accounttime))<=30  limit 1', array(':uniacid' => $_W['uniacid'])));
		return $totals;
	}
	...
	public function getClearTotals()
	{
		global $_W;
		global $_GPC;
		$totals = array('status0' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_merch_clearing') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])), 'status1' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_merch_clearing') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])), 'status2' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_merch_clearing') . ' where uniacid=:uniacid and status=2 limit 1', array(':uniacid' => $_W['uniacid'])));
		return $totals;
	}
	...
	public function getMerchOrderTotals($type = 0)
	{
		...
		$sql = 'select sum(o.price) as totalmoney from ' . tablename('vcshop_order') . ' o ' . ' left join ' . tablename('vcshop_merch_user') . ' u on u.id = o.merchid ' . (' where 1 ' . $condition . ' ');
		$price = pdo_fetch($sql, $params);
		$totalmoney = round($price['totalmoney'], 2);
		$sql = 'select count(o.id) as totalcount from ' . tablename('vcshop_order') . ' o ' . ' left join ' . tablename('vcshop_merch_user') . ' u on u.id = o.merchid ' . (' where 1 ' . $condition . ' ');
				...
				if (!empty($openid)) {
					foreach ($openid as $v) {
						...
						if (!empty($notice_redis['notice_redis'])) {
							...
							if ($open_redis) {
								$key = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . 'vc_' . 'notice_uniacid' . $_W['uniacid'] . '_list';
								$redis = redis();
...
请到后台查看~', 'color' => '#000000')
	...
	public function getMerchOrderTotalPrice($merchid)
	{
		...
		$sql = 'select *  from ' . tablename('vcshop_merch_bill') . (' where 1 ' . $condition . ' and status=1 and creditstatus =2');
		...
		if (0 < count($status1orderids)) {
			$status1order = pdo_fetchall('SELECT id FROM ' . tablename('vcshop_order') . ' WHERE id in(' . join(',', $status1orderids) . ') AND uniacid=' . $_W['uniacid']);
			$commission1 = 0;
		...
		$sql = 'select * from ' . tablename('vcshop_merch_bill') . (' where 1 ' . $condition . ' and status=2 and creditstatus =2');
		...
		if (0 < count($status2orderids)) {
			$status2order = pdo_fetchall('SELECT id FROM ' . tablename('vcshop_order') . ' WHERE id in(' . join(',', $status2orderids) . ') AND uniacid=' . $_W['uniacid']);
			$commission2 = 0;
		...
		$sql = 'select *  from ' . tablename('vcshop_merch_bill') . (' where 1 ' . $condition . ' and status=3 and creditstatus =2');
		...
		if (0 < count($status3orderids)) {
			$status3order = pdo_fetchall('SELECT id FROM ' . tablename('vcshop_order') . ' WHERE id in(' . join(',', $status3orderids) . ') AND uniacid=' . $_W['uniacid']);
			$commission3 = 0;
	...
	public function getMerchPrice($merchid, $flag = 0)
	{
		...
		$orderids = array();
		$sql = 'select ' . $con . ' from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_order') . ' o on u.id=o.merchid' . (' where 1 ' . $condition . ' limit 1');
		$list = pdo_fetch($sql, $params);
		$merchcouponprice = pdo_fetchcolumn('select sum(o.couponprice) from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_order') . ' o on u.id=o.merchid' . (' where o.couponmerchid>0 ' . $condition . ' limit 1'), $params);
		$refundlist = pdo_fetchall('select o.id,r.orderprice,r.applyprice from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_order') . ' o on u.id=o.merchid' . ' left join ' . tablename('vcshop_order_refund') . ' r on o.refundid=r.id' . (' where o.refundid>0 and r.status=1 and r.applyprice<r.orderprice  ' . $conditionrefund . ' '), $paramsrefund, 'id');
		$refundprice = 0;
		$refundorderprice = 0;
		...
		if (0 < $flag) {
			$sql = 'select o.id,o.agentid,comprice.commission_price,comprice.order_id from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_order') . ' o on u.id=o.merchid' . ' left join ' . tablename('vcshop_merch_commission_orderprice') . ' comprice on o.id=comprice.order_id' . (' where 1 ' . $condition);
			...
			if (!empty($order)) {
				foreach ($order as $k => $v) {
					...
					if (!empty($v['order_id'])) {
						...
						pdo_insert('vcshop_merch_commission_orderprice', array('order_id' => $v['id'], 'commission_price' => $commission_price));
	...
	public function getMerchPriceList($merchid, $orderid = 0, $flag = 0, $applyid = 0, $pindex = 0, $psize = 0)
	{
		...
		$sql = 'select ' . $con . ' from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_order') . ' o on u.id=o.merchid' . ' left join ' . tablename('vcshop_order_refund') . ' r on o.refundid=r.id' . ' left join ' . tablename('vcshop_merch_commission_orderprice') . ' comprice on o.id=comprice.order_id';
	...
	public function getMerchCreditList($merchid, $orderid = 0, $flag = 0, $creditrate = 0, $isbillcredit = 1)
	{
		...
		$sql = 'select ' . $con . ' from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_creditshop_log') . ' o on u.id=o.merchid' . (' where 1 ' . $condition);
		$order = pdo_fetchall($sql, $params);
		$_order = array();
	...
	public function getMerchCreditTotalPrice($merchid)
	{
		...
		$params = array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid);
		$condition = ' and uniacid=:uniacid and merchid=:merchid';
		$sql = 'select *  from ' . tablename('vcshop_merch_bill') . (' where 1 ' . $condition . ' and status=1 and creditstatus = 1');
		$status1 = pdo_fetchall($sql, $params);
		$status1price = 0;
		...
		$sql = 'select * from ' . tablename('vcshop_merch_bill') . (' where 1 ' . $condition . ' and status=2 and creditstatus = 1');
		$status2 = pdo_fetchall($sql, $params);
		$status2price = 0;
		...
		$sql = 'select *  from ' . tablename('vcshop_merch_bill') . (' where 1 ' . $condition . ' and status=3 and creditstatus = 1');
		$status3 = pdo_fetchall($sql, $params);
		$status3price = 0;
	...
	public function getMerchCredit($merchid, $flag = 0)
	{
		...
		$sql = 'select ' . $con . ' from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_creditshop_log') . ' o on u.id=o.merchid' . (' where 1 ' . $condition . ' limit 1');
		$list = pdo_fetch($sql, $params);
		$_condition = 'and uniacid=:uniacid and id=:merchid';
		$_sql = 'select iscreditmoney,creditrate,iscredit from' . tablename('vcshop_merch_user') . ('where 1 ' . $_condition);
		$iscredit = pdo_fetch($_sql, $params);

		if (0 < $flag) {
			$sql = 'select o.id,u.iscreditmoney,o.dispatch,o.money,o.credit from ' . tablename('vcshop_merch_user') . ' u ' . ' left join ' . tablename('vcshop_creditshop_log') . ' o on u.id=o.merchid' . (' where 1 ' . $condition);
			$order = pdo_fetchall($sql, $params);
			$orderids = array();
	...
	public function getOneApply($id)
	{
		...
		$sql = 'select b.*,u.merchname,u.realname,u.mobile,u.iscreditmoney from ' . tablename('vcshop_merch_bill') . ' b ' . ' left join ' . tablename('vcshop_merch_user') . ' u on b.merchid = u.id' . (' where 1 ' . $condition . ' Limit 1');
	...
	public function getMerchApplyTotals($merchid = 0)
	{
		...
		$totals['status1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_merch_bill') . (' WHERE 1 ' . $condition . ' and status=1'), $params);
		$totals['status2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_merch_bill') . (' WHERE 1 ' . $condition . ' and status=2'), $params);
		$totals['status3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_merch_bill') . (' WHERE 1 ' . $condition . ' and status=3'), $params);
		$totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_merch_bill') . (' WHERE 1 ' . $condition . ' and status=-1'), $params);
		return $totals;
	}
	...
	{
		global $_W;
		$totals = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_merch_user') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		return $totals;
	}
	...
	{
		global $_W;
		$data = pdo_fetch('select datas from ' . tablename('vcshop_perm_plugin') . ' where acid=:acid Limit 1', array(':acid' => $_W['uniacid']));
		$max_merch = 0;
	...
	public function tempData($type, $merchid = 0)
	{
		...
		$sql = 'SELECT id,expressname,expresscom,isdefault FROM ' . tablename('vcshop_exhelper_express') . (' where  1 and ' . $condition . ' ORDER BY isdefault desc, id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_exhelper_express') . (' where 1 and ' . $condition), $params);
	...
	public function setDefault($id, $type, $merchid = 0)
	{
		global $_W;
		$merchid = empty($merchid) ? $_W['merchid'] : $merchid;
		$item = pdo_fetch('SELECT id,expressname,type FROM ' . tablename('vcshop_exhelper_express') . ' WHERE id=:id and type=:type AND uniacid=:uniacid and merchid=:merchid', array(':id' => $id, ':type' => $type, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));

		if (!empty($item)) {
			pdo_update('vcshop_exhelper_express', array('isdefault' => 0), array('type' => $type, 'uniacid' => $_W['uniacid'], 'merchid' => $merchid));
			pdo_update('vcshop_exhelper_express', array('isdefault' => 1), array('id' => $id, 'merchid' => $merchid));
	...
	public function tempDelete($id, $type, $merchid = 0)
	{
		global $_W;
		$merchid = empty($merchid) ? $_W['merchid'] : $merchid;
		$items = pdo_fetchall('SELECT id,expressname FROM ' . tablename('vcshop_exhelper_express') . (' WHERE id in( ' . $id . ' ) and type=:type and uniacid=:uniacid and merchid=:merchid'), array(':type' => $type, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));

		foreach ($items as $item) {
			pdo_delete('vcshop_exhelper_express', array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $merchid));
	...
	public function getTemp($merchid = 0)
	{
		...
		global $_GPC;
		$merchid = empty($merchid) ? $_W['merchid'] : $merchid;
		$temp_sender = pdo_fetchall('SELECT id,isdefault,sendername,sendertel FROM ' . tablename('vcshop_exhelper_senduser') . ' WHERE uniacid=:uniacid and merchid=:merchid order by isdefault desc ', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
		$temp_express = pdo_fetchall('SELECT id,type,isdefault,expressname FROM ' . tablename('vcshop_exhelper_express') . ' WHERE type=1 and uniacid=:uniacid and merchid=:merchid order by isdefault desc ', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
		$temp_invoice = pdo_fetchall('SELECT id,type,isdefault,expressname FROM ' . tablename('vcshop_exhelper_express') . ' WHERE type=2 and uniacid=:uniacid and merchid=:merchid order by isdefault desc ', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
	...
	public function getPluginsetByMerch($key = '')
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$set = pdo_fetch('select * from ' . tablename('vcshop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
