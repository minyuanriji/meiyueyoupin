...
class Log_VcShopPage extends MobilePage
{
	...
	public function getlist()
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('vcshop_creditshop_log') . (' log where 1 ' . $condition);
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT log.id,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money,g.isverify,g.goodstype,log.addressid,log.storeid FROM ' . tablename('vcshop_creditshop_log') . ' log ' . ' left join ' . tablename('vcshop_creditshop_goods') . ' g on log.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
	...
	public function detail()
	{
		...
		if (empty($log['storeid'])) {
			...
			$store = pdo_fetch('select * from ' . tablename('vcshop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid']));
		}
