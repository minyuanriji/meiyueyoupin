...
class User_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select * from ' . tablename('vcshop_cashier_user') . (' where uniacid=:uniacid AND deleted=0 ' . $condition . ' ORDER BY id desc limit ') . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_cashier_user') . (' where uniacid=:uniacid AND deleted=0 ' . $condition), $params);
	...
	protected function post()
	{
		...
		if ($id) {
			$item = pdo_fetch('select * from ' . tablename('vcshop_cashier_user') . ' where id=:id AND deleted=0 and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			...
			if (!empty($item['management'])) {
				...
				$management = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_member') . (' WHERE  uniacid=:uniacid AND `openid` IN (\'' . $item['management'] . '\')'), array(':uniacid' => $_W['uniacid']));
			}
			...
			if (!empty($item['notice_openids'])) {
				...
				$operator = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('vcshop_member') . ' where openid in (' . implode(',', $openids) . (') and uniacid=' . $_W['uniacid']));
			}
		}
		...
		if ($_W['ispost']) {
			...
			$user_totle = (int) pdo_fetchcolumn('SELECT id FROM ' . tablename('vcshop_cashier_user') . ' WHERE username=:username AND uniacid=:uniacid AND deleted=0 LIMIT 1', array(':username' => $params['username'], ':uniacid' => $_W['uniacid']));
			$store = pdo_fetch('SELECT id,storeid FROM ' . tablename('vcshop_cashier_user') . ' WHERE uniacid=:uniacid AND deleted=0 LIMIT 1', array(':uniacid' => $_W['uniacid']));
			$merch = pdo_fetch('SELECT id,merchid FROM ' . tablename('vcshop_cashier_user') . ' WHERE uniacid=:uniacid AND deleted=0 LIMIT 1', array(':uniacid' => $_W['uniacid']));
		...
		if (!empty($item['storeid'])) {
			$store = pdo_fetch('SELECT id,storename FROM ' . tablename('vcshop_store') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['storeid']));
		}
		...
		if ($merch) {
			...
			if (!empty($item['merchid']) && !empty($data['is_openmerch'])) {
				$merchres = pdo_fetch('select id,merchname from ' . tablename('vcshop_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['merchid'], ':uniacid' => $_W['uniacid']));
			}
		}

		$wechatpay = json_decode($item['wechatpay'], true);
		$alipay = json_decode($item['alipay'], true);
		$order_template = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_member_printer_template') . ' WHERE uniacid=:uniacid  AND merchid=0', array(':uniacid' => $_W['uniacid']));
		$order_printer_array = array();

		if (!empty($userset['printer'])) {
			$order_printer_array = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_member_printer') . ' WHERE uniacid=:uniacid AND id IN (' . $userset['printer'] . ')', array(':uniacid' => $_W['uniacid']));
		}

		if (!empty($item['couponid'])) {
			$coupon = pdo_fetchall('SELECT id,couponname as title , thumb  FROM ' . tablename('vcshop_coupon') . ' WHERE uniacid = ' . $_W['uniacid'] . ' and id in (' . $item['couponid'] . ')');
		}
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_cashier_user') . (' WHERE id in(' . $id . ') AND deleted=0 AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('vcshop_cashier_user', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			pdo_update('vcshop_cashier_user', array('deleted' => 1), array('id' => $item['id']));
