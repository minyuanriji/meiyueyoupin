...
class Order_VcShopPage extends CashierWebPage
{
	public function main()
	{
		...
		$goods = pdo_fetchall('SELECT id,orderid,isgoods FROM ' . tablename('vcshop_cashier_pay_log') . ' WHERE uniacid=:uniacid AND cashierid=:cashierid AND status=0 AND createtime < ' . (time() - 3600 * 24), array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid']), 'id');
		$gids = array();
		$selfgids = array();
		...
		$sql = 'select * from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition . ' ORDER BY id desc');
		...
		$operator = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_cashier_operator') . ' WHERE uniacid=:uniacid AND cashierid=:cashierid ORDER BY id DESC', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid']), 'id');
