...
class Clearing_VcShopPage extends CashierWebPage
{
	...
	public function main()
	{
		...
		$sql = 'select sc.*,u.name,u.mobile,u.title,u.logo,u.openid from ' . tablename('vcshop_cashier_clearing') . ' sc left join ' . tablename('vcshop_cashier_user') . (' u ON sc.cashierid=u.id where ' . $condition . ' ORDER BY sc.id desc limit ') . ($pindex - 1) * $psize . ',' . $psize;
	...
	protected function post()
	{
		...
		if (!$id) {
			...
			$clearing = pdo_fetch('select sc.*,u.name,u.mobile,u.title,u.logo,u.openid,u.withdraw from ' . tablename('vcshop_cashier_clearing') . ' sc left join ' . tablename('vcshop_cashier_user') . ' u ON sc.cashierid=u.id where sc.uniacid=:uniacid AND sc.cashierid=:cashierid AND sc.id=:id AND sc.deleted=0', array(':uniacid' => $_W['uniacid'], ':cashierid' => $cashierid, ':id' => $id));
			empty($clearing) && $this->message('没有找到提现单!', '', 'error');
			$condition .= ' AND id IN(' . $clearing['orderids'] . ')';
			$payinfo = json_decode($clearing['payinfo'], true);
		}

		$total = (int) pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition . $payday_sql), $params);

		if (!empty($_W['cashieruser']['merchid'])) {
			$total_money = (double) pdo_fetchcolumn('select sum(money-orderprice+deduction) from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition . $payday_sql), $params);
		}
		else {
			$total_money = (double) pdo_fetchcolumn('select sum(money+deduction) from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition . $payday_sql), $params);
		}
		...
		if ($_W['ispost']) {
			...
			$order_ids = pdo_fetchall('select id from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition . $payday_sql), $params, 'id');
			...
			if (!$id) {
				pdo_insert('vcshop_cashier_clearing', $data);
				pdo_query('UPDATE ' . tablename('vcshop_cashier_pay_log') . ' SET is_applypay=1 WHERE uniacid=:uniacid AND id IN(' . $data['orderids'] . ')', $params);
		...
		$sql = 'select * from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition . $payday_sql . ' ORDER BY id desc limit ') . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$openids = '';
		...
		if (!empty($openids)) {
			...
			$user = pdo_fetchall('SELECT id,openid,nickname FROM ' . tablename('vcshop_member') . (' WHERE openid IN (' . $openids . ') AND uniacid=:uniacid'), array(':uniacid' => $_W['uniacid']), 'openid');
		}
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$orderids = pdo_fetch('select id,orderids,status from ' . tablename('vcshop_cashier_clearing') . ' where uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if (!empty($orderids) && $orderids['status'] != 2) {
			pdo_query('UPDATE ' . tablename('vcshop_cashier_clearing') . ' SET deleted=1 WHERE id=:id AND status<>2 AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			pdo_query('UPDATE ' . tablename('vcshop_cashier_pay_log') . ' SET is_applypay=0 WHERE id IN(' . $orderids['orderids'] . ') AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
			show_json(1);
		}
