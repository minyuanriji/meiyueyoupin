...
class Index_VcShopPage extends CashierWebPage {
...

    {
        ...
        if (!empty($item['goodsids'])){
            ...
            $goods = pdo_fetchall('select id,title,thumb from '.tablename('vcshop_goods')." where id in ({$item['goodsids']}) and cashier=1 and deleted=0 and uniacid={$_W['uniacid']}");
        ...
        if (!empty($item['goodsids1'])){
            ...
            $goods1 = pdo_fetchall('select id,title,thumb from '.tablename('vcshop_goods')." where id in ({$item['goodsids']}) and cashier=1 and deleted=0 and uniacid={$_W['uniacid']}");
        ...
        if (!empty($item['goodsids2'])){
            ...
            $goods2 = pdo_fetchall('select id,title,thumb from '.tablename('vcshop_goods')." where id in ({$item['goodsids']}) and cashier=1 and deleted=0 and uniacid={$_W['uniacid']}");
