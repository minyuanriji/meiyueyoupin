...
class Clearing_VcShopPage extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if (!$id) {
			...
			$clearing = pdo_fetch('select sc.*,u.name,u.mobile,u.title,u.logo,u.openid from ' . tablename('vcshop_cashier_clearing') . ' sc left join ' . tablename('vcshop_cashier_user') . ' u ON sc.cashierid=u.id where sc.uniacid=:uniacid AND sc.id=:id AND sc.deleted=0', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if ($_W['ispost']) {
			if ($clearing['status'] == 0) {
				pdo_update('vcshop_cashier_clearing', array('status' => 1), array('id' => $clearing['id'], 'uniacid' => $clearing['uniacid']));
				show_json(1);
			}
			...
			pdo_query('UPDATE ' . tablename('vcshop_cashier_clearing') . ' SET status=2,realmoney=:realmoney,remark=:remark,paytime=:paytime WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $clearing['uniacid'], ':id' => $clearing['id'], ':paytime' => time(), ':remark' => $remark, ':realmoney' => $money));
			pdo_query('UPDATE ' . tablename('vcshop_cashier_pay_log') . ' SET is_applypay=2 WHERE uniacid=:uniacid AND id IN(' . $clearing['orderids'] . ')', $params);
		...
		$sql = 'select * from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition . ' ORDER BY id desc limit ') . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$openids = '';
		...
		$total = (int) pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition), $params);

		if (!empty($clearing['merchid'])) {
			$total_money = (double) pdo_fetchcolumn('select sum(money-orderprice+deduction) from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition), $params);
		}
		else {
			$total_money = (double) pdo_fetchcolumn('select sum(money+deduction) from ' . tablename('vcshop_cashier_pay_log') . (' where uniacid=:uniacid ' . $condition), $params);
		}
