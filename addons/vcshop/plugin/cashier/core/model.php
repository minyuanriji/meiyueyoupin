...
class CashierModel extends PluginModel
{
	...
	public function updateUserSet($data = array(), $cashierid)
	{
		...
		return pdo_query('UPDATE ' . tablename('vcshop_cashier_user') . ' SET `set`=:set WHERE `uniacid` = :uniacid AND `id` = :id', array(':uniacid' => $_W['uniacid'], ':id' => $cashierid, ':set' => $set));
	}
	...
	public function payResult($logid, $return_log = false)
	{
		...
		if ($id != 0) {
			$log = pdo_fetch('SELECT * FROM ' . tablename('vcshop_cashier_pay_log') . ' WHERE `id`=:id and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		}
		else {
			$log = pdo_fetch('SELECT * FROM ' . tablename('vcshop_cashier_pay_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logid));
		}
	...
	public function categoryAll($status = 1)
	{
		...
		$item = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_cashier_category') . (' WHERE 1 ' . $condition . '  ORDER BY displayorder desc, id DESC'), $params);
		return $item;
	}
	...
	{
		global $_W;
		$item = pdo_fetch('select * from ' . tablename('vcshop_cashier_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		return $item;
	}
	...
	public function savaUser(array $params, $diyform = array())
	{
		...
		if (!isset($params['id'])) {
			...
			$params['deleted'] = 0;
			pdo_insert('vcshop_cashier_user', $params);
			$params['id'] = pdo_insertid();
		}
		else {
			pdo_update('vcshop_cashier_user', $params, array('id' => $params['id'], 'uniacid' => $params['uniacid']));
		}
	...
	public function userInfo($openid)
	{
		global $_W;
		$id = intval($openid);
		$sql = 'SELECT * FROM ' . tablename('vcshop_cashier_user') . ' WHERE uniacid=:uniacid AND deleted=0';
		$params = array(':uniacid' => $_W['uniacid']);
	...
	protected function sendNotice($notice, $tag, $datas, $message)
	{
		...
		if (!empty($notice[$tag])) {
			$advanced_template = pdo_fetch('select * from ' . tablename('vcshop_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $notice[$tag], ':uniacid' => $_W['uniacid']));
			...
			if (!empty($advanced_template)) {
				...
				if ($tag == 'saler_stockwarn') {
					$advanced_template = pdo_fetch('select templateid  from ' . tablename('vcshop_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $_W['uniacid']));
	...
	public function createOrder(array $array, $return = NULL, $can_sale = true)
	{
		...
		$res = pdo_insert('vcshop_cashier_pay_log', $data);
		...
		if (!empty($usecoupon)) {
			pdo_update('vcshop_coupon_data', array('used' => 1, 'usetime' => $data['createtime'], 'ordersn' => $data['logno']), array('id' => $array['usecoupon']));
		}
	...
	public function pay($data, $auth_code = NULL)
	{
		...
		if ($data['money'] <= 0 || $data['paytype'] == 3) {
			...
			$data['paytime'] = time();
			pdo_update('vcshop_cashier_pay_log', $data, array('id' => $data['id']));
	...
	public function refund($id)
	{
		...
		$_W['cashieruser'] = $this->userInfo((int) $_W['cashierid']);
		$id = (int) $id;
		$pay_log = pdo_fetch('SELECT * FROM ' . tablename('vcshop_cashier_pay_log') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		pdo_update('vcshop_cashier_pay_log', array('status' => -1, 'refundsn' => $out_trade_no, 'refunduser' => $refunduser), array('uniacid' => $_W['uniacid'], 'id' => $id));
		...
		if (!empty($pay_log['orderid'])) {
			pdo_update('vcshop_order', array('status' => -1), array('uniacid' => $_W['uniacid'], 'id' => $pay_log['orderid']));
			$this->refundbackGoodsStocks(1, $pay_log);
		}

		if ($pay_log['isgoods'] == 1) {
			$goodsinfo = pdo_getall('vcshop_cashier_pay_log_goods', array('logid' => $pay_log['id']), array('goodsid', 'total'));
	...
	public function refundbackGoodsStocks($status, $info = array())
	{
		...
		if ($status == 1) {
			...
			$order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent,paytype,isnewstore,storeid,istrade,status from ' . tablename('vcshop_order') . ' where id=:id limit 1', array(':id' => $orderid));
			...
			foreach ($goods as $g) {
				$goods_item = pdo_fetch('select total as goodstotal from' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
				...
				if (!empty($stocktype)) {
					if (!empty($g['optionid'])) {
						...
						if (!empty($option) && $option['stock'] != -1) {
							...
							pdo_update('vcshop_goods_option', array('stock' => $stock), array('uniacid' => $_W['uniacid'], 'goodsid' => $g['goodsid'], 'id' => $g['optionid']));
						}
					}
					...
					if ($stocktype == 1 && $g['goodstotal'] != -1) {
						...
						if ($totalstock != -1) {
							pdo_update('vcshop_goods', array('total' => $totalstock), array('uniacid' => $_W['uniacid'], 'id' => $g['goodsid']));
						}
					}
				}
			}
		...
		if ($status == 2) {
			if (!empty($info)) {
				foreach ($info as $key => $value) {
					$good = pdo_get('vcshop_cashier_goods', array('id' => $value['goodsid']));

					if ($good['total'] != -1) {
						$totalstock = $good['total'] + $value['total'];
						pdo_update('vcshop_cashier_goods', array('total' => $totalstock), array('id' => $value['goodsid']));
					}
				}
			}
		}
	...
	public function updateOrder($log)
	{
		...
		if ($res['trade_state'] == 'REFUND' || $res['trade_state'] == 'CLOSED' || $res['message'] == '该订单已经关闭或者已经退款') {
			pdo_update('vcshop_cashier_pay_log', array('status' => -1), array('uniacid' => $_W['uniacid'], 'id' => $log['id']));
			return false;
		}
		...
		if ($res['total_fee'] == round($realmoney * 100, 2) || $res['total_amount'] == round($realmoney, 2)) {
			$log = pdo_fetch('SELECT * FROM ' . tablename('vcshop_cashier_pay_log') . ' WHERE `id`=:id and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $log['id']));
	...
	public function paySuccess($log, $user)
	{
		...
		pdo_update('vcshop_cashier_pay_log', array('openid' => $log['openid'], 'payopenid' => $log['openid'], 'money' => $log['money'], 'status' => 1, 'paytime' => 0 < $log['paytime'] ? $log['paytime'] : time(), 'coupon' => $coupon), array('id' => $log['id']));
		...
		if (empty($user['notice_openids'])) {
			...
			if (!empty($log['operatorid'])) {
				$operator = pdo_fetch('select * from ' . tablename('vcshop_cashier_operator') . ' WHERE id=:id AND cashierid=:cashierid limit 1', array(':id' => $log['operatorid'], ':cashierid' => $log['cashierid']));
			}
		...
		if (!empty($log['orderid'])) {
			...
			pdo_update('vcshop_order', array('status' => 3, 'paytype' => $paytype, 'paytime' => time(), 'sendtime' => time(), 'finishtime' => time()), array('id' => $log['orderid'], 'uniacid' => $_W['uniacid']));
		...
		if (!empty($log['openid'])) {
			...
			if (0 < $present_credit1) {
				pdo_update('vcshop_cashier_pay_log', array('present_credit1' => $present_credit1), array('id' => $log['id']));
			}
		}
	...
	public function becomeDown($openid, $manageopenid)
	{
		...
		if ($member) {
			if (empty($member['isagent']) && empty($member['agentid'])) {
				...
				pdo_query('UPDATE ' . tablename('vcshop_member') . (' SET agentid=' . $store_member['id'] . ' WHERE `id` = :id'), array(':id' => $member['id']));
			}
		}
	}
	...
	public function getOrderMoney($starttime = 0, $endtime = 0)
	{
		...
		if ($starttime == 0 && $endtime == 0) {
			$money = (double) pdo_fetchcolumn('SELECT SUM(money+deduction) FROM ' . tablename('vcshop_cashier_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND cashierid=:cashierid ', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid']));
		}
		else {
			$money = (double) pdo_fetchcolumn('SELECT SUM(money+deduction) FROM ' . tablename('vcshop_cashier_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND cashierid=:cashierid AND createtime BETWEEN :starttime AND :endtime', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid'], ':starttime' => $starttime, ':endtime' => $endtime));
		}
	...
	public function saleCalculate($money = 0, $random = true)
	{
		...
		if ($random) {
			$randommoney_log = pdo_fetch('SELECT * FROM ' . tablename('vcshop_cashier_randommoney_log') . ' WHERE uniacid = :uniacid AND cashierid = :cashierid AND clientip = :clientip and ordermoney=:ordermoney limit 1', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid'], ':clientip' => ip2long(CLIENT_IP), ':ordermoney' => $money));
			$time = time();
			if (empty($randommoney_log) || $randommoney_log['expires_time'] < $time) {
				...
				pdo_insert('vcshop_cashier_randommoney_log', $data);
			}
			else {
	...
	public function createGoodsOrder($goods, $openid = '', $setmoney = NULL)
	{
		...
		foreach ($goods as $g) {
			...
			if (!empty($optionid)) {
				$option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`,weight from ' . tablename('vcshop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid, ':id' => $optionid));
	...
	public function goodsCalculate($selfgoods = array(), $shopgoods = array(), $params, $return = false)
	{
		...
		if (!empty($selfgoods)) {
			foreach ($selfgoods as $key => $val) {
				...
				$g = pdo_get('vcshop_cashier_goods', array('cashierid' => $data['cashierid'], 'id' => $data['goodsid']));
				if ($g['total'] < $data['total'] && $g['total'] != -1) {
					pdo_delete('vcshop_cashier_pay_log_goods', array('cashierid' => $data['cashierid'], 'logid' => $data['logid']));
					return array('res' => error('-101', $g['title'] . ' 库存不足, 剩余库存 ' . $g['total']));
				}

				pdo_insert('vcshop_cashier_pay_log_goods', $data);
			}
		}
		...
		pdo_update('vcshop_cashier_pay_log', $order, array('id' => $order['id']));
	...
	public function setStocks($orderid = 0, $manageopenid = '')
	{
		...
		$order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent from ' . tablename('vcshop_order') . ' where id=:id limit 1', array(':id' => $orderid));
		$param = array();
		$param[':uniacid'] = $_W['uniacid'];
		...
		$goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.title from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . (' where ' . $condition . ' and og.uniacid=:uniacid '), $param);
...
', 'color' => '#ff0000'),
			...
			if (1 <= $order['status']) {
				if ($g['totalcnf'] != 1) {
					pdo_update('vcshop_goods', array('sales' => $g['sales'] - $g['total']), array('uniacid' => $_W['uniacid'], 'id' => $g['goodsid']));
				}

				$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
				pdo_update('vcshop_goods', array('salesreal' => $salesreal), array('id' => $g['goodsid']));
			}
		}
	}
	...
	public function setSelfGoodsStocks($logid = 0, $manageopenid = '')
	{
		...
		$goods = pdo_fetchall('SELECT log.* ,g.total as goodstotal,g.title  FROM ' . tablename('vcshop_cashier_pay_log_goods') . 'log' . ' left join' . tablename('vcshop_cashier_goods') . 'g on g.id=log.goodsid ' . ' WHERE log.cashierid=:cashierid AND log.logid=:logid', array(':cashierid' => $_W['cashierid'], ':logid' => $logid));

		foreach ($goods as $g) {
			pdo_query('UPDATE ' . tablename('vcshop_cashier_goods') . (' SET total=total-' . $g['total'] . ' WHERE cashierid=:cashierid AND id=:id AND total<>-1'), array(':cashierid' => $_W['cashierid'], ':id' => $g['goodsid']));
...
', 'color' => '#ff0000'),
	...
	public function querycoupons($kwd = '')
	{
		...
		if (empty($_W['cashieruser']['couponid'])) {
			$list = pdo_fetchall('SELECT couponid  FROM ' . tablename('vcshop_cashier_user') . (' WHERE 1 ' . $condition1), $params);
		...
		$ds = pdo_fetchall('SELECT id,couponname as title , thumb FROM ' . tablename('vcshop_coupon') . (' WHERE 1 ' . $condition . ' order by createtime desc'), $params);
	...
	public function refundWechat($openid, $out_trade_no, $out_refund_no, $totalmoney, $refundmoney = 0, $app = false, $refund_account = false)
	{
		...
		if (is_array($certs)) {
			...
			$certfile = IA_ROOT . '/addons/vcshop/cert/' . random(128);
			file_put_contents($certfile, $certs['cert']);
			$keyfile = IA_ROOT . '/addons/vcshop/cert/' . random(128);
			file_put_contents($keyfile, $certs['key']);
			$rootfile = IA_ROOT . '/addons/vcshop/cert/' . random(128);
