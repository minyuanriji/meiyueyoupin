...
require_once VCSHOP_PLUGIN . 'cashier/core/inc/common.php';
class CashierWebPage extends PluginWebPage
{
	...
	public function __construct($_com = '', $_init = false)
	{
		...
		$this->modulename = 'vcshop';
		$this->plugintitle = m('plugin')->getName($this->pluginname);
		$this->model = m('plugin')->loadModel($this->pluginname);
		$this->set = $this->model->getSet();
		$user = pdo_fetch('SELECT * FROM ' . tablename('vcshop_cashier_user') . ' WHERE id=:id AND uniacid=:uniacid AND status=1 AND deleted=0 LIMIT 1', array(':id' => $_W['cashieruser']['id'], ':uniacid' => $_W['uniacid']));
