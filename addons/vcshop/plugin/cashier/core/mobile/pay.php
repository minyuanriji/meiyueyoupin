...
class Pay_VcShopPage extends CashierMobilePage
{
	public $log;
	...
	public function main()
	{
		...
		if (!empty($_GPC['id'])) {
			$id = $_GPC['id'];
			$qrid = authcode(base64_decode($id), 'DECODE', 'vcshop_cashier');

			if (!empty($qrid)) {
				$item = pdo_get('vcshop_cashier_qrcode', array('uniacid' => $_W['uniacid'], 'cashierid' => $_W['cashierid'], 'id' => $qrid));
	...
	protected function paytype1()
	{
		...
		if (is_array($config)) {
			$config['return_url'] = $_W['siteroot'] . 'addons/vcshop/payment/alipay/return.php';
		}
		else {
			show_json(0);
		}
