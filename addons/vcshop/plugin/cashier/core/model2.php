...
class CashierModel extends PluginModel 
{
	...
	public function setStocks($orderid = 0) 
	{
		...
		$goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $condition . ' and og.uniacid=:uniacid ', $param);
		foreach ($goods as $g ) 
		{
			$stocktype = -1;
			if (!(empty($stocktype))) 
	...
	public function setSelfGoodsStocks($logid = 0) 
	{
		...
		$goods = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_cashier_pay_log_goods') . ' WHERE cashierid=:cashierid AND logid=:logid', array(':cashierid' => $_W['cashierid'], ':logid' => $logid));
		foreach ($goods as $g ) 
		{
