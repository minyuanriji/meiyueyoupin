...
class Setting_VcShopPage extends PluginWebPage
{
	...
	public function download()
	{
		global $_GPC;
		global $_W;
		$dir = IA_ROOT . ('/addons/vcshop/data/qrcode/' . $_W['uniacid'] . '/exchange');
	...
	public function other()
	{
		...
		if ($_W['ispost']) {
			...
			if (pdo_get('vcshop_exchange_setting', array('uniacid' => $_W['uniacid'])) == false) {
				pdo_insert('vcshop_exchange_setting', array('uniacid' => $_W['uniacid'], 'mistake' => $mistake, 'freeze' => $freeze, 'grouplimit' => $grouplimit, 'alllimit' => $alllimit, 'no_qrimg' => (int) $_GPC['no_qrimg'], 'rule' => $_GPC['rule']));
			}
			else {
				pdo_update('vcshop_exchange_setting', array('mistake' => $mistake, 'freeze' => $freeze, 'grouplimit' => $grouplimit, 'alllimit' => $alllimit, 'no_qrimg' => (int) $_GPC['no_qrimg'], 'rule' => $_GPC['rule'], 'coupon_templateid' => $coupon_templateid), array('uniacid' => $_W['uniacid']));
			}

			show_json(1, '保存成功');
		}
		else {
			$res = pdo_fetch('SELECT * FROM ' . tablename('vcshop_exchange_setting') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
		}
