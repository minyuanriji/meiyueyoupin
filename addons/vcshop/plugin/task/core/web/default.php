...
class Default_VcShopPage extends PluginWebPage
{
	...
	{
		global $_W;
		$defaultcount = pdo_fetchcolumn('select COUNT(*) from ' . tablename('vcshop_task_default') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		...
		if (0 < $defaultcount) {
			...
			$result = pdo_update('vcshop_task_default', $update_data, array('uniacid' => $_W['uniacid']));
			...
			$result = pdo_insert('vcshop_task_default', $insert_data);
	...
	public function setstart()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $defaultcount) {
				...
				if (!empty($keyword)) {
					if ($keyword['name'] != 'vcshop:task') {
						show_json(0, '关键字已存在!');
					}
				}

				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'vcshop:task'));
				...
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'vcshop:task', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
				...
				$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'vcshop', 'title' => $default['title'], 'description' => $default['desc'], 'thumb' => $default['thumb'], 'url' => mobileUrl('task'));
				...
				pdo_update('vcshop_task_default', $update_data, array('uniacid' => $_W['uniacid']));
				...
				$default = serialize($default);
				$insert_data = array('uniacid' => $_W['uniacid'], 'data' => $default, 'addtime' => time());
				pdo_insert('vcshop_task_default', $insert_data);
			}
		}

		$set = unserialize($default);
		$bgimg = pdo_get('vcshop_task_default', array('uniacid' => $_W['uniacid']), array('bgimg', 'open'));
