...
require VCSHOP_INC . 'plugin_processor.php';
class TaskProcessor extends PluginProcessor
{
	...
	private function responseText_new($obj)
	{
		...
		$id = pdo_fetchcolumn('select id from ' . tablename('vcshop_task_list') . ' where keyword_pick = :keyword_pick and uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':keyword_pick' => $content));
		$timeout = 4;
	...
	public function responseSubscribe_new($obj)
	{
		...
		$recordid = pdo_fetchcolumn('select recordid from ' . tablename('vcshop_task_qr') . ' where uniacid = :uniacid and sceneid = :sceneid', array(':uniacid' => $_W['uniacid'], ':sceneid' => $eventkey));
		$record = pdo_fetch('select openid,member_group,tasktitle,auto_pick,taskid from ' . tablename('vcshop_task_record') . ' where uniacid = :uniacid and id = :recordid', array(':uniacid' => $_W['uniacid'], ':recordid' => $recordid));
		$list = pdo_fetch('select member_level,followreward from ' . tablename('vcshop_task_list') . ' where id = :id and uniacid = :uniacid ', array(':id' => $record['taskid'], ':uniacid' => $_W['uniacid']));
				...
				if ($member['isagent'] != 1) {
					...
					if ($agent['isagent']) {
						if (!empty($beagent['beagent'])) {
							$a = pdo_update('vcshop_member', array('isagent' => 1, 'agentid' => $agent['id'], 'status' => $become_check, 'childtime' => time(), 'agenttime' => time()), array('id' => $member['id']));
							...
							pdo_update('vcshop_member', array('agentid' => $agent['id'], 'childtime' => time()), array('id' => $member['id']));
						}
					}
					else if (!empty($beagent['beagent'])) {
						pdo_update('vcshop_member', array('isagent' => 1, 'agentid' => $agent['id'], 'status' => $become_check, 'childtime' => time(), 'agenttime' => time()), array('id' => $member['id']));
	...
	public function saleVirtual($obj = NULL)
	{
		...
		if ($open_redis) {
			// $redis_key1 = 'vc_' . $_W['uniacid'] . '_member_salevirtual_isagent';
			// $redis_key2 = 'vc_' . $_W['uniacid'] . '_member_salevirtual';
			$redis_key1 = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_'.'vc_' . $_W['uniacid'] . '_member_salevirtual_isagent';
			$redis_key2 = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_'.'vc_' . $_W['uniacid'] . '_member_salevirtual';
			$redis = redis();
			...
			if (!is_error($redis)) {
				if ($redis->get($redis_key1) != false) {
					...
					$totalagent = pdo_fetchcolumn('select count(*) from' . tablename('vcshop_member') . ' where uniacid =' . $_W['uniacid'] . ' and isagent =1');
					$totalmember = pdo_fetchcolumn('select count(*) from' . tablename('vcshop_member') . ' where uniacid =' . $_W['uniacid']);
	...
	private function responseScan($obj)
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('vcshop_task_poster') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['posterid'], ':uniacid' => $_W['uniacid']));
		...
		if ($member_info['isnew'] || $_SESSION['postercontent']) {
			...
			$scan_count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_task_joiner') . ' where uniacid=:uniacid and task_user=:task_user and joiner_id=:joiner_id and join_id=:join_id ', $params);
				...
				if ($member['isagent'] != 1) {
					if ($qrmember['isagent'] == 1 && $qrmember['status'] == 1) {
						if (!empty($poster['bedown'])) {
							if (empty($member['agentid'])) {
								if (empty($member['fixagentid'])) {
									pdo_update('vcshop_member', array('agentid' => $qrmember['id'], 'childtime' => $time), array('id' => $member['id']));
						...
						if (!empty($poster['beagent'])) {
							$become_check = intval($cset['become_check']);
							pdo_update('vcshop_member', array('isagent' => 1, 'status' => $become_check, 'agenttime' => $time), array('id' => $member['id']));
