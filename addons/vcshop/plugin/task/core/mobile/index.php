...
class index_VcShopPage extends PluginMobilePage
{
	private $new = false;
	...
	{
		global $_W;
		pdo_update('vcshop_task_record', array('read' => 1), array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));
	}
	...
	public function main()
	{
		...
		$bgimg = pdo_get('vcshop_task_default', array('uniacid' => $_W['uniacid']), array('bgimg'));
	...
	public function detail()
	{
		...
		if ($poster) {
			$sql = 'SELECT * FROM ' . tablename('vcshop_task_poster') . ' WHERE id = :id';
			$detail = pdo_fetch($sql, array(':id' => $id));
			$reward_data = unserialize($detail['reward_data']);
		}
		else {
			$sql = 'SELECT * FROM ' . tablename('vcshop_task') . ' WHERE id = :id';
			$detail = pdo_fetch($sql, array(':id' => $id));
			$reward_data = unserialize($detail['reward_data']);
	...
	public function taskposter()
	{
		...
		$now_time = time();
		$task_sql = 'SELECT * FROM ' . tablename('vcshop_task_poster') . ' WHERE timestart<=' . $now_time . ' AND timeend>' . $now_time . ' AND uniacid=' . $_W['uniacid'] . ' AND `status`=1 AND `is_delete`=0 ORDER BY `createtime` DESC';
		...
		$running_sql = 'SELECT `join`.*,`task`.title,`task`.reward_data AS `poster_reward`,`task`.titleicon,`task`.poster_type FROM ' . tablename('vcshop_task_join') . ' AS `join` LEFT JOIN ' . tablename('vcshop_task_poster') . ' AS `task` ON `join`.task_id=`task`.`id` WHERE `join`.`failtime`>' . $now_time . ' AND `join`.`join_user`="' . $openid . '" AND `join`.uniacid=' . $_W['uniacid'] . ' AND `join`.`is_reward` = 0 AND `task`.`is_delete` = 0 ORDER BY `join`.`addtime` DESC LIMIT 0,15';
		...
		$complete_sql = 'SELECT `join`.*,`task`.title,`task`.titleicon,`task`.poster_type FROM ' . tablename('vcshop_task_join') . ' AS `join` LEFT JOIN ' . tablename('vcshop_task_poster') . ' AS `task` ON `join`.task_id=`task`.`id` WHERE `join`.uniacid=' . $_W['uniacid'] . ' AND `join`.`join_user`="' . $openid . '" AND `join`.`is_reward`=1 AND `task`.`is_delete` = 0 ORDER BY `join`.`addtime` DESC LIMIT 0,15';
		...
		$faile_sql = 'SELECT `join`.*,`task`.title,`task`.reward_data AS `poster_reward`,`task`.titleicon,`task`.poster_type FROM ' . tablename('vcshop_task_join') . ' AS `join` LEFT JOIN ' . tablename('vcshop_task_poster') . ' AS `task` ON `join`.task_id=`task`.`id` WHERE `join`.`failtime`<=' . $now_time . ' AND `join`.`join_user`="' . $openid . '" AND `join`.uniacid=' . $_W['uniacid'] . ' AND `join`.`is_reward`=0 AND `task`.`is_delete` = 0 ORDER BY `join`.`addtime` DESC LIMIT 0,15';
	...
	public function getreward()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$rewarded = pdo_get('vcshop_task_extension_join', array('uniacid' => $_W['uniacid'], 'id' => $_GPC['id']));
	...
	private function getpostericon($id)
	{
		global $_W;
		global $_GPC;
		return pdo_fetchcolumn('SELECT titleicon FROM ' . tablename('vcshop_task_poster') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	}

	private function checkJoined($taskid)
	{
		global $_W;
		$sql = 'SELECT COUNT(*) FROM ' . tablename('vcshop_task_extension_join') . ' WHERE openid = :openid AND taskid = :taskid';
	...
	{
		global $_W;
		$sql = 'SELECT `data` FROM ' . tablename('vcshop_task_default') . ' WHERE uniacid = :uniacid';
	...
	public function main_new()
	{
		...
		$tableList = tablename('vcshop_task_list');
		$tableRecord = tablename('vcshop_task_record');
		...
		$set = pdo_fetchcolumn('select bg_img from ' . tablename('vcshop_task_set') . (' where uniacid = ' . $_W['uniacid']));
	...
	public function rewardlist()
	{
		...
		$psize = 100;
		$pstart = ($page - 1) * $psize;
		$sql = 'select * from ' . tablename('vcshop_task_reward') . (' where openid = :openid and `get` = 1 and uniacid = :uniacid order by gettime desc limit ' . $pstart . ',' . $psize);
	...
	public function mine()
	{
        ...
        $sql = "select tr.* from " . tablename("vcshop_task_record") . " tr " . " inner join " . tablename("vcshop_task_list") . " t on  tr.taskid = t.id" . " where tr.openid = :openid and tr.uniacid = :uniacid  " . $condition . " order by tr.id desc";
		$list = pdo_fetchall($sql, array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		include $this->template();
	}
	...
	public function detail_new()
	{
		...
		if (!empty($rid)) {
			$sql = 'select * from ' . tablename('vcshop_task_record') . ' where id = :id and uniacid = :uniacid';
			$detail = pdo_fetch($sql, array(':id' => $rid, ':uniacid' => $_W['uniacid']));
			$reward = json_decode($detail['reward_data'], true);
			$goods = pdo_fetchall('select * from ' . tablename('vcshop_task_reward') . ' where recordid = :recordid and openid = :openid and uniacid = :uniacid and isjoiner = 0 and reward_type = \'goods\' and `level` = 0 group by reward_title', array(':recordid' => $detail['id'], ':openid' => trim($_W['openid']), ':uniacid' => intval($_W['uniacid'])));
			$reward_goods = pdo_fetchall('select * from ' . tablename('vcshop_task_reward') . ' where recordid = :recordid and openid = :openid and uniacid = :uniacid and isjoiner = 0 and reward_type = \'goods\' and `level` = 0', array(':recordid' => $detail['id'], ':openid' => trim($_W['openid']), ':uniacid' => intval($_W['uniacid'])));
			...
			if ($detail['tasktype'] == 'poster') {
				if ($detail['level2'] == 0) {
					...
					if ($detail['level2'] < $detail['task_demand'] && $detail['level1'] < $detail['task_demand']) {
						...
						$goods1 = pdo_fetchall('select * from ' . tablename('vcshop_task_reward') . (' where recordid = ' . $detail['id'] . ' and openid = \'' . $_W['openid'] . '\' and uniacid = ' . $_W['uniacid'] . ' and isjoiner = 0 and reward_type = \'goods\' and `level` = 1 group by reward_title'));
						$reward_goods1 = pdo_fetchall('select * from ' . tablename('vcshop_task_reward') . (' where recordid = ' . $detail['id'] . ' and openid = \'' . $_W['openid'] . '\' and uniacid = ' . $_W['uniacid'] . ' and isjoiner = 0 and reward_type = \'goods\' and `level` = 1'));
						...
						$goods2 = pdo_fetchall('select * from ' . tablename('vcshop_task_reward') . (' where recordid = ' . $detail['id'] . ' and openid = \'' . $_W['openid'] . '\' and uniacid = ' . $_W['uniacid'] . ' and isjoiner = 0 and reward_type = \'goods\' and `level` = 2 group by reward_title'));
						$reward_goods2 = pdo_fetchall('select * from ' . tablename('vcshop_task_reward') . (' where recordid = ' . $detail['id'] . ' and openid = \'' . $_W['openid'] . '\' and uniacid = ' . $_W['uniacid'] . ' and isjoiner = 0 and reward_type = \'goods\' and `level` = 2'));
			...
			$joiner = pdo_fetchall('select DISTINCT openid,headimg,nickname,gettime from ' . tablename('vcshop_task_reward') . (' where isjoiner = 1 and recordid = ' . $rid . ' and `get`=1 and uniacid = ' . $_W['uniacid']));
		}

		if (empty($detail) && !empty($id)) {
			$sql = 'select * from ' . tablename('vcshop_task_list') . ' where id = :id and uniacid = :uniacid';
		...
		$postfix = pdo_fetch('SELECT verb,unit FROM ' . tablename('vcshop_task_list') . ' WHERE id=:id and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $detail['taskid']));
	...
	public function getred()
	{
		...
		$money = pdo_fetchcolumn('select reward_data from ' . tablename('vcshop_task_reward') . (' where id = ' . $rewardid . ' and `get` = 1 and sent = 0 and openid = \'' . $_W['openid'] . '\''));
		...
		pdo_update('vcshop_task_reward', array('sent' => 1, 'senttime' => time()), array('id' => $rewardid));
		show_json(1);
	}
	...
	public function buygoods()
	{
		global $_W;
		global $_GPC;
		$sql = 'SELECT id,design_data FROM ' . tablename('vcshop_task_record') . ' WHERE id = :id AND uniacid = :uniacid';
