...
class build_VcShopPage extends PluginPfMobilePage
{
	/**
     * 接任务
     */
	...
	public function main()
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('vcshop_task_poster') . ' where keyword=:keyword and uniacid=:uniacid and `status`=1 and `is_delete`=0 limit 1', array(':keyword' => $content, ':uniacid' => $_W['uniacid']));
		...
		$img = '';
		$is_waiting = false;
		$task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('vcshop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $poster['poster_type'] . ' and is_reward=0 and failtime>' . time(), array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid']));

		if ($task_count) {
			$task_info = pdo_fetch('select `needcount`,`completecount`,`is_reward`,`failtime` from ' . tablename('vcshop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_id=:task_id and task_type=:task_type and  failtime>' . time() . ' order by `addtime` DESC limit 1', array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid'], ':task_id' => $poster['id'], ':task_type' => $poster['poster_type']));
			...
			$other_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('vcshop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $poster_type . ' and failtime>' . time(), array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid']));
			...
			if ($other_task_count) {
				...
				if ($default) {
					...
					if ($default['is_posterall'] == 1) {
						$end_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('vcshop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $poster['poster_type'] . ' and failtime<' . time(), array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid']));

						if ($end_task_count) {
							$end_task_info = pdo_fetch('select `needcount`,`completecount`,`failtime` from ' . tablename('vcshop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_id=:task_id and task_type=:task_type and failtime<' . time() . ' order by `addtime` DESC limit 1', array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid'], ':task_id' => $poster['id'], ':task_type' => $poster['poster_type']));
				...
				$end_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('vcshop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $poster['poster_type'] . ' and (is_reward=1 or failtime<' . time() . ')', array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid']));
...
当前海报有效期至：[海报有效期]';
	...
	private function join_task($member, $poster)
	{
		...
		pdo_insert('vcshop_task_join', $task_join);
		$id = pdo_insertid();
		$img = '';
