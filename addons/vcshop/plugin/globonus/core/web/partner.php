...
class Partner_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('vcshop_member') . ' dm  ' . ' left join ' . tablename('vcshop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('vcshop_globonus_level') . ' l on l.id = dm.partnerlevel' . ' where dm.uniacid =' . $_W['uniacid'] . (' and (dm.ispartner =1 or dm.ispartner=-1) ' . $condition), $params);
	...
	public function delete()
	{
		...
		foreach ($members as $member) {
			pdo_update('vcshop_member', array('ispartner' => 0, 'partnerstatus' => 0), array('id' => $member['id']));
	...
	public function check()
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,partnerstatus FROM ' . tablename('vcshop_member') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		$time = time();
		...
		foreach ($members as $member) {
			...
			if ($status == 1) {
				pdo_update('vcshop_member', array('partnerstatus' => 1, 'partnertime' => $time), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
				...
				pdo_update('vcshop_member', array('partnerstatus' => 0, 'partnertime' => 0), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
	...
	public function partnerblack()
	{
		...
		foreach ($members as $member) {
			...
			if ($partnerblack == 1) {
				pdo_update('vcshop_member', array('ispartner' => 1, 'partnerstatus' => 0, 'partnerblack' => 1), array('id' => $member['id']));
				...
				pdo_update('vcshop_member', array('ispartner' => 1, 'partnerstatus' => 1, 'partnerblack' => 0), array('id' => $member['id']));
