...
class Level_VcShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_globonus_level') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY bonus asc'));
	...
	protected function post()
	{
		...
		if ($id == 'default') {
			...
			$level = pdo_fetch('SELECT * FROM ' . tablename('vcshop_globonus_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => intval($id), ':uniacid' => $_W['uniacid']));
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if ($id == 'default') {
					...
					pdo_update('vcshop_globonus_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					plog('globonus.level.edit', '修改股东等级 ID: ' . $id . $updatecontent);
				}
			}
			else {
				pdo_insert('vcshop_globonus_level', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('vcshop_globonus_level') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('vcshop_globonus_level', array('id' => $item['id']));
