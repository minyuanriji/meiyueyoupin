...
class SeckillModel extends PluginModel 
{
	...
	{
		global $_W;
		return 'vcshop_' . $_W['uniacid'] . '_' . $_W['account']['key'] . '_seckill_';
	}
	...
	public function deleteSeckill() 
	{
		...
		foreach ($keys as $key ) 
		{
			...
			if (!(empty($orders))) 
			{
				...
				{
					$o = pdo_fetch('select  id,openid,deductcredit2,ordersn,isparent,deductcredit,deductprice   from ' . tablename('vcshop_order') . ' where id=:id  limit 1', array(':id' => $orderid));
	...
	public function getTaskSeckillInfo() 
	{
		...
		$dbgoods = pdo_fetchall('select id,thumb,marketprice from ' . tablename('vcshop_goods') . ' where id in (' . implode(',', $goodsids) . ') and uniacid=' . $_W['uniacid'], array(), 'id');
		$goods = array();
