	...
	public function setting()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($arr['keyword'])) {
				...
				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'vcshop', ':name' => 'vcshop:mmanage'));

				if (empty($rule)) {
					$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'vcshop手机管理入口'));
				}
				...
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'vcshop手机管理入口', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($arr['status']));
