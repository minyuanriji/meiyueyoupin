...
class PosteraProcessor extends PluginProcessor
{
	...
	private function responseScan($obj)
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('vcshop_postera') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['posterid'], ':uniacid' => $_W['uniacid']));
		...
		if ($poster['resptype'] == '0') {
			...
			if ($poster['type'] == 3) {
				$goods = pdo_fetch('select id,title, thumb,description from ' . tablename('vcshop_goods') . ' where id=:id limit 1', array(':id' => $poster['goodsid']));
...
' . $goods['description']);
	...
	private function responseSubscribe($obj)
	{
		...
		$qrmember = m('member')->getMember($qr['openid']);
		$log = pdo_fetch('select * from ' . tablename('vcshop_postera_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid']));
		if (empty($log) && $openid != $qr['openid']) {
			...
			pdo_insert('vcshop_postera_log', $log);
			$log['id'] = pdo_insertid();
...
谢谢您对我们的支持！', 'color' => '#4a5077')
			...
			if (!empty($upgrade)) {
				pdo_update('vcshop_postera_log', $upgrade, array('id' => $log['id']));
			}
		}
