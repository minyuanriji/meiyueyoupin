...
class InvitationModel extends PluginModel
{
	public function getQR($params = array())
	{
		...
		$expire = 2592000;
		$create = false;
		$qr = pdo_fetch('select * from ' . tablename('vcshop_invitation_qr') . ' where openid=:openid and acid=:acid and invitationid=:invitationid and roomid=:roomid limit 1', array(':openid' => $params['openid'], ':acid' => $_W['acid'], ':invitationid' => $params['invitationid'], ':roomid' => $params['roomid']));
		...
		if (empty($qr)) {
			...
			$ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'qrcid' => $scene_id, 'model' => 0, 'name' => 'VCSHOP_INVITATION_QRCODE', 'keyword' => 'VCSHOP_INVITATION', 'expire' => $expire, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
			...
			pdo_insert('vcshop_invitation_qr', $qr);
			$qr['id'] = pdo_insertid();
			$create = true;
		}
		else {
			if ($qr['createtime'] + $qr['expire'] <= time()) {
				...
				pdo_update('vcshop_invitation_qr', array('ticket' => $result['ticket'], 'qrimg' => $qrimg, 'url' => $result['url']), array('id' => $qr['id']));
	...
	{
		global $_W;
		$path = IA_ROOT . '/addons/vcshop/data/qrcode/invitation/' . $_W['uniacid'] . '/';
		...
		if (!$create && is_file($filename)) {
			return $_W['siteroot'] . 'addons/vcshop/data/qrcode/invitation/' . $_W['uniacid'] . '/' . $file;
		}
	...
	public function getQRByTicket($ticket)
	{
		...
		$qr = pdo_fetch('SELECT * FROM ' . tablename('vcshop_invitation_qr') . ' WHERE ticket=:ticket AND acid=:acid', array(':ticket' => $ticket, ':acid' => $_W['acid']));
	...
	public function scanLog($openid = NULL, $qr = array(), $follow = false)
	{
		...
		pdo_query('UPDATE ' . tablename('vcshop_invitation') . ' SET scan=scan+1 WHERE id=:id AND uniacid=:uniacid', array(':id' => $qr['invitationid'], ':uniacid' => $_W['uniacid']));

		if ($follow) {
			pdo_query('UPDATE ' . tablename('vcshop_invitation') . ' SET follow=follow+1 WHERE id=:id AND uniacid=:uniacid', array(':id' => $qr['invitationid'], ':uniacid' => $_W['uniacid']));
		}

		pdo_insert('vcshop_invitation_log', array('uniacid' => $_W['uniacid'], 'invitation_id' => $qr['invitationid'], 'openid' => $openid, 'invitation_openid' => $qr['openid'], 'scan_time' => time(), 'follow' => $follow ? 1 : 0));
	}
}
