						...
						if ($item['params']['goodsdata'] == '0') {
							...
							if (!empty($cateid)) {
								...
								if (empty($item['params']['goodstype'])) {
									...
									$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels, showgroups, `type`, goodstype from ' . tablename('vcshop_creditshop_goods') . (' where cate=:cate and status=1 and deleted=0 and uniacid=:uniacid order by ' . $orderby . ' limit ') . $limit, array(':cate' => $cateid, ':uniacid' => $_W['uniacid']));
								}
							...
							if ($item['params']['goodsdata'] == '2' && empty($item['params']['goodstype'])) {
								...
								if (2 < $item['params']['goodsdata']) {
									...
									if (empty($item['params']['goodstype'])) {
										...
										$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels,showgroups, `type`, goodstype from ' . tablename('vcshop_creditshop_goods') . (' where 1 ' . $condition . ' order by ' . $args['order'] . ' limit ') . $args['pagesize'], $params);
									}
