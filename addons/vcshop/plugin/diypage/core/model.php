...
class DiypageModel extends PluginModel
{
	...
	public function getPageList($type = 'allpage', $condition = NULL, $page = 0)
	{
		...
		$list = pdo_fetchall('select id, `name`, `type`, createtime, lastedittime, keyword,`data` from ' . tablename('vcshop_diypage') . ' where merch=:merch ' . $c . ' and uniacid=:uniacid order by `type` desc, id desc ' . $limit, array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_diypage') . ' where merch=:merch and uniacid=:uniacid ' . $c, array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
	...
	public function getPage($id, $mobile = false)
	{
		...
		$page = pdo_fetch('select * from ' . tablename('vcshop_diypage') . $where . ' limit 1 ', $params);
		if (!empty($page)) {
						...
						if ($item['params']['goodsdata'] == '0') {
							if (!empty($item['data']) && is_array($item['data'])) {
								...
								if (!empty($goodsids) && is_array($goodsids)) {
									...
									if ($creditshop) {
										$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showgroups, `type`, goodstype from ' . tablename('vcshop_creditshop_goods') . (' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc '), array(':uniacid' => $_W['uniacid']));
									}
									else {
										// 20200416 lin
										$goods = pdo_fetchall('select id,`presellend`,`preselltimeend`,isdiscount, isdiscount_time, title, subtitle, thumb, productprice, minprice, total,`type`,showlevels, showgroups,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice, bargain, merchid, sales, salesreal,ispresell,presellprice,credit,deduct from ' . tablename('vcshop_goods') . (' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc '), array(':uniacid' => $_W['uniacid']));
        ...
        g.minprice = (select min(marketprice) from ' . tablename('vcshop_goods_option') . ' where goodsid = ' . $good['id'] . '),
        g.maxprice = (select max(marketprice) from ' . tablename('vcshop_goods_option') . ' where goodsid = ' . $good['id'] . ')
        where g.id = ' . $good['id'] . ' and g.hasoption=1';
															pdo_query($sql);
														}
														else {
															$sql = 'update ' . tablename('vcshop_goods') . ' set minprice = marketprice,maxprice = marketprice where id = ' . $good['id'] . ' and hasoption=0';
															pdo_query($sql);
														}

														$goods_price = pdo_fetch('select title,minprice,maxprice,ispresell,presellend,preselltimeend,presellprice from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $good['id'], ':uniacid' => $_W['uniacid']));
							...
							if (!empty($item['params']['cateid'])) {
								if (empty($item['params']['goodstype'])) {
									$category = pdo_fetch('select id,`name`, enabled from ' . tablename('vcshop_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								}
								else {
									$category = pdo_fetch('select id,`name`, enabled from ' . tablename('vcshop_creditshop_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								}
							...
							if ($item['params']['goodsdata'] == '2' && empty($item['params']['goodstype'])) {
								if (!empty($item['params']['groupid'])) {
									$group = pdo_fetch('select id, `name` from ' . tablename('vcshop_goods_group') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['groupid'], ':uniacid' => $_W['uniacid']));
						...
						if ($item['id'] == 'merchgroup' && p('merch')) {
							if ($item['params']['merchdata'] == '0') {
								...
								if (!empty($merchids) && is_array($merchids)) {
									...
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('vcshop_merch_user') . (' where id in( ' . $newmerchids . ' ) and status=1 and uniacid=:uniacid order by  field (id,') . $newmerchids . ')', array(':uniacid' => $_W['uniacid']));
								...
								if (!empty($item['params']['cateid'])) {
									$category = pdo_fetch('select id, `catename`, status from ' . tablename('vcshop_merch_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								}
								...
								if (!empty($category) && !empty($category['status'])) {
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('vcshop_merch_user') . ' where cateid=:cateid and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid'], ':cateid' => $item['params']['cateid']));
									if (!empty($merchs) && is_array($merchs)) {
										$item['data'] = array();
								...
								if (!empty($item['params']['groupid'])) {
									$group = pdo_fetch('select id, groupname, status from ' . tablename('vcshop_merch_group') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['groupid'], ':uniacid' => $_W['uniacid']));
								...
								if (!empty($group) && !empty($group['status'])) {
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('vcshop_merch_user') . ' where groupid=:groupid and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid'], ':groupid' => $item['params']['groupid']));
									if (!empty($merchs) && is_array($merchs)) {
										$item['data'] = array();
								...
								if ($item['params']['merchdata'] == '3') {
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('vcshop_merch_user') . ' where isrecommand=1 and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid']));
									if (!empty($merchs) && is_array($merchs)) {
										$item['data'] = array();
							...
							if ($item['id'] == 'picture' || $item['id'] == 'picturew') {
								...
								if (!empty($item['data']) && is_array($item['data'])) {
									...
									if (!empty($couponids) && is_array($couponids)) {
										...
										$coupons = pdo_fetchall('select * from ' . tablename('vcshop_coupon') . (' where id in( ' . $newcouponids . ' ) and uniacid=:uniacid'), array(':uniacid' => $_W['uniacid']), 'id');
			...
			if ($mobile && !empty($page['data']['items']) && is_array($page['data']['items'])) {
				foreach ($page['data']['items'] as $itemid => &$item) {
					if ($item['id'] == 'goods') {
						if ($item['params']['goodsdata'] == '0') {
							if (!empty($item['data']) && is_array($item['data'])) {
								...
								if (!empty($goodsids) && is_array($goodsids)) {
									...
									if ($creditshop) {
										$goods = pdo_fetchall('select id, showlevels, showgroups from ' . tablename('vcshop_creditshop_goods') . (' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc '), array(':uniacid' => $_W['uniacid']));
									}
									else {
										$goods = pdo_fetchall('select id, showlevels,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,showgroups from ' . tablename('vcshop_goods') . (' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc '), array(':uniacid' => $_W['uniacid']));
									}
									if (!empty($goods) && is_array($goods)) {
							...
							if (!empty($cateid)) {
								...
								if (empty($item['params']['goodstype'])) {
									...
									$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels, showgroups, `type`, goodstype,deduct from ' . tablename('vcshop_creditshop_goods') . (' where cate=:cate and status=1 and deleted=0 and uniacid=:uniacid order by ' . $orderby . ' limit ') . $limit, array(':cate' => $cateid, ':uniacid' => $_W['uniacid']));
								}
								...
								if (!empty($goods) && is_array($goods)) {
									...
									foreach ($goods as $key => $value) {
										...
										$goods[$key]['dprice'] = (double)($goods[$key]['minprice']-$goods[$key]['deduct']);
									}
							...
							if ($item['params']['goodsdata'] == '2' && empty($item['params']['goodstype'])) {
								...
								if (!empty($groupid)) {
									$group = pdo_fetch('SELECT * FROM ' . tablename('vcshop_goods_group') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $groupid, ':uniacid' => $_W['uniacid']));
								}
								...
								if (2 < $item['params']['goodsdata']) {
									...
									if (empty($item['params']['goodstype'])) {
										...
										$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels,showgroups, `type`, goodstype,deduct from ' . tablename('vcshop_creditshop_goods') . (' where 1 ' . $condition . ' order by ' . $args['order'] . ' limit ') . $args['pagesize'], $params);
									}
									...
									if (!empty($goods) && is_array($goods)) {
										...
										foreach ($goods as $index => $good) {
											...
											if (!empty($showgoods)) {
												$childid = rand(1000000000, 9999999999);
												$childid = 'C' . $childid;
												$item['data'][$childid] = array('thumb' => $good['thumb'], 'title' => $good['title'], 'subtitle' => $good['subtitle'], 'price' => $good['minprice'], 'gid' => $good['id'], 'total' => $good['total'], 'bargain' => $good['bargain'], 'productprice' => $good['productprice'], 'credit' => $good['credit'], 'seecommission' => $good['seecommission'], 'cansee' => $good['cansee'], 'seetitle' => $good['seetitle'], 'ctype' => $good['type'], 'gtype' => $good['goodstype'], 'sales' => $good['sales'] + intval($good['salesreal']),'deduct'=>$good['deduct'],'dprice'=>$good['dprice']);
											}
										}
									}
								}
						...
						$params = array(":uniacid" => $_W["uniacid"]);
						$sql = "SELECT c.*,g.* FROM " . tablename("vcshop_groups_goods") . " AS g\n                                LEFT JOIN " . tablename("vcshop_groups_category") . " AS c ON g.category = c.id\n                                where  1 = 1 and " . $condition . " ORDER BY g.displayorder DESC,g.id DESC LIMIT " . $limit;
						...
						if ($item['params']['noticedata'] == '0') {
							...
							if (!empty($page['merch'])) {
								$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('vcshop_merch_notice') . (' where uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc limit ' . $limit), array(':uniacid' => $_W['uniacid'], ':merchid' => intval($page['merch'])));
							}
							else {
								$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('vcshop_notice') . (' where uniacid=:uniacid and iswxapp=0 and status=1 order by displayorder desc limit ' . $limit), array(':uniacid' => $_W['uniacid']));
							}
						...
						if ($item['params']['noticedata'] == '2') {
							$notices = pdo_fetchall('SELECT m.nickname, m.avatar as imgurl, o.createtime as time, o.id FROM' . tablename('vcshop_order') . ' o LEFT JOIN ' . tablename('vcshop_member') . ' m ON m.openid=o.openid WHERE o.uniacid=:uniacid AND m.nickname!=\'\' ORDER BY o.createtime DESC LIMIT 10', array(':uniacid' => $_W['uniacid']));
							$item['data'] = array();
							if (!empty($notices) && is_array($notices)) {
								foreach ($notices as $index => $notice) {
									$goods = pdo_fetchall('SELECT title,goodsid FROM' . tablename('vcshop_order_goods') . ' WHERE uniacid=:uniacid AND orderid = :id', array(':uniacid' => $_W['uniacid'],':id'=>$notice['id']));
									foreach ($goods as $g => $go) {
										$childid = rand(1000000000, 9999999999);
										$childid = 'C' . $childid;
										// if (strpos($notice['nickname'], '\'') !== false) {
										// 	$nickname = str_replace('\'', '`', $notice['nickname']);
										// 	$nickname = str_replace('"', '`', $nickname);
										// 	$nickname = str_replace(PHP_EOL, '', $nickname);
										// }
										$nickname = mb_substr($notice['nickname'], 0, 1, 'utf8').'**';
										$timerstr = p('diypage')->getDanmuTime(time() - $notice['time']);
										// $noticecon = $notice['nickname'].' '.$timerstr.' 购买了'.$go['title'];
										$item['data'][$childid] = array('id' => $go['goodsid'], 'nickname' => $nickname ,'timestr' => $timerstr , 'goodstitle'=>$go['title']);
									}
								}
							}
						}
	...
	public function savePage($id, $data, $update = true)
	{
		...
		if (!empty($keyword) && $update) {
			...
			if (!empty($result)) {
				if ($result['name'] != 'vcshop:diypage:' . $id) {
					show_json(0, '关键字已存在！');
				}
			}
			else {
		...
		if (!empty($id)) {
			...
			pdo_update('vcshop_diypage', $diypage, array('id' => $id, 'uniacid' => $_W['uniacid']));
			$savetype = 'update';
		}
		else {
			...
			pdo_insert('vcshop_diypage', $diypage);
			$id = pdo_insertid();
			$savetype = 'insert';
		}

		if (!empty($keyword) && $update) {
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'vcshop', ':name' => 'vcshop:diypage:' . $id));

			if (!empty($rule)) {
				pdo_update('rule_keyword', array('content' => $keyword), array('rid' => $rule['id']));
			}
			else {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'vcshop:diypage:' . $id, 'module' => 'vcshop', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'vcshop', 'content' => $keyword, 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
		}

		if ($update) {
			$item = pdo_fetch('select id, type, name, keyword from ' . tablename('vcshop_diypage') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	...
	public function delPage($id)
	{
		...
		$items = pdo_fetchall('SELECT id,`name`,`type`,keyword FROM ' . tablename('vcshop_diypage') . (' WHERE id in( ' . $id . ' ) and uniacid=:uniacid '), array(':uniacid' => $_W['uniacid']));

		foreach ($items as $item) {
			pdo_delete('vcshop_diypage', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			...
			$keyword = pdo_fetch('SELECT * FROM ' . tablename('rule_keyword') . ' WHERE content=:content and module=:module and uniacid=:uniacid limit 1 ', array(':content' => $item['keyword'], ':module' => 'vcshop', ':uniacid' => $_W['uniacid']));
	...
	public function saveImg($str)
	{
		...
		$str = preg_replace_callback('/"imgurl"\\:"([^\\\'" ]+)"/', function($matches) {
			...
			if (!strexists($preg, '../addons/vcshop')) {
	...
	public function saveTemp($temp)
	{
		...
		pdo_insert('vcshop_diypage_template', $temp);
	...
	public function verify($do, $pagetype)
	{
		...
		if ($do == 'add') {
			...
			if (!empty($tid)) {
				$template = pdo_fetch('select * from ' . tablename('vcshop_diypage_template') . ' where id=:id and (uniacid=:uniacid or uniacid=0) limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $tid));
	...
	public function calculate($str = NULL, $pagetype = false)
	{
		...
		if (strexists($str, 'r=commission')) {
			...
			// zhou 20191125 通知显示 s
			if (strexists($str, 'r=member.message')) {
				// $user = m('member')->getMember($_W['openid']);
				$user = pdo_fetch('select id from ' . tablename('vcshop_member') . ' where uniacid=:uniacid and openid=:openid',array(':uniacid'=>$_W['uniacid'],':openid'=>$_W['openid']));
				$this->$member['message'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_message_notice_log') . ' where is_read=0 and mid=:mid and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':mid' => $user['id']));
			}
			// zhou 20191125 通知显示 e
			if ($plugin_commissiom) {
				...
				// 	strexists($str, 'r=commission.order') || 
				// 	strexists($str, 'r=commission.log') || 
				// 	strexists($str, 'r=commission.down')
				// ){
				...
				if (strexists($str, 'r=commission.log')) {
					$this->commission['applycount'] = pdo_fetchcolumn('select count(id) from ' . tablename('vcshop_commission_apply') . ' where mid=:mid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
				}

				if (strexists($str, 'r=commission.down')) {
					$level1 = $level2 = $level3 = 0;
					$level1 = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member') . ' where agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));
					if (2 <= $plugin_commissiom_set['level'] && 0 < count($member['level1_agentids'])) {
						$level2 = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member') . ' where agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
					}

					if (3 <= $plugin_commissiom_set['level'] && 0 < count($member['level2_agentids'])) {
						$level3 = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member') . ' where agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
					}
		...
		if (strexists($str, 'memberc') && p('commission')) {
			// han 20201112 此处用不着括号里的数据
			// $member = p('commission')->getInfo($_W['openid'], array('total', 'ordercount0', 'ok', 'ordercount', 'wait', 'pay'));
			$member = p('commission')->getInfo($_W['openid'], array());
		...
		if (strexists($str, 'r=order') || strexists($str, 'r=member.cart') || strexists($str, 'r=coupon.my')) {
			...
			if (strexists($str, 'status=0')) {
				...
				$ordernum['status0'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . ' where ' . $condition . ' and openid=:openid and status=0 and (isparent=1 or (isparent=0 and parentid=0)) and uniacid=:uniacid and istrade=0 and userdeleted=0 limit 1', $params);
			}
			...
			if (strexists($str, 'status=1')) {
				...
				$ordernum['status1'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid limit 1', $params);
			}
			...
			if (strexists($str, 'status=2')) {
				...
				$ordernum['status2'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid limit 1', $params);
			}
			...
			if (strexists($str, 'status=4')) {
				...
				$ordernum['status4'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid limit 1', $params);
			}
			...
			if (strexists($str, 'r=member.cart')) {
				...
				$ordernum['cartnum'] = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('vcshop_member_cart') . ' where ' . $condition . ' and uniacid=:uniacid and openid=:openid ', $params);
			}
			...
			if (strexists($str, 'r=member.favorite')) {
				...
				$ordernum['favorite'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member_favorite') . ' where ' . $condition . ' and uniacid=:uniacid and openid=:openid ', $params);
			}

			if (strexists($str, 'r=sale.coupon.my')) {
				$time = time();
				$sql = 'select count(*) from ' . tablename('vcshop_coupon_data') . ' d';
	...
	public function judge($type = NULL, $str = NULL)
	{
		...
		if ($type == 'dot') {
			...
			if (strexists($str, 'r=order')) {
				...
				// zhou 20191125  增加显示数据
				if (strexists($str, 'r=member.message')) {
					return $this->$member['message'];
				}
			}
	...
	public function url($str)
	{
		...
		if (!empty($_W['openid'])) {
			...
			if (!empty($myid)) {
				$member = pdo_fetch('select id,isagent,status from' . tablename('vcshop_member') . 'where id=' . $myid);
	...
	{
		global $_W;
		$adv = pdo_fetch('SELECT * FROM' . tablename('vcshop_diypage_plu') . 'WHERE id=:id AND status=1 AND `type`=1 AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
