...
class ContactController extends Controller
{
	public function index()
	{
		...
		$casebanner = pdo_fetch('select contact from ' . tablename('vcshop_system_setting') . ' where uniacid = :uniacid ', array(':uniacid' => $uniacid));

		if (empty($casebanner)) {
			$casebanners = pdo_fetchall('select contact from ' . tablename('vcshop_system_setting') . ' where 1 ');
