...
class Printer_VcShopComModel extends ComModel
{
	const PRINTRT_365 = 0;
...
';
	...
	public function printer($params, $templateid, $printerid, $type = 0)
	{
		global $_W;
		$this->type = $type;
		$printer = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_printer') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $printerid));

		if (empty($printer)) {
			return error(-1, '小票打印机未找到');
		}

		$template = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_printer_template') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $templateid));
...
') . $content;
	...
	public function getPrinterSet($merchid = 0)
	{
		...
		if (!empty($data['order_printer'])) {
			$data['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_member_printer') . ' WHERE uniacid=:uniacid AND id IN (' . $data['order_printer'] . ')', array(':uniacid' => $_W['uniacid']));
		}
		else {
	...
	public function getStorePrinterSet($store)
	{
		...
		if (!empty($store['order_printer'])) {
			$store['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_member_printer') . ' WHERE uniacid=:uniacid AND id IN (' . $store['order_printer'] . ')', array(':uniacid' => $_W['uniacid']));
		}
		else {
	...
	public function sendOrderMessage($orderid = 0, $verify = array())
	{
		...
		$order = pdo_fetch('select * from ' . tablename('vcshop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
		...
		$order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.goodssn,og.productsn from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and' . $scondition, $param);
		$goods = array();
		$replace_str = array('\'', '"', '#', '&', '+', ' ');
...
联系电话: ' . $carrier['carrier_mobile'];
		...
		if (!empty($order['verifystoreid'])) {
			$store = pdo_fetch('select * from ' . tablename('vcshop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $order['verifystoreid'], ':uniacid' => $_W['uniacid']));
			$StorePrinterSet = $this->getStorePrinterSet($store);
		}
	...
	public function sendCashierMessage($log, $templateid, $printerid, $operator)
	{
		...
		if (!empty($log['isgoods'])) {
			$goodsAll = pdo_fetchall('SELECT g.title,g.goodssn,og.price,og.total FROM ' . tablename('vcshop_cashier_goods') . ' g LEFT JOIN ' . tablename('vcshop_cashier_pay_log_goods') . ' og ON g.id=og.goodsid WHERE og.cashierid=:cashierid AND og.logid=:logid', array(':cashierid' => $log['cashierid'], ':logid' => $log['id']));
		...
		if (!empty($log['orderid'])) {
			$order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $log['uniacid'], ':orderid' => $log['orderid']));
