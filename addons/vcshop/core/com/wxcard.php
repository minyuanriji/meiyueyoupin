...
class Wxcard_VcShopComModel extends ComModel
{
	...
	public function wxCardUpdateQuantity($card_id)
	{
		...
		if (!empty($id)) {
			$sql = 'select id,uniacid, card_id from ' . tablename('vcshop_wxcard');
		...
		pdo_update('vcshop_wxcard', $data, array('id' => $id));
		return true;
	}
	...
	public function consumeWxCardCount($openid, $merch_array, $goods_array)
	{
		...
		$sql = 'select id,uniacid, card_id,least_cost,reduce_cost,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids  from ' . tablename('vcshop_wxcard');
	...
	public function getAvailableWxcards($type, $money = 0, $merch_array, $goods_array = array())
	{
		...
		$sql = 'select id,uniacid,card_type,logo_url,title, card_id,least_cost,reduce_cost,discount,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids,datetype,end_timestamp,fixed_term  from ' . tablename('vcshop_wxcard');
		$sql .= '  where uniacid=:uniacid and merchid=0 and card_id in (' . $card_id . ')  order by id desc';
		$cardlist = pdo_fetchall($sql, $param);
	...
	{
		global $_W;
		$sql = 'select *  from ' . tablename('vcshop_member') . ' where  openid=:openid  and uniacid=:uniacid  limit 1';
		...
		if (empty($member['level'])) {
			...
			$level = pdo_fetchcolumn('select levelname from ' . tablename('vcshop_member_level') . ' where id=:id limit 1', array(':id' => $member['level']));
		}
	...
	{
		global $_W;
		$sql = 'select vg.*,c.card_id  from ' . tablename('vcshop_verifygoods') . '   vg
	 ...
	 inner  join ' . tablename('vcshop_goods_cards') . ' c on c.id = g.cardid
	 ...
	 and  ((vg.limittype=0   and vg.limitdays * 86400 + vg.starttime >=unix_timestamp() )or ( vg.limittype=1   and vg.limitdate >=unix_timestamp() ))  and  vg.used =0 and (vg.activecard=0 or vg.activecard is null) and g.cardid>0 and vg.id=:id ';
		...
		$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('vcshop_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $id));
		$verifynum = 0;
		...
		pdo_update('vcshop_verifygoods', array('activecard' => 1, 'cardcode' => $code), array('id' => $id));
		return true;
	}
	...
	{
		global $_W;
		$sql = 'select vg.*,c.card_id,vg.cardcode  from ' . tablename('vcshop_verifygoods') . '   vg
	 inner join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
	 inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
	 inner  join ' . tablename('vcshop_goods_cards') . ' c on c.id = g.cardid
	...
	{
		global $_W;
		$sql = 'select g.id  from ' . tablename('vcshop_goods') . ' g
         inner join  ' . tablename('vcshop_goods_cards') . '  c on c.id = g.cardid  where g.uniacid=:uniacid and c.card_id=:card_id';
	...
	public function updateMemberInfoWithWxcard($openid)
	{
		...
		$result = pdo_update('vcshop_member', $arr, array('openid' => $openid, 'uniacid' => $_W['uniacid']));
		return $result;
	}
}
