...
class Coupon_VcShopComModel extends ComModel
{
	...
	{
		global $_W;
		$coupon = pdo_fetch('SELECT id,total FROM ' . tablename('vcshop_coupon') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
		...
		$gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid ', array(':couponid' => $couponid, ':uniacid' => $_W['uniacid']));
	...
	public function creditshop($logid = 0)
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('vcshop_creditshop_log') . ' WHERE `id`=:id and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logid));
		...
		if (!empty($log)) {
			...
			pdo_insert('vcshop_coupon_log', $couponlog);
			...
			$coupon = pdo_fetch('select * from ' . tablename('vcshop_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
			$coupon = $this->setCoupon($coupon, time());
			$this->sendMessage($coupon, 1, $member);
			pdo_update('vcshop_creditshop_log', array('status' => 3), array('id' => $logid));
		}
	}
	...
	public function getAvailableCoupons($type, $money = 0, $merch_array, $goods_array = array())
	{
		...
		if (!empty($merch_array)) {
			foreach ($merch_array as $key => $value) {
				...
				if (0 < $merchid) {
					...
					$sql = 'select d.id,d.couponid,d.gettime,c.timelimit,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.bgcolor,c.thumb,c.merchid,c.limitgoodcatetype,c.limitgoodtype,c.limitgoodcateids,c.limitgoodids  from ' . tablename('vcshop_coupon_data') . ' d';
					$sql .= ' left join ' . tablename('vcshop_coupon') . ' c on d.couponid = c.id';
		...
		if (!empty($goods_array)) {
			foreach ($goods_array as $key => $value) {
                ...
                $sql = "select id,cates,marketprice,merchid,`type`,discounts,istime,isdiscount,timestart,timeend,isdiscount_time,isdiscount_discounts   from " . tablename("vcshop_goods");
				...
				foreach ($goodlist as $good) {
					...
					if (0 < $row['enough'] && $p == 1) {
						if (0 < $good['optionid'] && $good['type'] != 4) {
							...
							$sql = 'select  marketprice  from ' . tablename('vcshop_goods_option');
							$sql .= ' where uniacid=:uniacid and id =:id order by id desc LIMIT 1 ';
	...
	public function payResult($logno)
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('vcshop_coupon_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
		...
		if (empty($log['status'])) {
			...
			$update['status'] = 1;
			pdo_update('vcshop_coupon_log', $update, array('id' => $log['id']));
	...
	public function useRechargeCoupon($log)
	{
		...
		$data = pdo_fetch('select id,openid,couponid,used from ' . tablename('vcshop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['couponid'], ':uniacid' => $_W['uniacid']));
		...
		$coupon = pdo_fetch('select enough,backcredit,backmoney,backredpack,couponname from ' . tablename('vcshop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
		...
		pdo_update('vcshop_coupon_data', array('used' => 1, 'usetime' => time(), 'ordersn' => $log['logno']), array('id' => $data['id']));
	...
	public function consumeCouponCount($openid, $money = 0, $merch_array, $goods_array)
	{
		...
		$param[':openid'] = $openid;
		$param[':uniacid'] = $_W['uniacid'];
		$sql = 'select d.id,d.couponid,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids,c.limitdiscounttype  from ' . tablename('vcshop_coupon_data') . ' d';
		$sql .= ' left join ' . tablename('vcshop_coupon') . ' c on d.couponid = c.id';
		...
		if (!empty($merch_array)) {
			foreach ($merch_array as $key => $value) {
				...
				if (0 < $merchid) {
					...
					$param[':merchid'] = $merchid;
					$sql = 'select d.id,d.couponid,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids  from ' . tablename('vcshop_coupon_data') . ' d';
					$sql .= ' left join ' . tablename('vcshop_coupon') . ' c on d.couponid = c.id';
	...
	public function useConsumeCoupon($orderid = 0)
	{
		...
		$order = pdo_fetch('select ordersn,createtime,couponid from ' . tablename('vcshop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
		...
		pdo_update('vcshop_coupon_data', array('used' => 1, 'usetime' => $order['createtime'], 'ordersn' => $order['ordersn']), array('id' => $order['couponid']));
	...
	public function returnConsumeCoupon($order)
	{
		...
		if (!is_array($order)) {
			$order = pdo_fetch('select id,openid,ordersn,createtime,couponid,status,finishtime from ' . tablename('vcshop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => intval($order), ':uniacid' => $_W['uniacid']));
		}
		...
		if (!empty($coupon['used']) && $coupon['ordersn'] == $order['ordersn']) {
			pdo_update('vcshop_coupon_data', array('used' => 0, 'usetime' => 0, 'ordersn' => ''), array('id' => $order['couponid']));
	...
	public function backConsumeCoupon($orderid)
	{
		...
		if (!is_array($orderid)) {
			$order = pdo_fetch('select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid,coupongoodprice  from ' . tablename('vcshop_order') . ' where id=:id and status>=0 and couponid >0 and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
		}
		...
		if ($isparent == 1 || $parentid != 0) {
			...
			if ($isparent == 1) {
				if (0 < $couponmerchid) {
					$sql = 'select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid from ' . tablename('vcshop_order') . ' where parentid=:parentid and couponmerchid=:couponmerchid and status>=0 and uniacid=:uniacid limit 1';
		...
		if ($parentid != 0 && $couponmerchid == 0) {
			if ($all_done == 1) {
				$sql = 'select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid from ' . tablename('vcshop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1';
		...
		if ($canback) {
			if (0 < $parentid) {
				$ordermoney = pdo_fetchcolumn('select coupongoodprice from ' . tablename('vcshop_order') . ' where id=:id and status>=0 and couponid >0 and uniacid=:uniacid limit 1', array(':id' => intval($parentid), ':uniacid' => $_W['uniacid']));
			}
			else {
			...
			if ($ordermoney == 0) {
				$sql = 'select ifnull( sum(og.realprice),0) from ' . tablename('vcshop_order_goods') . ' og ';
				$sql .= ' left join ' . tablename('vcshop_order') . ' o on';
			...
			pdo_update('vcshop_coupon_data', array('back' => 1, 'backtime' => time()), array('id' => $order['couponid']));
	...
	{
		global $_W;
		return pdo_fetch('select * from ' . tablename('vcshop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
	}

	public function getCouponByDataID($dataid = 0)
	{
		global $_W;
		$data = pdo_fetch('select id,openid,couponid,used,back,backtime,ordersn from ' . tablename('vcshop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $dataid, ':uniacid' => $_W['uniacid']));

		if (empty($data)) {
			return false;
		}

		$coupon = pdo_fetch('select * from ' . tablename('vcshop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
	...
	public function setCoupon($row, $time, $withOpenid = true)
	{
		...
		if ($withOpenid) {
			...
			if (0 < $row['getmax']) {
				$gets = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and gettype=1 limit 1', array(':couponid' => $row['id'], ':openid' => $openid, ':uniacid' => $_W['uniacid']));
	...
	public function addtaskdata($orderid)
	{
		...
		$order = pdo_fetch('select id,openid,price  from ' . tablename('vcshop_order') . ' where id=:id   and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
		...
		if ($pdata['isopensendtask'] == 1) {
			$price = $order['price'];
			$sendtasks = pdo_fetch('select id,couponid,sendnum,num,sendpoint  from ' . tablename('vcshop_coupon_sendtasks') . '
			...
			if (!empty($sendtasks)) {
				...
				pdo_insert('vcshop_coupon_taskdata', $data);
				$num = intval($sendtasks['num']) - intval($sendtasks['sendnum']);
				pdo_update('vcshop_coupon_sendtasks', array('num' => $num), array('id' => $sendtasks['id']));
			}
		}
		...
		if ($pdata['isopengoodssendtask'] == 1) {
            ...
            from ' . tablename('vcshop_coupon_goodsendtask') . ' gst
            inner join ' . tablename('vcshop_order_goods') . ' og on og.goodsid =gst.goodsid  and (orderid=:orderid or parentorderid=:orderid)
			...
			foreach ($goodssendtasks as $task) {
				...
				pdo_update('vcshop_coupon_goodsendtask', array('num' => $num), array('id' => $task['taskid']));
			}
		}
	}
	...
	public function sendcouponsbytask($orderid)
	{
		...
		if (!is_array($orderid)) {
			$order = pdo_fetch('select id,openid,ordersn,createtime,status,finishtime,`virtual`,isvirtualsend,isparent,parentid  from ' . tablename('vcshop_order') . ' where id=:id and status>=0  and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
		}
		...
		if ($order['status'] == 2) {
			$sql = 'SELECT goodsid FROM ' . tablename('vcshop_order_goods') . ' WHERE uniacid=:uniacid AND orderid=:orderid ';
			$goodsids = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));

			foreach ($goodsids as $gidk => $gidv) {
				$gtype = pdo_fetch('SELECT type FROM ' . tablename('vcshop_goods') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $gidv['goodsid']));
		...
		if ($isonlyverifygoods || $order['status'] == 1) {
			...
			if ($order['status'] == 3) {
				if (0 < $parentid) {
					$num = pdo_fetchcolumn('select 1 from ' . tablename('vcshop_order') . '
	...
	public function getOrderSendCoupons($orderid, $sendpoint, $tasktype, $openid)
	{
		...
		if ($sendpoint == 2) {
			$taskdata = pdo_fetchall('select id, couponid, sendnum  from ' . tablename('vcshop_coupon_taskdata') . '
	...
	public function posterbylist($list, $openid, $gettype)
	{
		...
		foreach ($list as $taskdata) {
			...
			$num += $couponnum;
			$merchid = pdo_fetchcolumn("select merchid from " . tablename("vcshop_coupon") . " where id=:couponid limit 1", array(":couponid" => $taskdata["couponid"]));
			$i = 1;
			...
			while ($i <= $couponnum) {
				...
				pdo_insert('vcshop_coupon_sendshow', $data);
				++$i;
			}

			pdo_update('vcshop_coupon_taskdata', array('status' => 1), array('id' => $taskdata['id']));
		}
	...
	public function posterbyusesendtask($couponid, $openid)
	{
		...
		$list = pdo_fetchall('select  *  from ' . tablename('vcshop_coupon_usesendtasks') . '
		...
		foreach ($list as $taskdata) {
			...
			pdo_update('vcshop_coupon_usesendtasks', array('num' => $num2), array('id' => $taskdata['id']));
		}
	...
	public function getCoupons()
	{
		global $_W;
		$time = time();
		return pdo_fetchall('select * from ' . tablename('vcshop_coupon') . ' where  (timelimit = 0  or  (timelimit =1 and timestart<={$time} && timeend>={$time})) and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	}
	...
	public function getInfo()
	{
		...
		$osql = 'SELECT `id` FROM ' . tablename('vcshop_order') . $condition;
		$order = pdo_fetchall($osql, $paramso);

		if (empty($order)) {
			$sql2 = 'SELECT * FROM ' . tablename('vcshop_sendticket') . ' WHERE uniacid = ' . intval($_W['uniacid']);
			...
			if ($ticket['status'] == 1) {
				if ($ticket['expiration'] == 1) {
					if ($ticket['endtime'] < TIMESTAMP) {
						...
						pdo_update('vcshop_sendticket', $status, array('id' => $ticket['id']));
						return false;
					}
	...
	public function getCoupon_new($cpid)
	{
		...
		if (is_array($cpids)) {
			...
			foreach ($cpids as $cpk => $cpv) {
				$cpsql = 'SELECT * FROM ' . tablename('vcshop_coupon') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($cpv);
		...
		$cpsql = 'SELECT * FROM ' . tablename('vcshop_coupon') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($cpid);
	...
	public function insertDraw($openid, $cpinfo)
	{
		...
		$drawsql = 'SELECT * FROM ' . tablename('vcshop_sendticket_draw') . ' WHERE  openid = :openid AND uniacid = :uniacid ';
		...
		if (empty($drawdata)) {
			...
			$insert = pdo_insert('vcshop_sendticket_draw', $data);
			return $insert;
		}
	...
	{
		global $_W;
		$sql = 'SELECT * FROM ' . tablename('vcshop_sendticket_share') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND status = 1 AND (enough = ' . $money . ' OR enough <= ' . $money . ') AND (expiration = 0 OR (expiration = 1 AND endtime >= ' . TIMESTAMP . ')) ORDER BY enough DESC,createtime DESC LIMIT 1';
	...
	public function getCanGetCouponNum($merchid = 0)
	{
		...
		$param = array();
		$param[':uniacid'] = $_W['uniacid'];
		$sql = 'select id,timelimit,coupontype,timedays,timestart,timeend,couponname,enough,backtype,deduct,discount,backmoney,backcredit,backredpack,bgcolor,thumb,credit,money,getmax,merchid,total as t,tagtitle,settitlecolor,titlecolor  from ' . tablename('vcshop_coupon');
	...
	{
		global $_W;
		pdo_update('vcshop_member', array('hasnewcoupon' => $hasnewcoupon), array('openid' => $openid, 'uniacid' => $_W['uniacid']));
	}
