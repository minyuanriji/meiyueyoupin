...
class Perm_VcShopComModel extends ComModel {
    ...
    protected function perm_finance() {
        return array(
            ...
            "recharge" => array(
                ...
                "credit1" => "充值积分-log",
                "credit2" => "充值余额-log",
                "quota" => "充值名额-log"       // zhou 20191125 增加名额项
            ),
            "credit" => array(
                "text" => "积分余额明细",
                "credit1" => "积分明细",
    ...
    private function check($permtype = "") {
        ...
        $user = pdo_fetch("select u.status as userstatus,r.status as rolestatus,u.perms2 as userperms,r.perms2 as roleperms,u.roleid from " . tablename("vcshop_perm_user") . " u " . " left join " . tablename("vcshop_perm_role") . " r on u.roleid = r.id " . " where u.uid=:uid and u.uniacid = :uniacid limit 1 ", array(
            ":uid" => $uid,
            ":uniacid" => intval($_W["uniacid"])
        ));
    ...
    public function log($type = "", $op = "") {
        ...
        pdo_insert("vcshop_perm_log", $log);
    }
