...
class Member_VcShopModel
{
	...
	public function getInfo($openid = '')
	{
		...
		if ($uid == 0) {
			$info = pdo_fetch('select * from ' . tablename('vcshop_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			...
			if (empty($info)) {
				...
				if (!empty($condition)) {
					$info = pdo_fetch('select * from ' . tablename('vcshop_member') . ' where ' . $condition . '  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			...
			$info = pdo_fetch('select * from ' . tablename('vcshop_member') . ' where id=:id  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
		}
	...
	public function getMember($openid = '')
	{
		...
		if ($uid == 0) {
			...
			$info = pdo_fetch('select * from ' . tablename('vcshop_member') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $openid));
		}
		...
		if (!empty($info)) {
			...
			if (strpos($info['avatar'], '132132')) {
				...
				pdo_update('vcshop_member', $upgrade2, array('id' => $info['id']));
			}
		...
		if (empty($info['uid'])) {
			...
			if ($followed) {
				...
				if (!empty($uid)) {
					...
					if (!empty($upgrade)) {
						pdo_update('vcshop_member', $upgrade, array('id' => $info['id']));
					}
				}
			}
		}
	...
	{
		global $_W;
		$info = pdo_fetch('select * from ' . tablename('vcshop_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mobile' => $mobile));
	...
	public function setCredit($openid = '', $credittype = 'credit1', $credits = 0, $log = array())
	{
		...
		$log_data = array('uid' => intval($uid), 'credittype' => $credittype, 'uniacid' => $_W['uniacid'], 'num' => $credits, 'createtime' => TIMESTAMP, 'module' => 'vcshop', 'operator' => intval($log[0]), 'remark' => $log[1]);
		...
		if (!empty($uid)) {
			...
			$value = pdo_fetchcolumn('SELECT ' . $credittype . ' FROM ' . tablename('vcshop_member') . ' WHERE  uniacid=:uniacid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			$newcredit = $credits + $value;
			...
			pdo_update('vcshop_member', array($credittype => $newcredit), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
		...
		$member_log_table_flag = pdo_tableexists('vcshop_member_credit_record');

		if ($member_log_table_flag) {
			$log_data['openid'] = $openid;
			$log_data["presentcredit"] = $a;
			pdo_insert('vcshop_member_credit_record', $log_data);
		}
	...
	public function getCredit($openid = '', $credittype = 'credit1')
	{
		...
		$item = pdo_fetch('SELECT ' . $credittype . ' FROM ' . tablename('vcshop_member') . ' WHERE openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));

		if (empty($item)) {
			$item = pdo_fetch('SELECT ' . $credittype . ' FROM ' . tablename('vcshop_member') . ' WHERE openid_wa=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		}
	...
	public function getCredits($openid = '', $credittypes = array('credit1', 'credit2'))
	{
		...
		$item = pdo_fetch('SELECT ' . $types . ' FROM ' . tablename('vcshop_member') . ' WHERE openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));

		if (empty($item)) {
			$item = pdo_fetch('SELECT ' . $types . ' FROM ' . tablename('vcshop_member') . ' WHERE openid_wa=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		}
	...
	public function checkMember()
	{
		...
		if (empty($member) && !empty($openid)) {
			...
			if (!empty($upgrade)) {
				pdo_update('vcshop_member', $upgrade, array('id' => $member['id']));
			}
		}
$t0 = microtime(true);
		if (p('commission')) {
...
$t2 = microtime(true);
pdo_insert('a',array('info'=>json_encode(array(
	'a'=>'ch',
	'1'=>round($t1-$t0,2),
	'2'=>round($t2-$t1,2),
))));
	...
	public function getLevels($all = true)
	{
		...
		return pdo_fetchall('select * from ' . tablename('vcshop_member_level') . ' where uniacid=:uniacid' . $condition . ' order by level asc', array(':uniacid' => $_W['uniacid']));
	}
	...
	public function getLevel($openid)
	{
		...
		if (!empty($member) && !empty($member['level'])) {
			$level = pdo_fetch('select * from ' . tablename('vcshop_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $member['level'], ':uniacid' => $_W['uniacid']));
	...
	public function getOneGoodsLevel($openid, $goodsid)
	{
		...
		$data = array();
		$levels = pdo_fetchall('select * from ' . tablename('vcshop_member_level') . ' where uniacid=:uniacid and buygoods=1 and level and level > :level order by level asc', array(':uniacid' => $uniacid, ':level' => $level));
	...
	public function getGoodsLevel($openid, $orderid)
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$order_goods = pdo_fetchall('select goodsid from ' . tablename('vcshop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid', array(':uniacid' => $uniacid, ':orderid' => $orderid));
		$levels = array();
		$data = array();
	...
	public function upgradeLevel($openid, $orderid = 0, $status = 3)
	{
		...
		if (empty($leveltype)) {
			$ordermoney = pdo_fetchcolumn('select sum(price) from ' . tablename('vcshop_order') . ' where uniacid=:uniacid and openid=:openid and status=:status limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid'], ':status' => $status));
			$ordermoney = floatval($ordermoney);
			$level = pdo_fetch('select * from ' . tablename('vcshop_member_level') . (' where uniacid=:uniacid  and enabled=1 and ' . $ordermoney . ' >= ordermoney and ordermoney>0  order by level desc limit 1'), array(':uniacid' => $_W['uniacid']));
		}
		else {
			if ($leveltype == 1) {
				$ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . ' where openid=:openid and status=:status and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid'], ':status' => $status));
				$level = pdo_fetch('select * from ' . tablename('vcshop_member_level') . (' where uniacid=:uniacid and enabled=1 and ' . $ordercount . ' >= ordercount and ordercount>0  order by level desc limit 1'), array(':uniacid' => $_W['uniacid']));
			}
		}
		...
		if ($canupgrade) {
			pdo_update('vcshop_member', array('level' => $level['id']), array('id' => $member['id']));
			com_run('wxcard::updateMemberCardByOpenid', $openid);
	...
	public function upgradeLevelByLevelId($openid, $LevelID)
	{
		...
		$level = pdo_fetch('select *  from ' . tablename('vcshop_member_level') . ' where uniacid=:uniacid and enabled=1 and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $LevelID));
	...
	{
		global $_W;
		return pdo_fetchall('select * from ' . tablename('vcshop_member_group') . ' where uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
	}
	...
	public function setGroups($openid, $group_ids, $reason = '')
	{
		...
		$old_group_ids = pdo_getcolumn('vcshop_member', $condition, 'groupid');
		...
		pdo_update('vcshop_member', array('groupid' => $group_ids), $condition);

		foreach ($diff_ids as $groupid) {
			pdo_insert('vcshop_member_group_log', array('add_time' => date('Y-m-d H:i:s'), 'group_id' => $groupid, 'content' => $reason, 'mid' => intval($openid), 'openid' => $is_id ? '' : $openid));
		}
	...
	public function checkMemberFromPlatform($openid = '', $acc = '')
	{
		...
		if (!is_error($redis)) {
			...
			$member = $redis->get($_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_' . $_W['uniacid'] . '_' .$openid . '_checkMemberFromPlatform');
		...
		if (!is_error($redis)) {
			$redis->set($_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_' . $_W['uniacid'] . '_' . $openid . '_checkMemberFromPlatform', json_encode($member), 20);
		}
	...
	public function checkMemberSNS($sns)
	{
		...
		if (empty($member['bindsns']) || $member['bindsns'] == $sns) {
			pdo_update('vcshop_member', $data, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			return $member['id'];
		}
	}
	...
	public function memberRadisCountDelete()
	{
		...
		if ($open_redis) {
			$redis = redis();
			// $keysArr = $redis->keys('vc_' . $_W['uniacid'] . '_member*');
			$keysArr = $redis->keys($_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_'.'vc_' . $_W['uniacid'] . '_member*');
