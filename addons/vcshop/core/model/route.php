...
class Route_VcShopModel
{
	public function run($isweb = true)
	{
		...
		if (version_compare(VCSHOP_VERSION, '3.14.0', '>=')) {
			@include_once VCSHOP_PATH . 'vendor/autoload.php';
		}

		require_once IA_ROOT . '/addons/vcshop/core/inc/page.php';

		if ($isweb) {
			require_once VCSHOP_CORE . 'inc/page_web.php';
			require_once VCSHOP_CORE . 'inc/page_web_com.php';
		}
		else {
			require_once VCSHOP_CORE . 'inc/page_mobile.php';
			require_once VCSHOP_CORE . 'inc/page_mobile_login.php';
		}
		...
		$segs = count($routes);
		$method = 'main';
		$root = $isweb ? VCSHOP_CORE_WEB : VCSHOP_CORE_MOBILE;
		$isMerch = false;
		$isNewstore = false;
		...
		if (strexists($_W['siteurl'], 'web/merchant.php')) {
			...
			$isplugin = !empty($r) && is_dir(VCSHOP_PLUGIN . $routes[0]);
		}

		if ($isplugin) {
			if ($isweb) {
				require_once VCSHOP_CORE . 'inc/page_web_plugin.php';
			}
			else {
				require_once VCSHOP_CORE . 'inc/page_mobile_plugin.php';
				require_once VCSHOP_CORE . 'inc/page_mobile_plugin_login.php';
				require_once VCSHOP_CORE . 'inc/page_mobile_plugin_pf.php';
			}

			$_W['plugin'] = $routes[0];
			$root = VCSHOP_PLUGIN . $routes[0] . '/core/' . ($isweb ? 'web' : 'mobile') . '/';

			if ($isMerch) {
				$_W['plugin'] = 'merch';
				$root = VCSHOP_PLUGIN . 'merch/core/web/manage/';
			}
			else if ($isNewstore) {
				$_W['plugin'] = 'newstore';
				$root = VCSHOP_PLUGIN . 'newstore/core/web/manage/';
			}
			else {
			...
			if ($routes[0] == 'system') {
				require_once VCSHOP_CORE . 'inc/page_system.php';
			}
		}
		...
		if (isset($_GPC['r']) && strpos($_GPC['r'], 'pc') !== false) {
			...
			$class = ucfirst($class) . '_VcShopPage';
		}
