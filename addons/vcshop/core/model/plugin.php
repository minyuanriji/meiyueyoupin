...
class Plugin_VcShopModel
{
	...
	{
		$dbplugin = pdo_fetchall('select * from ' . tablename('vcshop_plugin') . ' where identity=:identyty limit  1', array(':identity' => $pluginName));
	...
	public function getAll($iscom = false, $status = '')
	{
		...
		if ($iscom) {
			...
			if (empty($plugins)) {
				$plugins = pdo_fetchall('select * from ' . tablename('vcshop_plugin') . ' where iscom=1 and deprecated=0 ' . $status . ' order by displayorder asc');
			...
			if (empty($plugins)) {
				$plugins = pdo_fetchall('select * from ' . tablename('vcshop_plugin') . ' where iscom=0 and deprecated=0 ' . $status . ' order by displayorder asc');
				$plugins = $this->arrayUnique($plugins);
	...
	public function refreshCache($status = '', $iscom = false)
	{
		...
		$com = pdo_fetchall('select * from ' . tablename('vcshop_plugin') . ' where iscom=1 and deprecated=0 ' . $status . ' order by displayorder asc');
	...
	public function getList($status = '')
	{
		...
		if (file_exists($filename)) {
			$item = pdo_fetch('select  plugins from ' . tablename('vcshop_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $_W['uniacid']));
			...
			foreach ($plugins as $key => $value) {
				if (!strstr($item['plugins'], $value['identity']) && !strstr($setting['plugin'], $value['identity']) && !strstr($setting['com'], $value['identity'])) {
					$plugin = pdo_fetch('SELECT max(permendtime) as permendtime FROM ' . tablename('vcshop_system_grant_log') . ' 
			...
			if (p('grant')) {
				...
				$item = pdo_fetch('select plugins from ' . tablename('vcshop_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $acid['acid']));
				$setting = pdo_fetch('select * from ' . tablename('vcshop_system_plugingrant_setting') . ' where 1 = 1 limit 1 ');

				foreach ($plugins as $key => $value) {
					if (!strstr($item['plugins'], $value['identity']) && !strstr($setting['plugin'], $value['identity']) && !strstr($setting['com'], $value['identity'])) {
						$plugin = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('vcshop_system_plugingrant_log') . '
                    WHERE `identity` = \'' . $value['identity'] . '\' and uniacid = ' . $acid['uniacid'] . ' and isperm = 1 and `month` = 0 ');

						if (0 < $plugin) {
							$plugin = pdo_fetch('SELECT max(permendtime) as permendtime,`month`,isperm FROM ' . tablename('vcshop_system_plugingrant_log') . '
                        WHERE `identity` = \'' . $value['identity'] . '\' and uniacid = ' . $acid['uniacid'] . ' and isperm = 1 and `month` = 0 ');
						}
						else {
							$plugin = pdo_fetch('SELECT max(permendtime) as permendtime,`month`,isperm FROM ' . tablename('vcshop_system_plugingrant_log') . ' 
	...
	public function loadModel($pluginname = '')
	{
		...
		if (!$_model) {
			$modelfile = IA_ROOT . '/addons/vcshop/plugin/' . $pluginname . '/core/model.php';
	...
	public function getConfig($pluginname = '')
	{
		...
		$config_file = $moduleroot = IA_ROOT . '/addons/vcshop/plugin/' . $pluginname . '/config.php';
	...
	public function permission($pluginName, $uniacid = 0)
	{
		...
		$allPlugins = pdo_fetchall('select * from ' . tablename('vcshop_plugin') . ' where iscom=0 and deprecated=0 and status=1 order by displayorder asc');
		$allPlugins = array_column($allPlugins, 'identity');

		if ($permset) {
			$plugins = pdo_fetchcolumn('SELECT plugins FROM ' . tablename('vcshop_perm_plugin') . ' WHERE acid =:id limit 1', array(':id' => $id));
