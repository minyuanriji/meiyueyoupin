...
class Notice_VcShopModel
{
    ...
    public function sendMerchSuccess($id)
    {
        ...
        $merch = pdo_get("vcshop_merch_reg", array("id" => $id, "uniacid" => $_W["uniacid"]));
        $template = pdo_get("vcshop_member_message_template_default", array("typecode" => "march_type_success"));
    ...
    public function sendMerchFail($id)
    {
        ...
        $template = pdo_get("vcshop_member_message_template_default", array("typecode" => "march_type_fail"));
	...
	public function sendTeamMessage($orderid = '0', $delRefund = false)
	{
		...
		$order = pdo_fetch('select * from ' . tablename('vcshop_groups_order') . ' where id=:id limit 1', array(':id' => $orderid));
		...
		$orderpricestr = ' ¥' . $price . '元 (包含'.$shop['freight'].': ¥' . $order['freight'] . '元，积分抵扣: ¥' . $order['creditmoney'] . '元)';
		...
		$datas = array(
			...
			array('name' => $shop['freight'], 'value' => $order['freight']),
...
您的订单我们已经收到，我们将尽快配送~~';
	...
	{
		global $_W;
		$order = pdo_fetch('select * from ' . tablename('vcshop_order') . ' where id=:id limit 1', array(':id' => $orderid));
		$shop = m('common')->getSysset('shop');
		if (empty($order)) {
			return NULL;
		}
		...
		$order_goods = pdo_fetchall('select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . (' where ' . $scondition . ' and og.uniacid=:uniacid '), $param);
		$goods = '';
		$goodsname = '';
		$goodsnum = 0;
...
';
		...
		$orderpricestr = ' 订单总价: ' . $order['price'] . '(包含'.$shop['freight'].':' . $order['dispatchprice'] . ')';
		$member = m('member')->getMember($openid);
		$carrier = false;
		$store = false;
		...
		if (!empty($order['storeid'])) {
			if (0 < $order['merchid']) {
				$store = pdo_fetch('select * from ' . tablename('vcshop_merch_store') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
			}
			else {
				$store = pdo_fetch('select * from ' . tablename('vcshop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid']));
			}
		}
		...
		if (!empty($order['address'])) {
			...
			if (is_array($address) && empty($address) || !is_array($address)) {
				...
				if (is_array($address) && empty($address) || !is_array($address)) {
					$address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('vcshop_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $order['addressid'], ':uniacid' => $_W['uniacid']));
				}
			}
...
联系电话: ' . $carrier['carrier_mobile'];
		...
		$datas = array(
			...
			array('name' => $shop['freight'], 'value' => $order['dispatchprice']),
			array('name' => '商品详情', 'value' => $goods),
			array('name' => '快递公司', 'value' => $order['expresscom']),
			array('name' => '快递单号', 'value' => $order['expresssn']),
		...
		if ($order['status'] == 0) {
			...
			pdo_update('vcshop_order', array('willcancelmessage' => 1), array('id' => $orderid));
...
感谢您关注，如有疑问请联系在线客服或点击查看详情！', 'color' => '#000000')
	...
	public function sendOrderMessage($orderid = '0', $delRefund = false, $raid = NULL)
	{
		...
		// han 20200306 add 买家通知
		$order_goods = pdo_fetchall('select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.sendtype,og.expresscom,og.expresssn,og.sendtime,g.noticebuyer_type,g.noticebuyer_text,g.noticebuyer_title from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . (' where ' . $scondition . ' and og.uniacid=:uniacid '), $param);
		$goods = '';
		$goodsname = '';
		$goodsnum = 0;
...
联系电话: ' . $carrier['carrier_mobile'];
		...
		if (!empty($order['virtual']) && empty($order['virtual_str'])) {
			...
			if ($open_redis) {
				...
				$virtual_str = $redis->get($_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_'.'vc_' .$order['id'] . '_virtual_str');
		...
		if ($delRefund) {
			...
			if (!empty($order['refundid'])) {
				$refund = pdo_fetch('select * from ' . tablename('vcshop_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));
...
点击快速查询物流信息', 'color' => '#000000')
					...
					if ($refund['rtype'] == 2 || $refund['rtype'] == 1) {
						if (0 < $order['merchid']) {
							...
							if (empty($raid)) {
								$salerefund = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where uniacid=:uniacid and isdefault=1 limit 1', array(':uniacid' => $_W['uniacid']));
							}
							else {
								$salerefund = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where uniacid=:uniacid and id=' . $raid . ' limit 1', array(':uniacid' => $_W['uniacid']));
							}
...
', 'color' => '#ff0000'),
					...
					// han 20200306 发送买家通知s
					if(!empty($og['noticebuyer_text']) && !empty($og['noticebuyer_type'])){
						$noticebuyer_type = explode(',', $og['noticebuyer_type']);
						if ($og['noticebuyer_type'] == '1' || is_array($noticebuyer_type) && in_array('1', $noticebuyer_type)) {
								$og['noticebuyer_text'] = htmlspecialchars_decode($og['noticebuyer_text'], ENT_QUOTES);
								$og['noticebuyer_text'] = str_replace('"', '\\"', $og['noticebuyer_text']);
								$msg = array(
									'first'    => array('value' => (empty($og['noticebuyer_title'])) ? '新的通知！' : $og['noticebuyer_title'], 'color' => '#ff0000'),
									'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
									'keyword2' => array('title' => '处理进度', 'value' => '商品付款通知', 'color' => '#000000'),
									'keyword3' => array('title' => '处理内容', 'value' => '已付款', 'color' => '#000000'),
									'remark'   => array('value' => '祝您购物愉快！', 'color' => '#000000')
								);
								$this->sendNotice(array('openid' => $order['openid'], 'tag' => 'buyernotice', 'cusdefault' => $og['noticebuyer_text'], 'default' => $msg, 'datas' => $datas));

						}
					}
					// han 20200306 发送买家通知e
				}
			}
			else {
				if ($order['status'] == 1 && !empty($order['istrade'])) {
					$item = pdo_fetch('select og.trade_time,g.title,s.storename,s.mobile,p.nickname,s.id as storeid  from ' . tablename('vcshop_order_goods') . '  og
				 left join ' . tablename('vcshop_order') . ' o  on  og.orderid = o.id
				 left join ' . tablename('vcshop_goods') . ' g  on  og.goodsid = g.id
				 left join ' . tablename('vcshop_store') . ' s  on  o.storeid = s.id
				 left join ' . tablename('vcshop_newstore_people') . ' p  on  og.peopleid = p.id
...
如有变动可登录管理后台进行操作！！', 'color' => '#000000')
						...
						$salers = pdo_fetchall('select *   from ' . tablename('vcshop_saler') . ' sr  inner join ' . tablename('vcshop_store') . ' s on s.id = sr.storeid  where  s.id=:id and sr.getnotice =1', array(':id' => $item['storeid']));
...
' . $buyerinfo;
										...
										if(!empty($og['noticebuyer_text']) && !empty($og['noticebuyer_type'])){
											$noticebuyer_type = explode(',', $og['noticebuyer_type']);
											if ($og['noticebuyer_type'] == '2' || is_array($noticebuyer_type) && in_array('2', $noticebuyer_type)) {
													$og['noticebuyer_text'] = htmlspecialchars_decode($og['noticebuyer_text'], ENT_QUOTES);
													$og['noticebuyer_text'] = str_replace('"', '\\"', $og['noticebuyer_text']);
													$msg = array(
														'first'    => array('value' => (empty($og['noticebuyer_title'])) ? '新的通知！' : $og['noticebuyer_title'], 'color' => '#ff0000'),
...
' . $_W['shopset']['shop']['name'] . '感谢您的支持，如有疑问请联系在线客服。', 'color' => '#000000')
     ...
     * @return type
     */
	public function sendMemberQuotaChange($openid, $pointchange = 0, $changetype = 0,  $level = 0)
	{
		global $_W;
		global $_GPC;
		$url = $this->getUrl('member');
		...
		$new_num = pdo_fetchcolumn('select num from ' . tablename('vcshop_member_quota') . ' where uniacid=:uniacid and mid=:mid and agentlevel=:agentlevel',array(':uniacid'=>$_W['uniacid'],':mid'=>$member['id'],':agentlevel'=>$level));
		$usernotice = unserialize($member['noticeset']);
		...
		$credittext = "【" . $levelname . "】" . " 名额 "; // empty($_W['shopset']['trade']['credittext']) ? '积分' : $_W['shopset']['trade']['credittext'];
		$pointtext = ''; 
		$pointcolor = '';
		...
		$datas = array(
			array('name' => '商城名称', 'value' => $_W['shopset']['shop']['name']),
			array('name' => '粉丝昵称', 'value' => $member['nickname']),
			array('name' => '名额变动', 'value' => $pointtext),
			array('name' => '赠送时间', 'value' => date('Y-m-d H:i', time())),
			array('name' => '名额余额', 'value' => (double) $new_num . $credittext)
		);
		$remark = '
[商城名称]感谢您的支持，如有疑问请联系在线客服。';
		$text = '亲爱的[粉丝昵称]， 您的' . $credittext . '发生变动，具体内容如下：
...
变动时间：[赠送时间]
充值方式：' . $fromstr . '
当前【' . $levelname . '】名额余额：[名额余额] 
' . $remark;
		$message = array(
			'first'    => array('value' => '亲爱的' . $member['nickname'] . '，您的' . $credittext . '发生变动，具体如下:', 'color' => '#ff0000'),
			'keyword1' => array('title' => '获得时间', 'value' => date('Y-m-d H:i', time()), 'color' => '#000000'),
			'keyword2' => array('title' => '获得名额', 'value' => $pointtext, 'color' => $pointcolor),
			'keyword3' => array('title' => '获得原因', 'value' => $fromstr, 'color' => '#000000'),
			'keyword4' => array('title' => '当前【' . $levelname . '】名额余额', 'value' => (double) $new_num . $credittext, 'color' => '#ff0000'),
			'remark'   => array('value' => '
' . $_W['shopset']['shop']['name'] . '感谢您的支持，如有疑问请联系在线客服。', 'color' => '#000000')
		);
		$this->sendNotice(array('openid' => $openid, 'tag' => 'backpoint_ok', 'default' => $message, 'cusdefault' => $text, 'url' => $url, 'datas' => $datas));
	...
	public function sendMemberLogMessage($log_id = '', $channel = 0, $isback = false)
	{
		...
		$log_info = pdo_fetch('select * from ' . tablename('vcshop_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log_id, ':uniacid' => $_W['uniacid']));
...
如果您选择银行卡提现，预计在1-2个工作日内到账，如有疑问请联系在线客服或点击查看详情。', 'color' => '#000000')
	...
	public function sendStockWarnMessage($goodsid, $optionid)
	{
		...
		$goods = pdo_fetch('select  * from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
		...
		if (!empty($optionid)) {
			$option = pdo_fetch('select  *  from ' . tablename('vcshop_goods_option') . ' where id=:id and uniacid=:uniacid and goodsid = :goodsid', array(':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid, ':id' => $optionid));
...
', 'color' => '#ff0000'),
	...
	public function sendNotice(array $params)
	{
		...
		if (!empty($templateid)) {
			$template = pdo_fetch('select * from ' . tablename('vcshop_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $templateid, ':uniacid' => $_W['uniacid']));
			...
			if (!empty($template)) {
				...
				if (is_error($ret)) {
					$templatetype = pdo_fetch('select templateid  from ' . tablename('vcshop_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $_W['uniacid']));
...
<a href=\'' . $url . '\'>点击查看详情</a>');
	...
	public function settemplateidbyback($tag, $uniacid = 0)
	{
		...
		$templatetype = pdo_fetch('select `name`,templatecode,content  from ' . tablename('vcshop_member_message_template_type') . ' where typecode=:typecode  limit 1', array(':typecode' => $tag));
...
', '', $value['content']) == $content) {
				$issnoet = false;
				$defaulttemp = pdo_fetch('select 1  from ' . tablename('vcshop_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $uniacid));

				if (empty($defaulttemp)) {
					pdo_insert('vcshop_member_message_template_default', array('typecode' => $tag, 'uniacid' => $uniacid, 'templateid' => $value['template_id']));
				}
				else {
					pdo_update('vcshop_member_message_template_default', array('templateid' => $value['template_id']), array('typecode' => $tag, 'uniacid' => $uniacid));
				}
	...
	{
		global $_W;
		$item = pdo_fetch('select vgl.*,vg.limitnum,s.storename,sa.salername,sa.openid as saleropenid,o.openid,g.title,o.ordersn  from ' . tablename('vcshop_verifygoods_log') . '   vgl
		left  join ' . tablename('vcshop_store') . ' s on s.id = vgl.storeid
		left  join ' . tablename('vcshop_saler') . ' sa on sa.id = vgl.salerid
		inner  join ' . tablename('vcshop_verifygoods') . ' vg on vg.id = vgl.verifygoodsid
		inner  join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
		inner  join ' . tablename('vcshop_order') . ' o on o.id = og.orderid
		left  join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
	  where  vgl.id =:id  and vgl.uniacid=:uniacid  limit 1', array(':id' => $verifygoodlogid, ':uniacid' => $_W['uniacid']));
		$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('vcshop_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $item['verifygoodsid']));
		$verifynum = 0;
