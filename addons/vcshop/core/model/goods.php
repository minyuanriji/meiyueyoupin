...
class Goods_VcShopModel
{
    ...
    {
        global $_W;
        return pdo_fetch("select * from " . tablename("vcshop_goods_option") . " where id=:id and goodsid=:goodsid and uniacid=:uniacid Limit 1", array(":id" => $optionid, ":uniacid" => $_W["uniacid"], ":goodsid" => $goodsid));
    }
    /**
     * 获取商品规格的价格
     * @param type $goodsid
    ...
    {
        global $_W;
        return pdo_fetchcolumn("select marketprice from " . tablename("vcshop_goods_option") . " where id=:id and goodsid=:goodsid and uniacid=:uniacid", array(":id" => $optionid, ":uniacid" => $_W["uniacid"], ":goodsid" => $goodsid));
    }
    /**
     * 获取宝贝
     * @param type $page
    ...
    public function getList($args = array())
    {
        ...
        if (!$random) {
            // 20200427 lin ,deduct
            $sql = "SELECT id,title,subtitle,thumb,thumb_url" . $officsql . ",marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice,buylevels,deduct,buygroups\r\n            FROM " . tablename("vcshop_goods") . " where  " . $condition . " ORDER BY " . $order . " " . $orderby . " LIMIT " . ($page - 1) * $pagesize . "," . $pagesize;
            $total = pdo_fetchcolumn("select count(*) from " . tablename("vcshop_goods") . " where  " . $condition . " ", $params);
        } else {
            $sql = "SELECT id,title,thumb,thumb_url" . $officsql . ",marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice,buylevels,deduct,buygroups\r\n            FROM " . tablename("vcshop_goods") . " where  " . $condition . " ORDER BY rand() LIMIT " . $pagesize;
            $total = $pagesize;
        }
        ...
        foreach ($list as $lk => $lv) {
            ...
            if ($lv["hasoption"] == 1) {
                $pricemax = array();
                $options = pdo_fetchall("select * from " . tablename("vcshop_goods_option") . " where goodsid=:goodsid and                               uniacid=:uniacid order by displayorder asc", array(":goodsid" => $lv["id"], ":uniacid" => $_W["uniacid"]));
            ...
            if ($lv["type"] == 3) {
                $vsql = "SELECT * FROM " . tablename("vcshop_virtual_type") . " WHERE uniacid = " . intval($_W["uniacid"]) . " AND id = " . intval($lv["virtual"]);
            ...
            if (empty($lv["isdiscount"]) || $lv["isdiscount_time"] < time()) {
                if (isset($options) && 0 < count($options) && $lv["hasoption"]) {
                    ...
                    $sql = "update " . tablename("vcshop_goods") . " g set\r\n        g.minprice = (select min(marketprice) from " . tablename("vcshop_goods_option") . " where goodsid = " . $lv["id"] . "),\r\n        g.maxprice = (select max(marketprice) from " . tablename("vcshop_goods_option") . " where goodsid = " . $lv["id"] . ")\r\n        where g.id = " . $lv["id"] . " and g.hasoption=1";
                    pdo_query($sql);
                } else {
                    $sql = "update " . tablename("vcshop_goods") . " set minprice = marketprice,maxprice = marketprice where id = " . $lv["id"] . " and hasoption=0;";
                    pdo_query($sql);
                }
                $goods_price = pdo_fetch("select minprice,maxprice from " . tablename("vcshop_goods") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $lv["id"], ":uniacid" => $_W["uniacid"]));
    ...
    public function getCommission($goods, $level, $set)
    {
        ...
        if (!empty($goods) && $goods["hasoption"]) {
            $option = pdo_fetchall("select * from " . tablename("vcshop_goods_option") . " where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc", array(":goodsid" => $goods["id"], ":uniacid" => $_W["uniacid"]));
        }
        if ($goods["hascommission"] == 1) {
            ...
            if ($level != "false" && !empty($level)) {
                ...
                // debug 必须加(float),否者如果$set["commission1"]为空会报警告
    ...
    public function getListbyCoupon($args = array())
    {
        ...
        $table = tablename("vcshop_goods") . " g";
        if (0 < $couponid) {
            $data = pdo_fetch("select c.*  from " . tablename("vcshop_coupon_data") . "  cd inner join  " . tablename("vcshop_coupon") . " c on cd.couponid = c.id  where cd.id=:id and cd.uniacid=:uniacid and coupontype =0  limit 1", array(":id" => $couponid, ":uniacid" => $_W["uniacid"]));
            if (!empty($data)) {
                $i = 0;
                $condition2 = "";
    ...
    {
        global $_W;
        return array("sale" => pdo_fetchcolumn("select count(1) from " . tablename("vcshop_goods") . " where status > 0 and checked=0 and deleted=0 and total>0 and  type !=30 and uniacid=:uniacid", array(":uniacid" => $_W["uniacid"])), "out" => pdo_fetchcolumn("select count(1) from " . tablename("vcshop_goods") . " where status > 0 and deleted=0 and total=0 and type !=30 and uniacid=:uniacid", array(":uniacid" => $_W["uniacid"])), "stock" => pdo_fetchcolumn("select count(1) from " . tablename("vcshop_goods") . " where (status=0 or checked=1) and deleted=0 and uniacid=:uniacid and type !=30", array(":uniacid" => $_W["uniacid"])), "cycle" => pdo_fetchcolumn("select count(1) from " . tablename("vcshop_goods") . " where deleted=1 and uniacid=:uniacid and type !=30", array(":uniacid" => $_W["uniacid"])), "verify" => pdo_fetchcolumn("select count(1) from " . tablename("vcshop_goods") . " where deleted=0  and uniacid=:uniacid and type !=30 and merchid>0 and checked=1", array(":uniacid" => $_W["uniacid"])));
    }
    /**
     * 获取宝贝评价
     * @param type $page
    ...
    public function getComments($goodsid = "0", $args = array())
    {
        ...
        $sql = "SELECT id,nickname,headimgurl,content,images FROM " . tablename("vcshop_goods_comment") . " where 1 " . $condition . " ORDER BY createtime desc LIMIT " . ($page - 1) * $pagesize . "," . $pagesize;
        $list = pdo_fetchall($sql, $params);
        foreach ($list as &$row) {
    ...
    {
        global $_W;
        $count = pdo_fetchcolumn("select count(*) from " . tablename("vcshop_member_favorite") . " where goodsid=:goodsid and deleted=0 and openid=:openid and uniacid=:uniacid limit 1", array(":goodsid" => $id, ":openid" => $_W["openid"], ":uniacid" => $_W["uniacid"]));
        return 0 < $count;
    }
    public function addHistory($goodsid = 0)
    {
        global $_W;
        pdo_query("update " . tablename("vcshop_goods") . " set viewcount=viewcount+1 where id=:id and uniacid='" . $_W[uniacid] . "' ", array(":id" => $goodsid));
        $history = pdo_fetch("select id,times from " . tablename("vcshop_member_history") . " where goodsid=:goodsid and uniacid=:uniacid and openid=:openid limit 1", array(":goodsid" => $goodsid, ":uniacid" => $_W["uniacid"], ":openid" => $_W["openid"]));
        if (empty($history)) {
            $history = array("uniacid" => $_W["uniacid"], "openid" => $_W["openid"], "goodsid" => $goodsid, "deleted" => 0, "createtime" => time(), "times" => 1);
            pdo_insert("vcshop_member_history", $history);
        } else {
            pdo_update("vcshop_member_history", array("deleted" => 0, "times" => $history["times"] + 1), array("id" => $history["id"]));
        }
    }
    public function getCartCount($isnewstore = 0)
    {
        ...
        $count = pdo_fetchcolumn("select sum(total) from " . tablename("vcshop_member_cart") . " where uniacid=:uniacid and openid=:openid " . $sqlcondition . " and deleted=0 limit 1", $paras);
        return $count;
    }
    /**
     * 获取商品规格图片
    ...
    public function getSpecThumb($specs)
    {
        ...
        $specid = $cartspecs[0];
        if (!empty($specid)) {
            $spec = pdo_fetch("select thumb from " . tablename("vcshop_goods_spec_item") . " " . " where id=:id and uniacid=:uniacid limit 1 ", array(":id" => $specid, ":uniacid" => $_W["uniacid"]));
    ...
    public function getMemberPrice($goods, $level)
    {
        ...
        $levelBak = $level;
        if (!empty($level["id"])) {
            $level = pdo_fetch("select * from " . tablename("vcshop_member_level") . " where id=:id and uniacid=:uniacid and enabled=1 limit 1", array(":id" => $level["id"], ":uniacid" => $_W["uniacid"]));
    ...
    public function getOptions($goods)
    {
        ...
        if (!empty($goods) && $goods["hasoption"]) {
            ...
            foreach ($specs as &$spec) {
                $spec["items"] = pdo_fetchall("select * from " . tablename("vcshop_goods_spec_item") . " where specid=:specid order by displayorder asc", array(":specid" => $spec["id"]));
            }
            unset($spec);
    ...
    public function canBuyAgain($goods)
    {
        ...
        $order_goods = pdo_fetchall("SELECT id,orderid FROM " . tablename("vcshop_order_goods") . " WHERE uniacid=:uniaicd AND openid=:openid AND goodsid=:goodsid " . $condition, array(":uniaicd" => $_W["uniacid"], ":openid" => $_W["openid"], ":goodsid" => $id), "orderid");
        if (empty($order_goods)) {
            return false;
        }
        $order = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("vcshop_order") . " WHERE uniacid=:uniaicd AND status>=:status AND id IN (" . implode(",", array_keys($order_goods)) . ")", array(":uniaicd" => $_W["uniacid"], ":status" => empty($goods["buyagain_condition"]) ? "1" : "3"));
        return !empty($order);
    }
    /**
     * 使用掉重复购买的变量
    ...
    {
        global $_W;
        $order_goods = pdo_fetchall("SELECT id,goodsid FROM " . tablename("vcshop_order_goods") . " WHERE uniacid=:uniaicd AND openid=:openid AND canbuyagain = 1 AND orderid <> :orderid", array(":uniaicd" => $_W["uniacid"], ":openid" => $_W["openid"], "orderid" => $orderid), "goodsid");
        if (empty($order_goods)) {
            return false;
        }
        pdo_query("UPDATE " . tablename("vcshop_order_goods") . " SET `canbuyagain`='0' WHERE uniacid=:uniacid AND goodsid IN (" . implode(",", array_keys($order_goods)) . ")", array(":uniacid" => $_W["uniacid"]));
    }
    ...
    {
        global $_W;
        $path = IA_ROOT . "/addons/vcshop/data/goodscode/" . $_W["uniacid"] . "/";
        ...
        $img = $_W["siteroot"] . "addons/vcshop/data/goodscode/" . $_W["uniacid"] . "/" . $file;
        return $img;
    }
