...
class Common_VcShopModel
{
	...
	public function updateSysset($values, $uniacid = 0)
	{
		...
		$setdata = pdo_fetch('select * from ' . tablename('vcshop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));

		if (empty($setdata)) {
			$res = pdo_insert('vcshop_sysset', array('sets' => iserializer($values), 'uniacid' => $uniacid));
			...
			$res = pdo_update('vcshop_sysset', array('sets' => iserializer($sets)), array('id' => $setdata['id']));
	...
	public function deleteSysset($key, $uniacid = 0)
	{
		...
		$setdata = pdo_fetch('select id, sets from ' . tablename('vcshop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
		...
		if (!empty($setdata)) {
			...
			pdo_update('vcshop_sysset', array('sets' => iserializer($sets)), array('id' => $setdata['id']));
		}
	...
	public function updatePluginset($values, $uniacid = 0)
	{
		...
		if (empty($setdata)) {
			$res = pdo_insert('vcshop_sysset', array('plugins' => iserializer($values), 'uniacid' => $uniacid));
			...
			$res = pdo_update('vcshop_sysset', array('plugins' => iserializer($plugins)), array('id' => $setdata['id']));
	...
	public function alipay_build($params, $alipay = array(), $type = 0, $openid = '')
	{
		...
		if (0 < $pay['alipay_id']) {
			...
			if (empty($type)) {
				$config['notify_url'] = $_W['siteroot'] . 'addons/vcshop/payment/alipay/notify.php';
		...
		if (empty($type)) {
			$set['notify_url'] = $_W['siteroot'] . 'addons/vcshop/payment/alipay/notify.php';
	...
	public function publicAliPay($params = array(), $return = NULL)
	{
		...
		$url = VCSHOP_DEBUG ? 'https://openapi.alipaydev.com/gateway.do' : 'https://openapi.alipay.com/gateway.do';
...
', true);
	...
	public function public_build($isapp = false)
	{
		...
		if (!empty($set['weixin_id']) && $isapp == false) {
			$payments = pdo_fetch('SELECT * FROM ' . tablename('vcshop_payment') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $set['weixin_id']));
	...
	public function wechat_child_build($params, $wechat, $type = 0)
	{
		...
		$package['nonce_str'] = random(32);
		$package['body'] = trim($params['title']);
		$package['device_info'] = isset($params['device_info']) ? 'vcshop:' . $params['device_info'] : 'vcshop';
		...
		$package['notify_url'] = empty($params['notify_url']) ? $_W['siteroot'] . 'addons/vcshop/payment/wechat/notify.php' : $params['notify_url'];
		$package['trade_type'] = 'JSAPI';
	...
	public function helibao_wechat_build($paramss, $helibao, $type = 0)
	{
		global $_W;
		// require '../payment/tonglian/AppConfig.php';
		require '../payment/helibao/AppUtil.php';
		$params["P1_bizType"] = 'AppPayPublic';//交易类型 公众号
	    $params["P2_orderId"] = $paramss['tid'];//订单号,自行生成
	    $params["P3_customerNumber"] = $helibao['mchid'];
	    $params["P4_payType"] = 'PUBLIC';//支付类型
	    $params["P5_appid"] = $_W['account']['key'];//公众账号appid
	    $params["P6_deviceInfo"] = 'WEB';//设备表示
	    $params["P7_isRaw"] = 1;//是否原生态1是 0不是
	    $params["P8_openid"] = $paramss['user'];//用户的微信openid
	    $params["P9_orderAmount"] = $paramss['fee'];//金额（单位：元）
	    $params["P10_currency"] = 'CNY';//人民币
	    $params["P11_appType"] = 'WXPAY';//微信
        $params["P12_notifyUrl"] = "{$_W['siteroot']}addons/vcshop/payment/helibao/notify_wechat.php";//交易结果通知地址
        $params["P13_successToUrl"] = "";//交易返回地址
        $params["P14_orderIp"] = CLIENT_IP;//交易IP
        $params["P15_goodsName"] = $paramss['title'];//商品名
        $params["P16_goodsDetail"] = '';//商品详情  json  优惠活动必填
	    $params["P17_limitCreditPay"] = 0;//1位禁用信用卡
	    $params["P18_desc"] = ((isset($params['uniacid']) ? $params['uniacid'] : $_W['uniacid'])) . ':' . $type;
	    // $params["P19_subscribeAppId"] = '';//关注appId  此参数排除签名
	    // $params["P21_goodsTag"] = '';//商品标记，微信平台配置的商品标记，用于优惠券或者满减使用..(不参与签名)

	    $s = AppUtil::ToSignParams($params);
	    $params["sign"] = AppUtil::SignArray($params,$helibao['key']);//签名
	    $paramsStr = AppUtil::ToUrlParams($params);
	    $url = "http://pay.trx.helipay.com/trx/app/interface.action";
	    $rsp = AppUtil::Request($url,$paramsStr);
	    $rspArray = json_decode($rsp, true); 
	    		// pdo_insert('a',array('info'=>$s));
	    		// pdo_insert('a',array('info'=>json_encode($params)));
	    		pdo_insert('a',array('info'=>$rsp));
    	$signRsp = strtolower($rspArray["sign"]);
    	$signarr = array(
    		'rt1_bizType' => $rspArray['rt1_bizType'],
    		'rt2_retCode' => $rspArray['rt2_retCode'],
    		'rt4_customerNumber' => $rspArray['rt4_customerNumber'],
    		'rt5_orderId' => $rspArray['rt5_orderId'],
    		'rt6_serialNumber' => $rspArray['rt6_serialNumber'],
    		'rt7_payType' => $rspArray['rt7_payType'],
    		'rt8_appid' => $rspArray['rt8_appid'],
    		'rt9_tokenId' => $rspArray['rt9_tokenId'],
    		'rt10_payInfo' => $rspArray['rt10_payInfo'],
    		'rt11_orderAmount' => $rspArray['rt11_orderAmount'],
    		'rt12_currency' => $rspArray['rt12_currency']
    	);
	    		// pdo_insert('a',array('info'=>json_encode($signarr)));

    	$sign =  strtolower(AppUtil::SignArray($signarr, $helibao['key']));
	    //验证签名
    	if($sign==$signRsp){
		    if("0000"==$rspArray["rt2_retCode"]){
	    		$wOpt = json_decode($rspArray['rt10_payInfo'], true);
	    		// pdo_insert('a',array('info'=>$rspArray["rt3_retMsg"]));
				return $wOpt;
		    }
		    else{
		    	// show_json(0,array('err'=>$rspArray["rt3_retMsg"]));
	    		// pdo_insert('a',array('info'=>$rspArray["rt3_retMsg"]));
				return error(-1, $rspArray["rt3_retMsg"]);
		    }
	    }else{
    		// echo "验签失败:".$signRsp."--".$sign;
	    	// show_json(0,array('err'=>'验签失败'));
	    		// pdo_insert('a',array('info'=>'验签失败'.$signRsp."--".$sign));
			return error(-1, '验签失败');
	    }
	}
	public function isHelibaoWeixinPay($out_trade_no, $money = 0,$app=false)
	{
		global $_W;
		...
		$helibao = $options;
		require '../payment/helibao/AppUtil.php';
		$params["P1_bizType"] = 'AppPayQuery';//订单查询接口
	    $params["P2_orderId"] = $out_trade_no;//订单号,自行生成
		$params["P3_customerNumber"] = $helibao['mchid'];
	    $params["P4_serialNumber"] = '';//合利宝流水号 预订单号二选一
	    $params["sign"] = AppUtil::SignArray($params,$helibao['key']);//签名
	    $paramsStr = AppUtil::ToUrlParams($params);
	    $url = "http://pay.trx.helipay.com/trx/app/interface.action";
	    $rsp = AppUtil::Request($url,$paramsStr);
    	...
    	$signarray = array(
    		...
    		'rt5_orderId' => $rspArray['rt5_orderId'],
    		'rt6_serialNumber' => $rspArray['rt6_serialNumber'],
    		'rt7_orderStatus' => $rspArray['rt7_orderStatus'],
    		'rt8_orderAmount' => $rspArray['rt8_orderAmount'],
    		'rt9_currency' => $rspArray['rt9_currency']
    	);
    	$sign =  strtolower(AppUtil::SignArray($signarray, $helibao['key']));
	    //验证签名
    	if($sign==$signRsp){
		    if("0000"==$rspArray["rt2_retCode"]){
	    		$wOpt = json_decode($rspArray['rt10_payInfo'], true);
	...
	public function sendredpack($params)
	{
		...
		if (strpos($params['openid'], 'sns_wa_') !== false) {
			...
			if (!empty($set['weixin_id'])) {
				$payment = pdo_fetch('SELECT * FROM ' . tablename('vcshop_payment') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $set['weixin_id']));
				...
				$payment = pdo_fetch('SELECT * FROM ' . tablename('vcshop_payment') . ' WHERE uniacid=:uniacid AND `type`=\'0\'', array(':uniacid' => $_W['uniacid']));
			}
	...
	public function getSec($uniacid = 0)
	{
		...
		$set = pdo_fetch('select sec from ' . tablename('vcshop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
		...
		if (0 < $data['alipay_id']) {
			$paymentalis = pdo_fetch('SELECT alipay_sec FROM ' . tablename('vcshop_payment') . ' WHERE id = :id and uniacid=:uniacid and paytype = 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $data['alipay_id']));
	...
	public function paylog($log = '')
	{
		...
		if (!empty($openpaylog)) {
			$path = IA_ROOT . '/addons/vcshop/data/paylog/' . $_W['uniacid'] . '/' . date('Ymd');
	...
	public function getAreas()
	{
		...
		if (!empty($new_area)) {
			$file = IA_ROOT . '/addons/vcshop/static/js/dist/area/AreaNew.xml';
		}
		else {
			$file = IA_ROOT . '/addons/vcshop/static/js/dist/area/Area.xml';
		}
	...
	{
		return pdo_fetchall('SELECT  a.uniacid,a.name FROM ' . tablename('vcshop_sysset') . ' s  ' . ' left join ' . tablename('account_wechats') . ' a on a.uniacid = s.uniacid' . ' left join ' . tablename('account') . ' acc on acc.uniacid = a.uniacid where acc.isdeleted=0 GROUP BY uniacid');
	}
	...
	public function getCopyright($ismanage = false)
	{
		...
		if (!is_array($copyrights)) {
			$copyrights = pdo_fetchall('select *  from ' . tablename('vcshop_system_copyright'));
		...
		if (!empty($merchid)) {
			$merch = pdo_fetch('select logo from ' . tablename('vcshop_merch_user') . ' where id = :id and uniacid = :uniacid ', array(':id' => $merchid, ':uniacid' => $_W['uniacid']));
