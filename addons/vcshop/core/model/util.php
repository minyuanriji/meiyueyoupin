...
class Util_VcShopModel
{
	...
	public function get_area_config_data($uniacid = 0)
	{
		...
		$sql = 'select * from ' . tablename('vcshop_area_config') . ' where uniacid=:uniacid limit 1';
	...
	private function getExpressBird($express, $expresssn, $mobile = '', $express_set)
	{
		...
		if (!empty($express_set['express_bird_cache']) && 0 < $express_set['express_bird_cache']) {
			...
			$cache = pdo_fetch('SELECT * FROM' . tablename('vcshop_express_cache') . 'WHERE express=:express AND expresssn=:expresssn LIMIT 1', array('express' => $express, 'expresssn' => $expresssn));
		...
		$expressInfo = pdo_fetch('SELECT * FROM' . tablename('vcshop_express') . 'WHERE express=:express LIMIT 1', array('express' => $express));
		...
		if (0 < $express_set['express_bird_cache'] && !empty($list)) {
			if (empty($cache)) {
				pdo_insert('vcshop_express_cache', array('expresssn' => $expresssn, 'express' => $express, 'lasttime' => time(), 'datas' => iserializer($list)));
			}
			else {
				pdo_update('vcshop_express_cache', array('lasttime' => time(), 'datas' => iserializer($list)), array('id' => $cache['id']));
			}
		}
