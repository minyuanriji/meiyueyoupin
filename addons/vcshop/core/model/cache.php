		...
		if (function_exists('redis')) {
			...
			if (!is_error($redis)) {
				if (stripos($uniacid, 'global') !== false) {
					return 'vcshop_syscache_' . $_W['setting']['site']['key'] . '_global_' . $key;
				}
				...
				return 'vcshop_syscache_' . $_W['setting']['site']['key'] . '_' . $uniacid . '_' . $account_key . '_' . $key;
			}
		}

		return VCSHOP_PREFIX . md5($uniacid . '_new_' . $key);
	}
