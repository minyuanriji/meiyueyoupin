...
class Verifygoods_VcShopModel
{
	...
	{
		global $_W;
		$verifygoods = pdo_fetchall('select  *   from  ' . tablename('vcshop_verifygoods') . ' where  orderid=:orderid ', array(':orderid' => $orderid));
		...
		if (empty($goods['verifygoodstype'])) {
			foreach ($ordergoods as $ordergood) {
				...
				while ($i < $total) {
					...
					pdo_insert('vcshop_verifygoods', $data);
					++$i;
				}
			}
		}
	...
	public function getCanUseVerifygoods($openid)
	{
		global $_W;
		$time = time();
		$sql = 'select vg.*,g.title,g.subtitle,g.thumb,c.card_id  from ' . tablename('vcshop_verifygoods') . '   vg
                 inner join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
                 left  join ' . tablename('vcshop_order') . ' o on vg.orderid = o.id
                 left  join ' . tablename('vcshop_order_refund') . ' orf on o.refundid = orf.id
                 inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
                 left  join ' . tablename('vcshop_goods_cards') . ' c on c.id = g.cardid
	...
	public function search($verifycode)
	{
		...
		global $_GPC;
		$lastVerifyNumber = 0;
		$verifygood = pdo_fetch('select *  from ' . tablename('vcshop_verifygoods') . ' where uniacid=:uniacid and  verifycode=:verifycode and invalid =0  limit 1 ', array(':uniacid' => $_W['uniacid'], ':verifycode' => $verifycode));
		...
		$saler = pdo_fetch('select * from ' . tablename('vcshop_saler') . ' where  status=1  and  openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
	...
	public function allow($verifycode, $times)
	{
		...
		$order = pdo_fetch('select * from ' . tablename('vcshop_order') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $verifygood['orderid'], ':uniacid' => $verifygood['uniacid']));
		...
		if (!empty($verifygood['limitnum'])) {
			$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('vcshop_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $verifygood['id']));
			$verifynum = 0;
		...
		$verifygoods_detail = pdo_fetch('select vg.*,g.id as goodsid ,g.title,g.subtitle,g.thumb,vg.storeid  from ' . tablename('vcshop_verifygoods') . '   vg
		 inner join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
		 inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
	...
	public function complete($verifycode, $times, $remarks)
	{
		...
		pdo_insert('vcshop_verifygoods_log', $data);
		$logid = pdo_insertid();
		m('notice')->sendVerifygoodMessage($logid);
		pdo_query('update ' . tablename('vcshop_verifygoods') . ' set used=:used , verifycode=null ,codeinvalidtime=0 where id=:id', array(':id' => $verifygood['id'], ':used' => $used));
		...
		if ($isonlyverifygood) {
			$status = pdo_fetchcolumn('select status  from ' . tablename('vcshop_order') . ' where uniacid=:uniacid and id=:id  limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $verifygood['orderid']));

			$verifytypexzx = pdo_fetchall("select verifytype  from " . tablename("vcshop_order") . " where uniacid=:uniacid and id=:id  limit 1 ", array(":uniacid" => $_W["uniacid"], ":id" => $verifygood["orderid"]));
				$finishorderid = $verifygood['orderid'];
				$this->finishorder($finishorderid, array("times" => $times, "lastverifys" => $allow["lv"]));
		}
	...
	public function finishorder($id, $arr = array())
	{
		...
		pdo_update('vcshop_order', array('status' => 3, 'sendtime' => time(), 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		if ($item["status"] == 2) {
	...
	{
		global $_W;
		$sql = 'select  COUNT(1)  from ' . tablename('vcshop_verifygoods') . '   vg
                 inner join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
                 left  join ' . tablename('vcshop_order') . ' o on vg.orderid = o.id
                 left  join ' . tablename('vcshop_order_refund') . ' orf on o.refundid = orf.id
                 inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
