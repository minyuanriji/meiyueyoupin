<?php
class Order_VcShopModel {
    public function fullback($orderid) {
        global $_W;
        $uniacid     = $_W['uniacid'];
        $order_goods = pdo_fetchall('select o.openid,og.optionid,og.goodsid,og.price,og.total from ' . tablename('vcshop_order_goods') . ' as og
                    left join ' . tablename('vcshop_order') . ' as o on og.orderid = o.id
                    where og.uniacid = ' . $uniacid . ' and og.orderid = ' . $orderid . ' ');
        foreach ($order_goods as $key => $value) {
            if (0 < $value['optionid']) {
                $goods = pdo_fetch('select g.hasoption,g.id,go.goodsid,go.isfullback from ' . tablename('vcshop_goods') . ' as g
                left join ' . tablename('vcshop_goods_option') . ' as go on go.goodsid = :id and go.id = ' . $value['optionid'] . '
                 where g.id=:id and g.uniacid=:uniacid limit 1', array(
                    ':id' => $value['goodsid'],
                    ':uniacid' => $uniacid
                ));
            } else {
                $goods = pdo_fetch('select * from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
                    ':id' => $value['goodsid'],
                    ':uniacid' => $uniacid
                ));
            }
            if (0 < $goods['isfullback']) {
                $fullbackgoods = pdo_fetch('SELECT id,minallfullbackallprice,maxallfullbackallprice,minallfullbackallratio,maxallfullbackallratio,`day`,
                          ...
                          FROM ' . tablename('vcshop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and status=1 and goodsid = ' . $value['goodsid'] . ' limit 1');
                $condition     = '';
                if (!empty($fullbackgoods) && $goods['hasoption'] && 0 < $value['optionid']) {
                    $option = pdo_fetch('select id,title,allfullbackprice,allfullbackratio,fullbackprice,fullbackratio,`day` from ' . tablename('vcshop_goods_option') . ' 
                ...
                if (!empty($fullbackgoods)) {
                    ...
                    $has_record = pdo_fetch('select id from ' . tablename('vcshop_fullback_log') . (' where ' . $condition), $params);
                    if (empty($has_record)) {
                        pdo_insert('vcshop_fullback_log', $data);
                    }
                }
            }
        }
    ...
    public function fullbackstop($orderid) {
        ...
        $uniacid      = $_W['uniacid'];
        $shopset      = $_S['shop'];
        $fullback_log = pdo_fetch('select * from ' . tablename('vcshop_fullback_log') . ' where uniacid = ' . $uniacid . ' and orderid = ' . $orderid . ' ');
        pdo_update('vcshop_fullback_log', array(
            'isfullback' => 1
        ), array(
            'id' => $fullback_log['id'],
            'uniacid' => $uniacid
    ...
    public function payResult($params) {
        ...
        $order        = pdo_fetch('select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus,iscycelbuy from ' . tablename('vcshop_order') . ' where  ordersn=:ordersn and uniacid=:uniacid limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':ordersn' => $ordersn
        ));
        ...
        if (!empty($ispeerpay)) {
            $peerpay_info = (double) pdo_fetchcolumn('select SUM(price) price from ' . tablename('vcshop_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(
                ':pid' => $ispeerpay['id']
            ));
            ...
            $order['status'] = 0;
            pdo_update('vcshop_order_peerpay', array(
                'status' => 1
            ), array(
                'id' => $ispeerpay['id']
            ));
        ...
        if ($params['from'] == 'return') {
            ...
            if (empty($order['dispatchtype'])) {
                $address = pdo_fetch('select realname,mobile,address from ' . tablename('vcshop_member_address') . ' where id=:id limit 1', array(
                    ':id' => $order['addressid']
                ));
            }
            $carrier = false;
            ...
            if ($params['type'] == 'cash') {
                if ($order['isparent'] == 1) {
                    ...
                    pdo_update('vcshop_order', $change_data, array(
                        'id' => $order['id']
                    ));
                    $this->setChildOrderPayResult($order, 0, 0);
                    $merchSql  = 'SELECT id,merchid FROM ' . tablename('vcshop_order') . ' WHERE uniacid = ' . intval($order['uniacid']) . ' AND parentid = ' . intval($order['id']);
            ...
            if ($order['istrade'] == 0) {
                if ($order['status'] == 0) {
                    ...
                    // han 20200610 防止用户支付完成后没有按完成,导致会员等级付款后不能及时升级 s
                    $memberSetting = m('common')->getSysset('member');

                    if ((int) $memberSetting['upgrade_condition'] === 2) {
                        m('member')->upgradeLevel($order['openid'], $orderid, 1);
                    }
                    // han 20200610 防止用户支付完成后没有按完成,导致会员等级付款后不能及时升级 e
    ...
    public function setOrderPayType($orderid, $paytype, $ordersn = '') {
        ...
        $peerpay = pdo_fetch("select id from " . tablename("vcshop_order_peerpay") . " where orderid=:orderid and uniacid=:uniacid", array( "orderid" => $orderid, "uniacid" => $_W["uniacid"] ));
        ...
        if (!empty($orderid)) {
            ...
            pdo_update("vcshop_order_peerpay_payinfo", array( "paytype" => $paytype ), array( "pid" => $peerpay["id"], "openid" => $_W["openid"] ));
    ...
    public function getChildOrder($orderid) {
        global $_W;
        $list = pdo_fetchall('select id,ordersn,status,finishtime,couponid,merchid  from ' . tablename('vcshop_order') . ' where  parentid=:parentid and uniacid=:uniacid', array(
            ':parentid' => $orderid,
            ':uniacid' => $_W['uniacid']
        ));
        return $list;
    ...
    public function payVirtualSend($orderid = 0, $ispeerpay = false) {
        global $_W;
        global $_GPC;
        $order       = pdo_fetch('select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus,iscycelbuy from ' . tablename('vcshop_order') . ' where  id=:id and uniacid=:uniacid limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':id' => $orderid
        ));
        $order_goods = pdo_fetch('select g.virtualsend,g.virtualsendcontent from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid limit 1', array(
    ...
    public function afterPayResult($order, $ispeerpay = false) {
        if (p('task')) {
            ...
            $goodslist = pdo_fetchall('SELECT goodsid FROM ' . tablename('vcshop_order_goods') . ' WHERE orderid = :orderid AND uniacid = :uniacid', array(
                ':orderid' => $order['id'],
                ':uniacid' => $order['uniacid']
            ));
            ...
            if (pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . (' where openid = \'' . $order['openid'] . '\' and uniacid = ' . $order['uniacid'])) == 1) {
    ...
    public function setGiveBalance($orderid = '', $type = 0, $status = 0) {
        global $_W;
        $order   = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status from ' . tablename('vcshop_order') . ' where id=:id limit 1', array(
            ':id' => $orderid
        ));
        $goods   = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.money,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid
        ));
        $balance = 0;
    ...
    public function setStocksAndCredits($orderid = '', $type = 0, $flag = false) {
        global $_W;
        $tempflag = false;
        $order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent,paytype,isnewstore,storeid,istrade,status from ' . tablename('vcshop_order') . ' where id=:id limit 1', array(
            ':id' => $orderid
        ));
        ...
        $goods   = pdo_fetchall('select og.goodsid,og.seckill,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.type from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . (' where ' . $condition . ' and og.uniacid=:uniacid '), $param);
        $credits = 0;
        foreach ($goods as $g) {
            if (0 < $newstoreid) {
                ...
                $goods_item      = pdo_fetch('select total as goodstotal from' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
                    ':id' => $g['goodsid'],
                    ':uniacid' => $_W['uniacid']
                ));
            ...
            if (!empty($stocktype)) {
                ...
                if (!empty($g['optionid'])) {
                    ...
                    if (!empty($option) && $option['stock'] != -1) {
                        ...
                        if ($stock != -1) {
                            if (0 < $newstoreid) {
                                pdo_update('vcshop_newstore_goods_option', array(
                                    'stock' => $stock
                                ), array(
                                    'uniacid' => $_W['uniacid'],
                                    'goodsid' => $g['goodsid'],
                                ...
                                pdo_update('vcshop_goods_option', array(
                                    'stock' => $stock
                                ), array(
                                    'uniacid' => $_W['uniacid'],
                                    'goodsid' => $g['goodsid'],
                ...
                if ((!empty($g['goodstotal']) && $stocktype == -1 || $stocktype == 1) && $g['goodstotal'] != -1) {
                    ...
                    if ($totalstock != -1) {
                        if (0 < $newstoreid) {
                            pdo_update('vcshop_newstore_goods', array(
                                'stotal' => $totalstock
                            ), array(
                                'uniacid' => $_W['uniacid'],
                                'id' => $store_goods['id']
            ...
            if ($type == 0) {
                ...
                if ($type == 1) {
                    if (1 <= $order['status']) {
                        $salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(
                            ':goodsid' => $g['goodsid'],
                            ':uniacid' => $_W['uniacid']
                        ));
                        pdo_update('vcshop_goods', array(
                        ...
                        $table_flag = pdo_tableexists('vcshop_order_buysend');
                        if (0 < $credits && $table_flag) {
                            ...
                            $send_record = pdo_fetch('SELECT * FROM ' . tablename('vcshop_order_buysend') . ' WHERE orderid = :orderid AND uniacid = :uniacid AND openid = :openid', array(
                                ':orderid' => $orderid,
                                ':uniacid' => $_W['uniacid'],
                                ':openid' => $order['openid']
                            ));
                            if ($send_record) {
                                pdo_update('vcshop_order_buysend', $send_data, array(
                                    'id' => $send_record['id']
                                ));
                            } else {
                                pdo_insert('vcshop_order_buysend', $send_data);
                            }
                        }
                    }
                }
    ...
    public function getTotals($merch = 0) {
        ...
        $totals['all']      = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and deleted=0'), $paras);
        $totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and status=-1 and refundtime=0 and deleted=0'), $paras);
        $totals['status0']  = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and status=0 and paytype<>3 and deleted=0'), $paras);
        $totals['status1']  = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and ( status=1 or ( status=0 and paytype=3) ) and deleted=0'), $paras);
        $totals['status2']  = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and ( status=2 or (status = 1 and sendtype > 0) ) and deleted=0'), $paras);
        $totals['status3']  = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and status=3 and deleted=0'), $paras);
        $totals['status4']  = pdo_fetchcolumn('SELECT COUNT(1)FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and ((refundstate>0 and refundid<>0 and refundtime=0) or (refundtime=0 and refundstate=3)) and deleted=0'), $paras);
        $totals['status5']  = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and refundtime<>0 and deleted=0'), $paras);
        return $totals;
    }
    ...
    public function getGoodsDiscountPrice($g, $level, $type = 0, $memberCardId = 0) {
        ...
        if (!empty($level['id'])) {
            $level = pdo_fetch('select * from ' . tablename('vcshop_member_level') . ' where id=:id and uniacid=:uniacid and enabled=1 limit 1', array(
                ':id' => $level['id'],
                ':uniacid' => $_W['uniacid']
            ));
            ...
            if (is_array($discounts)) {
                ...
                if (!isset($discounts['type']) || empty($discounts['type'])) {
                    if (!empty($discounts[$key])) {
                        ...
                        $dd = floatval((float)$discounts[$key . '_pay'] * $total);
    ...
    public function validate_city_express($address) {
        ...
        $city_express      = pdo_fetch('SELECT * FROM ' . tablename('vcshop_city_express') . ' WHERE uniacid=:uniacid and merchid=0 limit 1', array(
            ':uniacid' => $_W['uniacid']
        ));
        if (!empty($city_express['enabled'])) {
            ...
            if (!empty($address)) {
                ...
                if ($data['status'] == 1 && 0 < $data['count']) {
                    ...
                    pdo_update('vcshop_member_address', $addres, array(
                        'id' => $addres['id'],
                        'uniacid' => $_W['uniacid']
                    ));
    ...
    public function changeParentOrderPrice($parent_order) {
        global $_W;
        $id   = $parent_order['id'];
        $item = pdo_fetch('SELECT price,ordersn2,dispatchprice,changedispatchprice FROM ' . tablename('vcshop_order') . ' WHERE id = :id and uniacid=:uniacid', array(
            ':id' => $id,
            ':uniacid' => $_W['uniacid']
        ));
        if (!empty($item)) {
            ...
            if (!empty($orderupdate)) {
                pdo_update('vcshop_order', $orderupdate, array(
                    'id' => $id,
                    'uniacid' => $_W['uniacid']
                ));
            }
    ...
    public function getOrderCommission($orderid, $agentid = 0) {
        global $_W;
        if (empty($agentid)) {
            $item = pdo_fetch('select agentid from ' . tablename('vcshop_order') . ' where id=:id and uniacid=:uniacid Limit 1', array(
                'id' => $orderid,
                ':uniacid' => $_W['uniacid']
            ));
            if (!empty($item)) {
    ...
    public function checkOrderGoods($orderid) {
        ...
        $item      = pdo_fetch('select * from ' . tablename('vcshop_order') . '  where  id = :id and uniacid=:uniacid limit 1', array(
            ':id' => $orderid,
            ':uniacid' => $uniacid
        ));
        if ((empty($order['isnewstore']) || empty($order['storeid'])) && empty($order['istrade'])) {
            $order_goods = pdo_fetchall('select og.id,g.title, og.goodsid,og.optionid,g.total as stock,og.total as buycount,g.status,g.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill from  ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(
                ':uniacid' => $_W['uniacid'],
                ':orderid' => $orderid
            ));
                ...
                if ($seckillinfo && $seckillinfo['status'] == 0 || !empty($ispeerpay)) {
                    ...
                    if ($data['totalcnf'] == 1) {
                        if (!empty($data['optionid'])) {
                            $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual` from ' . tablename('vcshop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(
                                ':uniacid' => $uniacid,
                                ':goodsid' => $data['goodsid'],
                                ':id' => $data['optionid']
                            ));
            ...
            $sql  = 'select g.id,g.title,ng.gstatus,g.deleted' . ' from ' . tablename('vcshop_order_goods') . ' og left join  ' . tablename('vcshop_goods') . ' g  on g.id=og.goodsid and g.uniacid=og.uniacid' . ' inner join ' . tablename('vcshop_newstore_goods') . ' ng on ng.goodsid = g.id AND ng.storeid=' . $item['storeid'] . ' where og.orderid=:orderid and og.uniacid=:uniacid';
    ...
    public function checkpeerpay($orderid) {
        global $_W;
        $sql   = 'SELECT p.*,o.openid FROM ' . tablename('vcshop_order_peerpay') . ' AS p JOIN ' . tablename('vcshop_order') . ' AS o ON p.orderid = o.id WHERE p.orderid = :orderid AND p.uniacid = :uniacid AND (p.status = 0 OR p.status=1) AND o.status >= 0 LIMIT 1';
    ...
    public function peerStatus($param) {
        global $_W;
        if (!empty($param['tid'])) {
            $sql = 'SELECT id FROM ' . tablename('vcshop_order_peerpay_payinfo') . ' WHERE tid = :tid';
        ...
        pdo_insert('vcshop_order_peerpay_payinfo', $param);
    ...
    public function getVerifyCardNumByOrderid($orderid) {
        global $_W;
        $num = pdo_fetchcolumn('select SUM(og.total)  from ' . tablename('vcshop_order_goods') . ' og
         inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
    ...
    public function checkisonlyverifygoods($orderid) {
        global $_W;
        $num = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('vcshop_order_goods') . ' og
         inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
        ...
        $num2 = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('vcshop_order_goods') . ' og
             inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
             where og.uniacid=:uniacid  and og.orderid =:orderid and g.type=5', array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid
    ...
    public function checkhaveverifygoodlog($orderid) {
        global $_W;
        $num = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('vcshop_verifygoods_log') . ' vl
         inner join ' . tablename('vcshop_verifygoods') . ' v on vl.verifygoodsid = v.id
    ...
    public function getOrderVirtual($order = array()) {
        ...
        $ordervirtual = array();
        $virtual_type = pdo_fetch('select fields from ' . tablename('vcshop_virtual_type') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1 ', array(
            ':id' => $order['virtual'],
            ':uniacid' => $_W['uniacid'],
    ...
    public function dada_send($order) {
        ...
        $cityexpress = pdo_fetch('SELECT * FROM ' . tablename('vcshop_city_express') . ' WHERE uniacid=:uniacid AND merchid=:merchid', array(
            ':uniacid' => $_W['uniacid'],
            ':merchid' => 0
        ));
    ...
    public function CheckoodsStock($orderid = '', $type = 0, $flg = true) {
        ...
        $goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.type from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . (' where ' . $condition . ' and og.uniacid=:uniacid '), $param);
        if (!empty($goods)) {
            foreach ($goods as $g) {
                if (0 < $newstoreid) {
                    $store_goods = m('store')->getStoreGoodsInfo($g['goodsid'], $newstoreid);
                ...
                if (!empty($stocktype)) {
                    ...
                    if (!empty($g['optionid'])) {
                        ...
                        if (!empty($option) && $option['stock'] != -1) {
                            if ($stocktype == -1 && $type == 0) {
                                ...
                                if ($open_redis) {
                                    $redis_key = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_'.'vc_' . $_W['uniacid'] . '_goods_order_option_stock_' . $option['id'];
                                    $redis     = redis();
                    ...
                    if (!empty($g['goodstotal']) && $g['goodstotal'] != -1) {
                        if ($stocktype == -1 && $type == 0) {
                            $open_redis = function_exists('redis') && !is_error(redis());
                            if ($open_redis) {
                                $redis_key = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_'.'vc_' . $_W['uniacid'] . '_goods_order_stock_' . $g['goodsid'];
                                $redis     = redis();
