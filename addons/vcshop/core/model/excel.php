...
class Excel_VcShopModel
{
	...
	public function export($list, $params = array())
	{
		...
		$excel = new PHPExcel();
		$excel->getProperties()->setCreator(empty($data['name']) ? '微信商城' : $data['name'])->setLastModifiedBy(empty($data['name']) ? '微信商城' : $data['name'])->setTitle('Office 2007 XLSX Test Document')->setSubject('Office 2007 XLSX Test Document')->setDescription('Test document for Office 2007 XLSX, generated using PHP classes.')->setKeywords('office 2007 openxml php')->setCategory('report file');
	...
	public function temp($title, $columns = array())
	{
		...
		require_once IA_ROOT . '/framework/library/phpexcel/PHPExcel.php';
		$excel = new PHPExcel();
		$excel->getProperties()->setCreator('微信商城')->setLastModifiedBy('微信商城')->setTitle('Office 2007 XLSX Test Document')->setSubject('Office 2007 XLSX Test Document')->setDescription('Test document for Office 2007 XLSX, generated using PHP classes.')->setKeywords('office 2007 openxml php')->setCategory('report file');
		$sheet = $excel->setActiveSheetIndex(0);
		$rownum = 1;
	...
	public function import($excefile)
	{
		...
		$path = IA_ROOT . '/addons/vcshop/data/tmp/';
