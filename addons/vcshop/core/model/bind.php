...
class Bind_VcShopModel
{
	...
	public function update($mid = 0, $arr = array())
	{
		...
		pdo_update('vcshop_member', $arr, array('id' => $mid, 'uniacid' => $_W['uniacid']));
	}
	...
	public function merge($a = array(), $b = array())
	{
		...
		if (!empty($arr) && is_array($arr)) {
			pdo_update('vcshop_member', $arr, array('id' => $b['id']));
		}

		pdo_update('vcshop_commission_apply', array('mid' => $b['id']), array('uniacid' => $_W['uniacid'], 'mid' => $a['id']));
		pdo_update('vcshop_order', array('agentid' => $b['id']), array('agentid' => $a['id']));
		pdo_update('vcshop_member', array('agentid' => $b['id']), array('agentid' => $a['id']));
		...
		pdo_delete('vcshop_member', array('id' => $a['id'], 'uniacid' => $_W['uniacid']));
		...
		pdo_insert('vcshop_member_mergelog', array('uniacid' => $_W['uniacid'], 'mergetime' => time(), 'openid_a' => $a['openid'], 'openid_b' => $b['openid'], 'mid_a' => $a['id'], 'mid_b' => $b['id'], 'detail_a' => iserializer($a), 'detail_b' => iserializer($b), 'detail_c' => iserializer($c)));
		return error(1);
	}
	...
	public function mergeforuniacid($a = array(), $b = array())
	{
		...
		pdo_update('vcshop_commission_apply', array('mid' => $b['id']), array('mid' => $a['id']));
		pdo_update('vcshop_order', array('agentid' => $b['id']), array('agentid' => $a['id']));
		pdo_update('vcshop_member', array('agentid' => $b['id']), array('agentid' => $a['id']));
		$mergeinfo = ' 合并前用户: ' . $a['nickname'] . '(' . $a['id'] . ') 合并后用户: ' . $b['nickname'] . '(' . $b['id'] . ')';
		...
		$c = m('member')->getMember($b['openid']);
		pdo_insert('vcshop_member_mergelog', array('uniacid' => $_W['uniacid'], 'fromuniacid' => $_W['uniacid'], 'mergetime' => time(), 'openid_a' => $a['openid'], 'openid_b' => $b['openid'], 'mid_a' => $a['id'], 'mid_b' => $b['id'], 'detail_a' => iserializer($a), 'detail_b' => iserializer($b), 'detail_c' => iserializer($c)));
		return error(1);
	}
}
