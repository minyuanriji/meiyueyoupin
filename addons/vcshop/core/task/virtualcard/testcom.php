...
if (!empty($sets)) {
	...
	$thismonth = mktime(0,0,0,date('m')+1,1);//本月
	$lastmonth = mktime(0,0,0,date('m'),1);//上月
	$nextmonth = mktime(0,0,0,date('m')+2,1);//下月
	...
	foreach ($sets as $set) {
		...
		foreach ($datas as $k => $u) {
			...
			print_r($cardmember['nickname']);echo "<br>";
			print_r($cardmember['openid']);echo "<br>";
			print_r($cardmember['agentlevel']);echo "<br>";
			if (in_array($cardmember['agentlevel'], $level_ids)) {
				print_r($cardmember['nickname']);echo "<br>";echo "<br>";
				// 清除上月赠送的没用完的积分
				...
				if($count == 0){
					...
					print_r('last_send-'.$last_send);echo "<br>";
					if($last_send > 0){
						...
						print_r('last_used-'.$last_used);echo "<br>";
						if($last_used < $last_send){
							$decut = $last_send - $last_used;
							if($decut > 0){
								print_r($cardmember['nickname'].'扣除上月剩余的赠送积分 ' . $decut.' decutmonthlycredit');echo "<br>";
			...
			// 		print_r($cardmember['nickname'].' 已送完 ');echo "<br>";
			// 		continue;
			// 	}
			...
			// 	print_r($cardmember['nickname'].'虚拟卡券兑换送积分 ' . $credit1);echo "<br>";
			// 	if ($d['is_alltime'] == 1) {
			// 	    $operator = 5;
			// 	}else{
			// 	    $operator = 6;
			...
			// 			print_r($cardmember['nickname'].'最后一次虚拟卡券兑换送积分 ' . $credit1);echo "<br>";echo "<br>";
			// 		}
			// 	// }
			// }
		}
