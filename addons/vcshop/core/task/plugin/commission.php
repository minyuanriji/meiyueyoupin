...
if (!empty($sets)) {
	...
	foreach ($sets as $set) {
		...
		foreach ($user as $k => $u) {
			...
			// 以下为赠送积分
			// 本月已发放过或者已经发放够3年36次
			$count = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('vcshop_member_credit_send_record') . ' where uniacid=:uniacid and openid=:openid and operator=0 and createtime>:createtime',array(':uniacid'=>$uniacid,':openid'=>$u['openid'],':createtime'=>$thismonth));
			if($count >= 1){
				continue;
			}
			$month = 0;
			...
			if(empty($orders)){
				...
				foreach ($orders as  $order) {
					...
					// 24
					$count = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('vcshop_order_goods') . ' where uniacid=:uniacid and orderid=:orderid and cretidgoods=2',array(':uniacid'=>$uniacid,':orderid'=>$order['id']));
					if($count >= 1){
						$month = 24;
						$checkcredit = false;
						break;
					...
					// 12
					$count = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('vcshop_order_goods') . ' where uniacid=:uniacid and orderid=:orderid and cretidgoods=3',array(':uniacid'=>$uniacid,':orderid'=>$order['id']));
					if($count >= 1){
						$month = 12;
						$checkcredit = false;
						break;
			...
			$count = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('vcshop_member_credit_send_record') . ' where uniacid=:uniacid and openid=:openid and operator=0',array(':uniacid'=>$uniacid,':openid'=>$u['openid']));
			if($count >= $month){
				continue;
			}
		...
		$current = time();
		$tm = $commission['tm'];
		// han 20200424 已搬到creditnotice.php文件执行
		/*
		if ($lasttime + $interval <= $current  && false) {
