...
foreach ($sets as $set) {
	...
	$logs = pdo_fetchall('select id,`day`,fullbackday,openid,priceevery,price,fullbacktime,goodsid,optionid from ' . tablename('vcshop_fullback_log') . ' where uniacid = ' . $_W['uniacid'] . ' and isfullback = 0 and (fullbacktime =0 or fullbacktime < ' . strtotime('-1 days') . ') and fullbackday < day ');
	...
	foreach ($logs as $key => $value) {
		if (1 < $value['day'] - $value['fullbackday']) {
			...
			if (1 <= $count) {
				if ($value['day'] - $value['fullbackday'] <= $count) {
					...
					pdo_update('vcshop_fullback_log', array('fullbackday' => $value['day'], 'fullbacktime' => $today, 'isfullback' => 1), array('id' => $value['id']));
				}
				else {
					$value['priceevery'] = $value['priceevery'] * $count;
					$value['fullbackday'] = $value['fullbackday'] + $count;
					$result = m('member')->setCredit($value['openid'], 'credit2', $value['priceevery'], array('0', $_W['shopset']['shop']['name'] . '全返余额' . $value['priceevery']));
					pdo_update('vcshop_fullback_log', array('fullbackday' => $value['fullbackday'], 'fullbacktime' => $today), array('id' => $value['id']));
				}
			}
		}
		else {
			if ($value['day'] - $value['fullbackday'] == 1) {
				...
				$value['priceevery'] = $value['price'] - $value['priceevery'] * $value['fullbackday'];
				$result = m('member')->setCredit($value['openid'], 'credit2', $value['priceevery'], array('0', $_W['shopset']['shop']['name'] . '全返余额' . $value['priceevery']));
				pdo_update('vcshop_fullback_log', array('fullbackday' => $value['day'], 'fullbacktime' => time(), 'isfullback' => 1), array('id' => $value['id']));
			}
		}
		...
		if (1 < $count) {
			...
			while ($i <= $count - 1) {
				...
				$logdata['day'] = 0;
				pdo_insert('vcshop_fullback_log_map', $logdata);
				++$i;
			}
		}
