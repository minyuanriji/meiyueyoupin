...
foreach ($sets as $set) {
	...
	$goods = pdo_fetchall('select id,title,ispresell,presellover,presellovertime,presellstart,preselltimestart,presellend,preselltimeend from ' . tablename('vcshop_goods') . ' where uniacid = ' . $_W['uniacid'] . ' and ispresell > 0 and deleted = 0 ');
			...
			if ($value['ispresell'] == 1 && $value['presellover'] == 0 && $value['presellend'] == 1) {
				...
				if ($value['ispresell'] == 1 && $value['presellover'] == 1 && $value['presellend'] == 1) {
					...
					if ($value['preselltimeend'] + $time < time()) {
						$value['status'] = 0;
						pdo_update('vcshop_goods', array('ispresell' => $value['status']), array('id' => $value['id']));
