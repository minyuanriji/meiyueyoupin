...
class VerifyProcessor extends ComProcessor
{
	...
	public function __construct()
	{
		...
		$this->sessionkey = VCSHOP_PREFIX . 'order_wechat_verify_info';
		$this->codekey = VCSHOP_PREFIX . 'order_wechat_verify_code';
	}
			...
			if ($obj->inContext) {
				if (is_numeric($content)) {
					if (8 <= strlen($content)) {
						...
						$member = pdo_get('vcshop_member', array('openid' => $order['openid']));
