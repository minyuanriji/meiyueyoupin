...
class Index_VcShopPage extends WebPage 
{
	...
	public function search()
	{
		...
		if (!empty($keyword)) {
			...
			array_walk($list['shop']['items'], function(&$v) {
				if ($v['url'] == 'http://slf.clubmall.cn/web/index.php?c=site&a=entry&m=vcshop&do=web&r=shop.diypage') {
					$v['url'] = 'http://slf.clubmall.cn/web/index.php?c=site&a=entry&m=vcshop&do=web&r=diypage';
				}
			});
	...
	public function switchversion()
	{
		...
		$id = intval($_GPC['id']);
		$set = pdo_fetch('SELECT * FROM ' . tablename('vcshop_version') . ' WHERE uid=:uid AND `type`=0', array(':uid' => $_W['uid']));
