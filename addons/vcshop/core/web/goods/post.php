...
if (!empty($id)) {
	pdo_update('vcshop_goods', array('newgoods' => 0), array('id' => $id));
}

$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_goods') . ' WHERE id = :id and uniacid = :uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
if (!empty($item) && $item['type'] == 5 && !empty($item['opencard']) && !empty($item['cardid'])) {
	$card = pdo_fetch('SELECT * FROM ' . tablename('vcshop_goods_cards') . ' WHERE id = :id and uniacid = :uniacid', array(':id' => $item['cardid'], ':uniacid' => $_W['uniacid']));
}
...
unset($l);
// han 20200330s
$starlevel = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_commission_star') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY level asc'));
...
unset($value);
if(!empty($item['starbonus'])){
	if (is_serialized($item['starbonus'])) {
		$item['starbonus'] = iunserializer($item['starbonus']);
	}
}
// han 20200330 e
...
$commission_level = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_commission_level') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY commission1 asc'));
...
unset($l);
$commission_level2 = p('commission')->getLevels(true,true);//han 20200110 
...
if ($_W['ispost']) {
	...
	if ($_GPC['isverify'] == 2 && $id) {
		$nowtime = TIMESTAMP;
		$gift = pdo_fetch('select id,title from ' . tablename('vcshop_gift') . ' WHERE goodsid = :goodsid and uniacid=:uniacid and endtime>=:endtime', array(':goodsid' => $id, ':uniacid' => $_W['uniacid'], ':endtime' => $nowtime));
    ...
    // han 20200305 买家通知
    $data['noticebuyer_type'] = is_array($_GPC["noticebuyer_type"]) ? implode(",", $_GPC["noticebuyer_type"]) : "";
    $data['noticebuyer_text'] = trim($_GPC['noticebuyer_text']);
    $data['noticebuyer_title'] = trim($_GPC['noticebuyer_title']);
    // han 20200305
	if (p('offic')) {
	...
	// 20200513
	$data['blankgroup'] = m('common')->html_images($_GPC['blankgroup']);
	if (p('commission')) {
		...
		$data['stockgoods'] = intval($_GPC['stockgoods']);//han 20191120
		$data['cretidgoods'] = intval($_GPC['cretidgoods']);//han 20191120
		$data['stocklevel'] = intval($_GPC['stocklevel']);//han 20191120
		//han 20200327 s
		$data['starbonus'] = is_array($_GPC['starbonus']) && !empty($_GPC['starbonus']) ? iserializer($_GPC['starbonus']) : iserializer(array());
		$data['nostar'] = intval($_GPC['nostar']);
			$data['hasstar'] = intval($_GPC['hasstar']);
		//han 20200327 e
	}
		...
		if ($_GPC['diyformtype'] == 2) {
			...
			if ($_GPC['diysave'] == 1) {
				...
				if (empty($is_save)) {
					...
					pdo_insert('vcshop_diyform_type', $insert);
					$data['diysaveid'] = pdo_insertid();
				}
				else {
					pdo_update('vcshop_diyform_type', $insert, array('id' => $diysaveid));
				}
			}
			...
			if (!empty($opencard)) {
				...
				if (!empty($card)) {
					...
					if ($change) {
						...
						pdo_update('vcshop_goods_cards', $carddata, array('id' => $card['id']));
					}
					...
					pdo_insert('vcshop_goods_cards', $carddata);
					$cardid = pdo_insertid();
				}
	...
	if (empty($id)) {
		...
		$old_data = pdo_fetch('SELECT nocommission FROM ' . tablename('vcshop_goods') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		pdo_update('vcshop_goods', $data, array('id' => $id));
	...
	$param_ids = (array)$_POST['param_id'];
	...
	while ($k < $len) {
		...
		if (!is_numeric($get_param_id)) {
			pdo_insert('vcshop_goods_param', $a);
			$param_id = pdo_insertid();
		}
		else {
			pdo_update('vcshop_goods_param', $a, array('id' => $get_param_id));
	...
	if (0 < count($paramids)) {
		pdo_query('delete from ' . tablename('vcshop_goods_param') . (' where goodsid=' . $id . ' and id not in ( ') . implode(',', $paramids) . ')');
	}
	else {
		pdo_query('delete from ' . tablename('vcshop_goods_param') . (' where goodsid=' . $id));
	}

	$totalstocks = 0;
	$files = $_FILES;
	$spec_ids = (array)$_POST['spec_id'];
	...
	while ($k < $len) {
		...
		if (is_numeric($get_spec_id)) {
			pdo_update('vcshop_goods_spec', $a, array('id' => $get_spec_id));
			$spec_id = $get_spec_id;
		}
		else {
			pdo_insert('vcshop_goods_spec', $a);
			$spec_id = pdo_insertid();
		}

		$spec_item_ids = (array)$_POST['spec_item_id_' . $get_spec_id];
		...
		while ($n < $itemlen) {
			...
			if (is_numeric($get_item_id)) {
				pdo_update('vcshop_goods_spec_item', $d, array('id' => $get_item_id));
				$item_id = $get_item_id;
			}
			else {
		...
		if (0 < count($itemids)) {
			pdo_query('delete from ' . tablename('vcshop_goods_spec_item') . (' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id . ' and id not in (') . implode(',', $itemids) . ')');
		}
		else {
			pdo_query('delete from ' . tablename('vcshop_goods_spec_item') . (' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id));
		}

		pdo_update('vcshop_goods_spec', array('content' => serialize($itemids)), array('id' => $spec_id));
		$specids[] = $spec_id;
		++$k;
	}

	if (0 < count($specids)) {
		pdo_query('delete from ' . tablename('vcshop_goods_spec') . (' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id . ' and id not in (') . implode(',', $specids) . ')');
	}
	else {
		pdo_query('delete from ' . tablename('vcshop_goods_spec') . (' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id));
	}
	...
	while ($k < $len) {
		...
		if (empty($get_option_id)) {
			pdo_insert('vcshop_goods_option', $a);
			$option_id = pdo_insertid();
		}
		else {
			pdo_update('vcshop_goods_option', $a, array('id' => $get_option_id));

			if (p('groups')) {
				pdo_update('vcshop_groups_goods_option', array('title' => $a['title'], 'specs' => $newids), array('goods_option_id' => $get_option_id));
			}
	...
	pdo_update('vcshop_goods', array('discounts' => $discounts_json), array('id' => $id));
	$has_merch = 0;
	...
	pdo_update('vcshop_goods', array('isdiscount_discounts' => $is_discounts_json), array('id' => $id));
	...
	pdo_update('vcshop_goods', $commission_arr, array('id' => $id));
	if (0 < count($optionids) && $data['hasoption'] !== 0) {
		pdo_query('delete from ' . tablename('vcshop_goods_option') . (' where goodsid=' . $id . ' and id not in ( ') . implode(',', $optionids) . ')');

		if (p('groups')) {
			pdo_query('delete from ' . tablename('vcshop_groups_goods_option') . (' where goodsid=' . $id . ' and goods_option_id not in ( ') . implode(',', $optionids) . ')');
		}
		...
		pdo_query('delete from ' . tablename('vcshop_goods_option') . (' where goodsid=' . $id));
		$sql = 'update ' . tablename('vcshop_goods') . (' set minprice = marketprice,maxprice = marketprice where id = ' . $id . ' and hasoption=0;');
		pdo_query($sql);

		if (p('groups')) {
			pdo_query('delete from ' . tablename('vcshop_groups_goods_option') . (' where goodsid=' . $id));
			pdo_update('vcshop_groups_goods', array('more_spec' => 0), array('gid' => $id));
		}
	}

	$sqlgoods = 'SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,total,description,merchsale FROM ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1';
	$goodsinfo = pdo_fetch($sqlgoods, array(':id' => $id, ':uniacid' => $_W['uniacid']));
	$goodsinfo = m('goods')->getOneMinPrice($goodsinfo);
	pdo_update('vcshop_goods', array('minprice' => $goodsinfo['minprice'], 'maxprice' => $goodsinfo['maxprice']), array('id' => $id, 'uniacid' => $_W['uniacid']));
	if ($data['type'] == 3 && $com_virtual) {
		$com_virtual->updateGoodsStock($id);
	}
	else {
		if ($data['hasoption'] !== 0 && $data["totalcnf"] != 3 && empty($data['unite_total'])) {
			pdo_update('vcshop_goods', array('total' => $totalstocks), array('id' => $id));
		}
	}
...
if (!empty($id)) {
	...
	$noticebuyer_type = explode(',', $item['noticebuyer_type']);// han 20200305
	...
	$allspecs = pdo_fetchall('select * from ' . tablename('vcshop_goods_spec') . ' where goodsid=:id order by displayorder asc', array(':id' => $id));

	foreach ($allspecs as &$s) {
		$s['items'] = pdo_fetchall('select a.id,a.specid,a.title,a.thumb,a.show,a.displayorder,a.valueId,a.`virtual`,b.title as title2 from ' . tablename('vcshop_goods_spec_item') . ' a left join ' . tablename('vcshop_virtual_type') . ' b on b.id=a.`virtual`  where a.specid=:specid order by a.displayorder asc', array(':specid' => $s['id']));
	}

	unset($s);
	$params = pdo_fetchall('select * from ' . tablename('vcshop_goods_param') . ' where goodsid=:id order by displayorder asc', array(':id' => $id));
	...
	// 20200513 lin
	$item['blankgroup'] = m('common')->html_to_images($item['blankgroup']);
	$html = '';
	$discounts_html = '';
	$commission_html = '';
	$isdiscount_discounts_html = '';
	$options = pdo_fetchall('select * from ' . tablename('vcshop_goods_option') . ' where goodsid=:id order by id asc', array(':id' => $id));
	$specs = array();
	...
	if ($merchid == 0) {
		...
		if (!empty($item['storeids'])) {
			$stores = pdo_fetchall('select id,storename from ' . tablename('vcshop_store') . ' where id in (' . $item['storeids'] . ' ) and uniacid=' . $_W['uniacid']);
		}
	}
...
$dispatch_data = pdo_fetchall('select * from ' . tablename('vcshop_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
...
if ($com_virtual) {
	$virtual_types = pdo_fetchall('select * from ' . tablename('vcshop_virtual_type') . ' where uniacid=:uniacid and merchid=:merchid and recycled = 0 order by id asc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
}

if ($merchid == 0) {
	$details = pdo_fetchall('select detail_logo,detail_shopname,detail_btntext1, detail_btnurl1 ,detail_btntext2,detail_btnurl2,detail_totaltitle from ' . tablename('vcshop_goods') . ' where uniacid=:uniacid and detail_shopname<>\'\' group by detail_shopname', array(':uniacid' => $_W['uniacid']));
