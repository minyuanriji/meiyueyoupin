...
class Label_VcShopPage extends WebPage
{
	public function main()
	{
		...
		$label = pdo_fetchall('SELECT id,uniacid,label,labelname,status,displayorder FROM ' . tablename('vcshop_goods_label') . '
                WHERE uniacid=:uniacid ' . $condition . ' order by id limit ' . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('vcshop_goods_label') . (' WHERE uniacid=:uniacid ' . $condition), $params);
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT id,uniacid,label,labelname,status,displayorder FROM ' . tablename('vcshop_goods_label') . '
		...
		if ($_W['ispost']) {
			...
			if (!empty($item)) {
				pdo_update('vcshop_goods_label', $data, array('id' => $item['id']));
				plog('goods.label.edit', '修改标签组 ID: ' . $id);
			}
			else {
				$data['uniacid'] = $uniacid;
				pdo_insert('vcshop_goods_label', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,label FROM ' . tablename('vcshop_goods_label') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		...
		foreach ($items as $item) {
			pdo_delete('vcshop_goods_label', array('id' => $item['id']));
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			pdo_update('vcshop_goods_label', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function query()
	{
		...
		$labels = pdo_fetchall('SELECT id,label,labelname FROM ' . tablename('vcshop_goods_label') . (' WHERE 1 ' . $condition . ' order by id desc'), $params);
	...
	public function labelfile()
	{
		...
		$labels = pdo_fetch('SELECT id,label,labelname FROM ' . tablename('vcshop_goods_label') . (' WHERE 1 ' . $condition . ' order by id desc'), $params);
	...
	public function style()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($style)) {
				pdo_update('vcshop_goods_labelstyle', $data, array('uniacid' => $uniacid));
				plog('goods.labelstyle.edit', '修改标签组样式');
			}
			else {
				$data['uniacid'] = $uniacid;
				pdo_insert('vcshop_goods_labelstyle', $data);
				$id = pdo_insertid();
