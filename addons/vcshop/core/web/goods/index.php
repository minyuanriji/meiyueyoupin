...
class Index_VcShopPage extends WebPage
{
	public function main($goodsfrom = 'sale')
	{
		...
		if (!empty($_GPC['keyword'])) {
			...
			$sqlcondition = ' left join ' . tablename('vcshop_goods_option') . ' op on g.id = op.goodsid';

			if ($merch_plugin) {
				$sqlcondition .= ' left join ' . tablename('vcshop_merch_user') . ' merch on merch.id = g.merchid and merch.uniacid=g.uniacid';
			}
		...
		if (p('merch')) {
			$merchSql = ' LEFT JOIN ' . tablename('vcshop_merch_user') . ' merch_user ON merch_user.id = g.merchid ';
			$condition .= ' AND (g.merchid = 0 or (g.merchid > 0 and merch_user.accounttime > ' . time() . ')) ';
		}

		$sql = 'SELECT g.id FROM ' . tablename('vcshop_goods') . 'g ' . $merchSql . $sqlcondition . $condition . $groupcondition;
		$total_all = pdo_fetchall($sql, $params);
		$total = count($total_all);
		unset($total_all);

		if (!empty($total)) {
            $sql = "SELECT g.* FROM " . tablename("vcshop_goods") . "g " . $merchSql . $sqlcondition . $condition . $groupcondition . " ORDER BY g.`status` DESC, g.`displayorder` DESC,\n                g.`id` DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize;
            $sql1 = "SELECT g.* FROM " . tablename("vcshop_goods") . "g " . $merchSql . $sqlcondition . $condition . $groupcondition . " ORDER BY g.`status` DESC, g.`displayorder` DESC,\n                g.`id` DESC ";
			...
			foreach ($list as $key => &$value) {
				...
				$sale_cpcount = pdo_fetch('SELECT sum(og.total)  as sale_count   FROM ims_vcshop_order_goods  og LEFT JOIN ims_vcshop_order o on og.orderid=o.id  WHERE og.goodsid=:gsid and o.`status`>=:status and o.refundstate = 0 and og.uniacid=:uniacid', array(':gsid' => $value['id'], ':status' => 1, ':uniacid' => $_W['uniacid']));
            ...
            if ($_GPC["export"] == 1) {
                foreach ($list1 as $k => $v) {
                    ...
                    if ($v["isnodiscount"] == 1) {
                        $list1[$k]["shuxing"] .= " 不参与折扣";
                    }
                }
                m("excel")->export($list1, array("title" => "商品列表明细-" . date("Y-m-d-H-i", time()), "columns" => array(array("title" => "商品名称", "field" => "title", "width" => 24), array("title" => "商品价格", "field" => "marketprice", "width" => 12), array("title" => "商品库存", "field" => "total", "width" => 24), array("title" => "销量", "field" => "salesreal", "width" => 12), array("title" => "实际销量", "field" => "sale_cpcount", "width" => 12), array("title" => "属性", "field" => "shuxing", "width" => 64))));
	...
	public function create()
	{
		...
		if ($com_virtual) {
			$virtual_types = pdo_fetchall('select * from ' . tablename('vcshop_virtual_type') . ' where uniacid=:uniacid and merchid=:merchid and recycled = 0 order by id asc', array(':uniacid' => $_W['uniacid'], ':merchid' => 0));
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_goods') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('vcshop_goods', array('deleted' => 1), array('id' => $item['id']));
	...
	public function status()
	{
		...
		pdo_query('update ' . tablename('vcshop_goods') . (' set newgoods = 0 where id in ( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		$items = pdo_fetchall('SELECT id,title,status,isstatustime,statustimestart,statustimeend FROM ' . tablename('vcshop_goods') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		...
		foreach ($items as $item) {
			...
			pdo_update('vcshop_goods', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function checked()
	{
		...
		foreach ($items as $item) {
			pdo_update('vcshop_goods', array('checked' => intval($_GPC['checked'])), array('id' => $item['id']));
	...
	public function delete1()
	{
		...
		foreach ($items as $item) {
			pdo_delete('vcshop_goods', array('id' => $item['id']));
	...
	public function restore()
	{
		...
		foreach ($items as $item) {
			pdo_update('vcshop_goods', array('deleted' => 0), array('id' => $item['id']));
	...
	public function property()
	{
		...
		if (in_array($type, array('new', 'hot', 'recommand', 'discount', 'time', 'sendfree', 'nodiscount'))) {
			pdo_update('vcshop_goods', array('is' . $type => $data), array('id' => $id, 'uniacid' => $_W['uniacid']));
		...
		if (in_array($type, array('status'))) {
			pdo_update('vcshop_goods', array($type => $data), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function change()
	{
		...
		$goods = pdo_fetch('select id,hasoption,marketprice,dowpayment,`type`,isdiscount,isdiscount_time from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		$result = pdo_update('vcshop_goods', array($type => $value), array('id' => $id));
		...
		if ($goods['hasoption'] == 0 && !in_array($type, array('displayorder', 'total'))) {
			if ($goods['isdiscount'] == 0 || $goods['isdiscount_time'] < time()) {
				$sql = 'update ' . tablename('vcshop_goods') . (' set minprice = marketprice,maxprice = marketprice where id = ' . $goods['id'] . ' and hasoption=0;');
				pdo_query($sql);
			}
			else {
				$sqlgoods = 'SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,total,description,merchsale FROM ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1';
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice,costprice,total,sales,islive,liveprice FROM ' . tablename('vcshop_goods') . (' WHERE 1 ' . $condition . ' order by createtime desc'), $params);
	...
	public function goodsprice()
	{
		global $_W;
		$sql = 'update ' . tablename('vcshop_goods') . ' g set 
g.minprice = (select min(marketprice) from ' . tablename('vcshop_goods_option') . ' where g.id = goodsid),
g.maxprice = (select max(marketprice) from ' . tablename('vcshop_goods_option') . ' where g.id = goodsid)
where g.hasoption=1 and g.uniacid=' . $_W['uniacid'] . ';
update ' . tablename('vcshop_goods') . ' set minprice = marketprice,maxprice = marketprice where hasoption=0 and uniacid=' . $_W['uniacid'] . ';';
		pdo_run($sql);
		show_json(1);
	}
	...
	public function ajax_batchcates()
	{
		...
		foreach ($goodsids as $goodsid) {
			if (!empty($iscover)) {
				...
				$goods = pdo_fetch('select pcate,ccate,tcate,cates,pcates,ccates,tcates  from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
