...
class Open_VcShopPage extends WebPage
{
	public function main()
	{
		...
		$key = pdo_fetch('SELECT * FROM ' . tablename('vcshop_open_plugin') . ' WHERE plugin = :plugin', array(':plugin' => $plugin));
		...
		if ($_W['ispost']) {
			...
			if ($key) {
				...
				$num = pdo_get('vcshop_open_plugin', array('plugin' => $plugin), 'status');

				if (!empty($num)) {
					pdo_update('vcshop_open_plugin', array('status' => 1, 'expirtime' => time() + 172800), array('plugin' => $plugin));
				...
				pdo_insert('vcshop_open_plugin', $data);
				$id = pdo_insertid();
	...
	private function checkOpen($key = '', $plugin = '', $domain = '')
	{
		...
		$resp = ihttp_post(VCSHOP_AUTH_URL . '/grant', $data);
