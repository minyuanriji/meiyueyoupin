...
class List_VcShopPage extends WebPage
{
    protected function orderData($status, $st, $index)
    {
        ...
        if ($order_type !== "") {
            ...
            if ($order_type == "general") {
                if (in_array($searchfield, array("goodstitle", "goodssn", "goodsoptiontitle"))) {
                    ...
                    $fullback_sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename("vcshop_order_goods") . " og  where og.uniacid = '" . $uniacid . "' and og.fullbackid=0) gs on gs.orderid=o.id";
                }
            } else {
                if ($order_type == "fullback") {
                    ...
                    if (in_array($searchfield, array("goodstitle", "goodssn", "goodsoptiontitle"))) {
                        ...
                        $fullback_sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename("vcshop_order_goods") . " og  where og.uniacid = '" . $uniacid . "' and og.fullbackid<>0 " . $fullback_goods_where . ") gs on gs.orderid=o.id";
                    }
                }
            }
        }
                                    ...
                                    if ($searchfield == "saler") {
                                        ...
                                        if ($searchfield == "verifycode") {
                                            ...
                                            if ($searchfield == "store") {
                                                if (p("merch")) {
                                                    $sqlcondition = " left join " . tablename("vcshop_store") . " store on store.id = o.verifystoreid and store.uniacid=o.uniacid left join " . tablename("vcshop_merch_store") . " merstore on merstore.id = o.verifystoreid and merstore.uniacid=o.uniacid";
                                                    $condition .= " AND (locate(:keyword,store.storename)>0 or locate(:keyword,merstore.storename)>0)";
                                                } else {
                                                    $sqlcondition = " left join " . tablename("vcshop_store") . " store on store.id = o.verifystoreid and store.uniacid=o.uniacid ";
                                                    $condition .= " AND (locate(:keyword,store.storename)>0)";
                                                }
                                            } else {
                                                if ($searchfield == "goodstitle") {
                                                    $sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename("vcshop_order_goods") . " og left join " . tablename("vcshop_goods") . " g on g.id=og.goodsid where og.uniacid = '" . $uniacid . "' and (locate(:keyword,g.title)>0) " . $fullback_where . ") gs on gs.orderid=o.id";
                                                } else {
                                                    if ($searchfield == "goodssn") {
                                                        $sqlcondition = " inner join ( select DISTINCT(og.orderid) from " . tablename("vcshop_order_goods") . " og left join " . tablename("vcshop_goods") . " g on g.id=og.goodsid where og.uniacid = '" . $uniacid . "' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0)) " . $fullback_where . ") gs on gs.orderid=o.id";
                                                    } else {
                                                        if ($searchfield == "goodsoptiontitle") {
                                                            $sqlcondition = " inner join ( select  DISTINCT(og.orderid) from " . tablename("vcshop_order_goods") . " og left join " . tablename("vcshop_goods") . " g on g.id=og.goodsid where og.uniacid = '" . $uniacid . "' and (locate(:keyword,og.optionname)>0) " . $fullback_where . ") gs on gs.orderid=o.id";
                                                        } else {
                                                            if ($searchfield == "merch") {
                                                                if ($merch_plugin) {
                                                                    $condition .= " AND (locate(:keyword,merch.merchname)>0)";
                                                                    $sqlcondition = " left join " . tablename("vcshop_merch_user") . " merch on merch.id = o.merchid and merch.uniacid=o.uniacid";
                                                                }
                                                            } else {
                                                                if ($searchfield == "selfget") {
                                                                    $condition .= " AND (locate(:keyword,store.storename)>0)";
                                                                    $sqlcondition = " left join " . tablename("vcshop_store") . " store on store.id = o.storeid and store.uniacid=o.uniacid";
                                                                }
                                                            }
                                                        }
                                                    }
        ...
        if (!empty($agentid) && 0 < $level) {
            ...
            if (empty($olevel)) {
                ...
                // han 20200113 改成显示整个团队的订单
                $alldowns = pdo_fetchall('select id from ' . tablename('vcshop_commission_relation') . ' where pid=:pid',array(':pid'=>$agentid),'id');
                if(!empty($alldowns)){
                    $condition .= " or o.agentid in( " . implode(",", array_keys($alldowns)) . ")";
                }
                // if ($cset["selfbuy"] == 1) {
        ...
        if ($condition != " o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 " || !empty($sqlcondition)) {
            if ($searchfield == "member") {
                ...
                $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,\n                      d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,m.uid,m.groupid,\n                      r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . tablename("vcshop_order") . " o" . " left join " . tablename("vcshop_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("vcshop_member") . " m on m.openid=o.openid and m.uniacid =  o.uniacid " . " left join " . tablename("vcshop_member_address") . " a on a.id=o.addressid " . " left join " . tablename("vcshop_dispatch") . " d on d.id = o.dispatchid " . " " . $sqlcondition . " where " . $condition . " " . $statuscondition . " ORDER BY " . $orderbuy . " DESC  ";
            } else {
                if ($searchfield == "saler") {
                    $condition .= " AND (locate(:keyword,sm.realname)>0 or locate(:keyword,sm.mobile)>0 or locate(:keyword,sm.nickname)>0 or locate(:keyword,s.salername)>0 )";
                    $sql = "select o.* ,sm.groupid, a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,\n                      d.dispatchname,\n                      r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . tablename("vcshop_order") . " o" . " left join " . tablename("vcshop_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("vcshop_member_address") . " a on a.id=o.addressid " . " left join " . tablename("vcshop_dispatch") . " d on d.id = o.dispatchid " . " left join " . tablename("vcshop_saler") . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid" . " left join " . tablename("vcshop_member") . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid" . " " . $sqlcondition . " where " . $condition . " " . $statuscondition . " ORDER BY " . $orderbuy . " DESC  ";
                } else {
                    if ($searchfield == "mid") {
                        $paras[":keyword"] = htmlspecialchars_decode($_GPC["keyword"], ENT_QUOTES);
                        $condition .= " AND (m.id=:keyword)";
            ...
            $sql = "select * from " . tablename("vcshop_order") . " where uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 " . $status_condition . " GROUP BY id ORDER BY createtime DESC  ";
            $sql .= "LIMIT " . ($pindex - 1) * $psize . "," . $psize;
            $list = pdo_fetchall($sql, $paras);
            if (!empty($list)) {
                $refundid = "";
                ...
                $refundid_array = pdo_fetchall("SELECT id,rtype,status as rstatus FROM " . tablename("vcshop_order_refund") . " WHERE id IN (" . $refundid . ")", array(), "id");
                $openid_array = pdo_fetchall("SELECT openid,nickname,id,groupid as mid,realname as mrealname,uid,mobile as mmobile FROM " . tablename("vcshop_member") . " WHERE openid IN (" . $openid . ") AND uniacid=" . $_W["uniacid"], array(), "openid");
                $addressid_array = pdo_fetchall("SELECT id,realname as arealname,mobile as amobile,province as aprovince ,city as acity , area as aarea,address as aaddress FROM " . tablename("vcshop_member_address") . " WHERE id IN (" . $addressid . ")", array(), "id");
                $dispatchid_array = pdo_fetchall("SELECT id,dispatchname FROM " . tablename("vcshop_dispatch") . " WHERE id IN (" . $dispatchid . ")", array(), "id");
                $verifyopenid_array = pdo_fetchall("SELECT sm.id as salerid,sm.nickname as salernickname,sm.openid,s.salername FROM " . tablename("vcshop_saler") . " s LEFT JOIN " . tablename("vcshop_member") . " sm ON sm.openid = s.openid and sm.uniacid=s.uniacid WHERE s.openid IN (" . $verifyopenid . ")", array(), "openid");
                foreach ($list as $key => &$value) {
        ...
        $gdata = pdo_fetchall("select * from " . tablename("vcshop_member_group") . " where uniacid = " . $uniacid);
        $paytype = array(array("css" => "default", "name" => "未支付"), array("css" => "danger", "name" => "余额支付"), 11 => array("css" => "default", "name" => "后台付款"), 2 => array("css" => "danger", "name" => "在线支付"), 21 => array("css" => "success", "name" => "微信支付"), 33 => array("css" => "success", "name" => "合利宝微信支付"), 22 => array("css" => "warning", "name" => "支付宝支付"), 23 => array("css" => "warning", "name" => "银联支付"), 3 => array("css" => "primary", "name" => "货到付款"), 4 => array("css" => "primary", "name" => "收银台现金收款"));
        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                if (!empty($value["refundid"]) && !empty($_GPC["export"])) {
                    $refund_apply = pdo_fetch("select id,status,applyprice from " . tablename("vcshop_order_refund") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $value["refundid"], ":uniacid" => $_W["uniacid"]));
                ...
                $commission2 = -1;
                $commission3 = -1;
                $commission_team_bonus = -1;// han 20190618
                $commission_team_bonus_data = array();// han 20190618
                $commission_star_bonus = -1;// han 20200402
                $commission_star_bonus_data = array();// han 20200402
                $m1 = false;
                $m2 = false;
                $m3 = false;
                $m_team_bonus = false;// han 20190618
                $m_star_bonus = false;// han 20200402
                ...
                if (!empty($level) && empty($agentid) && !empty($value["agentid"])) {
                    ...
                    // han 20190618 团队分红 s
                    $reward_logs = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_rewardlog') . ' WHERE uniacid=:uniacid AND orderid=:orderid ORDER BY id ASC,floot ASC',array(':uniacid' => $_W['uniacid'], ':orderid' => $value['id']));
                    if(!empty($reward_logs) && is_array($reward_logs)){
                        $commission_express = 0;
                        $commission_team_bonus = 0;
                        foreach ($reward_logs as $team) {
                            $team_user = pdo_fetch('SELECT id,nickname FROM ' . tablename('vcshop_member') . ' WHERE uniacid=:uniacid AND id=:id',array(':uniacid' => $_W['uniacid'], ':id' => $team['agentid']));
                            if($team_user){
                                if($team['type'] == 2){
                                    $commission_team_bonus += $team['money'];
                                    $commission_team_bonus_data[$team['floot']] = array('user'=>$team_user, 'money' => $team['money']);
                                }
                                // han 20200402 提成
                                if($team['type'] == 3){
                                    $commission_star_bonus += $team['money'];
                                    $commission_star_bonus_data[$team['floot']] = array('user'=>$team_user, 'money' => $team['money']);
                                }
                            }
                        }
                    }
                ...
                if (!empty($level) && empty($agentid)) {
                    ...
                    $value["commission_team_bonus"] = $commission_team_bonus;// han 20190618
                    $value["commission_team_bonus_data"] = $commission_team_bonus_data;// han 20190618

                    $value["commission_star_bonus"] = $commission_star_bonus;// han 20200402
                    $value["commission_star_bonus_data"] = $commission_star_bonus_data;// han 20200402
                }
                $value["goods"] = set_medias($order_goods, "thumb");
                $value["goods_str"] = $goods;
                if (!empty($agentid) && 0 < $level) {
                    $commission_level = 0;
                    $commission_level_team_bonus = 0;//han 20190618
                    $commission_level_team_bonus_data = array();//han 20190618
                    $commission_level_star_bonus = 0;//han 20200402
                    $commission_level_star_bonus_data = array();//han 20200402
                    // han 20190618 快递&&团队级差分红 s
                    $reward_agent = pdo_fetch('SELECT id,nickname,openid FROM ' . tablename('vcshop_member') . ' WHERE uniacid=:uniacid AND id=:id',array(':uniacid' => $_W['uniacid'], ':id' => $agentid));
                    if($reward_agent){
                        $reward_logs = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_rewardlog') . ' WHERE uniacid=:uniacid AND orderid=:orderid AND agentid=:agentid ORDER BY id ASC,floot ASC',array(':uniacid' => $_W['uniacid'], ':orderid' => $value['id'], ':agentid' => $reward_agent['id']));
                        if(!empty($reward_logs) && is_array($reward_logs)){
                            foreach ($reward_logs as $team) {
                                if($team['type'] == 2){
                                    $commission_level_team_bonus += $team['money'];
                                    $commission_level_team_bonus_data[$team['floot']] = array('user'=>$reward_agent, 'money' => $team['money']);
                                }
                                // han 20200402 提成
                                if($team['type'] == 3){
                                    $commission_level_star_bonus += $team['money'];
                                    $commission_level_star_bonus_data[$team['floot']] = array('user'=>$reward_agent, 'money' => $team['money']);
                                }
                            }
                        }
                    }
                    // han 20190618 快递&&团队级差分红 e
                    if ($value["agentid"] == $agentid) {
                        $value["level"] = 1;
                        $level1_commissions = pdo_fetchall("select commission1,commissions  from " . tablename("vcshop_order_goods") . " og " . " left join  " . tablename("vcshop_order") . " o on o.id = og.orderid " . " where og.orderid=:orderid and o.agentid= " . $agentid . "  and o.uniacid=:uniacid", array(":orderid" => $value["id"], ":uniacid" => $uniacid));
                        ...
                        // han 20200113 add !empty($agent["level1_agentids"])
                        if (!empty($agent["level1_agentids"]) && in_array($value["agentid"], array_keys($agent["level1_agentids"]))) {
                            $value["level"] = 2;
                            if (0 < $agent["level2"]) {
                                $level2_commissions = pdo_fetchall("select commission2,commissions  from " . tablename("vcshop_order_goods") . " og " . " left join  " . tablename("vcshop_order") . " o on o.id = og.orderid " . " where og.orderid=:orderid and  o.agentid in ( " . implode(",", array_keys($agent["level1_agentids"])) . ")  and o.uniacid=:uniacid", array(":orderid" => $value["id"], ":uniacid" => $uniacid));
                            ...
                            // han 20200113 add !empty($agent["level1_agentids"])
                            if (!empty($agent["level2_agentids"]) && in_array($value["agentid"], array_keys($agent["level2_agentids"]))) {
                                $value["level"] = 3;
                                if (0 < $agent["level3"]) {
                                    $level3_commissions = pdo_fetchall("select commission3,commissions from " . tablename("vcshop_order_goods") . " og " . " left join  " . tablename("vcshop_order") . " o on o.id = og.orderid " . " where og.orderid=:orderid and  o.agentid in ( " . implode(",", array_keys($agent["level2_agentids"])) . ")  and o.uniacid=:uniacid", array(":orderid" => $value["id"], ":uniacid" => $uniacid));
                    ...
                    // han 20191122 团队分红
                    $value['commission_level_team_bonus'] = $commission_level_team_bonus;
                    $value['commission_level_team_bonus_data'] = $commission_level_team_bonus_data;
                    // han 20200402 提成
                    $value['commission_level_star_bonus'] = $commission_level_star_bonus;
                    $value['commission_level_star_bonus_data'] = $commission_level_star_bonus_data;
                }
                $is_peerpay = 0;
            ...
            unset($value);//han 20191122 上面循环用了引用，unset一下
            $oopenid = "'" . implode("','", $openids) . "'";
            $verifyopenid = "'" . implode("','", $verifyopenids) . "'";
            $omember = pdo_fetchall("select openid,nickname,id as mid,realname as mrealname,mobile as mmobile,uid from " . tablename("vcshop_member") . " where openid in(" . $oopenid . ") and uniacid = :uniacid", array(":uniacid" => $_W["uniacid"]), "openid");
            $verifyopenid_array = pdo_fetchall("SELECT sm.id as salerid,sm.nickname as salernickname,sm.openid,s.salername FROM " . tablename("vcshop_saler") . " s LEFT JOIN " . tablename("vcshop_member") . " sm ON sm.openid = s.openid and sm.uniacid=s.uniacid WHERE s.openid IN (" . $verifyopenid . ") and s.uniacid = :uniacid", array(":uniacid" => $_W["uniacid"]), "openid");
        ...
        if ($_GPC["export"] == 1) {
            ...
            foreach ($list as &$row) {
                ...
                if (!empty($row["verifystoreid"])) {
                    if (0 < $row["merchid"]) {
                        $row["storeinfo"] = pdo_fetchcolumn("select storename from " . tablename("vcshop_merch_store") . " where id=:storeid limit 1 ", array(":storeid" => $row["verifystoreid"]));
                    } else {
                        $row["storeinfo"] = pdo_fetchcolumn("select storename from " . tablename("vcshop_store") . " where id=:storeid limit 1 ", array(":storeid" => $row["verifystoreid"]));
                    }
                } else {
                    if (!empty($row["verifyinfo"])) {
                        $verifyinfo = iunserializer($row["verifyinfo"]);
                ...
                if (!empty($row["storeid"])) {
                    if (0 < $row["merchid"]) {
                        $row["pickname"] = pdo_fetchcolumn("select storename from " . tablename("vcshop_merch_store") . " where id=:storeid limit 1 ", array(":storeid" => $row["storeid"]));
                    } else {
                        $row["pickname"] = pdo_fetchcolumn("select storename from " . tablename("vcshop_store") . " where id=:storeid limit 1 ", array(":storeid" => $row["storeid"]));
                    }
                }
                ...
                if ($row["isverify"] && is_array($verifyinfo) && empty($row["dispatchtype"])) {
                    ...
                    if ($v["verified"] || $row["verifytype"] == 1) {
                        ...
                        $v["salername"] = pdo_fetchcolumn("select salername from " . tablename("vcshop_merch_saler") . " where openid=:openid and uniacid=:uniacid and merchid = :merchid limit 1", array(":openid" => $v["verifyopenid"], ":uniacid" => $_W["uniacid"], ":merchid" => $_W["merchid"]));
                ...
                foreach ($row["goods"] as $k => $g) {
                    ...
                    $res_multi_arr = pdo_fetchall("select applyprice,rtype,status from " . tablename("vcshop_order_single_refund") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $g["single_refundid"], ":uniacid" => $_W["uniacid"]));
            ...
            m("excel")->exportCSV($exportlist, array("title" => "订单数据", "columns" => $columns), VCSHOP_DATA . "order/", $index, $exflag);
            unset($exportlist);
            unset($list);
            if (!$exflag) {
                $pindex++;
            ...
            if ($condition != " o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0" || !empty($sqlcondition)) {
                if ($searchfield == "member") {
                    ...
                    $openidArr = pdo_fetchall("SELECT openid FROM " . tablename("vcshop_member") . " WHERE uniacid = " . $_W["uniacid"] . $priceCondition1);
                    if (!empty($openidArr)) {
                        foreach ($openidArr as $openid) {
                            $openids[] = $openid["openid"];
                        }
                        $inOpenid = "'" . implode("','", $openids) . "'";
                        $orderPrice = pdo_fetch("SELECT count(1) as count,sum(price) as sumprice FROM " . tablename("vcshop_order") . " o WHERE o.uniacid = " . $_W["uniacid"] . " AND o.deleted=0 AND o.isparent=0 AND o.openid IN (" . $inOpenid . ")" . $timeCondition . $statuscondition);
                    } else {
                    ...
                    if ($searchfield == "address") {
                        $orderPrice = pdo_fetch("SELECT count(1) as count,sum(o.price) as sumprice FROM " . tablename("vcshop_order") . " o left join " . tablename("vcshop_member_address") . " a on o.addressid = a.id WHERE o.uniacid = " . $_W["uniacid"] . "  AND o.deleted=0 AND o.isparent=0 " . $priceCondition . $statuscondition);
                        $totalmoney = $orderPrice["sumprice"];
                        $total = $orderPrice["count"];
                        ...
                        if ($searchfield == "location") {
                            ...
                            if ($status === "" && empty($_GPC["keyword"]) && empty($_GPC["time"])) {
                                $redis = redis();
                                if (!empty($heads)) {
                                    $redis_key_total = $_W["uniacid"] . "_vcshop_order_list_total" . $heads;
                                    $redis_key_totalmoney = $_W["uniacid"] . "_vcshop_order_list_totalmoney" . $heads;
                                }
                                if (!is_error($redis)) {
                                    if (false && $redis->get($redis_key_total) != false && $redis->get($redis_key_totalmoney) != false) {
                                        ...
                                        $sumprice = 0;
                                        if (empty($agentid)) {
                                            $t = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM " . tablename("vcshop_order") . " o " . " left join " . tablename("vcshop_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("vcshop_saler") . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid" . " left join " . tablename("vcshop_member") . " m on m.openid = s.openid and m.uniacid=s.uniacid" . " " . $sqlcondition . " WHERE " . $condition . " " . $statuscondition, $paras);
                                            $total = $t["count"];
                                ...
                                $t = pdo_fetch("SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM " . tablename("vcshop_order") . " o " . " left join " . tablename("vcshop_order_refund") . " r on r.id =o.refundid " . " left join " . tablename("vcshop_saler") . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid" . " left join " . tablename("vcshop_member") . " m on m.openid = o.openid and m.uniacid=o.uniacid" . " " . $sqlcondition . " WHERE " . $condition . " " . $statuscondition, $paras);
                                $total = $t["count"];
                                $totalmoney = $t["sumprice"];
                            }
                        }
                ...
                $t = pdo_fetch("SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM " . tablename("vcshop_order") . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 " . $status_condition, $paras);
                $total = $t["count"];
                $totalmoney = $t["sumprice"];
            }
            $pager = pagination2($total, $pindex, $psize);
            $stores = pdo_fetchall("select id,storename from " . tablename("vcshop_store") . " where uniacid=:uniacid ", array(":uniacid" => $uniacid));
            $r_type = array("退款", "退货退款", "换货");
            load()->func("tpl");
    ...
    public function updateChildOrderPay()
    {
        ...
        $sql = "select id,parentid from " . tablename("vcshop_order") . " where parentid>0 and status>0 and paytype=0 and uniacid=:uniacid";
        $list = pdo_fetchall($sql, $params);
        if (!empty($list)) {
            foreach ($list as $k => $v) {
                $params[":orderid"] = $v["parentid"];
                $sql1 = "select paytype from " . tablename("vcshop_order") . " where id=:orderid and status>0 and paytype>0 and uniacid=:uniacid";
                $item = pdo_fetch($sql1, $params);
                if (0 < $item["paytype"]) {
                    pdo_update("vcshop_order", array("paytype" => $item["paytype"]), array("id" => $v["id"]));
                }
            }
        }
    }
