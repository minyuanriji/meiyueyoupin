...
class Op_VcShopPage extends WebPage
{
	public function delete()
	{
		...
		pdo_update('vcshop_order', array('deleted' => 1), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	protected function opData()
	{
		...
		$order_goods = pdo_fetchall('select single_refundstate from ' . tablename('vcshop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function pay($a = array(), $b = array())
	{
		...
		if (!empty($item['virtual']) && com('virtual')) {
			...
			pdo_update('vcshop_order', $updateorder, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function close()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $item['refundstate'] && !empty($item['refundid'])) {
				...
				pdo_update('vcshop_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid']));
			}
			...
			pdo_update('vcshop_order', array('status' => -1, 'refundstate' => 0, 'canceltime' => $time, 'remarkclose' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			if (!empty($item['virtual']) && $item['virtual'] != 0) {
				$goodsid = pdo_fetch('SELECT goodsid FROM ' . tablename('vcshop_order_goods') . ' WHERE uniacid = ' . $_W['uniacid'] . ' AND orderid = ' . $item['id']);
				$typeid = $item['virtual'];
				...
				$vkeynum = count($arr);
				pdo_query('update ' . tablename('vcshop_virtual_data') . ' set openid="",usetime=0,orderid=0,ordersn="",price=0,merchid=' . $item['merchid'] . ' where typeid=' . intval($typeid) . ' and orderid = ' . $item['id']);
				pdo_query('update ' . tablename('vcshop_virtual_type') . ' set usedata=usedata-' . $vkeynum . ' where id=' . intval($typeid));
			}
	...
	public function paycancel()
	{
		...
		if ($_W['ispost']) {
			m('order')->setStocksAndCredits($item['id'], 2);
			pdo_update('vcshop_order', array('status' => 0, 'cancelpaytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function fetchcancel()
	{
		...
		pdo_update('vcshop_order', array('status' => 1, 'finishtime' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
...
' . $remark;
			...
			if (0 < $item['sendtype']) {
				...
				$data['sendtype'] = 0;
				pdo_update('vcshop_order_goods', $data, array('orderid' => $item['id'], 'sendtype' => $sendtype, 'uniacid' => $_W['uniacid']));
		...
		if (0 < $sendtype) {
			...
			if (0 < $item['sendtype']) {
				...
				while ($i <= intval($item['sendtype'])) {
					$bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $i . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function send()
	{
			...
			if ($item['city_express_state'] == 0) {
				...
				if (intval($_GPC['sendtype']) == 1 || 0 < $item['sendtype']) {
					...
					foreach ($goodsid as $key => $value) {
						pdo_update('vcshop_order_goods', $data, array('id' => $value, 'uniacid' => $_W['uniacid']));
					}
		...
		if (0 < $item['sendtype']) {
			$noshipped = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = 0 and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$i = 1;

			while ($i <= $item['sendtype']) {
				$shipped[$i]['sendtype'] = $i;
				$shipped[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = ' . $i . ' and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
				++$i;
			}
		}

		$order_goods = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid and og.single_refundtime=0', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function remarksaler()
	{
		...
		if ($_W['ispost']) {
			pdo_update('vcshop_order', array('remarksaler' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($expresssn)) {
				...
				if (0 < $item['sendtype']) {
					...
					pdo_update('vcshop_order_goods', $change_data, array('orderid' => $id, 'sendtype' => $sendtype, 'uniacid' => $_W['uniacid']));
				}
				else {
					pdo_update('vcshop_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				}
	...
	public function changeaddress()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				$item = pdo_fetch('SELECT id, ordersn, address,openid FROM ' . tablename('vcshop_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('vcshop_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function upload_invoice()
	{
		...
		if (!$_W['ispost']) {
			$invoice = pdo_fetch('select invoicename,invoice_img from ' . tablename('vcshop_order') . ' where id = :order_id and uniacid = :uniacid limit 1', array(':order_id' => $order_id, ':uniacid' => $_W['uniacid']));
		...
		$update_ret = pdo_update('vcshop_order', array('invoice_img' => $invoice_img), array('id' => $order_id));
	...
	public function peerpay()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('vcshop_order_peerpay_payinfo') . ' where pid=:pid';
