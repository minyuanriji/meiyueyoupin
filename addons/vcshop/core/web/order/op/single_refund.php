...
class Single_Refund_VcShopPage extends WebPage
{
	protected function opData()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('select o.*,og.id as ordergoodsid,og.single_refundid,og.single_refundstate,og.single_refundtime from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on o.id=og.orderid' . ' where og.id=:ordergoodsid limit 1', array(':ordergoodsid' => $id));
		...
		if (!empty($item['single_refundid'])) {
			$refund = pdo_fetch('select * from ' . tablename('vcshop_order_single_refund') . ' where id=:id limit 1', array(':id' => $item['single_refundid']));
	...
	public function submit()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($refund['refundno'])) {
				...
				pdo_update('vcshop_order_single_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
			}
			...
			if ($refundstatus == 0) {
				...
				if ($raid == 0) {
					$raddress = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where isdefault=1 and uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $uniacid));
				}
				else {
					$raddress = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where id=:id and uniacid=:uniacid and merchid=0 limit 1', array(':id' => $raid, ':uniacid' => $uniacid));
				}

				if (empty($raddress)) {
					$raddress = pdo_fetch('select * from ' . tablename('vcshop_refund_address') . ' where uniacid=:uniacid and merchid=0 order by id desc limit 1', array(':uniacid' => $uniacid));
				}
				...
				pdo_update('vcshop_order_single_refund', $change_refund, array('id' => $item['single_refundid']));
				...
				pdo_update('vcshop_order_single_refund', $refund_data, array('id' => $item['single_refundid'], 'uniacid' => $uniacid));
				$order_data = array();
				$order_data['single_refundstate'] = 9;
				pdo_update('vcshop_order_goods', $order_data, array('id' => $item['ordergoodsid'], 'uniacid' => $uniacid));
				m('notice')->sendOrderMessage($item['id'], true);
			}
			else if ($refundstatus == 1) {
				if (0 < $item['parentid']) {
					$parent_item = pdo_fetch('SELECT id,ordersn,ordersn2,price,transid,paytype,apppay FROM ' . tablename('vcshop_order') . ' WHERE id = :id and uniacid=:uniacid Limit 1', array(':id' => $item['parentid'], ':uniacid' => $_W['uniacid']));
				...
				pdo_update('vcshop_order_single_refund', array('reply' => $refundcontent, 'status' => -1, 'endtime' => $time), array('id' => $item['single_refundid']));
				plog('order.op.refund.submit', '订单退款拒绝 ID: ' . $item['id'] . ' 订单号: ' . $item['ordersn'] . ' 原因: ' . $refundcontent);
				pdo_update('vcshop_order_goods', array('single_refundstate' => 8), array('id' => $item['ordergoodsid'], 'uniacid' => $uniacid));
				$is_single_refund = pdo_fetchcolumn('select count(id) from ' . tablename('vcshop_order_goods') . 'where orderid=:orderid and (single_refundstate=1 or single_refundstate=2)', array(':orderid' => $item['id']));

				if (empty($is_single_refund)) {
					pdo_update('vcshop_order', array('refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));
				}
		...
		$refund_address = pdo_fetchall('select * from ' . tablename('vcshop_refund_address') . ' where uniacid=:uniacid and merchid=0', array(':uniacid' => $_W['uniacid']));
	...
	public function refund_after($item, $time, $shopset)
	{
		global $_W;
		global $_GPC;
		$goods = pdo_fetch('SELECT og.goodsid,og.total,g.totalcnf,og.realprice,g.money,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.credit,og.seckill,og.consume FROM ' . tablename('vcshop_order_goods') . 'og' . ' left join ' . tablename('vcshop_goods') . ' g on og.goodsid=g.id ' . ' WHERE og.id=:ordergoodsid and og.uniacid=:uniacid limit 1', array(':ordergoodsid' => $item['ordergoodsid'], ':uniacid' => $_W['uniacid']));
		...
		pdo_update('vcshop_order_goods', array('single_refundstate' => 9, 'single_refundtime' => $time, 'nocommission' => 1), array('id' => $item['ordergoodsid'], 'uniacid' => $_W['uniacid']));
		$order_goods = pdo_fetchall('select single_refundid,single_refundstate,single_refundtime from ' . tablename('vcshop_order_goods') . ' where orderid=:orderid', array(':orderid' => $item['id']));
		$refund_num = 0;
		$apply_refund_num = 0;
		...
		if ($refund_num == count($order_goods)) {
			...
			pdo_update('vcshop_order', array('status' => -1, 'canceltime' => $time, 'refundid' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			plog('order.op.close', '订单关闭 ID: ' . $item['id'] . ' 订单号: ' . $item['ordersn']);
		}

		if (empty($apply_refund_num)) {
			pdo_update('vcshop_order', array('refundtime' => $time, 'refundid' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		}

		$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $item['ordergoodsid'], ':uniacid' => $_W['uniacid']));
		pdo_update('vcshop_goods', array('salesreal' => $salesreal), array('id' => $item['ordergoodsid']));
