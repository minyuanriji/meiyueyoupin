...
class Refund_VcShopPage extends WebPage
{
	protected function opData()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_order') . ' WHERE id = :id and uniacid=:uniacid Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function submit()
	{
		...
		if ($_W['ispost']) {
			...
			if ($refund['status'] < 0 || $refund['status'] == 1) {
				pdo_update('vcshop_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			...
			if ($refundstatus == 0) {
				...
				pdo_update('vcshop_order', $order_data, array('id' => $item['id'], 'uniacid' => $uniacid));
				m('notice')->sendOrderMessage($item['id'], true);
			}
				...
				pdo_update('vcshop_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));

				foreach ($goods as $g) {
					$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $g['id'], ':uniacid' => $uniacid));
				...
				pdo_update('vcshop_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid));
				m('notice')->sendOrderMessage($item['id'], true);
			}
			else {
				if ($refundstatus == 2) {
					...
					pdo_update('vcshop_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));
					$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice FROM ' . tablename('vcshop_order_goods') . ' o left join ' . tablename('vcshop_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid', array(':orderid' => $item['id'], ':uniacid' => $uniacid));
