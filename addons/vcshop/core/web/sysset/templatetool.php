...
class Templatetool_VcShopPage extends WebPage
{
	...
	{
		$items = pdo_fetchall('select typecode from ' . tablename('vcshop_member_message_template_type') . ' where templatecode is not null');
		$typecode = array();
...
', ' '), '', $value['content']);
			...
			if ($valuecontent == $content) {
				$issnoet = false;
				$defaulttemp = pdo_fetch('select 1  from ' . tablename('vcshop_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $_W['uniacid']));

				if (empty($defaulttemp)) {
					pdo_insert('vcshop_member_message_template_default', array('typecode' => $tag, 'uniacid' => $_W['uniacid'], 'templateid' => $value['template_id']));
				}
				else {
					pdo_update('vcshop_member_message_template_default', array('templateid' => $value['template_id']), array('typecode' => $tag, 'uniacid' => $_W['uniacid']));
				}
