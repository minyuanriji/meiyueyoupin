...
class Payment_VcShopPage extends WebPage
{
	...
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_payment') . (' WHERE 1 ' . $condition . '  ORDER BY id asc limit ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('vcshop_payment') . (' WHERE 1 ' . $condition), $params);
		...
		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_payment') . ' WHERE  uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				...
				pdo_insert('vcshop_payment', $data);
				$id = pdo_insertid();
				plog('sysset.payment.add', '添加支付信息一条 ID: ' . $id . ' 标题: ' . $data['title'] . ' ');
			}
			else {
				pdo_update('vcshop_payment', $data, array('id' => $id));
		...
		if (!empty($id)) {
			$data = pdo_fetch('SELECT * FROM ' . tablename('vcshop_payment') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_payment') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_payment') . ' WHERE  uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));

		if (count($items) < $count) {
			foreach ($items as $item) {
				pdo_delete('vcshop_payment', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_member_printer_template') . (' WHERE 1 ' . $condition . ' order by id asc'), $params);
