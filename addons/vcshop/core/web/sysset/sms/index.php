...
class Index_VcShopPage extends ComWebPage
{
	...
	public function set()
	{
		global $_W;
		global $_GPC;
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_sms_set') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (empty($item)) {
				...
				pdo_insert('vcshop_sms_set', $arr);
				$id = pdo_insertid();
			}
			else {
				pdo_update('vcshop_sms_set', $arr, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
