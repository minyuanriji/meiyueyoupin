...
class Index_VcShopPage extends SystemPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			if (!empty($_GPC['displayorder'])) {
				foreach ($_GPC['displayorder'] as $id => $displayorder) {
					pdo_update('vcshop_plugin', array('status' => $_GPC['status'][$id], 'displayorder' => $displayorder, 'name' => $_GPC['name'][$id], 'thumb' => $_GPC['thumb'][$id], 'desc' => $_GPC['desc'][$id]), array('id' => $id));
				}
		...
		$list = pdo_fetchall('select * from ' . tablename('vcshop_plugin') . (' where 1 ' . $condition . ' order by displayorder asc'), $params);
		$bucket = array();
	...
	public function apps()
	{
		...
		$auth = base64_encode(authcode($domain . '|' . $id . '|' . $authcode, 'ENCODE', 'vcshop_apps'));
