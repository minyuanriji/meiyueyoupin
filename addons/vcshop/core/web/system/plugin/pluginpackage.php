...
class Pluginpackage_VcShopPage extends SystemPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT gp.*,p.identity,p.category,p.version,p.author,p.status,p.name FROM ' . tablename('vcshop_system_plugingrant_package') . ' as gp
		...
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_system_plugingrant_package') . ' as gp
                left join ' . tablename('vcshop_plugin') . (' as p on p.id = gp.pluginid
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				pdo_update('vcshop_system_plugingrant_package', $data, array('id' => $id));
			}
			else {
				pdo_insert('vcshop_system_plugingrant_package', $data);
				$id = pdo_insertid();
			}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,text FROM ' . tablename('vcshop_system_plugingrant_package') . (' WHERE id in( ' . $id . ' ) '));

		foreach ($items as $item) {
			pdo_delete('vcshop_system_plugingrant_package', array('id' => $item['id']));
	...
	public function property()
	{
		...
		if (in_array($type, array('state', 'rec', 'displayorder'))) {
			pdo_update('vcshop_system_plugingrant_package', array($type => $value), array('id' => $id));
			$statusstr = '';
	...
	public function query()
	{
		...
		$pluginid = '';
		$grantPlugin = array();
		$grantPlugin = pdo_fetchall('select pluginid from ' . tablename('vcshop_system_plugingrant_package') . ' where 1 ');
		...
		$plugins = pdo_fetchall('select id,`name` as title,thumb,`desc` from ' . tablename('vcshop_plugin') . ' where 1 ' . $condition . ' order by displayorder asc LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_plugin') . ' WHERE 1 ' . $condition . ' ', $params);
