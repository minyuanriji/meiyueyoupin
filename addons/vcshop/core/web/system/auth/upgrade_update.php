...
class Upgrade_Update_VcShopPage extends SystemPage {
    ...
    function checkUpdate($output = false) {
        ...
        $resp = ihttp_post(VCSHOP_NEW_AUTH_URL . "/index", array(
            'ip' => $auth['ip'],
            'id' => $auth['id'],
            'code' => $auth['code'],
        ...
        if ($_W['ispost'] || !empty($gcp))
        {
            ...
            switch ($type)
            {
                ...
                case 'files':
                    ...
                    $resp = ihttp_post(VCSHOP_NEW_AUTH_URL . "/download", array(
                        'ip' => $auth['ip'],
                        'id' => $auth['id'],
                        'code' => $auth['code'],
                        'domain' => trim( preg_replace( "/http(s)?:\/\//", "", rtrim($_W['siteroot'],"/") )  ),
...
//		$ret['engine'] = $result['Engine'];
    ...
    public function log()
    {
        ...
        load()->func('communication');
        $resp = ihttp_post(VCSHOP_AUTH_URL . "log", array(
            'ip' => $auth['ip'],
            'id' => $auth['id'],
            'code' => $auth['code'],
            'domain' => trim( preg_replace( "/http(s)?:\/\//", "", trim($_W['siteroot'],"/") )  ),
