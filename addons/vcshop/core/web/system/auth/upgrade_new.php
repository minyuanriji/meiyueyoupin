...
class Upgrade_new_VcShopPage extends SystemPage
{
	public function main()
	{
		...
		$uni = pdo_fetchall('SELECT uniacid FROM ' . tablename('vcshop_sysset'), array(), 'uniacid');
		$options = array();
		if (!empty($uni) && date('w') % 7 == 1) {
			...
			$members = pdo_fetchall('SELECT COUNT(*) as count,uniacid FROM ' . tablename('vcshop_member') . '  GROUP BY uniacid', array(), 'uniacid');
			$orders = pdo_fetchall('SELECT SUM(price) as price,uniacid,COUNT(*) as `count` FROM ' . tablename('vcshop_order') . ' WHERE `status`=3 AND paytime>' . (strtotime(date('y-m-d')) - 3600 * 24 * 7) . ' GROUP BY uniacid', array(), 'uniacid');
		...
		$plugins = scandir(VCSHOP_PLUGIN);
		array_shift($plugins);
		array_shift($plugins);
		$resp = ihttp_request(VCSHOP_NEW_AUTH_URL, array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain, 'options' => $options, 'dirplugin' => $plugins));
	...
	public function check()
	{
		global $_W;
		global $_GPC;
		$plugins = pdo_fetchall('select `identity` from ' . tablename('vcshop_plugin'), array(), 'identity');
		load()->func('db');
		load()->func('communication');
		$check = (int) $_GPC['check'];
		set_time_limit(0);
		$auth = get_auth();
		$version = defined('VCSHOP_VERSION') ? VCSHOP_VERSION : '2.0.0';
		$release = defined('VCSHOP_RELEASE') ? VCSHOP_RELEASE : '201605010000';
		$file_md5 = $this->fileGlob(VCSHOP_PATH, true);
		$start = microtime(true);
		$resp = ihttp_post(VCSHOP_NEW_AUTH_URL . '/index', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => $check, 'file_md5' => json_encode($file_md5), 'plugins' => array_keys($plugins), 'phpversion' => PHP_VERSION_ID));
		$end = microtime(true);
		...
		if (is_file(VCSHOP_PATH . 'tmp')) {
			@unlink(VCSHOP_PATH . 'tmp');
		}
	...
	public function process()
	{
		...
		if ($_W['ispost']) {
			...
			case 'files':
				$resp = ihttp_post(VCSHOP_NEW_AUTH_URL . '/download', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'md5' => $content));
				...
				if (is_array($ret['result'])) {
					foreach ((array) $ret['result']['files'] as $files) {
						if (strexists($files['path'], 'pcsite/')) {
							...
							mkdirs(VCSHOP_PATH . $dirpath);
							copy(IA_ROOT . '/' . $files['path'], VCSHOP_PATH . '/' . $files['path']);
						}
						else {
							$dirpath = dirname($files['path']);
							mkdirs(VCSHOP_PATH . $dirpath);
							$content = base64_decode($files['content']);
							file_put_contents(VCSHOP_PATH . $files['path'], $content);
						}
					}
	...
	{
		load()->func('file');
		file_put_contents(VCSHOP_PATH . 'version.php', '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'VCSHOP_VERSION\')) {define(\'VCSHOP_VERSION\', \'' . $version . '\');}if(!defined(\'VCSHOP_RELEASE\')) {define(\'VCSHOP_RELEASE\', \'' . $release . '\');}');
	...
	{
		load()->model('cache');
		file_put_contents(IA_ROOT . '/addons/vcshop/version.php', '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'VCSHOP_VERSION\')) {define(\'VCSHOP_VERSION\', \'2.0.0\');}if(!defined(\'VCSHOP_RELEASE\')) {define(\'VCSHOP_RELEASE\', \'201605010000\');}');
		pdo_update('modules', array('version' => '3.0.0'), array('name' => 'vcshop'));
		cache_clean();
	...
	public function log()
	{
		...
		$psize = 15;
		load()->func('communication');
		$resp = ihttp_post(VCSHOP_AUTH_URL . 'log', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins), 'pindex' => $pindex, 'psize' => $psize));
	...
	public function fileGlob($path, $md5 = false, $only_dir = false, $recursive = true)
	{
		...
		if (strexists($path, VCSHOP_DATA)) {
			return array();
		}

		foreach (glob($path) as $file) {
			if ($file != '.' && $file != '..') {
				$relative_path = str_replace(VCSHOP_PATH, '', $file);
