...
class Upgrade_VcShopPage extends SystemPage
{
	...
	public function check()
	{
		...
		set_time_limit(0);
		$auth = get_auth();
		if(!$auth['code'])show_json(0, '请先授权然后更新！<a href="index.php?c=site&a=entry&m=vcshop&do=web&r=system.auth">立即授权</a>');
		$version = ((defined('VCSHOP_VERSION') ? VCSHOP_VERSION : '2.0.0'));
		$release = ((defined('VCSHOP_RELEASE') ? VCSHOP_RELEASE : '201605010000'));
		show_json(0);
		// $resp = ihttp_request('http://admin.zheyitianshi.com/gateway.php/a=rr&b=check', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				show_json(0);
				// $resp = ihttp_request('http://admin.zheyitianshi.com/gateway.php/a=rr&b=download', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
				...
				if (is_array($ret)) {
					...
					if (!is_dir(VCSHOP_PATH . $dirpath)) {
						mkdirs(VCSHOP_PATH . $dirpath);
						@chmod(VCSHOP_PATH . $dirpath, 511);
					}


					$content = base64_decode($ret['content']);
					file_put_contents(VCSHOP_PATH . $path, $content);

					if (isset($ret['path1'])) {
						$path1 = $ret['path1'];
						$dirpath1 = dirname($path1);

						if (!is_dir(VCSHOP_PATH . $dirpath1)) {
							mkdirs(VCSHOP_PATH . $dirpath1);
							@chmod(VCSHOP_PATH . $dirpath1, 511);
						}


						$content1 = base64_decode($ret['content1']);
						file_put_contents(VCSHOP_PATH . $path1, $content1);
					}
					...
					if (isset($ret['path2'])) {
						...
						if (!is_dir(VCSHOP_PATH . $dirpath2)) {
							mkdirs(VCSHOP_PATH . $dirpath2);
							@chmod(VCSHOP_PATH . $dirpath2, 511);
						}


						$content2 = base64_decode($ret['content2']);
						file_put_contents(VCSHOP_PATH . $path2, $content2);
					}
