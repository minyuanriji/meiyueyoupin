...
class Index_VcShopPage extends SystemPage
{
	...
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			$res = pdo_fetch('select id from ' . tablename('vcshop_system_site') . ' where `type`=:type', array(':type' => $this->type));

			if (empty($res)) {
				$ok = pdo_insert('vcshop_system_site', $data);
				$ok ? show_json(1) : show_json(0);
			}
			else {
				$ok = pdo_update('vcshop_system_site', $data, array('id' => $res['id']));
				show_json(1);
			}
		}
		...
		$res = pdo_fetch('select * from ' . tablename('vcshop_system_site') . ' where `type`=:type', array(':type' => $this->type));
