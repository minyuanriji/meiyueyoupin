...
class Credit_VcShopPage extends WebPage
{
	protected function main($type = 'credit1')
	{
		...
		if ($_GPC['groupid'] || $_GPC['level'] || $_GPC['keyword']) {
			...
			if ($type == 'credit1') {
				$table2 = 'select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname  from ' . tablename('vcshop_member_credit_record') . ' log ' . ' left join ' . tablename('users') . ' u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid' . ' left join ' . tablename('vcshop_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('vcshop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('vcshop_member_level') . ' l on m.level =l.id' . (' where 1 ' . $condition . ' and log.uid<>0');
			...
			$table2 = 'select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,u.username,\'0\' as uid,log.openid  from ' . tablename('vcshop_member_credit_record') . ' log ' . ' left join ' . tablename('users') . ' u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid' . (' where 1 ' . $condition . ' and log.uid<>0');
			$sql = 'select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,u.username,\'0\' as uid,log.openid  from ' . tablename('vcshop_member_credit_record') . ' log ' . ' left join ' . tablename('users') . ' u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid' . (' where 1 ' . $condition . ' order by log.createtime desc');
		}
		...
		if ($list && $search_flag == 0) {
			foreach ($list as $key => $val) {
				...
				if ($val['uid']) {
					$member = pdo_fetch('select * from ' . tablename('vcshop_member') . ' where uniacid=:uniacid and uid=:uid', array(':uniacid' => $_W['uniacid'], ':uid' => $val['uid']));
				}
				else {
					$member = pdo_fetch('select * from ' . tablename('vcshop_member') . ' where uniacid=:uniacid and openid=:openid', array(':uniacid' => $_W['uniacid'], ':openid' => $val['openid']));
				}

				$groupname = pdo_fetchcolumn('select groupname from ' . tablename('vcshop_member_group') . ' where uniacid=:uniacid and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $member['groupid']));
				$levelname = pdo_fetchcolumn('select levelname from ' . tablename('vcshop_member_level') . ' where uniacid=:uniacid and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $member['level']));
		...
		if ($type == 'credit1') {
			...
			if (empty($search_flag)) {
                $total_sql = "select count(*) as ccc  from " . tablename("vcshop_member_credit_record") . " log " . " left join " . tablename("users") . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid" . " where 1 " . $condition;
