...
class Log_VcShopPage extends WebPage
{
	protected function main($type = 0)
	{
		...
		if ($condition1 != '') {
			$member_sql = ' and openid IN (SELECT openid FROM ' . tablename('vcshop_member') . ('m WHERE uniacid = :uniacid ' . $condition1 . ') OR openid IN (SELECT CONCAT(\'sns_wa_\',openid_wa) FROM ') . tablename('vcshop_member') . (' m WHERE uniacid = :uniacid ' . $condition1 . ')');
		}
		...
		// han 20191214 add log.bankbranch
		$sql = 'select log.id,log.openid,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,log.money,log.createtime,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.bankbranch,log.realname as applyrealname,log.applytype,m.nickname,m.id as mid,m.avatar,m.level,m.groupid,m.realname,m.mobile,g.groupname,l.levelname from ' . tablename('vcshop_member_log') . ' log ' . ' left join ' . tablename('vcshop_member') . ' m on m.openid = log.openid ' . ' left join ' . tablename('vcshop_member_group') . ' g on g.id = m.groupid ' . ' left join ' . tablename('vcshop_member_level') . ' l on l.id = m.level ' . (' where 1 ' . $condition . ' ' . $condition1 . ' GROUP BY log.id ORDER BY log.createtime DESC ');
		...
		if (!empty($list)) {
			...
			$members_sql = 'select id as mid, realname,avatar,weixin,nickname,mobile,openid,openid_wa from ' . tablename('vcshop_member') . ' m where uniacid=:uniacid and openid IN (\'' . implode('\',\'', array_unique($openids)) . '\') OR openid_wa IN (\'' . implode('\',\'', array_unique($openids)) . '\')';
		...
		if ($_GPC['export'] == 1) {
			...
			unset($row);
			$columns = array();
			$columns[] = array('title' => '单号', 'field' => 'logno', 'width' => 12);
			...
			if (!empty($type)) {
				...
				$columns[] = array('title' => '银行支行', 'field' => 'bankbranch', 'width' => 24);
		...
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_member_log') . ' log ' . (' where 1 ' . $condition . ' ' . $member_sql), $params);
