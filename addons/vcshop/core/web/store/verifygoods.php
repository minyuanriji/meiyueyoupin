...
class Verifygoods_VcShopPage extends WebPage
{
	public function main()
	{
		...
		$sql = 'select vg.*,g.id as goodsid ,g.title,g.thumb,o.ordersn,o.paytime,s.storename,o.openid  from ' . tablename('vcshop_verifygoods') . '   vg
		 left join ' . tablename('vcshop_store') . ' s  on s.id = vg.storeid
		 left join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
		 left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid
		...
		foreach ($list as &$row) {
			...
			$verifygoodlogs = pdo_fetchall('select  *  from ' . tablename('vcshop_verifygoods_log') . '  where  verifygoodsid =:id  ', array(':id' => $row['id']));
			$verifynum = 0;
		...
		unset($row);
		$total = pdo_fetchcolumn('select  COUNT(*)   from ' . tablename('vcshop_verifygoods') . '   vg
		 left join ' . tablename('vcshop_store') . ' s  on s.id = vg.storeid
		 left join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
		 left join ' . tablename('vcshop_order') . ' o on o.id = og.orderid
		 left join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
	...
	public function detail()
	{
		...
		$id = $_GPC['id'];
		$uniacid = $_W['uniacid'];
		$item = pdo_fetch('select vg.*,g.title,g.subtitle,g.thumb,o.ordersn,o.paytime,o.openid  from ' . tablename('vcshop_verifygoods') . '   vg
		...
		$verifygoodlogs = pdo_fetchall('select vgl.*,s.storename,sa.salername  from ' . tablename('vcshop_verifygoods_log') . '   vgl
		...
		if ($_W['ispost']) {
			...
			pdo_update('vcshop_verifygoods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function invalid()
	{
		...
		$item = pdo_fetch('SELECT id FROM ' . tablename('vcshop_verifygoods') . ' WHERE id=' . $id . '  AND uniacid=' . $_W['uniacid']);

		if (!empty($item)) {
			pdo_update('vcshop_verifygoods', array('invalid' => $type), array('id' => $item['id']));
			com('wxcard')->updateusercardbyvarifygoodid($id);
		}
	...
	public function verifygoodslog()
	{
		...
		$sql = 'select vgl.id,g.id as goodsid ,g.title,g.thumb,o.ordersn,vgl.verifydate,vgl.verifynum,o.paytime,s.storename,sa.salername,vgl.remarks,o.openid  from ' . tablename('vcshop_verifygoods_log') . '   vgl
		...
		unset($rom);
		$total = pdo_fetchcolumn('select  COUNT(*)   from ' . tablename('vcshop_verifygoods_log') . '   vgl
		 left join ' . tablename('vcshop_verifygoods') . ' vg on vg.id = vgl.verifygoodsid
		 left join ' . tablename('vcshop_store') . ' s  on s.id = vgl.storeid
		 left join ' . tablename('vcshop_saler') . ' sa  on sa.id = vgl.salerid
		 left join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
	...
	public function verifygoodslogdelete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,verifygoodsid FROM ' . tablename('vcshop_verifygoods_log') . ' WHERE id=' . $id . ' AND uniacid=' . $_W['uniacid']);

		if (!empty($item)) {
			pdo_delete('vcshop_verifygoods_log', array('id' => $item['id']));
			pdo_update('vcshop_verifygoods', array('used' => 0), array('id' => $item['verifygoodsid']));
