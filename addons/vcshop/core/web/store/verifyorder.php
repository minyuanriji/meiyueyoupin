...
class Verifyorder_VcShopPage extends WebPage
{
	protected function orderData()
	{
		...
		if (p('merch')) {
			$leftsql = ' left join ' . tablename('vcshop_merch_saler') . ' ms on ms.openid = o.verifyopenid and ms.uniacid=o.uniacid';
			$leftsql .= ' left join ' . tablename('vcshop_merch_store') . ' mstore on mstore.id = o.verifystoreid';
		}
		...
		if ($searchtime == 'finish') {
			...
			$leftsql .= ' left join ' . tablename('vcshop_verifygoods') . ' vg on o.id = vg.orderid';
			$leftsql .= ' left join ' . tablename('vcshop_verifygoods_log') . ' vgl on vgl.verifygoodsid = vg.id';
		}
