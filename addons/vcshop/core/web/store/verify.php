...
class Verify_VcShopPage extends WebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if ($verifytype == 2) {
				...
				$verifygood = pdo_fetch('select *  from ' . tablename('vcshop_verifygoods') . ' where uniacid=:uniacid and  verifycode=:verifycode  limit 1 ', array(':uniacid' => $_W['uniacid'], ':verifycode' => $verifycode));
	...
	public function verifygoods()
	{
		...
		$verifygood = pdo_fetch('select vg.*,g.id as goodsid ,g.title,g.subtitle,g.thumb  from ' . tablename('vcshop_verifygoods') . '   vg
		...
		if (!empty($verifygood['storeid'])) {
			$store = pdo_fetch('select * from ' . tablename('vcshop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $verifygood['storeid'], ':uniacid' => $_W['uniacid']));
		}
	...
	public function log()
	{
		...
		$sql = 'select vg.*,g.id as goodsid ,g.title,g.thumb,o.ordersn,vgl.verifydate,vgl.verifynum,o.paytime,s.storename,sa.salername,vgl.remarks,o.openid,o.carrier  from ' . tablename('vcshop_verifygoods_log') . '   vgl
