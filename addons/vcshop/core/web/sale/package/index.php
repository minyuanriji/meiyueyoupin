...
class Index_VcShopPage extends WebPage
{
	public function main()
	{
		...
		$packages = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_package') . '
		...
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_package') . ' WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($goodsid as $key => $value) {
				$good_data = pdo_fetch('select title,thumb,marketprice,goodssn,productsn,hasoption,merchid
                            from ' . tablename('vcshop_goods') . ' where id = ' . $value . ' and uniacid = ' . $uniacid . ' ');
			...
			if (!empty($id)) {
				foreach ($goodsid as $key => $value) {
					$packagenum = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_package_goods') . ' where goodsid = :goodsid and uniacid = :uniacid', array(':goodsid' => $value, ':uniacid' => $value));
					$thisgoods = pdo_fetch('select id from ' . tablename('vcshop_package_goods') . ' where pid = :pid and goodsid = :goodsid and uniacid = :uniacid ', array(':pid' => $id, 'goodsid' => $value, ':uniacid' => $uniacid));
					if (!$thisgoods && 3 <= $packagenum) {
						show_json(0, '同一件商品最多参与三个套餐活动!');
					}
				}

				$package_update = pdo_update('vcshop_package', $data, array('id' => $id, 'uniacid' => $uniacid));
				$package_goods_del = pdo_delete('vcshop_package_goods', array('pid' => $id, 'uniacid' => $uniacid));
				$package_goods_option_del = pdo_delete('vcshop_package_goods_option', array('pid' => $id, 'goodsid' => $value, 'uniacid' => $uniacid));
				...
				foreach ($goodsid as $key => $value) {
					...
					$package_goods_insert = pdo_insert('vcshop_package_goods', $good_data);

					if (!empty($good_data['option'])) {
						$packageGoodsOption = array_filter(explode(',', $good_data['option']));

						foreach ($packageGoodsOption as $k => $val) {
							$op = pdo_fetch('SELECT id,title,marketprice,goodssn,productsn FROM ' . tablename('vcshop_goods_option') . '
							...
							$package_goods_option_insert = pdo_insert('vcshop_package_goods_option', $optionData);
				...
				foreach ($goodsid as $key => $value) {
					$packagenum = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_package_goods') . ' where uniacid = ' . $uniacid . ' and goodsid = ' . $value . ' ');
				...
				$package_insert = pdo_insert('vcshop_package', $data);
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_package') . ' WHERE uniacid = ' . $uniacid . (' and id = ' . $id . ' '));
		...
		if ($item) {
			...
			$package_goods = pdo_fetchall('select id,pid,title,thumb,packageprice,hasoption,goodsid,`option`,commission1,commission2,commission3
                            from ' . tablename('vcshop_package_goods') . ' where pid = ' . $id . ' and uniacid = ' . $uniacid . ' ');
			...
			foreach ($package_goods as $key => $value) {
				if ($value['hasoption']) {
					$package_goods[$key]['optiontitle'] = pdo_fetchall('select id,goodsid,optionid,pid,packageprice,title,marketprice,commission1,commission2,commission3
                            from ' . tablename('vcshop_package_goods_option') . ' where pid = ' . $id . ' and goodsid = ' . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
				}
			}
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('vcshop_package') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('vcshop_package', array('deleted' => 1, 'status' => 0), array('id' => $item['id']));
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			pdo_update('vcshop_package', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function delete1()
	{
		...
		foreach ($items as $item) {
			pdo_delete('vcshop_package', array('id' => $item['id']));
			pdo_delete('vcshop_package_goods', array('pid' => $item['id']));
			pdo_delete('vcshop_package_goods_option', array('pid' => $item['id']));
	...
	public function restore()
	{
		...
		foreach ($items as $item) {
			pdo_update('vcshop_package', array('deleted' => 0), array('id' => $item['id']));
	...
	public function change()
	{
		...
		$package = pdo_fetch('select id from ' . tablename('vcshop_package') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));

		if (empty($package)) {
			show_json(0, array('message' => '参数错误'));
		}

		pdo_update('vcshop_package', array($type => $value), array('id' => $id));
		show_json(1);
	}
	...
	public function query()
	{
		...
		foreach ($ds as $key => $row) {
			if (0 < $row['merchid']) {
				$merch = pdo_fetch('select merchname from ' . tablename('vcshop_merch_user') . ' where id = :merchid and uniacid = :uniacid ', array(':merchid' => $row['merchid'], ':uniacid' => $uniacid));
