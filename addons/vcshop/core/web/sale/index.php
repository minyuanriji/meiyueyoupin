...
class Index_VcShopPage extends ComWebPage
{
	...
	public function main()
	{
		...
		// 20201104 lin
		else if (cv('sale.virtualcard')) {
			header('location: ' . webUrl('sale/virtualcard'));
		}
		else {
	...
	public function enoughfree()
	{
		...
		if (!empty($data['goodsids'])) {
			$goods = pdo_fetchall('SELECT id,uniacid,title,thumb FROM ' . tablename('vcshop_goods') . ' WHERE uniacid=:uniacid AND id IN (' . implode(',', $data['goodsids']) . ')', array(':uniacid' => $_W['uniacid']));
		}
