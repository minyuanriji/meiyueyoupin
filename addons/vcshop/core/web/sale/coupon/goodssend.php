...
class Goodssend_VcShopPage extends ComWebPage
{
	...
	public function main()
	{
		...
		$goodssends = pdo_fetchall('SELECT cg.*, c.couponname, c.thumb as cthumb,g.title,g.thumb as ghumb  FROM ' . tablename('vcshop_coupon_goodsendtask') . '  cg
            left  join  ' . tablename('vcshop_goods') . '  g on cg.goodsid =g.id
            left  join  ' . tablename('vcshop_coupon') . '  c on cg.couponid =c.id
            WHERE 1 ' . $condition . '   LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_coupon_goodsendtask') . '  cg
            left  join  ' . tablename('vcshop_goods') . '  g on cg.goodsid =g.id
            left  join  ' . tablename('vcshop_coupon') . '  c on cg.couponid =c.id
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT *  FROM ' . tablename('vcshop_coupon_goodsendtask') . ' WHERE uniacid = ' . $uniacid . ' and id =' . $id);
			...
			if (!empty($item['goodsid'])) {
				$goods = pdo_fetch('SELECT id, title , thumb  FROM ' . tablename('vcshop_goods') . ' WHERE uniacid = ' . $uniacid . ' and id =' . $item['goodsid']);
			}
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				pdo_update('vcshop_coupon_goodsendtask', $data, array('id' => $id));
				plog('coupon.goodsendtask.edit', '修改优惠券发送任务 ID: ' . $id);
			}
			else {
				pdo_insert('vcshop_coupon_goodsendtask', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_coupon_goodsendtask') . ' WHERE id  = :id  and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));

		if (!empty($item)) {
			pdo_delete('vcshop_coupon_goodsendtask', array('id' => $id, 'uniacid' => $_W['uniacid']));
		}
