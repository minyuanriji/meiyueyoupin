...
class Index_VcShopPage extends ComWebPage
{
	...
	public function main()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('vcshop_coupon') . ' ' . (' where  1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);

		foreach ($list as &$row) {
			$row['gettotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['usetotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_coupon_data') . ' where used = 1 and couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['pwdjoins'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['pwdoks'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and ok=1 limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
		...
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_coupon') . (' where 1 and ' . $condition), $params);
		$pager = pagination2($total, $pindex, $psize);
		$category = pdo_fetchall('select * from ' . tablename('vcshop_coupon_category') . ' where uniacid=:uniacid and merchid=0 order by id desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if (!empty($data['pwdkey2'])) {
					$pwdkey2 = pdo_fetch('SELECT pwdkey2 FROM ' . tablename('vcshop_coupon') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('vcshop_coupon', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			if (!empty($data['pwdkey2'])) {
				if (empty($rule)) {
					$rule_data = array('uniacid' => $_W['uniacid'], 'name' => $key, 'module' => 'vcshop', 'displayorder' => 0, 'status' => $data['pwdopen']);
					pdo_insert('rule', $rule_data);
					$rid = pdo_insertid();
					$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'vcshop', 'content' => $data['pwdkey2'], 'type' => 1, 'displayorder' => 0, 'status' => $data['pwdopen']);
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_coupon') . ' WHERE id =:id and uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if (empty($item)) {
			...
			if ($item['limitgoodtype'] == 1 || $item['limitgoodtype'] == 2) {
				if ($item['limitgoodids']) {
					$goods = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('vcshop_goods') . (' WHERE uniacid = :uniacid and id in (' . $item['limitgoodids'] . ') '), array(':uniacid' => $_W['uniacid']));
				}
			}
			...
			if ($item['total'] == '-1') {
				...
				$gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function query()
	{
		...
		if ($merch) {
			$ds = pdo_fetchall('SELECT c.*,m.merchname FROM ' . tablename('vcshop_coupon') . ' as c
                 left join ' . tablename('vcshop_merch_user') . (' as m on m.id = c.merchid
                 WHERE 1 ' . $condition . ' ORDER BY c.id asc'), $params);
		}
		else {
			$ds = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_coupon') . ('  WHERE 1 ' . $condition . ' ORDER BY id asc'), $params);
		}
	...
	public function querygoods()
	{
		...
		$ds = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('vcshop_goods') . (' WHERE 1 ' . $condition . ' order by createtime desc'), $params);
		$replace_str = array('“', '"');
	...
	public function send()
	{
		...
		$coupon = pdo_fetch('SELECT * FROM ' . tablename('vcshop_coupon') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			pdo_update('vcshop_coupon', $upgrade, array('id' => $coupon['id']));
		...
		$coupons = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_coupon') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc'));
		$hascommission = false;
		$plugin_com = p('commission');
