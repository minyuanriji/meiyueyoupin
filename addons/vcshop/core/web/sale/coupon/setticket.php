...
class Setticket_VcShopPage extends WebPage
{
    public function main()
    {
        ...
        if ($_W["ispost"]) {
            ...
            if (empty($id)) {
                if (is_array($_GPC["couponid"])) {
                    ...
                    foreach ($_GPC["couponid"] as $cpk => $cpv) {
                        ...
                        $sql = "SELECT * FROM " . tablename("vcshop_coupon") . $where;
            ...
            if (empty($id)) {
                $result = pdo_insert("vcshop_sendticket", $data);
                plog("sendticket.set", "设置发券内容");
            } else {
                $params = array("id" => $id);
                $result = pdo_update("vcshop_sendticket", $data, $params);
            }
            if (!$result) {
        ...
        $sql = "SELECT * FROM " . tablename("vcshop_sendticket") . " WHERE uniacid = " . intval($_W["uniacid"]);
        $item = pdo_fetch($sql);
        if (!empty($item)) {
            ...
            foreach ($cpids as $cpidk => $cpidv) {
                $cpsql = "SELECT * FROM " . tablename("vcshop_coupon") . " WHERE uniacid = " . intval($_W["uniacid"]) . " AND id = " . intval($cpidv);
