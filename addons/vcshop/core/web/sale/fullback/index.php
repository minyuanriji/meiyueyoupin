...
class Index_VcShopPage extends WebPage
{
	public function main()
	{
		...
		$gifts = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_fullback_goods') . '
		...
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vcshop_fullback_goods') . ' WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				...
				$item = pdo_fetch('SELECT goodsid FROM ' . tablename('vcshop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $id . ' ');
			...
			$fullbackgoodsnum = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_fullback_goods') . ' where goodsid = :goodsid and uniacid = :uniacid ', array(':goodsid' => $data['goodsid'], ':uniacid' => $uniacid));
			...
			$good_data = pdo_fetch('select title,thumb,marketprice,goodssn,productsn,hasoption
                            from ' . tablename('vcshop_goods') . ' where id = ' . $data['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
			...
			if (!empty($good_data['option'])) {
				...
				pdo_update('vcshop_goods_option', array('isfullback' => 0), array('uniacid' => $uniacid, 'goodsid' => $data['goodsid']));
				...
				foreach ($fullbackOption as $val) {
					...
					pdo_update('vcshop_goods_option', $optionData, array('uniacid' => $uniacid, 'id' => intval($val)));
				}
			}

			if (!empty($id)) {
				pdo_update('vcshop_fullback_goods', $data, array('id' => $id));
				plog('sale.fullback.edit', '编辑全返 ID: ' . $id . ' <br/>全返名称: ' . $data['titles']);
			}
			else {
				pdo_insert('vcshop_fullback_goods', $data);
				$id = pdo_insertid();
				plog('sale.fullback.add', '添加全返 ID: ' . $id . '  <br/>全返名称: ' . $data['titles']);
			}

			$sql = 'update ' . tablename('vcshop_fullback_goods') . ' g set
                g.minallfullbackallprice = (select min(allfullbackprice) from ' . tablename('vcshop_goods_option') . ' where goodsid = ' . $data['goodsid'] . '),
                g.maxallfullbackallprice = (select max(allfullbackprice) from ' . tablename('vcshop_goods_option') . ' where goodsid = ' . $data['goodsid'] . '),
                g.minallfullbackallratio = (select min(allfullbackratio) from ' . tablename('vcshop_goods_option') . ' where goodsid = ' . $data['goodsid'] . '),
                g.maxallfullbackallratio = (select max(allfullbackratio) from ' . tablename('vcshop_goods_option') . ' where goodsid = ' . $data['goodsid'] . ')
                where g.goodsid = ' . $data['goodsid'] . ' and g.hasoption=1 and g.uniacid = ' . $uniacid . ' and g.id = ' . $id . ' ';
			pdo_query($sql);

			if (0 < $data['status']) {
				pdo_update('vcshop_goods', array('isfullback' => $id), array('id' => $data['goodsid']));
			}
			else {
				pdo_update('vcshop_goods', array('isfullback' => 0), array('id' => $data['goodsid']));
			}
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('vcshop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $id . ' ');
			$good_data = pdo_fetch('select title,thumb,hasoption
                            from ' . tablename('vcshop_goods') . ' where id = ' . $item['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
			...
			if (0 < $item['hasoption']) {
                ...
                FROM ' . tablename('vcshop_goods_option') . '
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,titles FROM ' . tablename('vcshop_fullback_goods') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('vcshop_fullback_goods', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
			pdo_update('vcshop_goods', array('isfullback' => intval($item['id'])), array('id' => $items['goodsid']));
	...
	public function delete1()
	{
		...
		$items = pdo_fetchall('SELECT id,titles,goodsid FROM ' . tablename('vcshop_fullback_goods') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('vcshop_fullback_goods', array('id' => $item['id']));
			pdo_update('vcshop_goods', array('isfullback' => 0), array('id' => $item['goodsid']));
	...
	public function change()
	{
		...
		$gift = pdo_fetch('select id from ' . tablename('vcshop_fullback_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));

		if (empty($gift)) {
			show_json(0, array('message' => '参数错误'));
		}

		pdo_update('vcshop_fullback_goods', array($type => $value), array('id' => $id));
		show_json(1);
	}
	...
	public function hasoption()
	{
		...
		$goods = pdo_fetch('select id,title,marketprice,hasoption,isfullback from ' . tablename('vcshop_goods') . ' where uniacid = :uniacid and id = :goodsid ', $params);

		if (!empty($id)) {
			$fullback = pdo_fetch('select * from ' . tablename('vcshop_fullback_goods') . '
	...
	private function deal_fullback_goods($goods = array())
	{
		...
		foreach ($goods as &$item) {
			if ($item['hasoption']) {
				...
				$options = pdo_fetchall('SELECT id,title,marketprice,`day`,allfullbackprice,fullbackprice FROM ' . tablename('vcshop_goods_option') . '
			...
			$item['order_count'] = pdo_fetchcolumn('SELECT COUNT(DISTINCT(orderid)) FROM ' . tablename('vcshop_order_goods') . ' WHERE uniacid = ' . $_W['uniacid'] . ' and fullbackid = ' . $item['id'] . ' and  goodsid = ' . $item['goodsid']);
		}
	...
	public function info()
	{
		...
		$pindex = max(1, intval($_GPC['page']));
		$psize = 20;
		$sql = 'select * from ' . tablename('vcshop_fullback_log') . ' where orderid = :orderid and optionid = :optionid and goodsid = :goodsid ';
		$fullbackInfo = pdo_fetch($sql, array(':orderid' => $orderId, ':goodsid' => $goodsId, 'optionid' => $optionId));
		$sql = 'select * from ' . tablename('vcshop_fullback_log_map') . ' where logid = :logid order by fullback_time desc LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
		$fullbackLogs = pdo_fetchall($sql, array(':logid' => $fullbackInfo['id']));
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_fullback_log_map') . ' where logid = :logid', array(':logid' => $fullbackInfo['id']));
		$pager = pagination2($total, $pindex, $psize, '', array('before' => 5, 'after' => 4, 'ajaxcallback' => 'select_page', 'callbackfuncname' => 'select_page'));
