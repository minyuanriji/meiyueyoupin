...
class Order_VcShopPage extends WebPage
{
	public function main()
	{
		...
		$sql = 'select ' . $field . ' from ' . tablename('vcshop_order') . ' o ' . ' left join ' . tablename('vcshop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('vcshop_member_address') . ' a on a.id = o.addressid ' . (' where 1 ' . $condition . ' ');
		...
		unset($row);
		$totalcount = $total = pdo_fetchall('select o.id from ' . tablename('vcshop_order') . ' o ' . ' left join ' . tablename('vcshop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('vcshop_member_address') . ' a on a.id = o.addressid ' . (' where 1 ' . $condition), $params);
