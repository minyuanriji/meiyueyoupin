...
class Goods_rank_VcShopPage extends WebPage
{
	public function main()
	{
        ...
        $sql .= 'FROM ' . tablename('vcshop_order_goods') . ' og ';
        $sql .= 'LEFT JOIN ' . tablename('vcshop_order') . ' o ON og.orderid = o.id  ';
        $sql .= 'LEFT JOIN ' . tablename('vcshop_order_refund') . ' re ON og.orderid = re.orderid AND re.STATUS = 1 ';
        $sql .= 'LEFT JOIN ' . tablename('vcshop_goods') . ' g ON g.id = og.goodsid ';
		...
		$refund_list = pdo_fetchall("SELECT orderid,price FROM " . tablename("vcshop_order_refund") . " WHERE uniacid=:uniacid ", array(":uniacid" => $_W["uniacid"]));
        $order_goods_list = pdo_fetchall("SELECT id,orderid,goodsid,price FROM " . tablename("vcshop_order_goods") . " WHERE uniacid=:uniacid", array(":uniacid" => $_W["uniacid"]));
        $ordersing_refund_list = pdo_fetchall("SELECT orderid,ordergoodsid,price FROM " . tablename("vcshop_order_single_refund") . " WHERE uniacid=:uniacid", array(":uniacid" => $_W["uniacid"]));
        $array_list = array();
        foreach ($refund_list as $k => $v) {
            ...
            $zhekou = $v["price"] / $sumprice;
            foreach ($order_goods_list as $kk2 => $vv2) {
                if ($v["orderid"] == $vv2["orderid"]) {
                    $array_list[$vv2["goodsid"]] += $vv2["price"] * $zhekou;
                }
            }
        }
        foreach ($list as $ks => $vs) {
            foreach ($array_list as $kks => $vvs) {
                if ($vs["id"] == $kks) {
                    $list[$ks]["refundprice"] = $vvs;
                }
            }
        }
        foreach ($list as $klist => $vlist) {
            foreach ($ordersing_refund_list as $kre => $vre) {
                foreach ($order_goods_list as $kgood => $vgood) {
                    if ($vre["ordergoodsid"] == $vgood["id"] && $vlist["id"] == $vgood["goodsid"]) {
                        $list[$klist]["refundprice"] += $vre["price"];
                    }
                }
            }
        }
		...
		$groups = pdo_fetchall('SELECT id,name,goodsids FROM ' . tablename('vcshop_goods_group') . (' WHERE 1 ' . $con . '  ORDER BY id DESC'), $params_g);
		load()->func('tpl');
