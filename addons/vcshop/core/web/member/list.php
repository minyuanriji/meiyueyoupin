...
class List_VcShopPage extends WebPage
{
	public function main()
	{
		...
		if ($_GPC['groupid'] != '') {
			...
			$join .= ' left join (select * from ' . tablename('vcshop_member_group_log') . ' order by log_id desc limit 1 ) glog on (glog.openid = dm.openid) and glog.group_id = ' . (int) $_GPC['groupid'];
		}
		...
		$sql = 'select * ,dm.openid from ' . tablename('vcshop_member') . (' dm ' . $join . ' where 1 ' . $condition . '  ORDER BY dm.id DESC,dm.createtime DESC ');
		...
		if (!empty($list_group)) {
			$res_group = pdo_fetchall('select id,groupname from ' . tablename('vcshop_member_group') . ' where id in (' . implode(',', $list_group) . ')', array(), 'id');
		}

		isset($list_level) && ($list_level = array_values(array_filter($list_level)));

		if (!empty($list_level)) {
			$res_level = pdo_fetchall('select id,levelname from ' . tablename('vcshop_member_level') . ' where id in (' . implode(',', $list_level) . ')', array(), 'id');
		}

		isset($list_agent) && ($list_agent = array_values(array_filter($list_agent)));

		if (!empty($list_agent)) {
			$res_agent = pdo_fetchall('select id,nickname as agentnickname,avatar as agentavatar from ' . tablename('vcshop_member') . ' where id in (' . implode(',', $list_agent) . ')', array(), 'id');
		}
		...
		foreach ($list as &$row) {
			...
			$row['ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
			$row['ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('vcshop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
		...
		if ($join == '' && $condition == ' and dm.uniacid=:uniacid') {
			if ($open_redis) {
				// $redis_key = 'vc_' . $_W['uniacid'] . '_member_list';
				$redis_key =  $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_'.'vc_' . $_W['uniacid'] . '_member_list';
				$total = m('member')->memberRadisCount($redis_key);

				if (!$total) {
					$total = pdo_fetchcolumn('select count(*) from' . tablename('vcshop_member') . (' dm ' . $join . ' where 1 ' . $condition . ' '), $params);
		...
		$com_levels = $plug_commission->getLevels(true,true);// han 20200218
	...
	public function detail()
	{
		...
		// zhou 20191121 获取用户的名额 s
		// 等级才显示
		if($member['isagent'] == 1 && $member['status']){
			$quota = pdo_fetchall('select q.*,c.levelname from ' . tablename('vcshop_member_quota') . ' as q INNER JOIN ' . tablename('vcshop_commission_level') .' as  c on c.id = q.agentlevel ' . ' where q.uniacid=:uniacid and q.openid=:openid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));

			// INNER JOIN ims_vcshop_commission_level AS b ON b.level <= a.level
			$check_level = pdo_fetchall('select b.id,b.level,b.levelname from ' . tablename('vcshop_commission_level') . ' as a INNER JOIN ' . tablename('vcshop_commission_level') . ' as  b on b.level <= a.level ' . ' where a.uniacid=:uniacid and a.id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$member['agentlevel']));
			// 不需要游客
		...
		$member['self_ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		$member['self_ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('vcshop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));

		if (!empty($member['agentid'])) {
			$parentagent = m('member')->getMember($member['agentid']);
		}

		$order = pdo_fetch('select finishtime from ' . tablename('vcshop_order') . ' where uniacid=:uniacid and openid=:openid and status>=1 Limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		...
		if ($plugin_sns) {
			...
			$sns_member = pdo_fetch('select * from ' . tablename('vcshop_sns_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $member['openid'], ':uniacid' => $_W['uniacid']));
			$hassns = !empty($sns_member);
			$sns_member['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid=0 and deleted = 0 and checked=1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
			$sns_member['replycount'] = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid>0 and deleted = 0 and checked=1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		...
		// 20200422 lin
		$starlevel =  pdo_fetchall('SELECT * FROM ' . tablename('vcshop_commission_star') . ' WHERE uniacid=:uniacid AND find_in_set(\'' . $member['agentlevel'] . '\',bindlevel) order by level desc',array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if ($openbind) {
				if (!empty($data['mobileverify'])) {
					...
					$m = pdo_fetch('select id from ' . tablename('vcshop_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniaicd limit 1 ', array(':mobile' => $data['mobile'], ':uniaicd' => $_W['uniacid']));
					if (!empty($m) && $m['id'] != $id) {
			...
			pdo_update('vcshop_member', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			if ($hascommission) {
				if (cv('commission.agent.edit')) {
					...
					if (!empty($adata)) {
						...
						// 以下为检查奖励s
						$checkIsReward = false;
						if($adata['isagent'] == 1 && $adata['status'] == 1 && $adata['agentlevel'] > 0){
							$level = pdo_fetch('select * from ' . tablename('vcshop_commission_level') . ' where uniacid=:uniacid and id=:id', array(':uniacid' => $_W['uniacid'],':id'=>$adata['agentlevel']));
							$checkIsReward = $plugin_com->checkIsReward($member['openid'],$level,'commission');
						}
						// 以上为奖励e
						// han 20191116 e
						if (!empty($agent_flag)) {
						...
						pdo_update('vcshop_member', $adata, array('id' => $id, 'uniacid' => $_W['uniacid']));

						if ($adata['agentid'] != $member['agentid']) {
							if (p('dividend')) {
								$dividend = pdo_fetch('select id,isheads,headsid,headsstatus from ' . tablename('vcshop_member') . ' where id = :id', array(':id' => $adata['agentid']));
								$dividend_init = pdo_fetch('select * from ' . tablename('vcshop_dividend_init') . ' where headsid = :headsid', array(':headsid' => $adata['agentid']));
								if (!empty($dividend['isheads']) && !empty($dividend['headsstatus']) && !empty($dividend_init['status'])) {
									pdo_update('vcshop_member', array('headsid' => $adata['agentid']), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
									$data = pdo_fetchall('select id from ' . tablename('vcshop_commission_relation') . ' where pid = :pid', array(':pid' => $member['id']));
									...
									if (!empty($data)) {
										...
										pdo_update('vcshop_member', array('headsid' => $adata['agentid']), array('id' => $ids));
									}
								}
								else {
									if (empty($dividend['isheads']) && !empty($dividend['headsid'])) {
										pdo_update('vcshop_member', array('headsid' => $dividend['headsid']), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
										$data = pdo_fetchall('select id from ' . tablename('vcshop_commission_relation') . ' where pid = :pid', array(':pid' => $member['id']));
										...
										if (!empty($data)) {
											...
											pdo_update('vcshop_member', array('headsid' => $dividend['headsid']), array('id' => $ids));
										}
									}
									else {
										pdo_update('vcshop_member', array('headsid' => 0), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
										$data = pdo_fetchall('select id from ' . tablename('vcshop_commission_relation') . ' where pid = :pid', array(':pid' => $member['id']));
										...
										if (!empty($data)) {
											...
											pdo_update('vcshop_member', array('headsid' => 0), array('id' => $ids));
										}
									}
								}
							}
						...
						// han 20190723 以下为奖励s
						if($checkIsReward){
							$plugin_com->agentReward($member['id'],$level);
						}
						// han 20190723 以上为奖励e
					}
				}
			...
			if ($hasglobonus) {
				if (cv('globonus.partner.check')) {
					...
					if (!empty($gdata)) {
						...
						pdo_update('vcshop_member', $gdata, array('id' => $id, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			...
			if ($hasauthor) {
				if (cv('author.partner.check')) {
					...
					if (!empty($author_data)) {
						...
						pdo_update('vcshop_member', $author_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			...
			if ($hasabonus) {
				if (cv('abonus.agent.check')) {
					...
					if (!empty($aadata)) {
						...
						pdo_update('vcshop_member', $aadata, array('id' => $id, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			...
			if ($hassns) {
				if (cv('sns.member.edit')) {
					...
					if (!empty($snsdata)) {
						...
						pdo_update('vcshop_sns_member', $snsdata, array('id' => $sns_member['id'], 'uniacid' => $_W['uniacid']));
					}
				}
			}
		...
		$order = pdo_fetch('select finishtime from ' . tablename('vcshop_order') . ' where uniacid=:uniacid and openid=:openid and status=3 order by id desc limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		$member['last_ordertime'] = $order['finishtime'];
	...
	public function delete()
	{
		...
		foreach ($members as $member) {
			pdo_delete('vcshop_member', array('id' => $member['id']));
	...
	public function setblack()
	{
		...
		$members = pdo_fetchall('select id,openid,nickname,realname,mobile from ' . tablename('vcshop_member') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
		$black = intval($_GPC['isblack']);

		foreach ($members as $member) {
			if (!empty($black)) {
				pdo_update('vcshop_member', array('isblack' => 1), array('id' => $member['id']));
				plog('member.list.edit', '设置黑名单 <br/>用户信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else {
				pdo_update('vcshop_member', array('isblack' => 0), array('id' => $member['id']));
	...
	public function changelevel()
	{
		...
		if ($_W['ispost']) {
			...
			// han 20200218 s
			$clevelid = $_GPC['clevel'];
			!strpos($clevelid, ',') && ($clevelid = intval($_GPC['clevel']));
			// han 20200218 e
			...
			if ($toggle == 'group') {
				if (!empty($levelid)) {
					...
					if (!empty($levelid_arr)) {
						foreach ($levelid_arr as $id) {
							$group = pdo_fetch('select * from ' . tablename('vcshop_member_group') . ' where id = :id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				if (!empty($levelid)) {
					$level = pdo_fetch('select * from ' . tablename('vcshop_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $levelid, ':uniacid' => $_W['uniacid']));

					if (empty($level)) {
						show_json(0, '此会员等级不存在');
					}
				}
				else {
					$set = m('common')->getSysset();
				...
				// han 20200218
				if (!empty($clevelid)) {
					$clevel = pdo_fetch('select * from ' . tablename('vcshop_commission_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $clevelid, ':uniacid' => $_W['uniacid']));

					if (empty($clevel)) {
						show_json(0, '此分销等级不存在');
					}
					$arr['agentlevel'] = $clevelid;
					$arr['isagent'] = 1;
					$arr['status'] = 1;
					$loginfo .= ' 用户分销等级 分销等级名称：' . $clevel['levelname'];

				}
				else {
					$cset = $_S['commission'];
					$cdefault = array('id' => 0, 'level' => 0, 'levelname' => empty($cset['levelname']) ? '默认等级' : $cset['levelname']);
					$clevel = array('levelname' => empty($cdefault['levelname']) ? '普通等级' : $cdefault['levelname']);
					$arr['agentlevel'] = $clevelid;
					$arr['isagent'] = 1;
					$arr['status'] = 1;
					$loginfo .= ' 用户分销等级 分销等级名称：' . $clevel['levelname'];
			...
			$changeids = array();
			$members = pdo_fetchall('select id,openid,nickname,realname,mobile from ' . tablename('vcshop_member') . (' WHERE id in( ' . $idsstr . ' ) AND uniacid=') . $_W['uniacid']);
			...
			if (!empty($members)) {
				foreach ($members as $member) {
					if ($toggle == 'group') {
						...
						// 以下为检查奖励s
						$checkIsReward = false;
						if($arr['isagent'] == 1 && $arr['status'] == 1 && $arr['agentlevel'] > 0){
							$clevel = pdo_fetch('select * from ' . tablename('vcshop_commission_level') . ' where uniacid=:uniacid and id=:id', array(':uniacid' => $_W['uniacid'],':id'=>$arr['agentlevel']));
							$checkIsReward = p('commission')->checkIsReward($member['openid'],$clevel,'commission');
						}
						// 以上为奖励e
						// han 20191116 e
						pdo_update('vcshop_member', $arr, array('id' => $member['id']));
						$changeids[] = $member['id'];
						// han 20190723 以下为奖励s
						if($checkIsReward){
							p('commission')->agentReward($member['id'],$clevel);
						}
						// han 20190723 以上为奖励e
					}
				}
