...
class Level_VcShopPage extends WebPage
{
	public function main()
	{
		...
		$shopset = $set['shop'];
		$default = array('id' => 'default', 'levelname' => empty($set['shop']['levelname']) ? '普通等级' : $set['shop']['levelname'], 'discount' => $set['shop']['leveldiscount'], 'ordermoney' => 0, 'ordercount' => 0, 'membercount' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_member') . ' where uniacid=:uniacid and level=0 limit 1', array(':uniacid' => $_W['uniacid'])));
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_member_level') . (' WHERE 1 ' . $condition . ' ORDER BY level asc'), $params);

		foreach ($others as &$row) {
			$row['membercount'] = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_member') . ' where uniacid=:uniacid and level=:level limit 1', array(':uniacid' => $_W['uniacid'], ':level' => $row['id']));
		}
	...
	protected function post()
	{
		...
		$id = trim($_GPC['id']);
		$set = m('common')->getSysset();
		$setdata = pdo_fetch('select * from ' . tablename('vcshop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		...
		if ($id == 'default') {
			...
			$level = pdo_fetch('SELECT * FROM ' . tablename('vcshop_member_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => intval($id)));
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if ($id == 'default') {
					...
					pdo_update('vcshop_member_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				$data['goodsids'] = $goodsids;
				$data['buygoods'] = $buygoods;
				pdo_insert('vcshop_member_level', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('vcshop_member_level') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		foreach ($items as $item) {
			$res = pdo_delete('vcshop_member_level', array('id' => $item['id']));
			if ($res) {
                pdo_update("vcshop_member", array("level" => 0), array("uniacid" => $_W["uniacid"], "level" => $item["id"]));
            }
	...
	public function enabled()
	{
		...
		foreach ($items as $item) {
			pdo_update('vcshop_member_level', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
