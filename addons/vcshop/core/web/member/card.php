...
class Card_VcShopPage extends WebPage
{
	...
	public function activationset()
	{
		...
		if (!empty($item['couponid'])) {
			$coupon = pdo_fetch('SELECT id,couponname as title , thumb  FROM ' . tablename('vcshop_coupon') . ' WHERE uniacid = ' . $_W['uniacid'] . ' and id =' . $item['couponid']);
		}

		$levels = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_member_level') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY level asc'));
