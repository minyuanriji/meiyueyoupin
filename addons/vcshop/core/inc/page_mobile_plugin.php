...
class PluginMobilePage extends MobilePage
{
    ...
    protected function view($template, $params = array())
    {
        ...
        $pluginTemplatePath = VCSHOP_PLUGIN . (string) $plugin . "/template/mobile/default/";
        if ($plugin == "pc") {
        ...
        $defaultParams = array("basePath" => VCSHOP_LOCAL . "plugin/" . $plugin . "/static", "staticPath" => VCSHOP_LOCAL . "static/", "appJsPath" => VCSHOP_LOCAL . "static/js/app", "title" => "微信商城");
        if (empty($params)) {
            $params = array();
        }
