...
class PluginWebPage extends WebPage
{
	...
	public function __construct($_init = true)
	{
		...
		if (strpos($this->pluginname, 'open_messikefu') !== false) {
			...
			$key = pdo_fetch('SELECT * FROM ' . tablename('vcshop_open_plugin') . ' WHERE plugin = :plugin', array(':plugin' => $this->pluginname));
			...
			if ($key['expirtime'] <= time() || is_null($redis->get($redis_key))) {
				...
				pdo_update('vcshop_open_plugin', array('expirtime' => time() + 172800), array('id' => $key['id']));
			}
		}
