...
class Page extends WeModuleSite
{
	public function runTasks()
	{
		...
		if ($lasttime + $interval <= $current) {
			...
			ihttp_request(VCSHOP_TASK_URL . 'order/receive.php', NULL, NULL, 10);
		}
		...
		if ($lasttime + $interval <= $current) {
			...
			ihttp_request(VCSHOP_TASK_URL . 'order/close.php', NULL, NULL, 10);
		}
		...
		if ($lasttime + $interval_v <= $current) {
			...
			ihttp_request(VCSHOP_TASK_URL . 'order/close.php', array('uniacid' => $_W['uniacid']), NULL, 10);
		}
		...
		if ($lasttime + $interval <= $current) {
			...
			ihttp_request(VCSHOP_TASK_URL . 'order/fullback.php', NULL, NULL, 10);
		}
		...
		if ($lasttime + $interval <= $current) {
			...
			ihttp_request(VCSHOP_TASK_URL . 'goods/presell.php', NULL, NULL, 10);
		}
		...
		if ($lasttime + $interval <= $current) {
			...
			ihttp_request(VCSHOP_TASK_URL . 'goods/status.php', NULL, NULL, 10);
		}
		...
		if (com('coupon')) {
			...
			if ($lasttime + $interval <= $current) {
				...
				ihttp_request(VCSHOP_TASK_URL . 'order/willclose.php', NULL, NULL, 10);
			}
		}
		...
		if (com('coupon')) {
			...
			if ($lasttime + $interval <= $current) {
				...
				ihttp_request(VCSHOP_TASK_URL . 'coupon/back.php', NULL, NULL, 10);
			}
		}
		...
		if ($lasttime + $interval <= $current) {
			...
			ihttp_request(VCSHOP_TASK_URL . 'notice/sendnotice.php', array('uniacid' => $_W['uniacid']), NULL, 10);
		}
		...
		if ($lasttime + $interval <= $current) {
			...
			ihttp_request(VCSHOP_TASK_URL . 'cycelbuy/sendnotice.php', array('uniacid' => $_W['uniacid']), NULL, 10);
		}
		...
		if ($lasttime + $interval <= $current) {
			...
			ihttp_request(VCSHOP_TASK_URL . 'cycelbuy/receive.php', array('uniacid' => $_W['uniacid']), NULL, 10);
		}
		...
		if (p('groups')) {
			...
			if ($groups_order_lasttime + $groups_order_interval <= $groups_order_current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/vcshop/plugin/groups/task/order.php', NULL, NULL, 10);
			}
			...
			if ($groups_team_lasttime + $groups_team_interval <= $groups_team_current) {
				...
				ihttp_request($_W['siteroot'] . ('addons/vcshop/plugin/groups/task/refund.php?uniacid=' . $_W['uniacid']), NULL, NULL, 10);
			}
			...
			if ($groups_receive_lasttime + $groups_receive_interval <= $groups_receive_current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/vcshop/plugin/groups/task/receive.php', NULL, NULL, 10);
			}
		}
		...
		if (p('seckill')) {
			...
			if ($lasttime + $interval <= $current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/vcshop/plugin/seckill/task/delete.php', NULL, NULL, 10);
			}
		}
		...
		if (p('friendcoupon')) {
			...
			if ($lasttime + $interval <= $current) {
				...
				ihttp_request($_W['siteroot'] . ('addons/vcshop/plugin/friendcoupon/task/sendMessage.php?uniacid=' . $_W['uniacid']), NULL, NULL, 10);
			}
		}
		...
		if (p('merch')) {
			...
			if ($lasttime + $interval <= $current) {
				...
				ihttp_request(VCSHOP_TASK_URL . 'plugin/merch.php', NULL, NULL, 10);
			}
		}
		if(p('commission')){
			$lasttime = strtotime(m('cache')->getString('usermonthlycredit', 'global'));
			$interval = 2;
			$interval *= 60;
			$current = time();

			if ($lasttime + $interval <= $current) {
				m('cache')->set('usermonthlycredit', date('Y-m-d H:i:s', $current), 'global');
				ihttp_request(VCSHOP_TASK_URL . 'plugin/commission.php', array('uniacid' => $_W['uniacid']), NULL, 10);
			}
			...
			if ($lasttime + $interval <= $current) {
				...
				ihttp_request(VCSHOP_TASK_URL . 'plugin/expriecreditnotice.php', array('uniacid' => $_W['uniacid']), NULL, 10);
			}
		}

		// 20201106 lin
		if (com('virtualcard')) {
			$lasttime = strtotime(m('cache')->getString('virtualcardsend', 'global'));
			...
			if ($lasttime + $interval <= $current) {
				m('cache')->set('virtualcardsend', date('Y-m-d H:i:s', $current), 'global');
				ihttp_request(VCSHOP_TASK_URL . 'virtualcard/credit.php', NULL, NULL, 10);
			}

			// $lasttime = strtotime(m('cache')->getString('vrcreditnotice', 'global'));
			// $interval = 60;
			// $interval *= 60;
			// $current = time();

			// if ($lasttime + $interval <= $current) {
			// 	m('cache')->set('vrcreditnotice', date('Y-m-d H:i:s', $current), 'global');
			// 	ihttp_request(VCSHOP_TASK_URL . 'virtualcard/creditnotice.php', array('uniacid' => $_W['uniacid']), NULL, 10);
			// }
		}
	}
	...
	public function template($filename = '', $type = TEMPLATE_INCLUDEPATH, $account = false)
	{
		...
		$name = 'vcshop';
		$moduleroot = IA_ROOT . '/addons/vcshop';
