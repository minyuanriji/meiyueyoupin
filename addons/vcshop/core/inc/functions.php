...
if( !function_exists("m") ) 
{
	function m($name = "") 
	{
		...
		$model = VCSHOP_CORE . "model/" . strtolower($name) . ".php";
		if( !is_file($model) ) 
		{
			exit( " Model " . $name . " Not Found!" );
		}
		require_once($model);
		$class_name = ucfirst($name) . "_VcShopModel";
...
if( !function_exists("d") ) 
{
	function d($name = "") 
	{
		...
		$model = VCSHOP_CORE . "data/" . strtolower($name) . ".php";
		if( !is_file($model) ) 
		{
			exit( " Data Model " . $name . " Not Found!" );
		}
		require_once(VCSHOP_INC . "data_model.php");
		require_once($model);
		$class_name = ucfirst($name) . "_VcShopDataModel";
		$_modules[$name] = new $class_name();
		return $_modules[$name];
	}
}
...
if( !function_exists("socket") ) 
{
	function socket($name) 
	{
		...
		$socket = VCSHOP_PLUGIN . strtolower($name) . "/core/socket.php";
		if( !is_file($socket) ) 
		{
			return false;
		}
...
if( !function_exists("p") ) 
{
	function p($name = "") 
	{
		...
		$model = VCSHOP_PLUGIN . strtolower($name) . "/core/model.php";
		if( !is_file($model) ) 
		{
			return false;
		}
...
if( !function_exists("com") ) 
{
	function com($name = "") 
	{
		...
		$model = VCSHOP_CORE . "com/" . strtolower($name) . ".php";
		if( !is_file($model) ) 
		{
			return false;
		}
		require_once(VCSHOP_CORE . "inc/com_model.php");
		require_once($model);
		$class_name = ucfirst($name) . "_VcShopComModel";
...
if( !function_exists("com_perm_check_plugin") ) 
{
	function com_perm_check_plugin($pluginname = "") 
	{
		...
		$ac_perm = pdo_fetch("select  plugins from " . tablename("vcshop_perm_plugin") . " where acid=:acid limit 1", array( ":acid" => $acid ));
		if( !empty($ac_perm) ) 
		{
			$allow_plugins = explode(",", $ac_perm["plugins"]);
			if( !in_array($pluginname, $allow_plugins) ) 
			{
				$filename = "../addons/vcshop/core/model/grant.php";
...
if( !function_exists("com_perm_check_com") ) 
{
	function com_perm_check_com($comname) 
	{
		...
		$allow = true;
		$acid = pdo_fetchcolumn("SELECT acid FROM " . tablename("account_wechats") . " WHERE `uniacid`=:uniacid LIMIT 1", array( ":uniacid" => $_W["uniacid"] ));
		$ac_perm = pdo_fetch("select  coms from " . tablename("vcshop_perm_plugin") . " where acid=:acid limit 1", array( ":acid" => $acid ));
		if( !empty($ac_perm) ) 
		{
...
if( !function_exists("check_operator_perm") ) 
{
	function check_operator_perm($pluginname = "") 
	{
		...
		$user_perms = array( );
		$role_perms = array( );
		$item = pdo_fetch("SELECT * FROM " . tablename("vcshop_perm_user") . " WHERE uid =:uid and deleted=0 and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":uid" => $uid ));
		if( !empty($item) ) 
		{
			$role = pdo_fetch("SELECT * FROM " . tablename("vcshop_perm_role") . " WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":id" => $item["roleid"] ));
			if( !empty($role) ) 
			{
...
if( !function_exists("get_auth") ) 
{
	...
	{
		global $_W;
		$set = pdo_fetch("select sets from " . tablename("vcshop_sysset") . " order by id asc limit 1");
...
if( !function_exists("rc") ) 
{
	function rc($plugin = "") 
	{
		...
		// $resp = ihttp_request(VCSHOP_AUTH_URL, array( "ip" => $ip, "id" => $id, "code" => $auth["code"], "domain" => $domain, "plugin" => $plugin ), NULL, 1);
...
if( !function_exists("mobileUrl") ) 
{
	function mobileUrl($do = "", $query = NULL, $full = false) 
	{
		...
		$query = array_merge(array( "m" => "vcshop" ), $query);
		if( empty($query["mid"]) ) 
		{
			$mid = intval($_GPC["mid"]);
			if( !empty($mid) ) 
...
if( !function_exists("redis") ) 
{
	function redis() 
	{
		...
		if( is_null($redis) ) 
		{
			...
			if( !isset($_W["config"]["setting"]["redis"]) && !isset($_W["config"]["setting"]["vcshop_redis"]) ) 
			{
				return error(-1, "未配置 redis, 请检查 data/config.php 中参数设置");
			}
			$config = (isset($_W["config"]["setting"]["vcshop_redis"]) ? $_W["config"]["setting"]["vcshop_redis"] : $_W["config"]["setting"]["redis"]);
...
if( !function_exists("tpl_form_field_image2") ) 
{
	function tpl_form_field_image2($name, $value = "", $default = "", $options = array( )) 
	{
		...
		if( empty($default) ) 
		{
			$default = "../addons/vcshop/static/images/nopic.png";
		}
		$val = $default;
		if( !empty($value) ) 
		{
		...
		{
			$val = "../addons/vcshop/static/images/default-pic.jpg";
		}
...
if( !function_exists("tpl_form_field_multi_image2") ) 
{
	function tpl_form_field_multi_image2($name, $value = array( ), $options = array( )) 
	{
		...
		{
			$s = "\n<script type=\"text/javascript\">\n\tfunction uploadMultiImage(elm) {\n\t\tvar name = \$(elm).next().val();\n\t\tutil.image( \"\", function(urls){\n\t\t\t\$.each(urls, function(idx, url){\n\t\t\t\t\$(elm).parent().parent().next().append('<div class=\"multi-item\"><img onerror=\"this.src=\\'../addons/vcshop/static/images/nopic.png\\'; this.title=\\'图片未找到.\\'\" src=\"'+url.url+'\" class=\"img-responsive img-thumbnail\"><input type=\"hidden\" name=\"'+name+'[]\" value=\"'+url.attachment+'\"><em class=\"close\" title=\"删除这张图片\" onclick=\"deleteMultiImage(this)\">×</em></div>');\n\t\t\t});\n\t\t}, " . json_encode($options) . ");\n\t}\n\tfunction deleteMultiImage(elm){\n\t\trequire([\"jquery\"], function(\$){\n\t\t\t\$(elm).parent().remove();\n\t\t});\n\t}\n</script>";
		...
		if( is_array($value) && 0 < count($value) ) 
		{
			...
			{
				$s .= "\n<div class=\"multi-item\">\n\t<img src=\"" . tomedia($row) . "\" onerror=\"this.src='../addons/vcshop/static/images/nopic.png'; this.title='图片未找到.'\" class=\"img-responsive img-thumbnail\">\n\t<input type=\"hidden\" name=\"" . $name . "[]\" value=\"" . $row . "\" >\n\t<em class=\"close\" title=\"删除这张图片\" onclick=\"deleteMultiImage(this)\">×</em>\n</div>";
			}
		}
		$s .= "</div>";
		return $s;
...
if( !function_exists("tpl_form_field_vcshop_daterange") ) 
{
	function tpl_form_field_vcshop_daterange($name, $value = array( ), $time = false) 
	{
		$s = "";
		if( empty($time) && !defined("TPL_INIT_DATERANGE_DATE") ) 
		{
...
if( !function_exists("tpl_form_field_vcshop_date") ) 
{
	function tpl_form_field_vcshop_date($name, $value = "", $withtime = false) 
	{
		$s = "";
...
if( !function_exists("tpl_goods_selector") ) 
{
	function tpl_goods_selector($name, $data_gs = "", $option = array( )) 
	{
		...
		$path = IA_ROOT . "/addons/vcshop/template/web_v3/util/tpl_goods_selector.html";
		include($path);
