...
class Refund_VcShopPage extends MobileLoginPage
{
    protected function globalData()
    {
        ...
        if (1 <= $order["status"]) {
            $goods = pdo_fetchall("select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund,g.refund,g.returngoods,g.exchange,g.type, g.thumb, g.title,g.isfullback from" . tablename("vcshop_order_goods") . " og left join " . tablename("vcshop_goods") . " g on g.id=og.goodsid where og.orderid=" . $order["id"]);
            if (!empty($goods)) {
                foreach ($goods as $g) {
        ...
        $fullback_log = pdo_fetchall("select * from " . tablename("vcshop_fullback_log") . " where orderid = " . $orderid . " and uniacid = " . $uniacid . " ");
        $fullbackkprice = 0;
        if ($fullback_log && is_array($fullback_log)) {
            foreach ($fullback_log as $key => $value) {
                $fullbackgoods = pdo_fetch("select refund from " . tablename("vcshop_fullback_goods") . " where goodsid = " . $value["goodsid"] . " and uniacid = " . $uniacid . " ");
    ...
    public function main()
    {
        ...
        if (0 < $order["refundstate"]) {
            if (!empty($refundid)) {
                $refund = pdo_fetch("select * from " . tablename("vcshop_order_refund") . " where id=:id and uniacid=:uniacid and orderid=:orderid limit 1", array(":id" => $refundid, ":uniacid" => $uniacid, ":orderid" => $orderid));
    ...
    public function submit()
    {
        ...
        if ($order["refundstate"] == 0) {
            ...
            pdo_insert("vcshop_order_refund", $refund);
            $refundid = pdo_insertid();
            pdo_update("vcshop_order", array("refundid" => $refundid, "refundstate" => $refundstate), array("id" => $orderid, "uniacid" => $uniacid));
        } else {
            pdo_update("vcshop_order", array("refundstate" => $refundstate), array("id" => $orderid, "uniacid" => $uniacid));
            pdo_update("vcshop_order_refund", $refund, array("id" => $refundid, "uniacid" => $uniacid));
        }
    ...
    public function cancel()
    {
        ...
        pdo_update("vcshop_order_refund", $change_refund, array("id" => $refundid, "uniacid" => $uniacid));
        pdo_update("vcshop_order", array("refundstate" => 0), array("id" => $orderid, "uniacid" => $uniacid));
        show_json(1);
    }
    ...
    public function receive()
    {
        ...
        pdo_update("vcshop_order_refund", $refund_data, array("id" => $refundid, "uniacid" => $uniacid));
        $order_data = array();
        $order_data["refundstate"] = 0;
        $order_data["status"] = 3;
        $order_data["refundtime"] = $time;
        $order_data["finishtime"] = $time;
        pdo_update("vcshop_order", $order_data, array("id" => $orderid, "uniacid" => $uniacid));
        show_json(1);
    }
