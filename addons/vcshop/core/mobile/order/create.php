...
class Create_VcShopPage extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
			...
			if (p('bargain') && !(empty($bargain_id))) 
			{
				...
				$bargain_act = pdo_fetch('SELECT *  FROM ' . tablename('vcshop_bargain_actor') . ' WHERE id = :id AND openid = :openid AND status = \'0\'', array(':id' => $bargain_id, ':openid' => $_W['openid']));
				if (empty($bargain_act)) 
				{
					exit('没有这个商品!');
				}
				$bargain_act_id = pdo_fetch('SELECT *  FROM ' . tablename('vcshop_bargain_goods') . ' WHERE id = \'' . $bargain_act['goods_id'] . '\'');
				if (empty($bargain_act_id)) 
				{
					exit('没有这个商品!');
				}
				$if_bargain = pdo_fetch('SELECT bargain FROM ' . tablename('vcshop_goods') . ' WHERE id = :id AND uniacid = :uniacid ', array(':id' => $bargain_act_id['goods_id'], ':uniacid' => $_W['uniacid']));
			...
			if (empty($id)) 
			{
				if (!(empty($quickid))) 
				{
					$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,' . ' g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.manydeduct2,g.virtual,o.virtual as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates,g.isfullback, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,g.unite_total,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption, g.threen' . ' FROM ' . tablename('vcshop_quick_cart') . ' c ' . ' left join ' . tablename('vcshop_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('vcshop_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid and c.quickid=' . $quickid . '  order by c.id desc';
					$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
				}
				else if (empty($exchangeOrder)) 
				{
					$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,' . ' g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.manydeduct2,g.virtual,o.virtual as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption' . ' FROM ' . tablename('vcshop_member_cart') . ' c ' . ' left join ' . tablename('vcshop_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('vcshop_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc';
					$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
				}
				else if (p('exchange')) 
				{
					$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,' . ' g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.manydeduct2,g.virtual,o.virtual as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates,g.isfullback, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,g.unite_total,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption' . ' FROM ' . tablename('vcshop_exchange_cart') . ' c ' . ' left join ' . tablename('vcshop_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('vcshop_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc';
					$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
				}
				if (empty($goods)) 
				{
				...
				else 
				{
					...
					foreach ($goods as $k => $v ) 
					{
						...
						if ($v['isfullback']) 
						{
							$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('vcshop_fullback_goods') . ' WHERE goodsid = :goodsid and uniacid = :uniacid and status = 1 limit 1 ', array(':goodsid' => $v['goodsid'], ':uniacid' => $uniacid));
						}
						if ($is_openmerch == 0) 
						{
							if (0 < $v['merchid']) 
			...
			else if (!(empty($id)) && !(empty($iswholesale))) 
			{
				...
				foreach ($optionsdata as $option ) 
				{
					...
					{
						$option = pdo_fetch('select id,title,marketprice,presellprice,goodssn,productsn,`virtual`,stock,weight,specs from ' . tablename('vcshop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $option['optionid']));
						if (!(empty($option))) 
						{
			...
			else 
			{
				...
				$sql = 'SELECT id as goodsid,type,title,weight,issendfree,isnodiscount,ispresell,presellprice,' . ' thumb,marketprice,liveprice,islive,storeids,isverify,isforceverifystore,deduct,hasoption,preselltimeend,presellsendstatrttime,presellsendtime,presellsendtype,' . ' manydeduct,manydeduct2,`virtual`,maxbuy,usermaxbuy,discounts,total as stock,deduct2,showlevels,' . ' ednum,edmoney,edareas,edareas_code,unite_total,diyfields,' . ' diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy, ' . ' isdiscount,isdiscount_time,isdiscount_discounts,isfullback, ' . ' virtualsend,invoice,needfollow,followtip,followurl,merchid,checked,merchsale, ' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale,bargain' . $threensql . $ishotelsql . ' FROM ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
				$data = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $id));
				...
				if (!(empty($liveid))) 
				{
					...
					if (!(empty($isLiveGoods))) 
					{
						...
						if ($defaultcardid) 
						{
							$live_product = pdo_fetch('SELECT *  FROM ' . tablename('vcshop_goods') . ' WHERE id = \'' . $data['goodsid'] . '\'');
							if ($live_product) 
							{
				...
				{
					$gift = pdo_fetch('select id,title,thumb,activity,giftgoodsid,goodsid from ' . tablename('vcshop_gift') . "\n" . '                where uniacid = ' . $uniacid . ' and id = ' . $giftid . ' and status = 1 and starttime <= ' . time() . ' and endtime >= ' . time() . ' ');
					...
					if (!(empty($gift['giftgoodsid']))) 
					{
						...
						if ($giftGoodsid) 
						{
							...
							{
								$giftGood[$key] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('vcshop_goods') . ' where uniacid = ' . $uniacid . ' and total > 0 and status = 2 and id = ' . $value . ' and deleted = 0 ');
							}
				...
				if ($data['isfullback']) 
				{
					$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('vcshop_fullback_goods') . ' WHERE goodsid = :goodsid and uniacid = :uniacid and status = 1 limit 1 ', array(':goodsid' => $data['goodsid'], ':uniacid' => $uniacid));
				}
			...
			foreach ($goods as &$g ) 
			{
				...
				else 
				{
					if (p('task')) 
					{
						...
						if (!(empty($task_id))) 
						{
							$rewarded = pdo_fetchcolumn('SELECT `rewarded` FROM ' . tablename('vcshop_task_extension_join') . ' WHERE id = :id AND openid = :openid AND  uniacid = :uniacid', array(':id' => $task_id, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
				...
				else 
				{
					...
					if (0 < $g['usermaxbuy']) 
					{
						$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=0 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $g['goodsid'], ':uniacid' => $uniacid, ':openid' => $openid));
			...
			if ($hasinvoice) 
			{
				$invoicename = pdo_fetchcolumn('select invoicename from ' . tablename('vcshop_order') . ' where openid=:openid and uniacid=:uniacid and ifnull(invoicename,\'\')<>\'\' order by id desc limit 1', array(':openid' => $openid, ':uniacid' => $uniacid));
			...
			if (!($isverify) && !($isvirtual) && !($ismerch)) 
			{
				if (0 < $merch_id) 
				{
					$carrier_list = pdo_fetchall('select * from ' . tablename('vcshop_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(1,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merch_id));
				}
				else 
				{
					$carrier_list = pdo_fetchall('select * from ' . tablename('vcshop_store') . ' where  uniacid=:uniacid and status=1 and type in(1,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
				}
			}
			...
			if ($isverify) 
			{
				...
				if (empty($storeids)) 
				{
					if (0 < $merchid) 
					{
						$stores = pdo_fetchall('select * from ' . tablename('vcshop_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
					}
					else 
					{
						$stores = pdo_fetchall('select * from ' . tablename('vcshop_store') . ' where  uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
					}
				}
				else if (0 < $merchid) 
				{
					$stores = pdo_fetchall('select * from ' . tablename('vcshop_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else 
				{
					$stores = pdo_fetchall('select * from ' . tablename('vcshop_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
				}
			}
			else 
			{
				$address = pdo_fetch('select * from ' . tablename('vcshop_member_address') . ' where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1', array(':uniacid' => $uniacid, ':openid' => $openid));
			...
			foreach ($goods as $g ) 
			{
				...
				{
					$gift = array();
					$giftdata = pdo_fetch('select giftgoodsid from ' . tablename('vcshop_gift') . ' where uniacid = ' . $uniacid . ' and id = ' . $giftid . ' and status = 1 and starttime <= ' . time() . ' and endtime >= ' . time() . ' ');
					if ($giftdata['giftgoodsid']) 
					{
						$giftgoodsid = explode(',', $giftdata['giftgoodsid']);
						foreach ($giftgoodsid as $key => $value ) 
						{
							$giftinfo = pdo_fetch('select id as goodsid,title,thumb from ' . tablename('vcshop_goods') . ' where uniacid = ' . $uniacid . ' and total > 0 and status = 2 and id = ' . $value . ' and deleted = 0 ');
							if ($giftinfo) 
							{
								$gift[$key] = $giftinfo;
								$gift[$key]['total'] = 1;
				...
				else 
				{
					...
					foreach ($gifts as $key => $value ) 
					{
						...
						{
							$giftgoodsdetail = pdo_fetch('select id,title,thumb,marketprice,total from ' . tablename('vcshop_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and status = 2 and id = ' . $val . ' ');
							if ($giftgoodsdetail) 
							{
		...
		else 
		{
			...
			$package = pdo_fetch('SELECT * FROM ' . tablename('vcshop_package') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $packageid . ' ');
			...
			{
				$tablename = tablename('vcshop_goods');
		...
		// 20200429 lin
		$createInfo['freight'] = $shop['freight'];
		include $this->template();
	}
	...
	public function caculatecoupon($contype, $couponid, $wxid, $wxcardid, $wxcode, $goodsarr, $totalprice, $discountprice, $isdiscountprice, $isSubmit = 0, $discountprice_array = array(), $merchisdiscountprice = 0, $real_price = 0) 
	{
		...
		if ($contype == 1) 
		{
			$sql = 'select id,uniacid,card_type,logo_url,title, card_id,least_cost,reduce_cost,discount,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids,merchid,limitdiscounttype  from ' . tablename('vcshop_wxcard');
		...
		{
			$sql = 'SELECT d.id,d.couponid,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodids,c.limitgoodcateids,c.limitdiscounttype  FROM ' . tablename('vcshop_coupon_data') . ' d';
			$sql .= ' left join ' . tablename('vcshop_coupon') . ' c on d.couponid = c.id';
	...
	public function caculate() 
	{
		...
		if (!(empty($packageid))) 
		{
			$package = pdo_fetch('SELECT id,title,price,freight,cash,starttime,endtime,dispatchtype FROM ' . tablename('vcshop_package') . "\n" . '                    WHERE uniacid = ' . $uniacid . ' and id = ' . $packageid . ' and deleted = 0 and status = 1  ORDER BY id DESC');
		}
		$dispatchid = intval($_GPC['dispatchid']);
		$totalprice = floatval($_GPC['totalprice']);
		$dflag = $_GPC['dflag'];
		$addressid = intval($_GPC['addressid']);
		$address = pdo_fetch('select * from ' . tablename('vcshop_member_address') . ' where  id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $uniacid, ':openid' => $openid, ':id' => $addressid));
		$member = m('member')->getMember($openid, true);
		$level = m('member')->getLevel($openid);
		...
		if (is_array($goodsarr)) 
		{
			...
			foreach ($goodsarr as &$g ) 
			{
				...
				if (!(empty($optionid))) 
				{
					$option = pdo_fetch('select id,title,marketprice,presellprice,goodssn,productsn,stock,`virtual`,weight,liveprice,islive from ' . tablename('vcshop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
					if (!(empty($option))) 
					{
						$data['optionid'] = $optionid;
						$data['optiontitle'] = $option['title'];
				...
				if (p('bargain')) 
				{
					$bargain_act_id = pdo_fetchall('SELECT *  FROM ' . tablename('vcshop_bargain_goods') . ' WHERE goods_id = \'' . $g['goodsid'] . '\' and status = 0');
					if (!(empty($bargain_act_id))) 
					{
						$allow_sale = false;
					}
	...
	public function submit() 
	{
		...
		if ($open_redis) 
		{
			...
			$redis_key = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_' . $_W['uniacid'] . '_order_submit_' . $openid;
			$redis = redis();
			if (!(is_error($redis))) 
			{
		...
		// han 20191115 s
		if(!(empty($_W['shopset']['wap']['mustagent']))){
			if(empty($member['agentid']) && (empty($member['isagent']) || empty($member['status']) || empty($member['agentlevel']))){
				show_json(0,'暂时无法访问，请稍后再试!');
			}
		}
		// han 20191115 e
		...
		if (!(empty($packageid))) 
		{
			...
			$packgoods = pdo_fetchall('SELECT id,title,thumb,packageprice,`option`,goodsid FROM ' . tablename('vcshop_package_goods') . "\n" . '                    WHERE uniacid = ' . $uniacid . ' and pid = ' . $packageid . '  ORDER BY id DESC');
			if (empty($packgoods)) 
			{
				show_json(0, '未找到套餐商品！');
			}
		...
		$sql = 'SELECT diyformtype,diyfields,bargain,diyformid FROM ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
		...
		if ($giftid) 
		{
			...
			if ($giftdata['giftgoodsid']) 
			{
				...
				foreach ($giftgoodsid as $key => $value ) 
				{
					$giftinfo = pdo_fetch('select id as goodsid,title,thumb from ' . tablename('vcshop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and total>0 and id = ' . $value . ' and deleted = 0 ');
					if (empty($giftinfo)) 
					{
						continue;
					}
		...
		foreach ($goods as $g ) 
		{
			...
			else 
			{
				...
				if (p('lottery')) 
				{
					$lottery_id = pdo_get('vcshop_lottery_log', array('log_id' => $log_id), array('lottery_id'));
					if ($lottery_id['lottery_id']) 
					{
						$is_goods = pdo_get('vcshop_lottery', array('lottery_id' => $lottery_id['lottery_id']), array('is_goods'));
			...
			else 
			{
				if ($data['type'] != 4) 
				{
					...
					if (0 < $data['maxbuy']) 
					{
						...
						// han 20191213  多规格限购问题
						if($data['maxbuy'] < $total_array[$goodsid]['total'])
						{
							show_json(0, $data['title'] . '<br/> 一次限购 ' . $data['maxbuy'] . $unit . '!');
						}
					}
				...
				{
					$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=0 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $data['goodsid'], ':uniacid' => $uniacid, ':openid' => $openid));
			...
			if (!(empty($optionid))) 
			{
				$option = pdo_fetch('select id,title,marketprice,liveprice,presellprice,goodssn,productsn,stock,`virtual`,weight' . $sql_condition . ' from ' . tablename('vcshop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
				if (!(empty($option))) 
				{
					if (!(empty($_SESSION['exchange'])) && p('exchange')) 
					{
						...
						{
							pdo_query('UPDATE ' . tablename('vcshop_goods_option') . ' SET exchange_stock = exchange_stock - 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $optionid, ':uniacid' => $_W['uniacid']));
						}
					}
					else 
					{
					...
					if ($data['type'] != 4) 
					{
						...
						if ($packageid) 
						{
							$packageoption = pdo_fetch('select packageprice ,marketprice from ' . tablename('vcshop_package_goods_option') . "\n" . '                                where uniacid = ' . $uniacid . ' and goodsid = ' . $goodsid . ' and optionid = ' . $optionid . ' and pid = ' . $packageid . ' ');
			...
			if ($packageid) 
			{
				...
				{
					$pg = pdo_fetch('select packageprice,marketprice from ' . tablename('vcshop_package_goods') . "\n" . '                                    where uniacid = ' . $uniacid . ' and goodsid = ' . $goodsid . ' and pid = ' . $packageid . ' ');
			...
			else if (!(empty($_SESSION['exchange'])) && p('exchange')) 
			{
				...
				{
					pdo_query('UPDATE ' . tablename('vcshop_goods') . ' SET exchange_stock = exchange_stock - 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $data['goodsid'], ':uniacid' => $_W['uniacid']));
				}
			...
			if (intval($_GPC['fromcart']) == 1) 
			{
				if ($diyform_plugin) 
				{
					$cartdata = pdo_fetch('select id,diyformdataid,diyformfields,diyformdata from ' . tablename('vcshop_member_cart') . ' ' . ' where goodsid=:goodsid and optionid=:optionid and openid=:openid and deleted=0 order by id desc limit 1', array(':goodsid' => $data['goodsid'], ':optionid' => intval($data['optionid']), ':openid' => $openid));
					if (!(empty($cartdata))) 
					{
		...
		if (($isverify || $dispatchtype) && !($isonlyverifygoods)) 
		{
			if ($isverify) 
			{
				if (($verifytype == 0) || ($verifytype == 1)) 
				{
					...
					while (1) 
					{
						$count = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
						if ($count <= 0) 
						{
							break;
						}
				...
				else if ($verifytype == 2) 
				{
					...
					while ($i <= $totaltimes) 
					{
						...
						while (1) 
						{
							$count = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order') . ' where concat(verifycodes,\'|\' + verifycode +\'|\' ) like :verifycodes and uniacid=:uniacid limit 1', array(':verifycodes' => '%' . $verifycode . '%', ':uniacid' => $_W['uniacid']));
							if ($count <= 0) 
							{
								break;
							}
		...
		{
			$bargain_act = pdo_fetch('SELECT * FROM ' . tablename('vcshop_bargain_actor') . ' WHERE id = :id AND openid = :openid ', array(':id' => $goods[0]['bargain_id'], ':openid' => $_W['openid']));
			if (empty($bargain_act)) 
			{
				exit('没有这个商品');
			}
			...
			if (!(pdo_update('vcshop_bargain_actor', array('status' => 1), array('id' => $goods[0]['bargain_id'], 'openid' => $_W['openid'])))) 
			{
				exit('下单失败');
			}
		...
		if ($taskgoodsprice) 
		{
			...
			pdo_update('vcshop_task_extension_join', array('rewarded' => serialize($taskGoodsInfo0)), array('id' => $task_id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		...
		if (!(empty($goods[0]['bargain_id'])) && p('bargain')) 
		{
			pdo_update('vcshop_bargain_actor', array('order' => $orderid), array('id' => $goods[0]['bargain_id'], 'openid' => $_W['openid']));
		}
		if ($multiple_order == 0) 
		{
			...
			foreach ($allgoods as $goods ) 
			{
				...
				if (!(empty($bargain_act)) && p('bargain')) 
				{
					...
					pdo_query('UPDATE ' . tablename('vcshop_goods') . ' SET sales = sales + 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $goods['goodsid'], ':uniacid' => $uniacid));
				}
				...
				$order_goods['cretidgoods'] = $goods['cretidgoods'];//han 20191205
				$order_goods['stockgoods'] = $goods['stockgoods'];//han 20191205
				$order_goods['stocklevel'] = $goods['stocklevel'];//han 20191205
				...
				pdo_insert('vcshop_order_goods', $order_goods);
				if ($goods['seckillinfo'] && ($goods['seckillinfo']['status'] == 0)) 
				{
		...
		if ($data['type'] == 3) 
		{
			...
			if (com('virtual')) 
			{
				...
				{
					pdo_update('vcshop_order', array('status' => -1, 'canceltime' => time()), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
		...
		if (!(empty($_SESSION['exchange'])) && p('exchange')) 
		{
			$codeResult = pdo_fetch('SELECT * FROM ' . tablename('vcshop_exchange_code') . ' WHERE `key` = :key AND uniacid =:uniacid', array(':key' => $_SESSION['exchange_key'], ':uniacid' => $_W['uniacid']));
			if ($codeResult['status'] == 2) 
			{
				show_json(0, '兑换失败:此兑换码已兑换');
			}
			$groupResult = pdo_fetch('SELECT * FROM ' . tablename('vcshop_exchange_group') . ' WHERE id = :id AND uniacid = :uniacid ', array(':uniacid' => $_W['uniacid'], ':id' => $codeResult['groupid']));
			$record_exsit = pdo_fetch('SELECT * FROM ' . tablename('vcshop_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':key' => $_SESSION['exchange_key'], ':uniacid' => $_W['uniacid']));
			if (empty($record_exsit)) 
			{
				...
				pdo_insert('vcshop_exchange_record', $data);
			}
			...
			else 
			{
				...
				{
					$exchange_res = pdo_update('vcshop_exchange_code', array('status' => 2), array('key' => $_SESSION['exchange_key'], 'status' => 1, 'uniacid' => $_W['uniacid']));
					pdo_query('UPDATE ' . tablename('vcshop_exchange_group') . ' SET `use` = `use` + 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $_SESSION['groupid'], ':uniacid' => $_W['uniacid']));
				}
				else 
				{
					pdo_update('vcshop_exchange_code', array('goodsstatus' => 2), array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid']));
					...
					{
						pdo_update('vcshop_exchange_code', array('status' => 2), array('key' => $_SESSION['exchange_key'], 'status' => 1, 'uniacid' => $_W['uniacid']));
					}
				}
			}
		}
		if (!(is_null($_SESSION['exchangeprice'])) && !(empty($_SESSION['exchange'])) && p('exchange')) 
		{
			...
			if ($groupResult['mode'] != 6) 
			{
				...
				pdo_update('vcshop_exchange_record', $exchangedata, array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid']));
			}
			else 
			{
			...
			pdo_update('vcshop_exchange_cart', array('selected' => 0), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'selected' => 1));
		}
		...
		if ($_GPC['fromcart'] == 1) 
		{
			pdo_query('update ' . tablename('vcshop_member_cart') . ' set deleted=1 where  openid=:openid and uniacid=:uniacid and selected=1 ', array(':uniacid' => $uniacid, ':openid' => $openid));
		}
		if (p('quick') && !(empty($_GPC['fromquick']))) 
		{
			pdo_update('vcshop_quick_cart', array('deleted' => 1), array('quickid' => intval($_GPC['fromquick']), 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		}
		if (0 < $deductcredit) 
		{
		...
		if (empty($virtualid)) 
		{
			...
			{
				pdo_update('vcshop_order', array('status' => -1), array('id' => $orderid));
	...
	protected function singleDiyformData($id = 0) 
	{
		...
		{
			$sql = 'SELECT id as goodsid,type,diyformtype,diyformid,diymode,diyfields FROM ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
	...
	public function diyform() 
	{
		...
		if (empty($goods_temp)) 
		{
			pdo_insert('vcshop_diyform_temp', $insert);
			$gdid = pdo_insertid();
		}
		else 
		{
			pdo_update('vcshop_diyform_temp', $insert, array('id' => $goods_temp['id']));
			$gdid = $goods_temp['id'];
		}
		if (!(empty($cartid))) 
		{
			$cart_data = array('diyformid' => $insert['diyformid'], 'diyformfields' => $insert['diyformfields'], 'diyformdata' => $insert['diyformdata']);
			pdo_update('vcshop_member_cart', $cart_data, array('id' => $cartid));
		}
	...
	public function getcardprice() 
	{
		...
		if (0 < $liveid) 
		{
			$live_product = pdo_fetch('SELECT *  FROM ' . tablename('vcshop_goods') . ' WHERE id = \'' . $goodsarr[0]['goodsid'] . '\'');
			if ($live_product) 
			{
		...
		if (is_array($goodsarr)) 
		{
			...
			foreach ($goodsarr as $k => &$g ) 
			{
				...
				if ($g['status'] != 2) 
				{
					if (0 < $liveid) 
					{
						$live_product = pdo_fetch('SELECT *  FROM ' . tablename('vcshop_goods') . ' WHERE id = \'' . $g['goodsid'] . '\'');
						if ($live_product) 
						{
	...
	public function isfullbackgoods($goodsid = 0, $optionid = 0) 
	{
		...
		if (0 < $optionid) 
		{
			$fullback_goods = pdo_fetch('select id from' . tablename('vcshop_fullback_goods') . 'where goodsid=:goodsid and hasoption =1 and status=1 and uniacid=:uniacid and find_in_set(\'' . $optionid . '\',optionid)', array(':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid));
		}
		else 
		{
			$fullback_goods = pdo_get('vcshop_fullback_goods', array('status' => 1, 'goodsid' => $goodsid, 'uniacid' => $_W['uniacid']), 'id');
		}
		if ($fullback_goods) 
		{
