...
class Pay_VcShopPage extends MobileLoginPage
{
	const AFTER_PAY = 1;
	...
	public function main()
	{
		...
		if (!empty($ispeerpay)) {
			if (pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vcshop_order_peerpay_payinfo') . ' WHERE openid = :openid AND pid = :pid', array(':openid' => $_W['openid'], ':pid' => $ispeerpay['id']))) {
			...
			if (empty($peerprice) || $peerprice <= 0) {
				...
				$openid = pdo_fetchcolumn('SELECT openid FROM ' . tablename('vcshop_order') . ' WHERE id = :id AND uniacid = :uniacid LIMIT 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
			}
		}

		$order = pdo_fetch('select * from ' . tablename('vcshop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		if ((empty($order['isnewstore']) || empty($order['storeid'])) && empty($order['istrade'])) {
			...
			foreach ($order_goods as $data) {
				...
				if ($data['totalcnf'] == 1) {
					if (!empty($data['optionid'])) {
						$option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual` from ' . tablename('vcshop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':id' => $data['optionid']));
			...
			$sql = 'select og.id,g.title, og.goodsid,og.optionid,ng.stotal as stock,og.total as buycount,ng.gstatus as status,ng.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill,og.storeid ' . ' from ' . tablename('vcshop_order_goods') . ' og left join  ' . tablename('vcshop_goods') . ' g  on g.id=og.goodsid and g.uniacid=og.uniacid' . ' inner join ' . tablename('vcshop_newstore_goods') . ' ng on g.id=ng.goodsid   ' . ' where og.orderid=:orderid and og.uniacid=:uniacid and ng.storeid=:storeid';
			...
			foreach ($order_goods as $data) {
				...
				if ($data['totalcnf'] == 1) {
					if (!empty($data['optionid'])) {
						$option = pdo_fetch('select id,marketprice,stock from ' . tablename('vcshop_newstore_goods_option') . ' where optionid=:optionid and goodsid=:goodsid and uniacid=:uniacid and storeid=:storeid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':optionid' => $data['optionid'], ':storeid' => $data['storeid']));
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'vcshop', ':tid' => $order['ordersn']));
		...
		$seckill_goods = pdo_fetchall('select goodsid,optionid,seckill from  ' . tablename('vcshop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid and seckill=1 ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		if (!empty($log) && $log['status'] == '0') {
		...
		if (empty($log)) {
			$log = array('uniacid' => $uniacid, 'openid' => trim($member['uid']), 'module' => 'vcshop', 'tid' => trim($order['ordersn']), 'fee' => $order['price'], 'status' => 0);
		...
		$helibao_wechat = array('success' => false);// han 20200317
		load()->model('payment');
		...
		if (is_weixin() && $order['price'] <7000) {
			...
			if (!empty($order['ordersn2'])) {
				...
				if (100 <= $var) {
					$var = $var + 1;
					pdo_update('vcshop_order', array('ordersn2' => $var), array('id' => $orderid, 'uniacid' => $uniacid, 'openid' => $openid));
				}
			...
			// han 20200317 helibao s
			if (isset($set['pay']) && ($set['pay']['helibao_wechat'] == 1)) 
			{
				$options = array();
				if (is_array($setting['payment']['helibao_wechat']) && $setting['payment']['helibao_wechat']['pay_switch']) {
					load()->model('payment');
					$setting = uni_setting($_W['uniacid'], array('payment'));

					if (is_array($setting['payment'])) {
						$options = $setting['payment']['helibao_wechat'];
					}
				}

				$helibao_wechat['wopt'] = m('common')->helibao_wechat_build($params, $options, 0);
				if (!is_error($helibao_wechat['wopt'])) {
					$helibao_wechat['success'] = true;
				}
			}
			// han 20200317 helibao e
		...
		$payinfo = array('orderid' => $orderid, 'ordersn' => trim($log['tid']), 'credit' => $credit, 'alipay' => $alipay, 'wechat' => $wechat, 'helibao_wechat' => $helibao_wechat, 'cash' => $cash, 'money' => $order['price']);
	...
	public function orderstatus()
	{
		...
		$uniacid = $_W['uniacid'];
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select status from ' . tablename('vcshop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));

		if (1 <= $order['status']) {
			@session_start();
			$_SESSION[VCSHOP_PREFIX . '_order_pay_complete'] = 1;
			show_json(1);
		}
	...
	public function complete()
	{
		...
		if (!empty($ispeerpay)) {
			...
			$openid = pdo_fetchcolumn('select openid from ' . tablename('vcshop_order') . ' where id=:orderid and uniacid=:uniacid limit 1', array(':orderid' => $orderid, ':uniacid' => $uniacid));
			$peerpay_info = (double) pdo_fetchcolumn('select SUM(price) price from ' . tablename('vcshop_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(':pid' => $ispeerpay['id']));
		}
		else {
		...
		if (is_h5app() && empty($orderid)) {
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('vcshop_order') . ' where ordersn=:ordersn and uniacid=:uniacid and openid=:openid limit 1', array(':ordersn' => $ordersn, ':uniacid' => $uniacid, ':openid' => $openid));
		}
		...
		if (0 < $order['ispackage']) {
			$package = pdo_fetch('SELECT * FROM ' . tablename('vcshop_package') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $order['packageid'] . ' ');
		...
		if (!in_array($type, array('wechat', 'alipay', 'credit', 'cash','helibao_wechat'))) {
			if ($_W['ispost']) {
				show_json(0, '未找到支付方式'.$type);
			}
			else {
		...
		if ($type == 'cash' && empty($ispeerpay)) {
			...
			pdo_update('vcshop_order', array('cashtime' => time()), array('id' => $order['id']));
		...
		if ($type == 'credit') {
			...
			if ($open_redis) {
				...
				$redis_key = $_W['setting']['site']['key'] . '_' . $_W['account']['key'] . '_'.'vc_' . $_W['uniacid'] . '_peerpay_order__pay_' . $ispeerpay['id'];
				$redis = redis();
			...
			if ($type == 'wechat') {
				...
				if (!is_error($payquery) || !is_error($payquery_jie) || !empty($ispeerpay)) {
					...
					if (is_h5app()) {
						pdo_update('vcshop_order', array('apppay' => 1), array('id' => $order['id']));
					}
					...
					if (!empty($ispeerpay)) {
						...
						pdo_update('vcshop_order_peerpay_payinfo', $udata, array('pid' => $ispeerpay['id'], 'openid' => $_W['openid']));
						m("order")->setOrderPayType($order["id"], 21, $gpc_ordersn);
				...
				if ((empty($set['pay']['helibao_wechat']))) {
					if ($_W['ispost']) {
						show_json(0, '未开启微信支付!!');
					}
					else {
				...
				// }
				// else {
					$payquery = m('common')->isHelibaoWeixinPay($ordersn, $order['price'], is_h5app() ? true : false);
				// }
				...
				if (!is_error($payquery)) {
					...
					pdo_update('core_paylog', $record, array('plid' => $log['plid']));
					m('order')->setOrderPayType($order['id'], 33, $gpc_ordersn);
					$ret = array();
					$ret['result'] = 'success';
					$ret['type'] = 'helibao_wechat';
					$ret['from'] = 'return';
					$ret['tid'] = $log['tid'];
					$ret['user'] = $log['openid'];
					$ret['fee'] = $log['fee'];
	...
	public function success()
	{
		...
		if (empty($_SESSION[VCSHOP_PREFIX . '_order_pay_complete'])) {
			if (empty($order['istrade'])) {
				header('location: ' . mobileUrl('order'));
			}
			else {
		...
		unset($_SESSION[VCSHOP_PREFIX . '_order_pay_complete']);
		...
		if (!empty($ispeerpay)) {
			...
			$peerprice = pdo_fetch('SELECT `price` FROM ' . tablename('vcshop_order_peerpay_payinfo') . ' WHERE uid = :uid ORDER BY id DESC LIMIT 1', array(':uid' => $peerpayuid['id']));
			$share = false;
			...
			$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
			$address = false;
			...
			if (!empty($order['addressid'])) {
				...
				if (!is_array($address)) {
					$address = pdo_fetch('select * from  ' . tablename('vcshop_member_address') . ' where id=:id limit 1', array(':id' => $order['addressid']));
				}
			}
			...
			if (!empty($order['storeid'])) {
				if (0 < $merchid) {
					$store = pdo_fetch('select * from  ' . tablename('vcshop_merch_store') . ' where id=:id limit 1', array(':id' => $order['storeid']));
				}
				else {
					$store = pdo_fetch('select * from  ' . tablename('vcshop_store') . ' where id=:id limit 1', array(':id' => $order['storeid']));
				}
			}
			...
			if ($order['isverify']) {
				...
				if (p('newstore') && !empty($order['isnewstore'])) {
					$stores = pdo_fetchall('select * from ' . tablename('vcshop_store') . ' where id = :id  and uniacid=:uniacid and status=1  order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':id' => $order['storeid']));
				}
					...
					$stores = pdo_fetchall('select * from ' . tablename('vcshop_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else {
					$stores = pdo_fetchall('select * from ' . tablename('vcshop_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
				}
			}
		...
		if (!empty($order['virtual']) && !empty($order['virtual_str'])) {
			...
			$virtualtemp = pdo_fetch('SELECT linktext, linkurl FROM ' . tablename('vcshop_virtual_type') . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid']));
		}
	...
	public function peerpay()
	{
		...
		$peerpay = (int) pdo_fetchcolumn('select orderid from ' . tablename('vcshop_order_peerpay') . ' where orderid=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		...
		$order = pdo_fetch('select * from ' . tablename('vcshop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));

		if (100 <= $order['ordersn2']) {
			$order['ordersn2'] = 0;
			pdo_update('vcshop_order', array('ordersn2' => 0), array('id' => $orderid, 'uniacid' => $uniacid));
		}
		...
		if ($_W['ispost']) {
			...
			$res = pdo_insert('vcshop_order_peerpay', $data);
		...
		$goods = pdo_fetchall('SELECT g.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,
                ...
                FROM ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id ' . ' left join ' . tablename('vcshop_goods_option') . ' op on og.optionid = op.id ' . (' where ' . $scondition . ' and og.uniacid=:uniacid order by og.id asc'), array(':id' => $orderid, ':uniacid' => $uniacid));
		$address = iunserializer($order['address']);
		$member = m('member')->getMember($openid, true);
	...
	public function peerpayshare()
	{
		...
		$uniacid = $_W['uniacid'];
		$peerpay = pdo_fetch('select p.*,o.openid from ' . tablename('vcshop_order_peerpay') . ' p join ' . tablename('vcshop_order') . ' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1', array(':id' => $peerid, ':uniacid' => $uniacid));
		...
		$peerpay_info = (double) pdo_fetchcolumn('select SUM(price) price from ' . tablename('vcshop_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(':pid' => $peerpay['id']));
		...
		$ordergoods = pdo_fetch('select * from ' . tablename('vcshop_order_goods') . ' where orderid=:id and uniacid=:uniacid limit 1', array(':id' => $peerpay['orderid'], ':uniacid' => $uniacid));
		$goods = pdo_fetch('select * from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $ordergoods['goodsid'], ':uniacid' => $uniacid));
	...
	public function peerpaydetail()
	{
		...
		$peerid = intval($_GPC['id']);
		$uniacid = $_W['uniacid'];
		$peerpay = pdo_fetch('select p.*,o.openid,o.address,o.isparent,o.id as oid from ' . tablename('vcshop_order_peerpay') . ' p join ' . tablename('vcshop_order') . ' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1', array(':id' => $peerid, ':uniacid' => $uniacid));
		...
		$goods = pdo_fetchall('SELECT g.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,
                og.sendtype,og.expresscom,og.expresssn,og.express,og.sendtime,og.finishtime,og.remarksend
                FROM ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id ' . ' left join ' . tablename('vcshop_goods_option') . ' op on og.optionid = op.id ' . (' where ' . $scondition . ' and og.uniacid=:uniacid order by og.id asc'), array(':id' => $peerpay['orderid'], ':uniacid' => $uniacid));
		...
		if ($peerpay['peerpay_type'] == 0) {
			...
			$message = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_order_peerpay_payinfo') . ' WHERE pid = :pid ORDER BY id DESC LIMIT 3', array(':pid' => $peerpay['id']));
