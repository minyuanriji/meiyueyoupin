...
class Single_Refund_VcShopPage extends MobileLoginPage
{
	protected function globalData()
	{
		...
		$openid = $_W['openid'];
		$uniacid = $_W['uniacid'];
        $order = pdo_fetch("select o.id,o.price,o.couponprice,o.iscycelbuy,o.status,o.virtual,o.isverify,o.refundstate,o.finishtime,o.deductprice,o.deductcredit2,o.dispatchprice,o.deductenough,o.merchdeductenough,g.cannotrefund,g.refund,g.returngoods,g.exchange,g.type,og.single_refundid,og.single_refundstate,og.single_refundtime,og.realprice as og_realprice,o.grprice,og.consume,o.ispackage,og.sendtime from " . tablename("vcshop_order") . " o " . " left join " . tablename("vcshop_order_goods") . " og on og.orderid=o.id" . " left join " . tablename("vcshop_goods") . " g on g.id=og.goodsid" . " where og.id=:ogid and o.openid=:openid and o.uniacid=:uniacid", array(":ogid" => $order_goodsid, ":openid" => $openid, ":uniacid" => $uniacid));
		...
		if ($order['iscycelbuy'] == 1) {
			$order_goods = pdo_fetch('select * from ' . tablename('vcshop_cycelbuy_periods') . ('where orderid = ' . $order['id'] . ' and status != 0'));
		...
		$fullback_goods_count = pdo_fetchcolumn('select count(og.id) from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid' . ' where og.orderid=:orderid and g.isfullback>0 and og.uniacid=:uniacid', array(':orderid' => $order['id'], ':uniacid' => $uniacid));
		...
		$order_goods = pdo_fetchall('select og.id,og.single_refundid,og.single_refundstate,r.applyprice from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order_single_refund') . ' r on r.id=og.single_refundid' . ' where og.orderid=:orderid and og.uniacid=:uniacid', array(':orderid' => $order['id'], ':uniacid' => $uniacid));
		$is_last = true;
		$refund_price = 0;
	...
	public function main()
	{
		...
		if (!empty($single_refundid)) {
			$refund = pdo_fetch('select * from ' . tablename('vcshop_order_single_refund') . ' where id=:id and ordergoodsid=:ordergoodsid and uniacid=:uniacid limit 1', array(':id' => $single_refundid, ':ordergoodsid' => $order_goodsid, ':uniacid' => $uniacid));
	...
	public function submit()
	{
		...
		if ($order['single_refundstate'] == 0) {
			...
			pdo_insert('vcshop_order_single_refund', $refund);
			$single_refundid = pdo_insertid();
			pdo_update('vcshop_order_goods', array('single_refundid' => $single_refundid, 'single_refundstate' => $refundstate), array('id' => $order_goodsid, 'uniacid' => $uniacid));
		}
		else {
			$refund['status'] = 0;
			pdo_update('vcshop_order_goods', array('single_refundstate' => $refundstate), array('id' => $order_goodsid, 'uniacid' => $uniacid));
			pdo_update('vcshop_order_single_refund', $refund, array('id' => $single_refundid, 'uniacid' => $uniacid));
		}

		pdo_update('vcshop_order', array('refundstate' => 3, 'refundtime' => 0), array('id' => $order['id'], 'uniacid' => $uniacid));
	...
	public function cancel()
	{
		...
		pdo_update('vcshop_order_single_refund', $change_refund, array('id' => $single_refundid, 'uniacid' => $uniacid));
		pdo_update('vcshop_order_goods', array('single_refundstate' => 0), array('id' => $order_goodsid, 'uniacid' => $uniacid));
		$order_goods = pdo_fetchall('select single_refundid,single_refundstate,single_refundtime from ' . tablename('vcshop_order_goods') . ' where orderid=:orderid', array(':orderid' => $order['id']));
		$refund_num = 0;
		$apply_refund_num = 0;
		...
		if (empty($apply_refund_num) && !empty($refund_num)) {
			pdo_update('vcshop_order', array('refundtime' => time()), array('id' => $order['id'], 'uniacid' => $uniacid));
		}

		if (empty($apply_refund_num) && empty($refund_num)) {
			pdo_update('vcshop_order', array('refundstate' => 0, 'refundtime' => 0), array('id' => $order['id'], 'uniacid' => $uniacid));
		}
	...
	public function receive()
	{
		...
		pdo_update('vcshop_order_single_refund', $refund_data, array('id' => $single_refundid, 'uniacid' => $uniacid));
		$order_data = array();
		$order_data['single_refundstate'] = 9;
		pdo_update('vcshop_order_goods', $order_data, array('id' => $order_goodsid, 'uniacid' => $uniacid));
		$is_single_refund = pdo_fetchcolumn('select count(id) from ' . tablename('vcshop_order_goods') . 'where orderid=:orderid and (single_refundstate=1 or single_refundstate=2)', array(':orderid' => $order['id']));
