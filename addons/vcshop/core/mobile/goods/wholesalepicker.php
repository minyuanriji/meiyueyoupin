...
class Wholesalepicker_VcShopPage extends MobilePage
{
	public function main()
	{
		...
		if (!empty($goods) && $goods['hasoption']) {
			$specs1 = pdo_fetch('select *  from ' . tablename('vcshop_goods_spec') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc limit 1', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));

			if (!empty($specs1)) {
				$hasoption = 1;
				$spec1items = pdo_fetchall('select *  from ' . tablename('vcshop_goods_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $specs1['id']));
			...
			$specs2 = pdo_fetch('select *  from ' . tablename('vcshop_goods_spec') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc limit 1,1', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));

			if (!empty($specs2)) {
				$hasoption = 2;
				$spec2items = pdo_fetchall('select *  from ' . tablename('vcshop_goods_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $specs2['id']));
			...
			$optionlist = pdo_fetchall('select *  from ' . tablename('vcshop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
			$options = array();
