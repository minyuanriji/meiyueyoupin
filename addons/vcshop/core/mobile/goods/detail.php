...
class Detail_VcShopPage extends MobilePage
{
	public function main()
	{
		...
		if (p('task')) {
			if (!empty($task_id)) {
				$rewarded = pdo_fetchcolumn('SELECT `rewarded` FROM ' . tablename('vcshop_task_extension_join') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $task_id, ':uniacid' => $_W['uniacid']));
			...
			$taskGoodsInfo = pdo_fetch('select * from ' . tablename('vcshop_task_reward') . ' where id = :id and openid = :openid and senttime = 0 and gettime > 0', array(':id' => $taskrewardgoodsid, ':openid' => $_W['openid']));
		...
		$gifts = array();
		$giftgoods = array();
		$gifts = pdo_fetchall('select id,goodsid,giftgoodsid,thumb,title from ' . tablename('vcshop_gift') . ' where uniacid = ' . $uniacid . ' and activity = 2 and status = 1 and starttime <= ' . time() . ' and endtime >= ' . time() . '  ');
			...
			foreach ($gid as $ke => $val) {
				if ($val == $id) {
					...
					foreach ($giftgoods as $k => $val) {
						$giftinfo = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('vcshop_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and total > 0 and status = 2 and id = ' . $val . ' ');
		...
		$goods = pdo_fetch('select * from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (0 < $goods["ispresell"] && ($goods["preselltimeend"] == 0 || time() < $goods["preselltimeend"]) && $goods["hasoption"] == 1 && !empty($goods["hasoption"])) {
            $presell = pdo_fetch("select min(presellprice) as minprice,max(presellprice) as maxprice from " . tablename("vcshop_goods_option") . " where goodsid = " . $id);
		...
		if ($goods['isfullback']) {
			$isfullback = true;
			$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('vcshop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $id . ' and status=1 limit 1 ');
			...
			if (empty($fullbackgoods)) {
				...
				$fullprice = pdo_fetch('select min(allfullbackprice) as minfullprice,max(allfullbackprice) as maxfullprice,min(allfullbackratio) as minfullratio
                            ...
                            from ' . tablename('vcshop_goods_option') . ' where goodsid = ' . $id . '');
		...
		$style = pdo_fetch('SELECT id,uniacid,style FROM ' . tablename('vcshop_goods_labelstyle') . ' WHERE uniacid=' . $uniacid);
		...
		if (empty($member['updateaddress'])) {
			$address_list = pdo_fetchall('select id,datavalue from ' . tablename('vcshop_member_address') . ' where openid=:openid and uniacid=:uniacid', array(':uniacid' => $uniacid, ':openid' => $openid));
				...
				foreach ($address_list as $k1 => $v1) {
					if (!empty($v1['datavalue'])) {
						...
						if (!empty($change_data['province']) && !empty($change_data['city']) && !empty($change_data['area'])) {
							pdo_update('vcshop_member_address', $change_data, array('id' => $v1['id']));
						}
					}
				}

				pdo_update('vcshop_member', array('updateaddress' => 1), array('id' => $member['id']));
			}
		}
		...
		if (empty($task_goods_data['is_task_goods'])) {
			...
			if (p('bargain')) {
				$bargain = pdo_fetch('SELECT * FROM ' . tablename('vcshop_bargain_goods') . ' WHERE id = :id AND unix_timestamp(start_time)<' . time() . ' AND unix_timestamp(end_time)>' . time() . ' AND status = 0', array(':id' => $goods['bargain']));
		...
		if ($goods['buyshow'] == 1) {
			$sql = 'select o.id from ' . tablename('vcshop_order') . ' o left join ' . tablename('vcshop_order_goods') . ' g on o.id = g.orderid';
		...
		$package_goods = pdo_fetch('select pg.id,pg.pid,pg.goodsid,p.displayorder from ' . tablename('vcshop_package_goods') . ' as pg
                        left join ' . tablename('vcshop_package') . ' as p on pg.pid = p.id
		...
		if ($package_goods['pid']) {
			$packages = pdo_fetchall('SELECT id,title,thumb,packageprice FROM ' . tablename('vcshop_package_goods') . '
		...
		$specs = pdo_fetchall('select * from ' . tablename('vcshop_goods_spec') . ' where goodsid=:goodsid and  uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
		$spec_titles = array();
		...
		$params = pdo_fetchall('SELECT * FROM ' . tablename('vcshop_goods_param') . ' WHERE uniacid=:uniacid and goodsid=:goodsid order by displayorder asc', array(':uniacid' => $uniacid, ':goodsid' => $goods['id']));
		$goods = set_medias($goods, 'thumb');
		$goods['canbuy'] = $goods['status'] == 1 && empty($goods['deleted']);

		if (!empty($goods['hasoption'])) {
			$options = pdo_fetchall('select id,stock,marketprice,islive,liveprice from ' . tablename('vcshop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $goods['id'], ':uniacid' => $_W['uniacid']), 'stock');
		...
		if (0 < $goods['usermaxbuy']) {
			$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $goods['id'], ':uniacid' => $uniacid, ':openid' => $openid));
        ...
        g.minprice = (select min(marketprice) from ' . tablename('vcshop_goods_option') . (' where goodsid = ' . $id . '),
        g.maxprice = (select max(marketprice) from ') . tablename('vcshop_goods_option') . (' where goodsid = ' . $id . ')
        where g.id = ' . $id . ' and g.hasoption=1');
				pdo_query($sql);
			}
			else {
				$sql = 'update ' . tablename('vcshop_goods') . (' set minprice = marketprice,maxprice = marketprice where id = ' . $id . ' and hasoption=0;');
				pdo_query($sql);
			}

			$goods_price = pdo_fetch('select minprice,maxprice from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if (0 < $goods['hasoption']) {
			$productprice = pdo_fetchcolumn('select max(productprice) as productprice from ' . tablename('vcshop_goods_option') . ' where goodsid = :goodsid', array(':goodsid' => $id));
		...
		if (empty($this->merch_user)) {
			$merch_flag = 0;
			if ($is_openmerch == 1 && 0 < $goods['merchid']) {
				$merch_user = pdo_fetch('select * from ' . tablename('vcshop_merch_user') . ' where id=:id limit 1', array(':id' => intval($goods['merchid'])));
			...
			if (empty($shop['selectgoods'])) {
				$statics = array('all' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . (' where uniacid=:uniacid ' . $sqlcon . ' and status=1 and deleted=0'), $param), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . (' where uniacid=:uniacid ' . $sqlcon . ' and isnew=1 and status=1 and deleted=0'), $param), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . (' where uniacid=:uniacid ' . $sqlcon . ' and isdiscount=1 and status=1 and deleted=0'), $param));
			}
			else {
				$goodsids = explode(',', $shop['goodsids']);
				$goodsids = array_filter($goodsids);
				$shop['goodsids'] = implode(',', $goodsids);
				$statics = array('all' => count($goodsids), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . (' where uniacid=:uniacid ' . $sqlcon . ' and id in(') . $shop['goodsids'] . ') and isnew=1 and status=1 and deleted=0', $param), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . (' where uniacid=:uniacid ' . $sqlcon . ' and id in(') . $shop['goodsids'] . ') and isdiscount=1 and status=1 and deleted=0', $param));
			}
		}
		else {
			if ($goods['checked'] == 1) {
			...
			if (empty($shop['selectgoods'])) {
				$statics = array('all' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . ' where uniacid=:uniacid and merchid=:merchid and status=1 and deleted=0', array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . ' where uniacid=:uniacid and merchid=:merchid and isnew=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . ' where uniacid=:uniacid and merchid=:merchid and isdiscount=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])));
			}
			else {
				$goodsids = explode(',', $shop['goodsids']);
				$statics = array('all' => count($goodsids), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . (' where uniacid=:uniacid and merchid=:merchid and id in( ' . $shop['goodsids'] . ' ) and isnew=1 and status=1 and deleted=0'), array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_goods') . (' where uniacid=:uniacid and merchid=:merchid and id in( ' . $shop['goodsids'] . ' ) and isdiscount=1 and status=1 and deleted=0'), array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])));
			}
		}
		...
		if ($com) {
			...
			if ($goods['nocommission'] == 0) {
				...
				if (!empty($goods) && $goods['hasoption']) {
					$optioncomm = pdo_fetchall('select * from ' . tablename('vcshop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
				}
		...
		if ($new_temp && $getComments) {
			$showComments = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order_comment') . ' where goodsid=:goodsid and level>=0 and deleted=0 and checked=0 and uniacid=:uniacid', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
		}

		$close_preview = intval($_W['shopset']['shop']['close_preview']);
		$goods['city_express_state'] = 1;
		$city_express = pdo_fetch('SELECT * FROM ' . tablename('vcshop_city_express') . ' WHERE uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $_W['uniacid']));
	...
	public function get_code()
	{
		...
		$goods = pdo_fetch('select id,minprice,minprice,maxprice,thumb_url,thumb,title from ' . tablename('vcshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function querygift()
	{
		...
		$gift = pdo_fetch('select * from ' . tablename('vcshop_gift') . ' where id=:id and status=1 and  uniacid =:uniacid limit 1', array(':id' => $giftid, ':uniacid' => $_W['uniacid']));
		show_json(1, $gift);
	}
	...
	public function get_comments()
	{
		...
		$count = array('all' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order_comment') . ' where goodsid=:goodsid and level>=0 and deleted=0 and checked=0 and uniacid=:uniacid', $params), 'good' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order_comment') . ' where goodsid=:goodsid and level>=5 and deleted=0 and checked=0 and uniacid=:uniacid', $params), 'normal' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order_comment') . ' where goodsid=:goodsid and level>=2 and level<=4 and deleted=0 and checked=0 and uniacid=:uniacid', $params), 'bad' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order_comment') . ' where goodsid=:goodsid and level<=1 and deleted=0 and checked=0 and uniacid=:uniacid', $params), 'pic' => pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order_comment') . ' where goodsid=:goodsid and ifnull(images,\'a:0:{}\')<>\'a:0:{}\' and deleted=0 and checked=0 and uniacid=:uniacid', $params));
		$list = array();

		if (0 < $count['all']) {
			$percent = intval($count['good'] / (empty($count['all']) ? 1 : $count['all']) * 100);
			$list = pdo_fetchall('select nickname,level,content,images,createtime from ' . tablename('vcshop_order_comment') . ' where goodsid=:goodsid and deleted=0 and checked=0 and uniacid=:uniacid order by istop desc, createtime desc, id desc limit 2', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function get_offic_list()
	{
		...
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('vcshop_offic') . (' as o where 1 ' . $condition), $params);
		$list = array();

		if (0 < $total) {
			$list = pdo_fetchall('SELECT o.*,m.avatar,m.nickname,g.thumb,g.title,g.minprice FROM ' . tablename('vcshop_offic') . ' as o
                    left join ' . tablename('vcshop_member') . ' as m on m.openid = o.openid
                    left join ' . tablename('vcshop_goods') . (' as g on g.id = o.goodsid
	...
	public function get_comment_list()
	{
		...
		$list = pdo_fetchall('select * from ' . tablename('vcshop_order_comment') . ' ' . ('  where goodsid=:goodsid and uniacid=:uniacid and deleted=0 and checked=0 ' . $condition . ' order by istop desc, createtime desc, id desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);
		...
		unset($row);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_order_comment') . (' where goodsid=:goodsid  and uniacid=:uniacid and deleted=0 and checked=0 ' . $condition), $params);
	...
	public function getCouponsbygood($goodid)
	{
		...
		if (!empty($goodid)) {
			...
			$goodparam[':id'] = $goodid;
			$sql = 'select id,cates,marketprice,merchid   from ' . tablename('vcshop_goods');
			...
			foreach ($list as $key => &$row) {
				...
				if ($row['islimitlevel'] == 1) {
					...
					if (!empty($row['limitmemberlevels']) || $row['limitmemberlevels'] == '0') {
						$level1 = pdo_fetchall('select * from ' . tablename('vcshop_member_level') . ' where uniacid=:uniacid and  id in (' . $row['limitmemberlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
					...
					if ((!empty($row['limitagentlevels']) || $row['limitagentlevels'] == '0') && $hascommission) {
						$level2 = pdo_fetchall('select * from ' . tablename('vcshop_commission_level') . ' where uniacid=:uniacid and id  in (' . $row['limitagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
						if ($member['isagent'] == '1' && $member['status'] == '1') {
					...
					if ((!empty($row['limitpartnerlevels']) || $row['limitpartnerlevels'] == '0') && $hasglobonus) {
						$level3 = pdo_fetchall('select * from ' . tablename('vcshop_globonus_level') . ' where uniacid=:uniacid and  id in(' . $row['limitpartnerlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
					...
					if ((!empty($row['limitaagentlevels']) || $row['limitaagentlevels'] == '0') && $hasabonus) {
						$level4 = pdo_fetchall('select * from ' . tablename('vcshop_abonus_level') . ' where uniacid=:uniacid and  id in (' . $row['limitaagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
	...
	public function pay($a = array(), $b = array())
	{
		...
		$openid = $_W['openid'];
		$id = intval($_GPC['id']);
		$coupon = pdo_fetch('select * from ' . tablename('vcshop_coupon') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		pdo_insert('vcshop_coupon_log', $log);
