...
class Index_VcShopPage extends MobilePage
{
	...
	public function gift()
	{
		...
		$gift = pdo_fetch('select * from ' . tablename('vcshop_gift') . ' where uniacid = ' . $uniacid . ' and id = ' . $giftid . ' and starttime <= ' . time() . ' and endtime >= ' . time() . ' and status = 1 ');
		$giftgoodsid = explode(',', $gift['giftgoodsid']);
		$giftgoods = array();

		if (!empty($giftgoodsid)) {
			foreach ($giftgoodsid as $key => $value) {
				$giftgoods[$key] = pdo_fetch('select id,status,title,thumb,marketprice,total from ' . tablename('vcshop_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0  and id = ' . $value . ' and status = 2 ');
			}
	...
	private function _condition($args)
	{
		...
		foreach ($goods['list'] as $key => &$value) {
			if(is_array($value)){
				$value['dprice'] = number_format($value['minprice']-$value['deduct'],2);
				$value['minprice'] = number_format($value['minprice'],2);
			}
		}
