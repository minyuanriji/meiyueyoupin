...
class Index_VcShopPage extends MobilePage
{
	...
	public function detail()
	{
		...
		if (!empty($_GPC['card_id']) && !empty($_GPC['encrypt_code']) && !empty($_GPC['openid'])) {
			...
			$code = $data['code'];
			$item = pdo_fetch('select vg.*,g.title,g.subtitle,g.thumb  from ' . tablename('vcshop_verifygoods') . '   vg
		 inner join ' . tablename('vcshop_order_goods') . ' og on vg.ordergoodsid = og.id
		 left  join ' . tablename('vcshop_order') . ' o on vg.orderid = o.id
		 left  join ' . tablename('vcshop_order_refund') . ' orf on o.refundid = orf.id
		 inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id
		...
		if (empty($verifycode) || $item['codeinvalidtime'] < time()) {
			...
			while (1) {
				$count = pdo_fetchcolumn('select count(*) from ' . tablename('vcshop_verifygoods') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
			...
			pdo_update('vcshop_verifygoods', $data, array('id' => $item['id']));
		}
		...
		$goodsstore = pdo_fetch('select * from ' . tablename('vcshop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['storeid'], ':uniacid' => $_W['uniacid']));
		include $this->template();
	}
