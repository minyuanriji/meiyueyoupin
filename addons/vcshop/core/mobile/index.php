...
class Index_VcShopPage extends MobilePage
{
	public function main()
	{
		...
		// han 20191130
		$member = pdo_fetch('select agentid,agentlevel from ' . tablename('vcshop_member') . ' where openid=:openid and uniacid=:uniacid',array(':openid'=>$_W['openid'],':uniacid'=>$_W['uniacid']));
		...
		if (empty($trade['shop_strengthen'])) {
			$order = pdo_fetch('select id,price,`virtual`,createtime  from ' . tablename('vcshop_order') . ' where uniacid=:uniacid and status = 0 and paytype<>3 and openid=:openid order by createtime desc limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
			...
			if (!empty($order)) {
				...
				$goods = pdo_fetchall('select g.*,og.total as totals  from ' . tablename('vcshop_order_goods') . ' og inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id   where og.uniacid=:uniacid    and og.orderid=:orderid  limit 3', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
				$goodstotal = pdo_fetchcolumn('select COUNT(*)  from ' . tablename('vcshop_order_goods') . ' og inner join ' . tablename('vcshop_goods') . ' g on og.goodsid = g.id   where og.uniacid=:uniacid    and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
			}
		}
		...
		$sale_sql = 'SELECT * FROM ' . tablename('vcshop_sendticket') . ' WHERE uniacid = ' . intval($_W['uniacid']);
	...
	public function getpage()
	{
		...
		$advs = pdo_fetchall('select id,advname,link,thumb from ' . tablename('vcshop_adv') . ' where uniacid=:uniacid and iswxapp=0 and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		$navs = pdo_fetchall('select id,navname,url,icon from ' . tablename('vcshop_nav') . ' where uniacid=:uniacid and iswxapp=0 and status=1 order by displayorder desc', array(':uniacid' => $uniacid));
		$cubes = is_array($_W['shopset']['shop']['cubes']) ? $_W['shopset']['shop']['cubes'] : array();
		$banners = pdo_fetchall('select id,bannername,link,thumb from ' . tablename('vcshop_banner') . ' where uniacid=:uniacid and iswxapp=0 and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		...
		if (!empty($_W['shopset']['shop']['indexrecommands'])) {
			...
			if (!empty($goodids)) {
				$indexrecommands = pdo_fetchall('select * from ' . tablename('vcshop_goods') . (' where id in( ' . $goodids . ' ) and uniacid=:uniacid and deleted = 0 and status=1 order by instr(\'' . $goodids . '\',id),displayorder desc'), array(':uniacid' => $uniacid));
		...
		$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('vcshop_notice') . ' where uniacid=:uniacid and iswxapp=0 and status=1 order by displayorder desc limit 5', array(':uniacid' => $uniacid));
