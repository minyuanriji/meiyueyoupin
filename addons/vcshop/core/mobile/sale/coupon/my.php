...
class My_VcShopPage extends MobileLoginPage
{
		...
		if ($data['limitgoodcatetype'] == 1 && !empty($data['limitgoodcateids'])) {
			...
			if (0 < count($limitcateids)) {
				...
				foreach ($limitcateids as $cateid) {
					...
					$table .= 'select * from ' . tablename('vcshop_goods') . ' where FIND_IN_SET(' . $cateid . ',cates) and deleted =0';
				}
