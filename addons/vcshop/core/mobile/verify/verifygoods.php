...
class Verifygoods_VcShopPage extends MobilePage
{
	/**
     * 会员核销卡核销页面
     */
	public function detail()
	{
		...
		$item = pdo_fetch('select vg.*,g.id as goodsid ,g.title,g.subtitle,g.thumb,vg.storeid  from ' . tablename('vcshop_verifygoods') . '   vg
		...
		$saler = pdo_fetch('select * from ' . tablename('vcshop_saler') . ' where  status=1  and openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		...
		if (!empty($item['limitnum'])) {
			$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('vcshop_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $item['id']));
			$verifynum = 0;
