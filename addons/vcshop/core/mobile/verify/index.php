...
class Index_VcShopPage extends MobilePage
{
	public function qrcode()
	{
		...
		$order = pdo_fetch('select istrade from ' . tablename('vcshop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
	...
	public function select()
	{
		...
		$order = pdo_fetch('select id,verifyinfo from ' . tablename('vcshop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
	...
	public function check()
	{
		...
		$uniacid = $_W['uniacid'];
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select id,status,isverify,verified from ' . tablename('vcshop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
