...
class Verifyorder_VcShopPage extends MobilePage
{
	public function orderData()
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$sqlcondition = ' inner join ( select DISTINCT(og.orderid) from ' . tablename('vcshop_order_goods') . ' og left join ' . tablename('vcshop_goods') . (' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id');
			}
			else {
				if ($searchfield == 'goodssn') {
					$sqlcondition = ' inner join ( select DISTINCT(og.orderid) from ' . tablename('vcshop_order_goods') . ' og left join ' . tablename('vcshop_goods') . (' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id');
				}
			}
		}
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 AND ( o.status = 3 and (isverify=1 or o.istrade=1)) and o.verifyopenid = :openid' || !empty($sqlcondition)) {
			if ($this->merchPluginExists()) {
				$sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress, d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname,s.salername,
                  r.rtype,r.status as rstatus,o.sendtype,0 as isverifygoods_order from ' . tablename('vcshop_order') . ' o' . ' left join ' . tablename('vcshop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('vcshop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . ' left join ' . tablename('vcshop_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('vcshop_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('vcshop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('vcshop_merch_saler') . ' ms on ms.openid = o.verifyopenid and ms.uniacid=o.uniacid' . ' left join ' . tablename('vcshop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . (' ' . $sqlcondition . ' where ' . $condition . '  GROUP BY o.id ORDER BY o.createtime DESC  ');
			}
			else {
				$sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress, d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname,s.salername,
			...
			if ($this->merchPluginExists()) {
				$sql = 'SELECT o.*,0 as isverifygoods_order FROM' . tablename('vcshop_order') . ' o ' . ' LEFT JOIN ' . tablename('vcshop_saler') . ' s ON o.verifyopenid=s.openid' . ' LEFT JOIN ' . tablename('vcshop_merch_saler') . ' ms ON o.verifyopenid=ms.openid' . '   WHERE (s.openid=:openid or ms.openid=:openid) AND o.ismr=0  AND   o.status =3  AND o.deleted=0 and o.uniacid=:uniacid   GROUP BY o.id ORDER BY o.createtime DESC  ';
			}
			else {
				$sql = 'SELECT o.*,0 as isverifygoods_order FROM' . tablename('vcshop_order') . ' o ' . ' LEFT JOIN ' . tablename('vcshop_saler') . ' s ON o.verifyopenid=s.openid' . '   WHERE o.verifyopenid = :openid AND o.ismr=0  AND   o.status =3  AND o.deleted=0 and o.uniacid=:uniacid   GROUP BY o.id ORDER BY o.createtime DESC  ';
			}
...
';
	...
	public function orderDataVerify()
	{
		...
		$pindex = max(1, intval($_GPC['page']));
		$psize = 20;
		$saler = pdo_fetch('select id,openid from ' . tablename('vcshop_saler') . ' where status=1  and openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		...
		if ($condition != ' log.salerid = :salerid and log.uniacid = :uniacid ' || !empty($sqlcondition)) {
			$sql = 'select o.* ,1 as isverifygoods_order,g.id as verifygoodsid from ' . tablename('vcshop_verifygoods_log') . ' log ' . ' left join  ' . tablename('vcshop_verifygoods') . ' g on log.verifygoodsid = g.id ' . ' left join  ' . tablename('vcshop_order') . ' o on g.orderid = o.id  ' . ' left join ' . tablename('vcshop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . (' ' . $sqlcondition . ' where ' . $condition . ' GROUP BY o.id ORDER BY log.verifydate DESC  ');
			...
			$sql = 'select o.* ,1 as isverifygoods_order,g.id as verifygoodsid from ' . tablename('vcshop_verifygoods_log') . ' log ' . ' left join  ' . tablename('vcshop_verifygoods') . ' g on log.verifygoodsid = g.id ' . ' left join  ' . tablename('vcshop_order') . ' o on g.orderid = o.id where log.uniacid = :uniacid and log.salerid=:salerid GROUP BY o.id ORDER BY log.verifydate DESC  ';
		...
		if (!empty($list)) {
			...
			foreach ($list as &$value) {
				$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('vcshop_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $value['verifygoodsid']));
				$verifynum = 0;
				...
				$verifygood = pdo_fetch('select *  from ' . tablename('vcshop_verifygoods') . '    where id =:id  ', array(':id' => $value['verifygoodsid']));
				...
				$order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,g.type,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    og.price,og.optionname as optiontitle, og.realprice,og.changeprice,og.oldprice,og.commission1,og.commission2,og.commission3,og.commissions,og.diyformdata,
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell from ' . tablename('vcshop_order_goods') . ' og ' . ' left join ' . tablename('vcshop_goods') . ' g on g.id=og.goodsid ' . ' left join ' . tablename('vcshop_goods_option') . ' op on og.optionid = op.id ' . ' where og.orderid=:orderid and og.uniacid=:uniacid and g.type=5 ', array(':orderid' => $value['id'], ':uniacid' => $uniacid));
				$goods = '';
...
';
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
			...
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('vcshop_order') . ' WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0', $paras);
		}
	...
	protected function selectOrderPrice($day = 0)
	{
		...
		if ($this->merchPluginExists()) {
			$sql = 'SELECT count(o.id) FROM' . tablename('vcshop_order') . ' o' . ' LEFT JOIN ' . tablename('vcshop_saler') . ' s ON o.verifyopenid=s.openid' . ' LEFT JOIN ' . tablename('vcshop_merch_saler') . ' ms ON o.verifyopenid=ms.openid' . '   WHERE (s.openid=:openid or ms.openid = :openid) and o.createtime between :createtime1 and :createtime2 AND o.ismr=0  AND   o.status =3  AND o.deleted=0 and o.uniacid=:uniacid   limit 1';
		}
		else {
			$sql = 'SELECT count(o.id) FROM' . tablename('vcshop_order') . ' o' . ' LEFT JOIN ' . tablename('vcshop_saler') . ' s ON o.verifyopenid=s.openid' . '   WHERE o.verifyopenid = :openid and o.createtime between :createtime1 and :createtime2 AND o.ismr=0  AND   o.status =3  AND o.deleted=0 and o.uniacid=:uniacid   limit 1';
		}

		$count = pdo_fetchcolumn($sql, $param);
		$saler = pdo_fetch('select id,openid from ' . tablename('vcshop_saler') . ' where status=1  and openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		$sql = 'SELECT sum(verifynum) FROM' . tablename('vcshop_verifygoods_log') . '    WHERE 
