...
class Rank_VcShopPage extends MobileLoginPage
{
	...
	protected function getRank($update = false)
	{
		...
		if (empty($rank_cache) || $rank_cache['time'] < TIMESTAMP || $update) {
			...
			$result = pdo_fetchall('SELECT sm.id,sm.uid,m.credit1,sm.nickname,sm.avatar,sm.openid FROM ' . tablename('vcshop_member') . ' sm RIGHT JOIN ' . tablename('mc_members') . (' m ON m.uid=sm.uid WHERE sm.uniacid = :uniacid ORDER BY m.credit1 DESC LIMIT ' . $num), array(':uniacid' => $_W['uniacid']));
			$result1 = pdo_fetchall('SELECT id,uid,credit1,nickname,avatar,openid FROM ' . tablename('vcshop_member') . (' WHERE uniacid = :uniacid AND uid=0 ORDER BY credit1 DESC LIMIT ' . $num), array(':uniacid' => $_W['uniacid']));
	...
	protected function creditChange($uid, $credittype = 'credit1', $day = 7, $openid = '')
	{
		...
		if ($uid) {
			$sql = 'select sum(num) from ' . tablename('mc_credits_record') . ' where uniacid = :uniacid and uid = :uid and credittype = :credittype and `module` = "vcshop" and createtime between :createtime1 and :createtime2 and uid<>0';
			$param = array(':uniacid' => $_W['uniacid'], ':uid' => (int) $uid, ':createtime1' => $createtime1, ':createtime2' => $createtime2, ':credittype' => $credittype);
		}
		else {
			$sql = 'select sum(num) from ' . tablename('vcshop_member_credit_record') . ' where uniacid = :uniacid and openid = :openid and credittype = :credittype and createtime between :createtime1 and :createtime2';
	...
	public function order_rank()
	{
		...
		$result_one = pdo_fetch('select a.openid, sum(price) as price,a.uniacid,a.status
			from (
			(select openid, sum(price) price,uniacid,status from ' . tablename('vcshop_order') . ' where uniacid = :uniacid and status = 1 group by openid)
			union  all (select openid, sum(price+freight) price,uniacid,status from ' . tablename('vcshop_groups_order') . ' where uniacid = :uniacid and status >= 1 group by openid)
	...
	public function ajaxorderpage()
	{
		...
		$sql = 'SELECT m.id,m.uid,m.nickname,m.avatar,m.openid,' . '(select ifnull(sum(o.price),0) from  ' . tablename('vcshop_order') . (' o where o.openid=m.openid and o.status>=1 ' . $condition . ')  as price') . ' from ' . tablename('vcshop_member') . ' m  ' . (' where 1 ' . $condition1 . ' order by price desc,createtime desc') . $limit;
	...
	protected function orderChange($openid, $day = 7)
	{
		...
		$sql = 'select sum(price) from ' . tablename('vcshop_order') . ' where uniacid = :uniacid and openid = :openid and status >= 1 and createtime between :createtime1 and :createtime2';
