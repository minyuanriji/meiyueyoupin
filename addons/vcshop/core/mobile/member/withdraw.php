...
class Withdraw_VcShopPage extends MobileLoginPage
{
	...
	public function submit()
	{
		...
		if ($applytype == 2) {
			...
			if ($applytype == 3) {
				...
				$bankbranch = trim($_GPC['bankbranch']);//han 20191214
				...
				$apply['realname'] = $realname;
				$apply['bankbranch'] = $bankbranch;//han 20191214
		...
		pdo_insert('vcshop_member_log', $apply);
		$logid = pdo_insertid();
	...
	public function getLastApply($openid, $applytype = -1)
	{
		...
		$sql = 'select applytype,alipay,bankname,bankcard,realname from ' . tablename('vcshop_member_log') . ' where openid=:openid and uniacid=:uniacid';
