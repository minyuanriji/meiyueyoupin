...
class Address_VcShopPage extends MobileLoginPage
{
	public function main()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('vcshop_member_address') . ' where 1 ' . $condition . ' ORDER BY `id` DESC';
	...
	public function post()
	{
		...
		if (!empty($id) || !empty($wx_address_info)) {
			...
			if (empty($address['datavalue'])) {
				...
				$citys_code = 0;
				$county_code = 0;
				$path = VCSHOP_PATH . 'static/js/dist/area/AreaNew.xml';
				...
				if ($province_code != 0 && $citys_code != 0 && $county_code != 0) {
					...
					if (empty($wx_address_info)) {
						pdo_update('vcshop_member_address', $address, array('id' => $id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
					}
				}
			}
	...
	public function selector()
	{
		...
		$condition = ' and openid=:openid and deleted=0 and  `uniacid` = :uniacid  ';
		$params = array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']);
		$sql = 'SELECT * FROM ' . tablename('vcshop_member_address') . ' where 1 ' . $condition . ' ORDER BY isdefault desc, id DESC ';
		$list = pdo_fetchall($sql, $params);
		include $this->template();
		exit();
	}
