...
class Finance_VcShopModel
{
    ...
    public function pay($openid = "", $paytype = 0, $money = 0, $trade_no = "", $desc = "", $return = true)
    {
        ...
        if (strpos($openid, "sns_wa_") !== false) {
            ...
            if (!empty($set["weixin_id"])) {
                ...
                $payment = pdo_fetch("SELECT * FROM " . tablename("vcshop_payment") . " WHERE uniacid=:uniacid AND  type=0  AND paytype=0 AND sub_appid=:appid", array(":uniacid" => $_W["uniacid"], ":appid" => $_W["account"]["key"]));
            }
            if (empty($payment)) {
                $payment = array();
    ...
    public function wxapp_refund($openid, $out_trade_no, $out_refund_no, $totalmoney, $refundmoney = 0, $app = false, $refund_account = false)
    {
        ...
        if (is_array($certs)) {
            ...
            $certfile = IA_ROOT . "/addons/vcshop/cert/" . random(64);
            file_put_contents($certfile, $certs["cert"]);
            $keyfile = IA_ROOT . "/addons/vcshop/cert/" . random(64);
            file_put_contents($keyfile, $certs["key"]);
            $extras["CURLOPT_SSLCERT"] = $certfile;
            $extras["CURLOPT_SSLKEY"] = $keyfile;
            if (!empty($certs["root"])) {
                $rootfile = IA_ROOT . "/addons/vcshop/cert/" . random(64);
                file_put_contents($rootfile, $certs["root"]);
                $extras["CURLOPT_CAINFO"] = $rootfile;
            }
        } else {
    ...
    public function AlipayRefund($params, $batch_no, $config)
    {
        ...
        $parameter = array("service" => "refund_fastpay_by_platform_pwd", "partner" => $config["partner"], "_input_charset" => "UTF-8", "notify_url" => isset($params["notify_url"]) ? $params["notify_url"] : $_W["siteroot"] . "addons/vcshop/payment/alipay/notify.php", "seller_user_id" => $config["partner"], "seller_email" => $config["account"], "refund_date" => date("Y-m-d H:i:s"), "batch_no" => $batch_no, "batch_num" => "1", "detail_data" => $params["trade_no"] . "^" . $params["refund_price"] . "^" . $params["refund_reason"]);
        ...
        foreach ($dates as $date) {
            ...
            if ($datatype) {
                ...
                foreach ($dc_arr as $key => $value) {
                    if ($key != 0 && $key < $len - 3 && !strstr($value, "vcshop")) {
