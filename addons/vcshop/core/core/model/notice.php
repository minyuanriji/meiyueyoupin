...
class Notice_VcShopModel
{
    ...
    public function sendOrderMessage($orderid = "0", $delRefund = false, $raid = NULL)
    {
        ...
        $order_goods = pdo_fetchall("select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.sendtype,og.expresscom,og.expresssn,og.sendtime from " . tablename("vcshop_order_goods") . " og " . " left join " . tablename("vcshop_goods") . " g on g.id=og.goodsid " . " where " . $scondition . " and og.uniacid=:uniacid ", $param);
        $goods = "";
        $goodsname = "";
        $goodsnum = 0;
        foreach ($order_goods as $og) {
