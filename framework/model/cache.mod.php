...
function cache_build_setting() {
	...
	if (is_array($setting)) {
		...
		if(empty($setting['site'])){
			$setting['site']['key'] = empty($setting['site']['key']) ? random(5, true) : $setting['site']['key'];
			$setting['site']['token'] = empty($setting['site']['token']) ? "ojWRS7m0OX18R1ZxC7M5fgdCyr5tJ629" : $setting['site']['token'];
			pdo_insert('core_settings',array('key'=>'site','value'=>serialize($setting['site'])));
		}
